...
class Prescription extends ORDataObject
{
    ...
    */
    ...
    function set_medication($med)
    {
        ...
        $dataRow = sqlQuery("select id from lists where type = 'medication' and activity = 1 and (enddate is null or cast(now() as date) < enddate) and upper(trim(title)) = upper(trim('" . add_escape_custom($this->drug) . "')) and pid = '" . add_escape_custom($this->client->id) . "' limit 1");

        if ($med && !isset($dataRow['id'])) {
            $dataRow = sqlQuery("select id from lists where type = 'medication' and activity = 0 and (enddate is null or cast(now() as date) < enddate) and upper(trim(title)) = upper(trim('" . add_escape_custom($this->drug) . "')) and pid = '" . add_escape_custom($this->client->id) . "' limit 1");

            if (!isset($dataRow['id'])) {
                //add the record to the medication list
                sqlStatement("insert into lists(date,begdate,type,activity,pid,user,groupname,title) values (now(),cast(now() as date),'medication',1,'" . add_escape_custom($this->client->id) . "','" . add_escape_custom($$_SESSION['authUser']) . "','" . add_escape_custom($$_SESSION['authProvider']) . "','" . add_escape_custom($this->drug) . "')");
            } else {
    ...
    function get_prescription_display()
    {
        ...
        $string .= ""
                ."Prescription For:" . "\t" .$this->client->get_name_display() . "\n"
    ...
    function get_prescription_florida_display()
    {
        ...
        $string .= "\n";
        $string .= strtoupper($this->client->lname) . ", " . ucfirst($this->client->fname) . " " . $this->client->mname . "\n";
        $string .= "DOB " .  $this->client->date_of_birth . "\n";
        $string .= "\n";
    ...
    static function prescriptions_factory(
        ...
        $prescriptions = array();
        $p = new Prescription();
        $sql = "SELECT id FROM " . escape_table_name($p->_table) . " WHERE client_id = ? " .
                "ORDER BY " . add_escape_custom($order_by);
        $results = sqlQ($sql, array($client_id));
