<?php
/**
 * soap_service/server_newclienttoapprove.php Server side code to unapproved clients.
 *
 * Functions to handle unapproved clients.
 *
...
//actual execution is done in the select_query function in Server_side
    ...
    public function query_formation($data)
    {
        ...
        switch ($data[0]) {
            case 'A1':
            //Select list of encounters for the clients
            ...
            case 'A4':
                        //Select encounter of the client
                $query="select * from form_encounter where pid=? limit 1 ";
                return array($query,array($pid));
            break;
            
            case 'A5':
            //Select encounters of the client
                        $enc_set_array=array();
                $enc_set_array[]=$pid;
            ...
            as provname,".$provider." from form_encounter fe left outer join users u
             on u.id =fe.".$provider." join client_data pd on pd.pid=fe.pid where 
             fe.pid=?".  $enc_set ." order by fe.date desc";
                return array($query, $enc_set_array);
            break;
            
            case 'A6':
                        //Select encounter and copay of the client
                $enc_set_array=array();
                $enc_set_array[]=$pid;
            ...
            case 'A9':
                        //Payment
                $query = "SELECT sum(pay_total)  as pay_total FROM ar_session WHERE client_id=? AND adjustment_code=?";
                return array($query,array($pid,'pre_payment'));
            break;
            case 'A10':
                        //Payment
                $query = "SELECT sum(pay_amount)  as pay_amount FROM ar_session,ar_activity WHERE client_id=? AND adjustment_code=?
        ...
        // Entries pending  for approval for Existing Client and New Client.
            ...
            case 'P3':
                        $query="SELECT * FROM audit_details WHERE audit_master_id = ? AND table_name = 'client_data'";
           ...
           //for building client Demo   Employer Data
            ...
            case 'P20':
                ...
                $query="select count(*) AS count from client_data where pubpid = ? $string_query";
                return array($query,$x);
            break;
        
            //getting DOB and SSN for verifying the duplicate client existance
            case 'P21':
                if ($pid) {
                      $string_query=" and pid !=?";
                }
                ...
                $query="select  ss,DOB  from client_data where DOB=? $string_query  ";
                return array($query,$x);
            break;
            ...
            case 'B4':
             //Check whether an entry exist in the form Encounter to decide whether client is an existing client.
                $query="select COUNT(*) AS cnt from form_encounter WHERE pid=?";
                return array($query,array($pid));
            break;
            
            case 'B5':
           //Existing appointments for a client
           ...
           //Calendar default visit time for visit category. value for Admin--->others-->calendar
            ...
            case 'G2':
                    ...
                    dlm_effective_date<>? AND dlm_upload_type = '0' AND dlm_document_id NOT IN (SELECT DISTINCT (ad2.field_value) FROM
										audit_details ad JOIN audit_details ad1 ON ad1.table_name = 'client_access_offsite' AND ad1.field_name = 'portal_pwd' 
										AND ad1.audit_master_id = ad.audit_master_id JOIN audit_details ad2 ON ad2.table_name = 'documents_legal_detail' AND
										ad2.field_name = 'dld_master_docid' JOIN audit_details ad3 ON ad3.table_name = 'client_access_offsite' AND
										ad3.field_name = 'portal_username' AND ad3.audit_master_id = ad2.audit_master_id WHERE ad.audit_master_id = ? AND
										ad.table_name = 'client_access_offsite' AND ad.field_name = 'portal_username' AND ad3.field_value = ad.field_value)";
            ...
            case 'G3':
                    $query = "SELECT *, ad5.field_value AS dld_filename FROM documents_legal_master AS dlm JOIN audit_details ad ON
										ad.table_name = 'client_access_offsite' AND ad.field_name = 'portal_username' JOIN audit_details ad1 ON
										ad1.table_name = 'client_access_offsite' AND ad1.field_name = 'portal_pwd' AND ad1.audit_master_id = ad.audit_master_id
										JOIN audit_details ad2 ON ad2.table_name = 'client_access_offsite' AND ad2.field_name = 'portal_username' AND
            ...
            case 'F8':
            // Entries to be approved demo  for new client, existing client and only documents
            ...
            case 'J1':
                ...
                $query = "SELECT field_value as fname FROM audit_details WHERE audit_master_id=? AND table_name='client_data' AND field_name = 'fname'";
                return array($query,array($auditmasterid));
            break;
            //Checking whether a new client entry is pending in the audit master
            case 'J2':
            ...
            case 'authorizenet_id':
                $query = "SELECT authorize_net_id FROM client_access_offsite WHERE pid=?";
                return array($query,array($pid));
            break;
        }
    }
