...
function generate_list_payment_category($tag_name, $list_id, $currvalue, $title, $empty_name = ' ', $class = '', $onchange = '', $PaymentType = 'insurance', $screen = 'new_payment')
{
    ...
    while ($lrow = sqlFetchArray($lres)) {
        ...
        if (($PaymentType == 'insurance' || $screen == 'new_payment') && ($lrow['option_id'] == 'family_payment' || $lrow['option_id'] == 'client_payment')) {
            $s .= " style='background-color:#DEDEDE' ";
        }
        if ($PaymentType == 'client' && $lrow['option_id'] == 'insurance_payment') {
            $s .= " style='background-color:#DEDEDE' ";
        }
...
if ($payment_id > 0) {
    ...
    $res = sqlStatement("SELECT check_date ,reference ,insurance_companies.name,
        payer_id,pay_total,payment_type,post_to_date,client_id ,
    ...
    } elseif ($row['payment_type'] == 'client' || $row['client_id'] * 1 > 0) {
        $res = sqlStatement("SELECT fname,lname,mname FROM client_data
            where pid =?", array($ClientId));
        $row = sqlFetchArray($res);
        $fname = $row['fname'];
        $lname = $row['lname'];
        $mname = $row['mname'];
    ...
    ?>
        ...
        ?>
                    ...
                    <div class="col-xs-12 oe-custom-line">
                        ...
                        <div class="col-xs-3">
                            <label class="control-label" for="div_insurance_or_client"><?php echo xlt('Payor ID'); ?>:</label>
                            <!--<input class="form-control" type="text"  value = '<?php //echo attr($TypeCode);?>' name="div_insurance_or_client" id="div_insurance_or_client" placeholder="Payor ID…"  />-->
                            <div  name="div_insurance_or_client" id="div_insurance_or_client" class="form-control"  ><?php echo text($TypeCode);?></div>
                        </div>
                    </div>
                    <div class="col-xs-12 oe-custom-line">
                        <div class="col-xs-3">
    ...
    ?>
                    ...
                    <div class="col-xs-12 oe-custom-line">
                        <div class="forms col-xs-6">
                            <label class="control-label" for="div_insurance_or_client"><?php echo xlt('Payment From'); ?>:</label>
                            <input name='div_insurance_or_client'  id='div_insurance_or_client' type="text" class="form-control" value="<?php echo attr($div_after_save);?>"  disabled />
                        </div>
                        <div class="forms col-xs-3">
