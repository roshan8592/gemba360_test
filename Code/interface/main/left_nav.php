 ...
 */
 ...
 // * interface/main/finder/client_select.php: loads stuff when a new client
 //   is selected.
 // * interface/client_file/summary/demographics.php: this is the first frame
 //   loaded when a new client is chosen, and in turn sets the current pid and
 //   then loads the initial bottom frame.
 // * interface/client_file/summary/demographics_full.php: added support for
 //   setting a new pid, needed for going to demographics from billing.
 // * interface/client_file/summary/demographics_save.php: redisplay
 //   demographics.php and not the frameset.
 // * interface/client_file/encounter/encounter_bottom.php: new frameset for
 //   the selected encounter, mosting coding/billing stuff, cloned from
 //   client_encounter.php.  This will also self-load the superbill pages
 //   as requested.
 ...
 // * interface/new/new_client_save.php: modified to load the demographics
 //   page to the current frame instead of loading a new frameset.
 // * interface/client_file/history/history.php: target change.
 // * interface/client_file/history/history_full.php: target changes.
 // * interface/client_file/history/history_save.php: target change.
 // * interface/client_file/history/encounters.php: link/target changes.
 // * interface/client_file/encounter/encounter_top.php: another new frameset
 //   cloned from client_encounter.php.
 // * interface/client_file/encounter/forms.php: link target removal.
 // * interface/forms/newclient/new.php, view.php, save.php: link/target
 //   changes.
 // * interface/client_file/summary/immunizations.php: removed back link.
 // * interface/client_file/summary/pnotes.php: changed link targets.
 // * interface/client_file/summary/pnotes_full.php: changed back link and
 //   added set_pid logic.
 // * interface/client_file/transaction/transactions.php: various changes.
 // * interface/client_file/transaction/add_transaction.php: new return js.
 // * interface/client_file/encounter/superbill_codes.php: target and link
 //   changes.
 // * interface/client_file/encounter/superbill_custom_full.php: target and
 //   link changes.
 // * interface/client_file/encounter/diagnosis.php: target changes.
 // * interface/client_file/encounter/diagnosis_full.php: target and link
 //   changes.
 // * interface/main/authorizations/authorizations.php: link and target changes.
 // * library/api.inc: url change.
 // * interface/client_file/summary/rx_frameset.php: new frameset.
 // * interface/client_file/summary/rx_left.php: new for prescriptions.
 // * all encounter forms: remove all instances of "target=Main" and change
 //   all instances of "client_encounter.php" to "encounter_top.php".

 // Our find_client form, when submitted, invokes client_select.php in the
 // upper frame. When the client is selected, demographics.php is invoked
 // with the set_pid parameter, which establishes the new session pid and also
 // calls the setClient() function (below).  In this case demographics.php
 ...
 // a new encounter.  interface/client_file/encounter/encounter_top.php
...
require_once($GLOBALS['fileroot']."/library/client.inc");
...
// * Usage: 0 = global, 1 = client-specific, 2 = encounter-specific, 3 = therapy group-specific, 4 = therapy group encounter - specific
// * The URL relative to the interface directory
//

$primary_docs = array(
'cal' => array(xl('Calendar')  , 0, 'main/main_info.php'),
'pfb' => array(xl('Client Flow Board')  , 0, '../interface/client_tracker/client_tracker.php?skip_timeout_reset=1'),
...
'aop' => array(xl('Portal Dashboard')  , 0, '../portal/client/provider'),
...
'sup' => array(xl('Superbill') , 0, 'client_file/encounter/superbill_custom_full.php'),
'aun' => array(xl('Authorizations'), 0, 'main/authorizations/authorizations.php'),
'new' => array(xl('New Pt')    , 0, 'new/new.php'),
'ped' => array(xl('Client Education'), 0, 'reports/client_edu_web_lookup.php'),
'lab' => array(xl('Check Lab Results')  , 0, 'orders/lab_exchange.php'),
'dem' => array(xl('Client')   , 1,  "client_file/summary/demographics.php"),
'his' => array(xl('History')   , 1, 'client_file/history/history.php'),
'ens' => array(xl('Visit History'), 1, 'client_file/history/encounters.php'),
'nen' => array(xl('Create Visit'), 1, 'forms/newclient/new.php?autoloaded=1&calenc='),
'pre' => array(xl('Rx')        , 1, 'client_file/summary/rx_frameset.php'),
'iss' => array(xl('Issues')    , 1, 'client_file/summary/stats_full.php?active=all'),
'imm' => array(xl('Immunize')  , 1, 'client_file/summary/immunizations.php'),
'doc' => array(xl('Documents') , 1, '../controller.php?document&list&client_id={PID}'),
'orp' => array(xl('Proc Pending Rev'), 1, 'orders/orders_results.php?review=1'),
'orr' => array(xl('Proc Res')  , 1, 'orders/orders_results.php'),
'lda' => array(xl('Lab overview')  , 1, 'client_file/summary/labdata.php'),
'tan' => array(xl('Configure Tracks')  , 0, 'forms/track_anything/create.php'),
'prp' => array(xl('Pt Report') , 1, 'client_file/report/client_report.php'),
'prq' => array(xl('Pt Rec Request') , 1, 'client_file/transaction/record_request.php'),
'pno' => array(xl('Pt Notes')  , 1, 'client_file/summary/pnotes.php'),
'tra' => array(xl('Transact')  , 1, 'client_file/transaction/transactions.php'),
'enc' => array(xl('Encounter') , 2, 'client_file/encounter/encounter_top.php'),
...
'pay' => array(xl('Payment') , 1, '../client_file/front_payment.php'),
...
if ($GLOBALS['use_charges_panel'] || $GLOBALS['menu_styling_vertical'] == 0) {
    $primary_docs['cod'] = array(xl('Charges'), 2, 'client_file/encounter/encounter_bottom.php');
}
...
if ($GLOBALS['enable_group_therapy']) {
    ...
    $primary_docs['gce'] = array(xl('Current') , 4, 'client_file/encounter/encounter_top.php');
    $primary_docs['gvh'] = array(xl('Visit History'), 3, 'client_file/history/encounters.php');
}
...
$disallowed['new'] = !(acl_check('clients', 'demo', '', array('write','addonly')));

$disallowed['fax'] = !(($GLOBALS['enable_hylafax'] || $GLOBALS['enable_scanner']) && acl_check('clients', 'docs'));

$disallowed['iss'] = !((acl_check('encounters', 'notes', '', 'write') ||
acl_check('encounters', 'notes_a', '', 'write') ) &&
acl_check('clients', 'med', '', 'write') );
...
$disallowed['pre'] = !(acl_check('clients', 'med'));
...
function genPopupsList($style = '')
{
    ...
    <?php if (!$disallowed['iss']) { ?>
     <option value='../client_file/problem_encounter.php'><?php echo xlt('Issues'); ?></option>
    <?php } ?>
    <?php if (!$GLOBALS['ippf_specific'] && acl_check('clients', 'demo')) { ?>
     <option value='../../custom/export_xml.php'><?php echo xlt('Export'); ?></option>
        <?php if (acl_check('clients', 'demo', '', 'write')) { ?>
     <option value='../../custom/import_xml.php'><?php echo xlt('Import'); ?></option>
    <?php }
    }

    if (!$GLOBALS['disable_calendar'] && acl_check('clients', 'appt')) { ?>
     <option value='../reports/appointments_report.php?client=<?php if (isset($pid)) {
            echo attr_url($pid);
                                                               } ?>'><?php echo xlt('Appts'); ?></option>
    <?php } ?>
        <?php if (acl_check('clients', 'med')) { ?>
     <option value='../client_file/printed_fee_sheet.php?fill=1'><?php echo xlt('Superbill'); ?></option>
    <?php } ?>
        <?php if (acl_check('acct', 'bill', '', 'write')) { ?>
     <option value='../client_file/front_payment.php'><?php echo xlt('Payment'); ?></option>
    <?php } ?>
        <?php if ($GLOBALS['inhouse_pharmacy'] && acl_check('acct', 'bill', '', 'write')) { ?>
     <option value='../client_file/pos_checkout.php'><?php echo xlt('Checkout'); ?></option>
    <?php } ?>
        <?php if (is_dir($GLOBALS['OE_SITE_DIR'] . "/documents/letter_templates") && acl_check('clients', 'med')) { ?>
     <option value='../client_file/letter.php'><?php echo xlt('Letter'); ?></option>
    <?php } ?>
        <?php if ($GLOBALS['chart_label_type'] != '0' && acl_check('clients', 'demo')) { ?>
    <option value='../client_file/label.php'><?php echo xlt('Chart Label'); ?></option>
    <?php } ?>
        <?php if ($GLOBALS['barcode_label_type'] != '0' && acl_check('clients', 'demo')) { ?>
    <option value='../client_file/barcode_label.php'><?php echo xlt('Barcode Label'); ?></option>
    <?php } ?>
        <?php if ($GLOBALS['addr_label_type'] && acl_check('clients', 'demo')) { ?>
    <option value='../client_file/addr_label.php'><?php echo xlt('Address Label'); ?></option>
    <?php } ?>
    </select>
        <?php
}
...
function genFindBlock()
{
  ...
  <td class='smalltext' colspan='2'>
   <input type="entry" size="7" name="client" class='inputtext' style='width:65px;' />
  </td>
 </tr>
 <tr>
      <td class='smalltext'><?php echo xlt('by') ?>:</td>
  <td class='smalltext'>
       <a href="javascript:findClient('Last');" class="navitem"><?php echo xlt('Name'); ?></a>
  </td>
  <td class='smalltext' align='right'>
       <a href="javascript:findClient('ID');"   class="navitem"><?php echo xlt('ID'); ?></a>
  </td>
 </tr>
 <tr>
  <td class='smalltext'>&nbsp;</td>
  <td class='smalltext'>
       <a href="javascript:findClient('SSN');"  class="navitem"><?php echo xlt('SSN'); ?></a>
  </td>
  <td class='smalltext' align='right'>
       <a href="javascript:findClient('DOB');"  class="navitem"><?php echo xlt('DOB'); ?></a>
  </td>
 </tr>
 <tr>
  <td class='smalltext'>&nbsp;</td>
  <td class='smalltext'>
       <a href="javascript:findClient('Any');"  class="navitem"><?php echo xlt('Any'); ?></a>
  </td>
  <td class='smalltext' align='right'>
 ...
 function loadFrame2(fname, frame, url) {
  ...
  if (active_pid == 0 && active_gid == 0  && (usage > '0' && usage < '5')){
    <?php if ($GLOBALS['enable_group_therapy']) { ?>
      alert(<?php echo xlj('You must first select or add a client or therapy group.'); ?>);
    <?php } else { ?>
      alert(<?php echo xlj('You must first select or add a client.'); ?>);
    <?php } ?>
   return false;
  }
...
function loadNewForm(formname, formdesc) {
  var url = '<?php echo "$rootdir/client_file/encounter/load_form.php?formname=" ?>' + encodeURIComponent(formname);
  if (parent.RBot.twAddFrameTab) {
    ...
    loadFrame2('enc2','RBot','client_file/encounter/encounter_top.php' +
 ...
 // depending on whether there is an active client or encounter.
 function syncRadios() {
  var f = document.forms[0];
...
//Function to clear active client and encounter in the server side
function clearactive() {
    top.restoreSession();
    //Ajax call to clear active client in session
 ...
 // Process the click to find a client by name, id, ssn or dob.
 function findClient(findby) {
  var f = document.forms[0];
  if (! f.cb_top.checked) {
 ...
 // This is called automatically when a new client is set, to make sure
 // there are no client-specific documents showing stale data.  If a frame
 // was just loaded with data for the correct client, its name is passed so
 ...
 // document for the new encounter will not work, so load client info instead.
 ...
 // Call this to announce that the client has changed.  You must call this
 // if you change the session PID, so that the navigation frame will show the
 // correct client and so that the other frame will be reloaded if it contains
 // client-specific information from the previous client.  frname is the name
 // of the frame that the call came from, so we know to only reload content
 // from the *other* frame if it is client-specific.
 function setClient(pname, pid, pubpid, frname, str_dob) {
  clearTherapyGroup();
  var str = '<a href=\'javascript:;\' onclick="parent.left_nav.loadCurrentClientFromTitle()" title="PID = ' + pid + '"><b>' + pname + ' (' + pubpid + ')<br /></b></a>';
  setDivContent('current_client', str);
  setTitleContent('current_client', str + str_dob);
  if (pid == active_pid) return;
  setDivContent('current_encounter', '<b>' + <?php echo xlj('None'); ?> + '</b>');
  active_pid = pid;
  active_encounter = 0;
  encounter_locked = false;
  if (frname) reloadClient(frname);
  syncRadios();
  $(parent.Title.document.getElementById('current_client_block')).show();
  ...
  $(parent.Title.document.getElementById('clear_active')).show();//To display Clear Active Client button on selecting a client
  $(parent.Title.document.getElementById('clear_active_group')).hide();//To hide Clear Active group button on selecting a client
 ...
 function setTherapyGroup(group_id, group_name) {
     ...
     $(parent.Title.document.querySelector('#current_client_block span.text')).hide();
     setTitleContent('current_client', '<span>' + <?php echo xlj('Therapy Group');?> + ' - <a href=\'javascript:;\' onclick="parent.left_nav.loadCurrentGroupFromTitle(' + group_id +')">' + group_name + ' (' + group_id + ')<a></span>' );
    ...
    setDivContent('current_encounter', '<b>' + <?php echo xlj('None'); ?> + '</b>');
     ...
     $(parent.Title.document.getElementById('clear_active_group')).show();//To display Clear Active group button on selecting a client
     $(parent.Title.document.getElementById('clear_active')).hide();//To hide Clear Active Client button on selecting a client
 }

 function setClientEncounter(EncounterIdArray,EncounterDateArray,CalendarCategoryArray) {
 //This function lists all encounters of the client.
 //This function writes the drop down in the top frame.
 //It is called when a new client is create/selected from the search menu.
...
<?php if (acl_check_form('newclient', '', array('write', 'addonly'))) { ?>
...
function loadCurrentClientFromTitle() {
    top.restoreSession();
    top.frames['RTop'].location='../client_file/summary/demographics.php';
}
...
function isEncounterLocked( encounterId ) {
 ...
 function loadCurrentEncounterFromTitle() {
      top.restoreSession();
      top.frames[ parent.left_nav.getEncounterTargetFrame('enc') ].location='../client_file/encounter/encounter_top.php';
 }

 // You must call this if you delete the active client (or if for any other
 // reason you "close" the active client without opening a new one), so that
 ...
 function clearClient() {
  ...
  active_encounter = 0;
  encounter_locked = false;
  setDivContent('current_client', '<b>' + <?php echo xlj('None'); ?> + '</b>');
  $(parent.Title.document.getElementById('current_client_block')).hide();
...
{//Removes an item from the Encounter drop down.
 ...
 function pidSanityCheck(pid) {
  if (pid != active_pid) {
   alert('Session client ID is ' + pid + ', expecting ' + active_pid +
...
<form method='post' name='find_client' target='RTop'
 action='<?php echo $rootdir ?>/main/finder/client_select.php'>
...
if (!$GLOBALS['disable_pat_trkr'] && !$GLOBALS['disable_calendar'] && acl_check('clients', 'appt')) {
    genTreeLink('RTop', 'pfb', xl('Flow Board'));
}
if (!$GLOBALS['disable_rcb'] && !$GLOBALS['disable_calendar'] && acl_check('clients', 'appt')) {
    genTreeLink('RBot', 'rcb', xl('Recall Board'));
}
if (acl_check('clients', 'notes')) {
    genTreeLink('RBot', 'msg', xl('Messages'));
}

if ($GLOBALS['portal_offsite_enable'] && $GLOBALS['portal_offsite_address'] && acl_check('clientportal', 'portal')) {
    genTreeLink('RTop', 'app', xl('Portal Activity'));
}

if ($GLOBALS['portal_onsite_two_enable'] && acl_check('clientportal', 'portal')) {
    genTreeLink('RTop', 'aop', xl('Portal Dashboard'));
}

if ($GLOBALS['gbl_portal_cms_enable'] && acl_check('clientportal', 'portal')) {
    genPopLink(xl('CMS Portal'), '../cmsportal/list_requests.php', 'ppo0');
}
?>

<?php if (acl_check('clients', 'demo') || acl_check('clients', 'appt') || acl_check_form('newclient', '', array('write', 'addonly')) || acl_check('clients', 'med')) { ?>
  <li class="open"><a class="collapsed" id="patimg" ><i class="fa fa-fw fa-user fa-2x"></i>&nbsp;<span><?php echo xlt('Client/Client') ?></span></a>
    <ul>
        <?php if (acl_check('clients', 'demo')) {
            genMiscLink('RTop', 'fin', '0', xl('Clients'), 'main/finder/dynamic_finder.php');
        } ?>
        <?php if (acl_check('clients', 'demo', '', 'write') || acl_check('clients', 'demo', '', 'addonly')) {
            genTreeLink('RTop', 'new', ($GLOBALS['full_new_client_form'] ? xl('New/Search') : xl('New')));
        } ?>
        <?php if (acl_check('clients', 'demo')) {
            genTreeLink('RTop', 'dem', xl('Summary'));
        } ?>

    <?php if (acl_check('clients', 'appt') || acl_check_form('newclient', '', array('write', 'addonly'))) { ?>
      <li class="open"><a class="expanded_lv2"><span><?php echo xlt('Visits') ?></span></a>
        <ul>
            <?php if (acl_check_form('newclient', '', array('write', 'addonly'))) {
                genTreeLink('RBot', 'nen', xl('Create Visit'));
            } ?>
            <?php if (acl_check('clients', 'appt')) {
    ...
    <?php if (acl_check('clients', 'med')) { ?>
      <li><a class="collapsed_lv2"><span><?php echo xlt('Records') ?></span></a>
        <ul>
            <?php genTreeLink('RTop', 'prq', xl('Client Record Request')); ?>
        </ul>
      </li>
    <?php } ?>

    <?php if ($GLOBALS['gbl_nav_visit_forms'] && acl_check('clients', 'demo')) { ?>
    ...
    //
      ...
      <li class="collapsed" ><a class="collapsed_lv2"><span><?php echo xlt('Import') ?></span></a>
        <ul>
            <?php genMiscLink('RTop', 'ccr', '0', xlt('Upload'), 'client_file/ccr_import.php'); ?>
            <?php genMiscLink('RTop', 'apr', '0', xlt('Pending Approval'), 'client_file/ccr_pending_approval.php'); ?>
        </ul>
      </li>
...
<?php } ?>
  ...
  <li><a class="collapsed" id="feeimg" ><span><?php echo xlt('Fees') ?></span></a>
    <ul>
        ...
        <?php if (acl_check('acct', 'bill', '', 'write')) {
            genMiscLink('RBot', 'pay', '1', xl('Payment'), 'client_file/front_payment.php');
        } ?>
        <?php if (acl_check('acct', 'bill', '', 'write')) {
            genMiscLink('RBot', 'bil', '1', xl('Checkout'), 'client_file/pos_checkout.php?framed=1');
        } ?>
...
<?php if (acl_check('clients', 'lab')) { ?>
  <li><a class="collapsed" id="proimg" ><span><?php echo xlt('Procedures') ?></span></a>
    <ul>
        ...
        <?php genTreeLink('RTop', 'orr', xl('Client Results')); ?>
...
<?php } ?>
    ...
    <?php
    if ($GLOBALS['erx_enable'] && acl_check('clients', 'rx')) {
  ...
  <li><a class="collapsed" id="admimg" ><span><?php echo xlt('Administration') ?></span></a>
    <ul>
      ...
      // Changed the target URL from practice settings -> Practice Settings - Pharmacy... Dec 09,09 .. Visolve ... This replaces empty frame with Pharmacy window
        ...
        <?php if (acl_check('admin', 'super') && $GLOBALS['enable_cdr']) {
            genMiscLink('RTop', 'adm', '0', xl('Client Reminders'), 'client_file/reminder/client_reminders.php?mode=admin&client_id=');
        } ?>
      ...
      <li><a class="collapsed_lv2"><span><?php echo xlt('Other') ?></span></a>
        <ul>
            ...
            <?php if (acl_check('admin', 'super')) {
                genMiscLink('RTop', 'adm', '0', xl('Merge Clients'), 'client_file/merge_clients.php');
            } ?>
            <?php if (acl_check('admin', 'super')) {
  ...
  <li><a class="collapsed" id="repimg" ><span><?php echo xlt('Reports') ?></span></a>
    <ul>
        ...
        <?php if (acl_check('clients', 'demo') || acl_check('clients', 'med') ||
        (acl_check('clients', 'rx') && !$GLOBALS['disable_prescriptions'])) { ?>
      <li><a class="collapsed_lv2"><span><?php echo xlt('Clients') ?></span></a>
        <ul>
        <?php if (acl_check('clients', 'demo')) {
            genMiscLink('RTop', 'rep', '0', xl('List'), 'reports/client_list.php');
        } ?>
    <?php if (acl_check('clients', 'rx') && !$GLOBALS['disable_prescriptions']) {
        genMiscLink('RTop', 'rep', '0', xl('Rx'), 'reports/prescriptions_report.php');
    } ?>
    <?php if (acl_check('clients', 'med')) {
        genMiscLink('RTop', 'rep', '0', xl('Client List Creation'), 'reports/client_list_creation.php');
    } ?>
    <?php if (acl_check('clients', 'med')) {
    ...
    <?php } ?>
      ...
      <li><a class="collapsed_lv2"><span><?php echo xlt('Clinic') ?></span></a>
        <ul>
            <?php if (($GLOBALS['enable_cdr'] || $GLOBALS['enable_cqm']  || $GLOBALS['enable_amc']) && acl_check('clients', 'med')) {
                genMiscLink('RTop', 'rep', '0', xl('Report Results'), 'reports/report_results.php');
            } ?>
            <?php if ($GLOBALS['enable_cdr'] && acl_check('clients', 'med')) {
                genMiscLink('RTop', 'rep', '0', xl('Standard Measures'), 'reports/cqm.php?type=standard');
            } ?>
            <?php if ($GLOBALS['enable_cqm'] && acl_check('clients', 'med')) {
                genMiscLink('RTop', 'rep', '0', xl('Quality Measures (CQM)'), 'reports/cqm.php?type=cqm');
            } ?>
            <?php if ($GLOBALS['enable_amc'] && acl_check('clients', 'med')) {
                genMiscLink('RTop', 'rep', '0', xl('Automated Measures (AMC)'), 'reports/cqm.php?type=amc');
            } ?>
            <?php if ($GLOBALS['enable_amc_tracking'] && acl_check('clients', 'med')) {
                genMiscLink('RTop', 'rep', '0', xl('AMC Tracking'), 'reports/amc_tracking.php');
            } ?>
            <?php if ($GLOBALS['enable_cdr'] && $GLOBALS['enable_alert_log'] && acl_check('clients', 'med')) {
      ...
      <li><a class="collapsed_lv2"><span><?php echo xlt('Visits') ?></span></a>
        <ul>
            ...
            <?php if (!$GLOBALS['disable_calendar'] && acl_check('clients', 'appt')) {
                genMiscLink('RTop', 'rep', '0', xl('Appointments'), 'reports/appointments_report.php');
            } ?>
            <?php if (!$GLOBALS['disable_pat_trkr'] && !$GLOBALS['disable_calendar'] && acl_check('clients', 'appt')) {
                genMiscLink('RTop', 'rep', '0', xl('Client Flow Board'), 'reports/client_flow_board_report.php');
            } ?>
            ...
            <?php if (acl_check('clients', 'demo')) {
                ...
                <?php if (!$GLOBALS['disable_chart_tracker'] && acl_check('clients', 'appt')) {
...
<?php if (!$GLOBALS['simplified_demographics'] && (acl_check('acct', 'rep_a') || acl_check('clients', 'demo'))) { ?>
      <li><a class="collapsed_lv2"><span><?php echo xlt('Insurance') ?></span></a>
        <ul>
            ...
            <?php if (acl_check('acct', 'rep_a')) {
                genMiscLink('RTop', 'rep', '0', xl('Indigents'), 'billing/indigent_clients_report.php');
            } ?>
            <?php if (acl_check('clients', 'demo')) {
                genMiscLink('RTop', 'rep', '0', xl('Unique SP'), 'reports/unique_seen_clients_report.php');
            } ?>
        </ul>
      </li>
<?php } ?>
...
<?php
    ...
    genPopLink(xl('Demographics'), '../client_file/summary/demographics_print.php');
    genPopLink(xl('Superbill/Fee Sheet'), '../client_file/printed_fee_sheet.php');
    // genPopLink(xl('Referral'),'../client_file/transaction/print_referral.php');
    echo "        </ul></li>\n";
...
?>
  ...
  <li><a class="collapsed" id="misimg" ><span><?php echo xlt('Miscellaneous') ?></span></a>
    <ul>
        <?php genTreeLink('RTop', 'ped', xl('Client Education')); ?>
        <?php if (acl_check('encounters', 'auth')) {
            genTreeLink('RBot', 'aun', xl('Authorizations'));
        } ?>
        <?php if (acl_check('clients', 'docs')) {
        ...
        <?php if (acl_check('clients', 'docs', '', 'write') || acl_check('clients', 'docs', '', 'addonly')) {
            genMiscLink('RTop', 'adm', '0', xl('New Documents'), '../controller.php?document&list&client_id=00');
        } ?>
        <?php if (acl_check('admin', 'practice')) {
...
<div id='current_client' style = 'display:none'>
