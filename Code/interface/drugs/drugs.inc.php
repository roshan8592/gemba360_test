  ...
  // Get relevant options for this product.
    ...
    if (!$dispensable) {
        ...
        $sale_id = sqlInsert(
            ...
            "?, 0, ?, ?, ?, ?, ?, ?, ?)",
            array($drug_id, $prescription_id, $client_id, $encounter_id, $user, $sale_date, $quantity, $fee)
        );
        return $sale_id;
    }
  ...
  // if there is enough quantity on hand and check for lots to be destroyed.
    ...
    if ($bad_lot_list) {
        send_drug_email(
            ...
            "order for client $client_id: $bad_lot_list\n"
        );
    }
      ...
      if ($qty_left > 0) {
        ...
        *****************************************************************/
        ...
        $sale_id = sqlInsert(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            array($drug_id, $inventory_id, $prescription_id, $client_id, $encounter_id, $user,
            $sale_date,
            $thisqty,
            $thisfee,
            $pricelevel,
