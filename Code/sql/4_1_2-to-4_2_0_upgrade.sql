...
#IfMissingColumn client_data email_direct
ALTER TABLE `client_data` ADD COLUMN `email_direct` varchar(255) NOT NULL default '';
...
#IfMissingColumn client_data cmsportal_login
ALTER TABLE `client_data` ADD COLUMN `cmsportal_login` varchar(60) NOT NULL default '';
INSERT INTO `layout_options` (`form_id`, `field_id`, `group_name`, `title`, `seq`, `data_type`, `uor`, `fld_length`, `max_length`, `list_id`, `titlecols`, `datacols`, `default_value`, `edit_options`, `description`, `fld_rows`) VALUES
  ('DEM', 'cmsportal_login', '3Choices', 'CMS Portal Login', 15, 2, 1, 30, 60, '', 1, 1, '', '', 'Login ID for the CMS Client Portal', 0);
#EndIf
...
ALTER TABLE `clinical_rules` ADD COLUMN `amc_2011_flag` tinyint(1) COMMENT '2011 Automated Measure Calculation flag for (unable to customize per client)';
UPDATE `clinical_rules` SET `amc_2011_flag` = 1 WHERE `amc_flag` = 1;
#EndIf

#IfMissingColumn clinical_rules amc_2014_flag
ALTER TABLE `clinical_rules` ADD COLUMN `amc_2014_flag` tinyint(1) COMMENT '2014 Automated Measure Calculation flag for (unable to customize per client)';
#EndIf
...
ALTER TABLE `clinical_rules` ADD COLUMN `cqm_2011_flag` tinyint(1) COMMENT '2011 Clinical Quality Measure flag (unable to customize per client)';
UPDATE `clinical_rules` SET `cqm_2011_flag` = 1 WHERE `cqm_flag` = 1;
#EndIf

#IfMissingColumn clinical_rules cqm_2014_flag
ALTER TABLE `clinical_rules` ADD COLUMN `cqm_2014_flag` tinyint(1) COMMENT '2014 Clinical Quality Measure flag (unable to customize per client)';
#EndIf

#IfMissingColumn clinical_plans cqm_2011_flag
ALTER TABLE `clinical_plans` ADD COLUMN `cqm_2011_flag` tinyint(1) COMMENT '2011 Clinical Quality Measure flag (unable to customize per client)';
UPDATE `clinical_plans` SET `cqm_2011_flag` = 1 WHERE `cqm_flag` = 1;
#EndIf

#IfMissingColumn clinical_plans cqm_2014_flag
ALTER TABLE `clinical_plans` ADD COLUMN `cqm_2014_flag` tinyint(1) COMMENT '2014 Clinical Quality Measure flag (unable to customize per client)';
#EndIf
...
#IfNotRow2D list_options list_id lists option_id amendment_from	
	...
	INSERT INTO `list_options` ( `list_id`, `option_id`, `title`, `seq`, `is_default` ) VALUES
	('amendment_from' ,'client','Client', 10, 0),
...
#IfNotTable amendments
	CREATE TABLE `amendments` (
		...
		`pid`			int(11)			NOT NULL	COMMENT 'Client ID from client_data',
