...
class Userforms extends UserAudit
{
    ...
    {
        if (UserService::valid($data[0])=='existingclient') {
            global $ISSUE_TYPES;
    ...
    private function getIncudes($val)
    {
        ...
        if ($val == "demographics") {
            ...
            <?php
            // printRecDataOne($client_data_array, getRecClientData ($pid), $N);
            $result1 = getClientData($pid);
            $result2 = getEmployerData($pid);
            ?>
            <table>
            <tr><td><h6><?php echo htmlspecialchars(xl('Client Data').":", ENT_QUOTES);?></h6></td></tr>
            <?php
        ...
        <table>
            <tr><td width='400' class='bold'><?php echo htmlspecialchars(xl('Code'), ENT_QUOTES);?></td><td class='bold'><?php echo htmlspecialchars(xl('Fee'), ENT_QUOTES);?></td></tr>
                ...
                $total = 0.00;
                $copays = 0.00;
                foreach ($ar['newclient'] as $be) {
                    $ta = split(":", $be);
                    $billing = getClientBillingEncounter($pid, $ta[1]);
                    $billings[] = $billing;
        ...
        //print_r($billings);
            ...
            <h6><?php echo htmlspecialchars(xl('Client Immunization').":", ENT_QUOTES);?></h6>
            <?php
            $sql = "select i1.immunization_id as immunization_id, if(i1.administered_date,concat(i1.administered_date,' - ') ,substring(i1.note,1,20) ) as immunization_data from immunizations i1 where i1.client_id = ? order by administered_date desc";
            ...
            <h6><?php htmlspecialchars(xl('Client Communication sent').":", ENT_QUOTES);?></h6>
            <?php
            $sql="SELECT concat( 'Messsage Type: ', batchcom.msg_type, ', Message Subject: ', batchcom.msg_subject, ', Sent on:', batchcom.msg_date_sent ) AS batchcom_data, batchcom.msg_text, concat( users.fname, users.lname ) AS user_name FROM `batchcom` JOIN `users` ON users.id = batchcom.sent_by WHERE batchcom.client_id=?";
            $result = sqlStatement($sql, array($pid));
            while ($row=sqlFetchArray($result)) {
            ...
            <h6><?php echo htmlspecialchars(xl('Client Notes').":", ENT_QUOTES);?></h6>
            <?php
            printClientNotes($pid);
            ?>
        </div>
            <?php
            ...
            <hr />
            <div class='text transactions'>
            <h6><?php echo htmlspecialchars(xl('Client Transactions').":", ENT_QUOTES);?></h6>
            <?php
            printClientTransactions($pid);
            ?>
        </div>
            <?php
        }
    ...
    public function ccdaFetching($data)
    {
        ...
        if (UserService::valid($data[0])=='existingclient') {
            if ($this->checkModuleInstalled($moduleName = 'Carecoordination')) {
                ...
                try {
                    $event = isset($data['event']) ? $data['event'] : 'client-record';
                    $menu_item = isset($data['menu_item']) ? $data['menu_item'] : 'Dashboard';
                    EventAuditLogger::instance()->newEvent($event, 1, '', 1, '', $pid, $log_from = 'client-portal', $menu_item);
                } catch (Exception $e) {
                }
			...
			<note>
			<heading>WARNING!</heading>
			<body>Existing client checking failed!</body>
			</note>';
        }
    ...
    */
    public function getEventLog($data)
    {
        ...
        if (UserService::valid($data[0])=='existingclient') {
            ...
            $arrBinds = array();
            $cols = "DISTINCT log.date, event, user, groupname, client_id, success, comments,checksum,crt_user";
            $sql = "SELECT $cols, CONCAT(fname, ' ', lname) as client_ful_name, client_portal_menu.`menu_name`,
            client_portal_menu_group.`menu_group_name`, ccda_doc_id FROM log
			JOIN client_data ON log.client_id = client_data.pid
			JOIN client_access_offsite ON log.client_id = client_access_offsite.pid
			JOIN client_portal_menu ON client_portal_menu.`client_portal_menu_id` = log.menu_item_id
			JOIN client_portal_menu_group ON client_portal_menu_group.`client_portal_menu_group_id` = client_portal_menu.`client_portal_menu_group_id`
			WHERE log.date >= ? AND log.date <= ?";

            $sql .= " AND log_from = 'client-portal'";
            $sql .= " AND client_id = ?";
     ...
     * @param string requested_by user | client
    ...
    function transmitCCD($data = array())
    {
        ...
        if (UserService::valid($data[0])=='existingclient') {
            try {
                ...
                global $pid;
                //get client name in Last_First format (used for CCDA filename) and
                //First Last for the message text.
                $clientData = getClientPID(array("pid"=>$pid));
                if (empty($clientData[0]['lname'])) {
                    $att_filename = "";
                    $clientName2 = "";
                } else {
                   //spaces are the argument delimiter for the phiMail API calls and must be removed
                    ...
                    $att_filename = " " .
                    str_replace(" ", "_", $xml_type . "_" . $clientData[0]['lname']
                    . "_" . $clientData[0]['fname']) . "." . $extension;
                    $clientName2 = $clientData[0]['fname'] . " " . $clientData[0]['lname'];
                }
                ...
                if ($requested_by=="client") {
                    $text_out = xl("Delivery of the attached clinical document was requested by the client") .
                    ($clientName2=="" ? "." : ", " . $clientName2 . ".");
                } else {
                    $text_out = xl("A clinical document is attached") .
                     ($clientName2=="" ? "." : " " . xl("for client") . " " . $clientName2 . ".");
                }
            ...
            //            $sql="INSERT INTO direct_message_log (msg_type,msg_id,sender,recipient,status,status_ts,client_id,user_id) " .
            //             "VALUES ('S', ?, ?, ?, 'S', NOW(), ?, ?)";
