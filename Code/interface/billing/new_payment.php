...
$hidden_client_code     = isset($_REQUEST['hidden_client_code']) ? $_REQUEST['hidden_client_code'] : '';
$default_search_client  = isset($_POST['default_search_client']) ? $_POST['default_search_client'] : '';
...
if ($mode == "new_payment" || $mode == "distribute") {
    if (trim($_POST['type_name'])=='insurance') {
        $QueryPart="payer_id = '" . add_escape_custom($hidden_type_code) . "', client_id = '0" ;
    } elseif (trim($_POST['type_name'])=='client') {
        $QueryPart="payer_id = '0', client_id = '" .add_escape_custom($hidden_type_code);
    }
...
<head>
    ...
    {//Used in the cancel button.Helpful while cancelling the distribution.
       if(confirm(<?php echo xlj('Would you like to Cancel Distribution for this Client?') ?>))
        {
           document.getElementById('hidden_client_code').value='';
    ...
    {//Used in saving the allocation
       ...
       if(!CheckPayingEntityAndDistributionPostFor())//Ensures that Insurance payment is distributed under Ins1,Ins2,Ins3 and Client paymentat under Pat.
        {
         return false;
        }
    ...
    {//Displays message after saving to master table.
     ...
     if(after_value=='distribute')
      ...
      {
       if(document.getElementById('TableClientPortion'))
        {
           document.getElementById('TableClientPortion').style.display='none';
        }
       if(confirm(<?php echo xlj('Successfully Saved.Would you like to Allocate?'); ?>))
        {
           if(document.getElementById('TableClientPortion'))
            {
               document.getElementById('TableClientPortion').style.display='';
            }
        }
      }
    ...
    {//Resets form used in the 'Cancel Changes' button in the master screen.
     ...
     document.getElementById('div_insurance_or_client').innerHTML='&nbsp;';
                ...
                if ($payment_id*1==0) {
                    ...
                    <fieldset>
                        ...
                        if ($payment_id*1>0) {
                            ?>
                            <?php
                            if ($PaymentType=='client' && $default_search_client != "default_search_client") {
                                $default_search_client = "default_search_client";
                                $_POST['default_search_client'] = $default_search_client;
                                $hidden_client_code=$TypeCode;
                                $_REQUEST['hidden_client_code']=$hidden_client_code;
                                $_REQUEST['RadioPaid']='Show_Paid';
                            }
                                require_once("payment_pat_sel.inc.php"); //Client ajax section and listing of charges.
                            ?>
                            <?php
                    ...
                    <input id="hidden_client_code" name="hidden_client_code" type="hidden" value="<?php echo attr($hidden_client_code);?>"> <input id='mode' name='mode' type='hidden' value=''>
                    <input id='default_search_client' name='default_search_client' type='hidden' value='<?php echo attr($default_search_client); ?>'>
