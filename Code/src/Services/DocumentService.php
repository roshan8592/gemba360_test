...
class DocumentService
{
    ...
    public function insertAtPath($pid, $path, $fileData)
    {
        ...
        if ($GLOBALS['secure_upload'] && !isWhiteFile($fileData["tmp_name"])) {
            error_log("OpenEMR API Error: Attempt to upload unsecure client document was declined");
            return false;
        }

        // Ensure category exists
        if (!$this->isValidPath($path)) {
            error_log("OpenEMR API Error: Attempt to upload client document to category that did not exist was declined");
            return false;
        }
        ...
        if (empty($file)) {
            error_log("OpenEMR API Error: Client document was empty, so declined request");
            return false;
        }
        ...
        if (!empty($ret)) {
            error_log("OpenEMR API Error: There was an error in attempt to upload a client document");
            return false;
        }
    ...
    public function getFile($pid, $did)
    {
        ...
        if (empty($document)) {
            error_log("OpenEMR API Error: Requested client document was empty, so declined request");
            return false;
        }
