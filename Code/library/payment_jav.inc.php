...
function PayingEntityAction()
 {
  //Which ajax is to be active(client,insurance), is decided by the 'Paying Entity' drop down, where this function is called.
  ...
  if(document.getElementById('ajax_div_insurance'))
   {
    ...
    if(document.getElementById('type_name').options[document.getElementById('type_name').selectedIndex].value=='client')
     {
      document.getElementById('adjustment_code').value='client_payment';
     }
    ...
    if (newValue=='client') {
        ...
        if(document.getElementById('option_client_payment'))
            document.getElementById('option_client_payment').style.backgroundColor='#ffffff';
    }
    if (newValue=='insurance') {
        if(document.getElementById('option_family_payment'))
            document.getElementById('option_family_payment').style.backgroundColor='#DEDEDE';
        if(document.getElementById('option_client_payment'))
            document.getElementById('option_client_payment').style.backgroundColor='#DEDEDE';
        if(document.getElementById('option_insurance_payment'))
...
function FilterSelection(listSelected) {
    ...
    if (newValue=='insurance') {
        ...
        if(newValueSelected=='client_payment')
            listSelected.value=ValueToPut;//Putting values back
    }
}
...
function ConvertToUpperCase(ObjectPassed)
 ...
 //'Non Paid', 'Show Primary Complete', 'Show All Transactions' uses this when a client is selected through ajax.
    if(document.getElementById('hidden_client_code').value*1>0)
     {
     ...
     {
        alert("<?php echo htmlspecialchars(xl('Please Select a Client.'), ENT_QUOTES) ?>")
     }
 }
...
function ScreenAdjustment(PassedObject,CountIndex)
 ...
 //It moves to secondary or client balance.
...
function CheckPayingEntityAndDistributionPostFor()
 {//Ensures that Insurance payment is distributed under Ins1,Ins2,Ins3 and Client paymentat under Pat.
    ...
    for(RowCount=CountIndexAbove+1;;RowCount++)
     {
      ...
      else if(document.getElementById('Allowed'+RowCount).value=='' && document.getElementById('Payment'+RowCount).value=='' && document.getElementById('AdjAmount'+RowCount).value=='' && document.getElementById('Deductible'+RowCount).value=='' && document.getElementById('Takeback'+RowCount).value=='' && document.getElementById('FollowUp'+RowCount).checked==false)
      {
     ...
     else
       {
        ...
        if(PayingEntity=='client' && InsPatDropDownValue>0)
         {
          alert("<?php echo htmlspecialchars(xl('Cannot Post for Insurance.The Paying Entity selected is Client.'), ENT_QUOTES) ?>");
          return false;
         }
        else if(PayingEntity=='insurance' && InsPatDropDownValue==0)
         {
          alert("<?php echo htmlspecialchars(xl('Cannot Post for Client.The Paying Entity selected is Insurance.'), ENT_QUOTES) ?>");
          return false;
         }
       }
     }
...
function FormValidations()
 {//Screen validations are done here.
  ...
  if(document.getElementById('hidden_type_code').value!=document.getElementById('div_insurance_or_client').innerHTML)
   {
