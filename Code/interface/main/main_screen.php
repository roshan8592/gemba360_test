...
///////////////////////////////////////////////////////////////////////
    ...
    // (allows creations of separate OpenEMR frames to view clients concurrently
...
if ($is_expired) {
  //display the php file containing the password expiration message.
    ...
    $clientID = 0 + $_POST['clientID'];
    if (empty($_POST['encounterID'])) {
        // Open client summary screen (without a specific encounter)
        $frame1url = "../client_file/summary/demographics.php?set_pid=" . attr_url($clientID);
        $frame1target = "pat";
    } else {
        // Open client summary screen with a specific encounter
        $encounterID = 0 + $_POST['encounterID'];
        $frame1url = "../client_file/summary/demographics.php?set_pid=" . attr_url($clientID) . "&set_encounterid=" . attr_url($encounterID);
        $frame1target = "pat";
    }
    ...
    $map_paths_to_targets = array(
        ...
        '../../interface/client_tracker/client_tracker.php?skip_timeout_reset=1' => ('flb'),
...
// from the Client Flow Board or the Client Finder.
window.opener = null;
