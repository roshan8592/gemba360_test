...
class Allergy extends ClinicalType
{
    ...
    /*
     *  Check to see if a client had an allergy to THIS thing between $beginDate and $endDate
     *  $beginDate and $endDate can be the same, indicating a check for allergy on particular date
     *
     *  @param  (RsClient) $client    Client to check
    ...
    public function doClientCheck(RsClient $client, $beginDate = null, $endDate = null, $options = null)
    {
        ...
        foreach ($data as $codeType => $codes) {
            foreach ($codes as $code) {
                if (exist_lists_item($client->id, $type, $codeType.'::'.$code, $endDate)) {
                    return true;
                }
            }
        }
