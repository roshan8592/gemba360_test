...
require_once("$srcdir/clientvalidation.inc.php");
...
$result2 = getEmployerData($pid);
 ...
 // Check authorization.
if ($pid) {
    // Create and fire the client demographics update event
...
<title><?php echo xlt('Edit Current Client'); ?></title>
...
<?php  
  $sqlArray = array();
  //getting document ID based on client ID from categories_to_document table
  $sql = "SELECT c.id, c.name, c.aco_spec, d.id AS document_id, d.type, d.url, d.docdate"
            ...
            . " AND d.foreign_id = ? ";
  array_push($sqlArray, attr($result['id']));

  $document_result = sqlQuery($sql, $sqlArray);

  $document_id = $document_result['document_id'];  
?>

<!-- Cutome functions and implementation for profile pic view and upload -->
<script type="text/javascript">
  $(document).ready(function(){
    // retrieving and storing client's document ID
    var document_id = "<?php echo $document_id; ?>";

    // retrieving and storing client_ID
    var client_id = "<?php echo attr($result['id']); ?>";

    // retrieving and storing application's context
    var context = "<?php echo $web_root;?>";

    // Creating Upload profile pic button by clicking on which would replace image sections with input file upload
    var changeProfilePicBtn = "<button type='button'>Update Profile Pic</button>";
    
    /* setting the client's profile pic image in the place of profile pic text field section. Here profile pic is fetched 
    *  from controller.php's act method, passing the respective parameter to controller.php. 
    *  Also if no profile image is uploaded previously, setting default user image as profile image using "onerror" event trigger.
    */
    $("#value_id_text_profile_pic").html("<img src='"+context+"/controller.php?document&retrieve&client_id="+client_id+"&document_id=-1&as_file=false&original_file=true&disable_exit=false&show_original=true&context=client_picture' style='float: left;padding: 5px;border: 1px solid #bbb;border-radius: 5px;margin: 0 10px 10px 10px;height:130px;' onerror='this.src = \"/openemr-5.0.2/public/images/client-picture-default.png\"'>"+changeProfilePicBtn);

    //set input type file on change profile pic button click
    $("#value_id_text_profile_pic button").on("click",function(){
        upload_file(client_id,'value_id_text_profile_pic', document_id);
    });

  });

  /*
  *  To replace the client profile pic image section with input file upload field and also to add hidden inputs relative to
  *  upload the newly added profile pic image.
  */
  function upload_file(client_id, tdID, document_id){
    var file_element = "<input type='file' name='file[]' class='form-control' style='margin-bottom: 8px;' id='form_profile_pic'><input type='hidden' name='MAX_FILE_SIZE' value='64000000'><input type='hidden' name='category_id' value='10'><input type='hidden' name='process' value='true'><input type='hidden' name='document_id' value='"+document_id+"'><input type='hidden' name='destination' title='Leave Blank To Keep Original Filename' id='destination-name'><input type='hidden' name='encrypted' title='Check the box if this is an encrypted file' id='encrypted'><input type='hidden' name='passphrase' title='Pass phrase to decrypt document' id='passphrase'>";

    //appending the input file element as well as set of hidden elements to profile pic TD tag
    $("#"+tdID).html(file_element);

    //setting 'enctype' attribute in order to support form action for file upload
    $("#DEM").attr("enctype","multipart/form-data");

    //opening the file browser of the local machine in order to choose the profile pic file to upload by triggering click event on input type file.
    $("#form_profile_pic").click();
  }
</script>
<!-- End custom function and implementation -->
        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="page-header">
                    <h2><?php echo xlt('Edit Current Client');?></h2>
                </div>
            </div>
            <div class="col-xs-12">
                <div class="btn-group">
...
<?php if ($set_pid) { ?>
 parent.left_nav.setClient(<?php echo js_escape($result['fname'] . " " . $result['lname']) . "," . js_escape($pid) . "," . js_escape($result['pubpid']) . ",''," . js_escape(" " . xl('DOB') . ": " . oeFormatShortDate($result['DOB_YMD']) . " " . xl('Age') . ": " . getClientAgeDisplay($result['DOB_YMD'])); ?>);
<?php } ?>
...
//This code deals with demographics before save action -
    <?php if (($GLOBALS['gbl_edit_client_form'] == '1') && (checkIfClientValidationHookIsActive())) :?>
                //Use the Zend client validation hook.
                //TODO - get the edit part of client validation hook to work smoothly and then
                //       remove the closeBeforeOpening=1 in the url below.

        var f = $("form");

        // Use hook to open the controller and get the new client validation .
        // when no params are sent this window will be closed from the zend controller.
        var url ='<?php echo  $GLOBALS['web_root']."/interface/modules/zend_modules/public/clientvalidation";?>';
