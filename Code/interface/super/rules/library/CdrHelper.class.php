...
class CdrResults
{
    ...
    function __construct($rule_id = "", $active_alert_flag = "", $passive_alert_flag = "", $client_reminder_flag = "", $access_control = "")
    {
        $this->id = $rule_id;
                $this->rule = getLabel($this->id, 'clinical_rules');
        $this->active_flag = $active_alert_flag;
        $this->passive_flag = $passive_alert_flag;
        $this->reminder_flag = $client_reminder_flag;
    ...
    function update_table()
    {
           ...
           sqlStatement($query, array($this->active_flag,$this->passive_flag,$this->reminder_flag,$this->id));
                    ...
                    // Set the settings that apply to all rules including the client custom rules (pid is > 0)
