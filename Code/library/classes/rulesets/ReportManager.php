...
class ReportManager
{
    ...
    public function runReport($rowRule, $clients, $dateTarget, $options = array())
    {
        $ruleId = $rowRule['id'];
        $clientData = array();
        foreach ($clients as $client) {
            $clientData []= $client['pid'];
        }
        ...
        if ($reportFactory instanceof  RsReportFactoryAbstract) {
            $report = $reportFactory->createReport(ReportTypes::getClassName($ruleId), $rowRule, $clientData, $dateTarget, $options);
        }
