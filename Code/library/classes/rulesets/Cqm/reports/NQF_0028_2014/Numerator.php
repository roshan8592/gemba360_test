...
class NQF_0028_2014_Numerator implements CqmFilterIF
{
    ...
    public function test(CqmClient $client, $beginDate, $endDate)
    {
        ...
        foreach ($this->getApplicableEncounters() as $encType) {
            $dates = Helper::fetchEncounterDates($encType, $client, $beginDate, $endDate);
            foreach ($dates as $date) {
                ...
                // this is basically a check to see if the client is an reported as an active smoker on their last encounter
                if (Helper::check(ClinicalType::CHARACTERISTIC, Characteristic::TOBACCO_USER, $client, $beginMinus24Months, $date)) {
                    //return true;
                    // encounters time stamp is always 00:00:00, so change it to 23:59:59 or 00:00:00 as applicable
                    $date = date("Y-m-d H:i:s", strtotime('+1 days', strtotime($date)));
                    $date = date('Y-m-d 23:59:59', strtotime($date));
                    $beginMinus24Months = strtotime('-24 month', strtotime($date));
                    $beginMinus24Months = date('Y-m-d 00:00:00', $beginMinus24Months);
                    $smoke_cess = sqlQuery("SELECT * FROM `rule_client_data` " .
                                           "WHERE `category`='act_cat_inter' AND `item`='act_tobacco' AND `complete`='YES' " .
                                           "AND `pid`=? AND `date`>=? AND `date`<=?", array($client->id,$beginMinus24Months,$date));
                    // this is basically a check to see if the client's action has occurred in the two years previous to encounter.
                    // TODO: how to check for the smoking cessation medication types (can also just be a smoking cessation order, ie. prescription)
                    if (!(empty($smoke_cess)) ||
                         Helper::checkMed(Medication::SMOKING_CESSATION, $client, $beginMinus24Months, $date) ||
                         Helper::checkMed(Medication::SMOKING_CESSATION_ORDER, $client, $beginMinus24Months, $date) ) {
                        return true;
                    }
                } else if (Helper::check(ClinicalType::CHARACTERISTIC, Characteristic::TOBACCO_NON_USER, $client, $beginMinus24Months, $date)) {
                    return false;
                } else {
