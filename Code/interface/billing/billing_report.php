...
<script>
    ...
    function toencounter(pid, pubpid, pname, enc, datestr, dobstr) {
        top.restoreSession();
        encurl = 'client_file/encounter/encounter_top.php?set_encounter=' + encodeURIComponent(enc) +
            '&pid=' + encodeURIComponent(pid);
        parent.left_nav.setClient(pname, pid, pubpid, '', dobstr);
    ...
    // Process a click to go to an client.
    function toclient(pid, pubpid, pname, enc, datestr, dobstr) {
        top.restoreSession();
        paturl = 'client_file/summary/demographics_full.php?pid=' + encodeURIComponent(pid);
        parent.left_nav.setClient(pname, pid, pubpid, '', dobstr);
        <?php if ($GLOBALS['new_tabs_layout']) { ?>
        parent.left_nav.loadFrame('ens1', 'enc',
            'client_file/history/encounters.php?pid=' + encodeURIComponent(pid));
    ...
    function popMBO(pid, enc, mboid) {
        ...
        let href = "<?php echo $GLOBALS['web_root']?>/interface/client_file/encounter/view_form.php?formname=misc_billing_options&isBilling=1" + qstring;
                    ...
                    if ($ret = getBillsBetween("%")) {
                        ...
                        foreach ($ret as $iter) {
                            ...
                            if ($last_encounter_id != $this_encounter_id) {
                                ...
                                $name = getClientData($iter['enc_pid'], "fname, mname, lname, pubpid, billing_note, DATE_FORMAT(DOB,'%Y-%m-%d') as DOB_YMD");

                                // Check if client has primary insurance and a subscriber exists for it.
                            ...
                            Count++;
                                ...
                                window.toencounter(" . attr_js($iter['enc_pid']) . "," . attr_js($name['pubpid']) . "," . attr_js($ptname) . "," . attr_js($iter['enc_encounter']) . "," . attr_js(oeFormatShortDate($raw_encounter_date)) . "," . attr_js(" ".xl('DOB').": ".oeFormatShortDate($name['DOB_YMD'])." ".xl('Age').": ".getClientAge($name['DOB_YMD'])) . ");
                                top.window.parent.left_nav.setClientEncounter(EncounterIdArray[" . attr($iter['enc_pid']) . "],EncounterDateArray[" . attr($iter['enc_pid']) . "], CalendarCategoryArray[" . attr($iter['enc_pid']) . "]);
                            ...
                            \">" . xlt('Encounter') . " " . text(oeFormatShortDate($raw_encounter_date)) . "</a>";
                                ...
                                $lhtml .= "<a class=\"btn btn-xs btn-default\" role=\"button\" " . "href=\"javascript:window.toclient(" . attr_js($iter['enc_pid']) . "," . attr_js($name['pubpid']) . "," . attr_js($ptname) . "," . attr_js($iter['enc_encounter']) . "," . attr_js(oeFormatShortDate($raw_encounter_date)) . "," . attr_js(" ".xl('DOB').": ".oeFormatShortDate($name['DOB_YMD'])." ".xl('Age').": ".getClientAge($name['DOB_YMD'])) . ");
                        top.window.parent.left_nav.setClientEncounter(EncounterIdArray[" . attr($iter['enc_pid']) . "],EncounterDateArray[" . attr($iter['enc_pid']) . "], CalendarCategoryArray[" . attr($iter['enc_pid']) . "])\">" . xlt('Client') . "</a>";
                                ...
                                if ($iter['id']) {
                                    ...
                                    $query = "SELECT * FROM claims WHERE client_id = ? AND encounter_id = ? ORDER BY version";
                                    $cres = sqlStatement(
                                        $query,
                                        array(
                                        $iter['enc_pid'],
                            ...
                            if ($last_encounter_id != $this_encounter_id) {
                                ...
                                $resMoneyGot = sqlStatement(
                                    "SELECT pay_amount AS ClientPay,date(post_time) AS date FROM ar_activity WHERE " .
                                ...
                                while ($rowMoneyGot = sqlFetchArray($resMoneyGot)) {
                                    ...
                                    if ($ClientPay > 0) {
                                        ...
                                        $rhtml2 .= "</td>\n";
                                        $rhtml2 .= "<td><span class='text'>" . text(oeFormatMoney($ClientPay)) . "</span><span style='font-size:8pt;'>&nbsp;</span></td>\n";
