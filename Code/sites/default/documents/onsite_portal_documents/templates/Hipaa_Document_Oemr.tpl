...
When using this medical information registration in front of clients the User should use the "Privacy" feature to hide PHI (Personal Health Information) for other clients in the Search screen. 
We have developed and will use standard operating procedures (SOPs) requiring any use of the Export Clients Medical or other information to be documented. 
...
Client Name: 	{ClientName}  Sex: {ClientSex} <em>hereby signs and agree to the terms of this agreement .</em>
Our external ID:{ClientID}		
Born: 		{ClientDOB}  
Home Address: 	{Address}   
Zip: 		{Zip}; City: {City}; State: {State}
Home Phone: 	{ClientPhone}
		Client Signature:{ClientSignature}
		Client:{ClientName} Date: {DOS}

<em>I do not accept these terms: </em>{CheckMark}
Client refusal to sign due to the following reason: {TextInput}
