...
require_once($GLOBALS['srcdir'].'/client_tracker.inc.php');
require_once($GLOBALS['incdir']."/main/holidays/Holidays_Controller.php");
require_once($GLOBALS['srcdir'].'/group.inc');

 //Check access control
if (!acl_check('clients', 'appt', '', array('write','wsome'))) {
...
<?php if ($group_disabled) {
    ...
    function DOBandEncounter($pc_eid)
    {
         ...
         $client_dob = trim($_POST['form_dob']);
        ...
        if ($client_dob && $_POST['form_pid']) {
            sqlStatement("UPDATE client_data SET DOB = ? WHERE " .
                                    "pid = ?", array($client_dob,$_POST['form_pid']));
        }
        ...
        if (!empty($_POST['form_pid'])) {
            ...
            if ((int)$GLOBALS['auto_create_new_encounters']
                ...
                # Capture the appt status and room number for client tracker. This will map the encounter to it also.
                ...
                # Capture the appt status and room number for client tracker.
                if (!empty($_GET['eid'])) {
...
//
    ...
    if ($_POST['form_action'] != "") {
        // Close this window and refresh the calendar (or the client_tracker) display.
        ...
        echo " if (opener && !opener.closed && opener.refreshme) {\n " .
         ...
         "  window.opener.pattrk.submit()\n " . // This is for client flow board page refresh}
          " }};\n";
        echo " dlgclose();\n";
        echo "</script>\n</body>\n</html>\n";
        exit();
...
*********************************************************************/
 ...
 // If we have a client ID, get the name and phone numbers to display.
    if ($clientid) {
        $prow = sqlQuery("SELECT lname, fname, phone_home, phone_biz, DOB " .
         "FROM client_data WHERE pid = ?", array($clientid));
        $clientname = $prow['lname'] . ", " . $prow['fname'];
        if ($prow['phone_home']) {
            $clienttitle['phone_home'] = xl("Home Phone").": " . $prow['phone_home'];
        }

        if ($prow['phone_biz']) {
            $clienttitle['phone_biz'] = xl("Work Phone").": " . $prow['phone_biz'];
        }
    }
...
<?php require($GLOBALS['srcdir'] . "/restoreSession.php"); ?>
 ...
 function sel_client() {
  let title = '<?php echo xlt('Client Search'); ?>';
  dlgopen('find_client_popup.php', 'findClient', 650, 300, '', title);
 }
...
<!-- ViSolve : Requirement - Redirect to Create New Client Page -->
  ...
  <td nowrap id='tdallday5'>
   <input class='input-sm'    type='text' size='4' name='form_duration' value='<?php echo attr($thisduration) ?>' title='<?php echo xla('Event duration in minutes'); ?>' />
    ...
    if ($_GET['prov']!=true && $_GET['group']!=true) {
        ?>
    <tr id="client_details">
     <td nowrap>
         <b><?php echo xlt('Client'); ?>:</b>
     </td>
     <td nowrap>
      <input class='input-sm'    type='text' size='10' name='form_client' id="form_client" style='width:100%;cursor:pointer;cursor:hand' placeholder='<?php echo xla('Click to select');?>' value='<?php echo is_null($clientname) ? '' : attr($clientname); ?>' onclick='sel_client()' title='<?php echo xla('Click to select client'); ?>' readonly />
      <input class='input-sm'    type='hidden' name='form_pid' value='<?php echo attr($clientid) ?>' />
     </td>
     <td colspan='3' nowrap style='font-size:8pt'>
      <span class="infobox">
        <?php
        foreach ($clienttitle as $value) {
            if ($value != "") {
                echo text(trim($value));
            }

            if (count($clienttitle) > 1) {
                echo "<br />";
            }
        }
        ?>
...
?>
    ...
    <tr>
     ...
     <td colspan='4' nowrap>
        <?php
        echo generate_select_list('form_room', 'client_flow_board_rooms', $pcroom, xl('Room Number'));
        ?>
     </td>
    </tr>
        <?php
...
<?php
 ...
 // in case the client-select popup is used.
 $client_dob = trim($prow['DOB']);
 $is_group = $groupname;
 $dobstyle = ($prow && (!$client_dob || substr($client_dob, 5) == '00-00') && !$is_group) ?
  '' : 'none';
?>
