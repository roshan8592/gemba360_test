...
if ($formid && !$visitid) {
    ...
    if ($frow['pid'] != $pid) {
        die("Internal error: client ID mismatch!");
    }
}
...
if (!empty($_POST['bn_save']) || !empty($_POST['bn_save_print']) || !empty($_POST['bn_save_continue'])) {
    ...
    while ($frow = sqlFetchArray($fres)) {
        ...
        if ($source == 'D' || $source == 'H') {
            // Save to client_data, employer_data or history_data.
            if ($source == 'H') {
                ...
                $esc_field_id = escape_sql_column_name($field_id, array('client_data'));
                sqlStatement(
                    "UPDATE client_data SET `$esc_field_id` = ? WHERE pid = ?",
                    array($value, $pid)
                );
            }
...
<div class='container'>
    ...
    if (!$from_trend_form) {
        ...
        "fe.date FROM " .
        "form_encounter AS fe, forms AS f, client_data AS p WHERE " .
        "p.pid = ? AND f.pid = p.pid AND f.encounter = ? AND " .
        "f.formdir = 'newclient' AND f.deleted = 0 AND " .
        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="page-header">
                    ...
                    // If appropriate build a drop-down selector of issues of this type for this client.
                        ...
                        if (!$from_issue_form) {
                            ...
                            if ($form_is_graphable) {
                                ...
                                <button type='button' class="btn btn-default btn-graph"
                                        onclick="top.restoreSession();location='../../client_file/encounter/trend_form.php?formname=<?php echo attr_url($formname); ?>'">
...
<?php } ?>
    ...
    <script language="JavaScript">
        ...
        // New form and this client has a portal login and we have not loaded portal data.
        // Check if there is portal data pending for this client and form type.
        if (!$alertmsg && !$formid && $GLOBALS['gbl_portal_cms_enable'] && $cmsportal_login && !$portalid) {
            $portalres = cms_portal_call(array('action' => 'checkptform', 'form' => $formname, 'client' => $cmsportal_login));
            if ($portalres['errmsg']) {
            ...
            if ($portalid) {
                echo "if (confirm(" . xlj('The portal has data for this client and form. Load it now?') . ")) {\n";
