...
function show_PRIOR_CANVAS_section(section, newValue) {
    var pid    =  $('#pid').val();
    var zone   = section;
    var result = base+'/controller.php?document&retrieve&client_id='+pid+'&document_id='+newValue+'&as_file=false';
...
function Suggest_visit_code() {
    //assume Eyes and established client
...
function openImage() {
    dlgopen(base+'/controller.php?document&retrieve&client_id=3&document_id=10&as_file=false', '_blank', 600, 475);
}

// Called to open a document in another tab for this encounter.
function openDocumentNewTab(doc_id) {
    var url = '../../interface/client_file/encounter/view_form.php?formname=' + formdir + '&id=' + formid;
...
/**
 *      Function to update the client's current pharmacy
 */

function  update_Pharma() {
    //$(#form_pharmacy_id) has changed value, update the client_data field pharmacy_id
...
function showpnotes(docid) {
    if (top.tab_mode) {
        let btnClose = 'Done';
        let url = base+'/interface/client_file/summary/pnotes.php?docid=' + docid;
