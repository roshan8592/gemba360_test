<?php /* Smarty version 2.6.31, created on 2019-09-19 14:29:05
...
<script language='JavaScript'>
    ...
    if(isset($_SESSION['pid']))
        {
            if($_SESSION['pid']>0)
                {
                    echo "+'&clientid=" . attr_url($_SESSION['pid']) . "'";
                }
        }
     ?>
   ,'_blank', 775, 500);
 ...
 function goPid(pid) {
  top.restoreSession();
<?php 
  		echo "  top.RTop.location = '../../client_file/summary/demographics.php' " .
...
foreach ($providers as $provider) {
    ...
    foreach ($A_EVENTS as $date => $events) {
        ...
        foreach ($events as $event) {
            ...
            $pccattype = 'true';
            $clientid = $event['pid'];
            $commapos = strpos($event['client_name'], ",");
            $lname = substr($event['client_name'], 0, $commapos);
	        $fname = substr($event['client_name'], $commapos + 2);
            $client_dob = $event['client_dob'];
            $client_age = $event['client_age'];
            $catid = $event['catid'];
            $comment = $event['hometext'];
            $catname = $event['catname'];
            $title = "Age $client_age ($client_dob)";
            ...
            if ($catid == 2 || $catid == 3 || $catid == 4 || $catid == 8 || $catid == 11) {
                ...
                // some sort of client appointment
                if($groupid)
                ...
                if ($clientid) {
                    ...
                    $link_title .= xl('Age') . ": " . $client_age . "\n" . xl('DOB') . ": " . $client_dob . $comment . "\n";
                    $link_title .= "(" . xl('Click to view') . ")";

                    $content .= "<a href='javascript:goPid(" . attr_js($clientid) . ")' title='" . attr($link_title) . "'>";
                    $content .= "<img src='{$this->_tpl_vars['TPL_IMAGE_PATH']}/user-green.gif'
                                      onmouseover=\"javascript:ShowImage(" . attr_js($GLOBALS['webroot']."/controller.php?document&retrieve&client_id=".attr($clientid)."&document_id=-1&as_file=false&original_file=true&disable_exit=false&show_original=true&context=client_picture") . ");\"
                                      onmouseout=\"javascript:HideImage();\"
                                      border='0' title='" . attr($link_title) . "' alt='" . xla('View Client') . "' />";
                ...
                elseif($groupid){
                    ...
                    $content .= "<img src='{$this->_tpl_vars['TPL_IMAGE_PATH']}/user-blue.gif' border='0' title='" . attr($link_title) . "' alt='" . xla('View Client') . "' />";
                    if ($catid == 1) $content .= "<strike>";
                    ...
                    //Add class to wrapping div so EditEvent js function can differentiate between click on group and client
                    $evtClass .= ' groups ';
                }
                else {
                    ...
                    if ( $catid ==6  || $catid == 7){
                        ...
                        // no client id, just output the category name
