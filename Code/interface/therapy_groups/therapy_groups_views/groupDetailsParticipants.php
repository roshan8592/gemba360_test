                                ...
                                <form  id="add-participant-form" name="add-participant-form" class="<?php echo isset($addStatus) ? 'showAddForm' : '' ?>" action="<?php echo $GLOBALS['rootdir'] . '/therapy_groups/index.php?method=addParticipant&group_id=' . attr_url($groupId)?>" method="post">
                                    ...
                                    <div class="col-md-12">
                                        <div class="row">
                                            ...
                                            <div class="col-md-5">
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-8">
                                                        <input type="text" id="group_client_start" name="group_client_start" class="full-width datepicker"  value="<?php echo !is_null($participant_data) ? attr(oeFormatShortDate($participant_data['group_client_start'])): oeFormatShortDate(date('Y-m-d'));?>">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            ...
                                            <div class="col-md-8">
                                                <input type="text" id="group_client_comment" name="group_client_comment" value="<?php echo !is_null($participant_data) ? attr($participant_data['group_client_comment']): ''?>" class="full-width">
                                            </div>
                                        </div>
                                        <div class="row">
                                    ...
                                    <form id="updateParticipants" method="post">
                                        ...
                                        <table  id="participants_table" class="dataTable display">
                                            ...
                                            <tr>
                                                ...
                                                <th><?php echo xlt("Client's number"); ?></th>
                                            ...
                                            <?php foreach ($participants as $i => $participant) : ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <select name="group_client_status[]" <?php echo $readonly; ?>>
                                                            <?php foreach ($statuses as $key => $status) : ?>
                                                                <option value="<?php echo attr($key);?>" <?php if ($key == $participant['group_client_status']) {
                                                                    echo 'selected';
                                                                               } ?> > <?php echo text($status); ?> </option>
                                                            <?php endforeach; ?>
                                                        </select>
                                                    </td>
                                                    <td><input type="text" name="group_client_start[]" id="start-date<?php echo $i+1?>" class="datepicker"  value="<?php echo attr(oeFormatShortDate($participant['group_client_start']));?>" <?php echo $readonly; ?>></td>
                                                    <td><input type="text" name="group_client_end[]" id="end-date<?php echo $i+1?>" class="datepicker" value="<?php echo $participant['group_client_end'] == '0000-00-00' ? '' : attr(oeFormatShortDate($participant['group_client_end'])) ;?>" <?php echo $readonly; ?>></td>
                                                    <td><input type="text" name="group_client_comment[]" class="full-width"  value="<?php echo attr($participant['group_client_comment']);?>" <?php echo $readonly; ?> /></td>
                                                    <?php if ($readonly == '') : ?>
                                                        <td class="delete_btn">
...
<script>
    $(function(){
        ...
        $('#cancelAddParticipant').on('click', function(e){
            ...
            $('#group_client_comment').val('');
            $('#group_client_start').val('<?php echo date('Y-m-d');?>');
        });

        $('#participant_name').on('click', function(){
            top.restoreSession();
            var url = <?php echo js_escape($GLOBALS['webroot']); ?> + '/interface/main/calendar/find_client_popup.php';
   ...
   // parent.left_nav.setTherapyGroup(<?php echo attr_js($group_id);?>,<?php echo attr_js('test'); ?>);
    ...
    ?>
    top.window.parent.left_nav.setClientEncounter(EncounterIdArray,EncounterDateArray,CalendarCategoryArray);
