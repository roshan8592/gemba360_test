...
// $patdata = getClientData($pid, 'fname,lname,pubpid');

$patdata = sqlQuery("SELECT " .
    "p.fname, p.mname, p.lname, p.pubpid,p.pid, i.copay " .
    "FROM client_data AS p " .
...
function toencounter(enc, datestr, topframe) {
    ...
    if(!top.tab_mode) {
        ...
        topframe.left_nav.loadFrame('enc2', 'RBot', 'client_file/encounter/encounter_top.php?set_encounter=' + encodeURIComponent(enc));
    } else {
        ...
        for (var i = 0; i < f.elements.length; ++i) {
            ...
            if (ename.indexOf('form_upay[0') == 0) //Today is this text box.
            {
                if (elem.value * 1 > 0) {//A warning message, if the amount is posted with out encounter.
                    if (confirm(<?php echo xlj('If client has appointment click OK to create encounter otherwise, cancel this and then create an encounter for today visit.'); ?>)) {
                        ok = 1;
                    } else {
                        elem.focus();
                        return false;
...
<title><?php echo xlt('Record Payment'); ?></title>
    ...
    'include_client_name' => true,// use only in appropriate pages
    'expandable' => false,
                   ...
                   <input name='form_pid' type='hidden' value='<?php echo attr($pid) ?>'>
                    ...
                    <legend><?php echo xlt('Payment'); ?></legend>
                        ...
                        <div class="col-xs-12 oe-custom-line">
                            ...
                            <div class="col-xs-3">
                                <div id="ajax_div_client" style="display:none;"></div>
                                <input type='text'  id="check_number" name='form_source' class= 'form-control' value='<?php echo attr($payrow['source']); ?>'>
                            </div>
                        </div>
                        <div class="col-xs-12 oe-custom-line">
                            <div class="col-xs-3 col-lg-offset-3">
                                <label class="control-label" for="form_discount"><?php echo xla('Client Coverage'); ?>:</label>
                            </div>
                            <div class="col-xs-6">
                            ...
                            <table class = "table" id="table_display">
                                <thead>
                                    <tr bgcolor="#CCCCCC" id="tr_head">
                                        ...
                                        <td align="center" class="dehead" id="td_head_client_payment" width="80"><?php echo xlt('Client Payment'); ?></td>
                                        <td align="center" class="dehead" id="td_head_client_co_pay" width="55"><?php echo xlt('Co Pay Paid'); ?></td>
                                        ...
                                        <td align="center" class="dehead" id="td_head_client_balance" width="80"><?php echo xlt('Client Balance'); ?></td>
