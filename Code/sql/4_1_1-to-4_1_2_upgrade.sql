...
#IfMissingColumn documents_legal_detail dld_client_comments
ALTER TABLE `documents_legal_detail` ADD COLUMN `dld_client_comments` text COMMENT 'Client comments stored here';
#EndIf

#IfMissingColumn documents_legal_master dlm_upload_type
ALTER TABLE `documents_legal_master` ADD COLUMN `dlm_upload_type` tinyint(4) DEFAULT '0' COMMENT '0-Provider Uploaded,1-Client Uploaded';
#EndIf
...
INSERT INTO `users` (username,password,lname,authorized,active) 
  VALUES ('portal-user','NoLogin','Client Portal User',0,0);
#EndIf
...
#IfMissingColumn client_access_onsite portal_salt
ALTER TABLE `client_access_onsite` ADD COLUMN `portal_salt` VARCHAR(100) NULL;
#Endif
