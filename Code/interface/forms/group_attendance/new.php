...
if ($form_id) {//If editing a form or the form already exists (inwhich case will automatically go into edit mode for existing form)
    $participants_sql =  "SELECT tgpa.*, p.fname, p.lname " .
        ...
        "JOIN client_data as p ON tgpa.pid = p.id " .
        "WHERE tgpa.form_id = ?;";
...
<?php } ?>
    <div id="add_participant">
        ...
        <div id="add_participant_element"  style="display: none;">
            <div class="client_wrap">
                ...
                <input name="new_id" class="button-css new_client_id" type="hidden" value="">
                <input name="new_client" class="button-css new_client" type="text" value=""  readonly>
            ...
            <div class="button_wrap">
                <input class="button-css add_client_button" type="button" value="<?php echo xla('Add Client'); ?>">
    ...
    <table id="group_attendance_form_table">
        ...
        <tr>
            ...
            <th align="center"><?php echo xlt("Client's number"); ?></th>
        ...
        <?php foreach ($participants as $participant) {?>
            <tr>
                ...
                <td >
                    <select class="status_select" name="<?php echo "clientData[" . attr($participant['pid']) . "][status]" ;?>" <?php if (!$can_edit) {
                        ?> disabled <?php
                                                        } ?> >
                        <?php foreach ($statuses_in_meeting as $status_in_meeting) {?>
                            <option value="<?php echo attr($status_in_meeting['option_id']); ?>" <?php if ($participant['meeting_client_status'] == $status_in_meeting['option_id']) {
                                echo 'selected';
                ...
                <td >
                    <input class="comment" type="text" name="<?php echo "clientData[" . attr($participant['pid']) . "][comment]";  ?>" value="<?php echo attr($participant['meeting_client_comment']) ;?>" <?php if (!$can_edit) {
                        ?> disabled <?php
                                                             } ?> ></input>
                </td>
            </tr>
...
<script>
    $(function () {
        ...
        $('.new_client').on('click', function(){
            top.restoreSession();
            $('.new_client').css("border-color", "black");
            $('.error_wrap .error').html("");
            var url = '<?php echo $GLOBALS['webroot']?>/interface/main/calendar/find_client_popup.php';
        ...
        $('.add_client_button').click(function(e){
            var name = $('.new_client').val();

            if(name == ""){
                //If no client was chosen (validation)
                $('.new_client').css("border-color", "red");
                var err_msg = <?php echo xlj("Choose Client"); ?>;
                $('.error_wrap .error').html(err_msg);
            }
            else{

                // Get new participant details
                var new_client_id = $('.new_client_id').val();
                var new_client_name = $('.new_client').val();
                var new_client_comment = $('.new_comment').val();

                //Check if client already exists in form
                var ids_array = [];
                $('#group_attendance_form_table tbody tr td:nth-child(2)').each(function(){
                    ids_array.push($(this).text());
                });
                var exists = $.inArray(new_client_id, ids_array);
                if(exists >= 0){
                    $('.new_client').css("border-color", "red");
                    var err_msg = <?php echo xlj("Client already in form"); ?>;
                    $('.error_wrap .error').html(err_msg);
                    return;
                }
                ...
                var attended_sign = '@';
                select_element.attr('name', 'clientData[' + new_client_id + '][status]');
                ...
                comment_element.attr('name', 'clientData[' + new_client_id + '][comment]');
                comment_element.attr('value', new_client_comment);
    ...
    /* For client popup search */
    function setclient(pid, lname, fname, dob){
        $('.new_client_id').val(pid);
        $('.new_client').val(fname + " " + lname);
    }
