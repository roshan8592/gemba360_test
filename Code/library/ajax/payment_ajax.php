<?php
/**
 * This section handles ajax for insurance,client and for encounters.
 *
 * @package   OpenEMR
...
//===============================================================================
    if ($_POST["ajax_mode"] == "set_client") {//client.
    //From 2 areas this ajax is called.So 2 pairs of functions are used.
        ...
        if (isset($_POST['client_code']) && $_POST['client_code']!='') {
            $client_code=trim((isset($_POST['client_code']) ? $_POST['client_code'] : ''));
            if (isset($_POST['submit_or_simple_type']) && $_POST['submit_or_simple_type']=='Simple') {
                $StringToAppend="PutTheValuesClickClient";
                $StringToAppend2="PlaceValuesClient";
            } else {
                $StringToAppend="PutTheValuesClickDistribute";
                $StringToAppend2="PlaceValuesDistribute";
            }

            $client_code_complete=$_POST['client_code'];//we need the spaces here
        } elseif (isset($_POST['insurance_text_ajax']) && $_POST['insurance_text_ajax']!='') {
            $client_code=trim((isset($_POST['insurance_text_ajax']) ? $_POST['insurance_text_ajax'] : ''));
            $StringToAppend="PutTheValuesClick";
            $StringToAppend2="PlaceValues";
            $client_code_complete=$_POST['insurance_text_ajax'];//we need the spaces here
        }

        $CountIndex=1;
        $StringForAjax="<div id='AjaxContainerClient'><table width='452' border='1' cellspacing='0' cellpadding='0'>
	  <tr class='text' bgcolor='#dddddd'>
		<td width='50'>" . xlt('Code') . "</td>
	  ...
	  ";
        $res = sqlStatement(
            "SELECT pid as id,fname,lname,mname,DOB FROM client_data
        ...
        $StringForAjax.="</table></div>";
        echo text(strlen($client_code_complete)).'~`~`'.$StringForAjax;
        die;
    }
...
//===============================================================================
    ...
    //PlaceValuesEncounter==>Used while -->KEY PRESS<-- over list.List vanishes and the clicked one gets listed in the parent page's text box.
        ...
        if (isset($_POST['encounter_client_code'])) {
            $client_code=trim((isset($_POST['encounter_client_code']) ? $_POST['encounter_client_code'] : ''));
	  ...
	  ";
        $res = sqlStatement("SELECT date,encounter FROM form_encounter
			 where pid =? ORDER BY encounter", array($client_code));
        while ($row = sqlFetchArray($res)) {
            if ($CountIndex%2==1) {
                $bgcolor='#ddddff';
            } else {
