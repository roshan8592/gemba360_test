...
// If a performing organization is provided, make a note for it also.
    ...
    // Get the LOINC code if one exists in the compendium for this order type.
        if (empty($GLOBALS['CLIENT_REPORT_ACTIVE'])) {
            $trow = sqlQuery(
...
// If this starts a new report or a new order, generate the report fields.
    ...
    if ($result_code !== '' || $result_document_id) {
        ...
        if (empty($GLOBALS['CLIENT_REPORT_ACTIVE']) && !empty($result_code)) {
    ...
    //
        if ($result_document_id) {
            ...
            if (empty($GLOBALS['CLIENT_REPORT_ACTIVE'])) {
                ...
                echo "&retrieve&client_id=" . attr_url($client_id) . "&document_id=" . attr_url($result_document_id) . "' ";
...
// Check authorization.
    ...
    $orow = sqlQuery(
        ...
        "po.order_status, po.specimen_type, po.client_id, po.order_diagnosis, " .
        ...
        "LEFT JOIN client_data AS pd ON pd.pid = po.client_id " .
        "LEFT JOIN procedure_providers AS pp ON pp.ppid = po.lab_id " .
        "LEFT JOIN users AS u ON u.id = po.provider_id " .
        "LEFT JOIN form_encounter AS fe ON fe.pid = po.client_id AND fe.encounter = po.encounter_id " .
...
<?php } // end if input form
    ...
    <?php if (empty($GLOBALS['CLIENT_REPORT_ACTIVE'])) { ?>
<script language="JavaScript">
    ...
    // Called to show client notes related to this order in the "other" frame.
    ...
    function showpnotes(orderid) {
        ...
        if (!top.tab_mode) {
            ...
            w.parent.left_nav.loadFrame('pno1', othername, 'client_file/summary/pnotes_full.php?orderid=' + <?php echo js_url($orderid); ?>);
        } else {
            let url = top.webroot_url + '/interface/client_file/summary/pnotes_full.php?orderid=' + <?php echo js_url($orderid); ?>;
            dlgopen(url, 'notes', 950, 750, false, '');
        }
        return false;
    }

    // Process click on LOINC code for client education popup.
    function educlick(codetype, codevalue) {
        dlgopen(top.webroot_url + '/interface/client_file/education.php' +
...
<form method='post' action='single_order_results.php?orderid=<?php echo attr_url($orderid); ?>'>
    ...
    <div class='labres'>
        ...
        <table width='100%' cellpadding='2' cellspacing='0'>
            <tr>
                <td class="td-label" width='5%' nowrap><?php echo xlt('Client ID'); ?></td>
            ...
            <tr>
                <td class="td-label" nowrap><?php echo xlt('Client Name'); ?></td>
        ...
        <table width='100%' style='border-width:0px;'>
            <tr>
                ...
                <td style='border-width:0px;' align='right' valign='top'>
                    ...
                    <?php if (empty($GLOBALS['CLIENT_REPORT_ACTIVE'])) { ?>
                        &nbsp;
                        <input type='button' value='<?php echo xla('Related Client Notes'); ?>'
    onclick='showpnotes(<?php echo attr_js($orderid); ?>)' />
                    ...
                    <?php if ($input_form && $ctx['sign_list']) { ?>
                        ...
                        <?php
                        // If this is a portal client, sending them a copy is an option.
