...
use OpenEMR\FHIR\R4\FHIRDomainResource\FHIRClient;
...
class FhirResourcesService
{
    ...
    public function createClientResource($resourceId = '', $data = '', $encode = true)
    {
        ...
        $clientResource = new FHIRClient($initResource);
        //$clientResource->setId($id);
        $clientResource->setActive(true);
        $clientResource->setGender($gender);
        $clientResource->addName($name);
        $clientResource->addAddress($address);

        if ($encode) {
            return json_encode($clientResource);
        } else {
    ...
    public function createEncounterResource($eid = '', $data = '', $encode = true)
    {
        ...
        $resource->setSubject(['reference' => "Client/$pid"]);
