...
class RuleManager
{
     ...
     * clinical_rules. An optional client id parameter allows you to get the
     * rules specific to the client.
     *
     * Returns null if no rule is found matching the id or client.
     * @param <type> $id
     * @param <type> $pid
     * @return Rule
     */
    ...
    private function fillRuleTypes($rule, $ruleResult)
    {
        ...
        if ($ruleResult['client_reminder_flag'] == 1) {
            $rule->addRuleType(RuleType::from(RuleType::ClientReminder));
        }
    }
    ...
    function updateSummary($ruleId, $types, $title, $developer, $funding, $release, $web_ref)
    {
        ...
        if (is_null($rule)) {
            ...
            sqlStatement(
                "INSERT INTO clinical_rules (id, pid, active_alert_flag, passive_alert_flag, cqm_flag, amc_flag, client_reminder_flag, developer, funding_source, release_version, web_reference ) " .
                    "VALUES (?,?,?,?,?,?,?,?,?,?,?) ",
                array(
                        $ruleId,
                        0,
                        ...
                        in_array(RuleType::ClientReminder, $types) ? 1 : 0,
                        $developer,
                        $funding,
                        $release,
                                                $web_ref
