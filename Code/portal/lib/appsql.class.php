...
class ApplicationTable
{
    ...
    public function getPortalAudit($clientid, $action = 'review', $activity = 'profile', $status = 'waiting', $auditflg = 1, $rtn = 'last', $oelog = true, $error = true)
    {
        $return = false;
        $result = false;
        $audit = array (
        ...
        try {
            $sql = "Select * From onsite_portal_activity As pa Where  pa.client_id = ? And  pa.activity = ? And  pa.require_audit = ? ".
     ...
     * Hoping to work both ends, client and user, from one or most two tables
     *
     * @param String $sql
    ...
    public function portalAudit($type = 'insert', $rec = '', array $auditvals, $oelog = true, $error = true)
    {
        ...
        $audit['client_id'] = $auditvals['client_id'] ? $auditvals['client_id'] : $_SESSION['pid'];
        ...
        try {
            if ($type != 'update') {
                $logsql = "INSERT INTO onsite_portal_activity".
                        "( date, client_id, activity, require_audit, pending_action, action_taken, status, narrative,".
                            "table_action, table_args, action_user, action_taken_time, checksum) ".
                                "VALUES (NOW(), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            } else {
                $logsql = "update onsite_portal_activity set date=?, client_id=?, activity=?, require_audit=?,".
                        "            pending_action=?, action_taken=?,status=?, narrative=?, table_action=?, table_args=?,".
                                        "action_user=?, action_taken_time=?, checksum=? ";
                $logsql .= "where id='" . add_escape_custom($rec) . "' And client_id='" . add_escape_custom($audit['client_id']) . "'";
            }
        ...
        if ($oelog) {
            $this->portalLog('profile audit transaction', $audit['client_id'], $logsql, $audit, $result, 'See portal audit activity');
    ...
    public function portalLog($event = '', $client_id = null, $comments = "", $binds = '', $success = '1', $user_notes = '', $ccda_doc_id = 0)
    {
        ...
        $this->portalNewEvent($event, $user, $groupname, $success, $comments, $client_id, $log_from, $user_notes, $ccda_doc_id);
    }
    /**
    ...
    public function portalNewEvent($event, $user, $groupname, $success, $comments = "", $client_id = null, $log_from = '', $user_notes = "", $ccda_doc_id = 0)
    {
        ...
        $sql = "insert into log ( date, event, user, groupname, success, comments, log_from, crt_user, client_id, user_notes) " . "values ( NOW(), " . $adodb->qstr($event) . "," .
            $adodb->qstr($user) . "," . $adodb->qstr($groupname) . "," . $adodb->qstr($success) . "," .
            $adodb->qstr($comments) . "," . $adodb->qstr($log_from) . "," . $adodb->qstr($crt_user) . "," .
            $adodb->qstr($client_id) . "," . $adodb->qstr($user_notes) .")";
        ...
        EventAuditLogger::instance()->send_atna_audit_msg($user, $groupname, $event, $client_id, $success, $comments);
    }
}// app query class
