...
class FeeSheetHtml extends FeeSheet
{
  ...
  // Generate a price level drop-down defaulting to the client's current price level.
  //
...
";
   ...
   if (!male_compatible_method) {
    if (!confirm('" . xls('Warning: Contraceptive method is not compatible with a male client.') . "'))
     return false;
   }
";
        } // end if male client
        if ($this->client_age < 10 || $this->client_age > 50) {
            $s .= "
   if (!confirm('" . xls('Warning: Contraception for a client under 10 or over 50.') . "'))
    return false;
";
        } // end if improper age
