...
#IfMissingColumn registry client_encounter
ALTER TABLE `registry` ADD `client_encounter` TINYINT NOT NULL DEFAULT '1';
#EndIf
...
INSERT INTO registry (name, state, directory, sql_run, unpackaged, date, priority, category, nickname, client_encounter, therapy_group_encounter) VALUES ('Group Attendance Form', 1, 'group_attendance', 1, 1, '2015-10-15 00:00:00', 0, 'Clinical', '',0,1);
#EndIf

#IfNotRow registry directory newGroupEncounter
INSERT INTO registry (name, state, directory, sql_run, unpackaged, date, priority, category, nickname, client_encounter, therapy_group_encounter) VALUES ('New Group Encounter Form', 1, 'newGroupEncounter', 1, 1, '2015-10-15 00:00:00', 0, 'Clinical', '',0,1);
#EndIf
...
INSERT INTO categories select (select MAX(id) from categories) + 1, 'Client', '', (select id from categories where name = 'Onsite Portal'), rght + 1, rght + 2 from categories where name = 'Categories';
...
UPDATE `registry` SET `aco_spec` = 'clients|appt'     WHERE directory = 'newclient';
UPDATE `registry` SET `aco_spec` = 'clients|appt'     WHERE directory = 'newGroupEncounter';
...
UPDATE `registry` SET `aco_spec` = 'clients|lab'      WHERE directory = 'procedure_order';
#EndIf
...
ALTER TABLE `issue_types` ADD `aco_spec` varchar(63) NOT NULL default 'clients|med';
#EndIf

#IfMissingColumn categories aco_spec
ALTER TABLE `categories` ADD `aco_spec` varchar(63) NOT NULL default 'clients|docs';
#EndIf
...
#IfNotTable client_birthday_alert
...
(15, 'AVM', 'FAILURE', '', '<span class="btn" title="AVM: Failed.  Check client\'s phone numbers." style="padding:5px;background-color:#ffc4c4;color:#000;"><i class="fa fa-phone fa-fw"></i></span>', ''),
...
#IfMissingColumn users client_menu_role
ALTER TABLE `users` ADD `client_menu_role` VARCHAR(50) NOT NULL DEFAULT 'standard';
#EndIf
