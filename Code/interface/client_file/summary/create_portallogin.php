...
$row = sqlQuery("SELECT pd.*,pao.portal_username,pao.portal_pwd,pao.portal_pwd_status FROM client_data AS pd LEFT OUTER JOIN client_access_" . escape_identifier($portalsite, array("on","off"), true) . "site AS pao ON pd.pid=pao.pid WHERE pd.pid=?", array($pid));
...
{
    $message = xlt("Client Portal Web Address") . ":<br>";
    if ($site == "on") {
        ...
        $offsite_portal_client_link = $GLOBALS['portal_offsite_address_client_link'] ?  $GLOBALS['portal_offsite_address_client_link'] : "https://mydocsportal.com";
        $message .= "<a href='" . attr($offsite_portal_client_link) . "'>" .
            text($offsite_portal_client_link) . "</a><br><br>";
...
function emailLogin($client_id, $message)
{
    ...
    $email_subject=xl('Access Your Client Portal');
...
function displayLogin($client_id, $message, $emailFlag)
{
    $clientData = sqlQuery("SELECT * FROM `client_data` WHERE `pid`=?", array($client_id));
    if ($emailFlag) {
        $message = "<br><br>" .
            xlt("Email was sent to following address") . ": " .
            text($clientData['email']) . "<br><br>" .
            $message;
    }
...
if (isset($_POST['form_save']) && $_POST['form_save']=='SUBMIT') {
    ...
    $res = sqlStatement("SELECT * FROM client_access_" . escape_identifier($portalsite, array("on","off"), true) . "site WHERE pid=?", array($pid));
    ...
    if (sqlNumRows($res)) {
        sqlStatement("UPDATE client_access_" . escape_identifier($portalsite, array("on","off"), true) . "site SET portal_username=?,portal_pwd=?,portal_pwd_status=0 " . $salt_clause . " WHERE pid=?", $query_parameters);
    } else {
        sqlStatement("INSERT INTO client_access_" . escape_identifier($portalsite, array("on","off"), true) . "site SET portal_username=?,portal_pwd=?,portal_pwd_status=0" . $salt_clause . " ,pid=?", $query_parameters);
    }
