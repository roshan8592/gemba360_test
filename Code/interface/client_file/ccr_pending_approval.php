<?php
/**
 * interface/client_file/ccr_pending_approval.php Approval screen for uploaded CCR XML.
 *
...
require_once(dirname(__FILE__) . "/../../library/parse_client_xml.php");
...
<table style="width:80%;" border="0" >
    ...
    $query = sqlStatement("SELECT *,am.id amid,CONCAT(ad.field_value,' ',ad1.field_value) as pat_name FROM audit_master am JOIN audit_details ad ON
		ad.audit_master_id = am.id AND ad.table_name = 'client_data' AND ad.field_name = 'lname' JOIN audit_details ad1 ON
		ad1.audit_master_id = am.id AND ad1.table_name = 'client_data' AND ad1.field_name = 'fname' WHERE type='11' AND approval_status='1'");
    if (sqlNumRows($query) > 0) {
        while ($res = sqlFetchArray($query)) {
            $dup_query = sqlStatement(
                "SELECT * FROM audit_master am JOIN audit_details ad ON ad.audit_master_id = am.id AND ad.table_name = 'client_data'
			AND ad.field_name = 'lname' JOIN audit_details ad1 ON ad1.audit_master_id = am.id AND ad1.table_name = 'client_data' AND
			ad1.field_name = 'fname' JOIN audit_details ad2 ON ad2.audit_master_id = am.id AND ad2.table_name = 'client_data' AND ad2.field_name = 'DOB'
			JOIN client_data pd ON pd.lname = ad.field_value AND pd.fname = ad1.field_value AND pd.DOB = DATE(ad2.field_value) WHERE am.id = ?",
                array($res['amid'])
            );
            ?>
    <tr>
