...
class UserMail
{
    ...
    public function getMails($data)
    {
        ...
        if (UserService::valid($data[0])=='existingclient') {
            ...
            if ($data[2] == "inbox") {
                if ($data[3] && $data[4]) {
                    $result_notes = getClientNotes($pid, '', '0', $data[3]);
                    $result_notifications = getClientNotifications($pid, '', '0', $data[4]);
                    $result = array_merge((array)$result_notes, (array)$result_notifications);
                } else {
                    $result_notes = getClientNotes($pid);
                    $result_notifications = getClientNotifications($pid);
                    $result = array_merge((array)$result_notes, (array)$result_notifications);
                }
                ...
                if ($data[3]) {
                    $result_sent_notes = getClientSentNotes($pid, '', '0', $data[3]);
                } else {
                    $result_sent_notes = getClientSentNotes($pid);
                }
    ...
    public function getMailDetails($data)
    {
        if (UserService::valid($data[0])=='existingclient') {
            ...
            if ($result['assigned_to'] == '-client-' && $result['message_status'] == 'New') {
