...
function register_email($client_id, $sent_by, $msg_type, $msg_subject, $msg_text)
{

    $sql = "INSERT INTO batchcom SET client_id=?, sent_by=?,
                                     msg_type=?, msg_subject=?,
                                     msg_text=?, msg_date_sent=NOW()";

    echo $sql;

    $res = sqlStatement($sql, array($client_id, $sent_by, $msg_type, $msg_subject, $msg_text));
}
