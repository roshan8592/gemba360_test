...
class C_Document extends Controller
{
    ...
    function __construct($template_mod = "general")
    {
        ...
        $this->_args = array("client_id" => $_GET['client_id']);
    ...
    function upload_action($client_id, $category_id)
    {
        ...
        /*checking whether request has came from Search/Add Client page or from Document page depending upon whether request_check is set into the POST method or not, if set then request from Search/Add Client page and no need to return anything else return $this->list_action*/
        if(isset($_POST["request_check"])){
            return "";
        }else{
            return $this->list_action($client_id);
        }
    }
    ...
    function upload_action_process()
    {
        ...
        $client_id = 0;
        if (isset($_GET['client_id']) && !$couchDB) {
    ...
    function note_action_process($client_id)
    {
        ...
        // Collect filename and path
            ...
            $url = $temp_url;
            $pdetails = getClientData($client_id);
      ...
      // remove the temporary couchdb file
        ...
        $this->_state = false;
        $_POST['process'] = "";
        return $this->view_action($client_id, $n->get_foreign_id());
    }
    ...
    function view_action($client_id = "", $doc_id)
    {
        ...
        $ires = sqlStatement("SELECT id, type, title, begdate FROM lists WHERE " .
            ...
            "ORDER BY type, begdate", array($client_id));
        ...
        // Populate the dropdown with client's encounter list
        ...
        $result_docs = sqlStatement("SELECT fe.encounter,fe.date,openemr_postcalendar_categories.pc_catname FROM form_encounter AS fe " .
            "LEFT JOIN openemr_postcalendar_categories ON fe.pc_catid=openemr_postcalendar_categories.pc_catid  WHERE fe.pid = ? ORDER BY fe.date desc", array($client_id));
        ...
        $imgOrders  = sqlStatement("select procedure_name,po.procedure_order_id,procedure_code from procedure_order po inner join procedure_order_code poc on poc.procedure_order_id = po.procedure_order_id where po.client_id = ?  and poc.procedure_order_title = 'imaging'", array($client_id));
     ...
     * @param (string) $context - given a special document scenario (e.g.: client avatar, custom image viewer document, etc), the context can be set so that a switch statement can execute a custom strategy.
     * */
    function retrieve_action($client_id = "", $document_id, $as_file = true, $original_file = true, $disable_exit = false, $show_original = false, $context = "normal")
    {
        $encrypted = $_POST['encrypted'];
        $passphrase = $_POST['passphrase'];
        $doEncryption = false;
        ...
        switch ($context) {
            case "client_picture":
                $this->clientService->setPid($client_id);
                $document_id = $this->clientService->getClientPictureDocumentId();
                break;
        }
        ...
        //different clients. Note that the path_depth is used to see how far down
        ...
        $from_pathname = implode("/", $from_pathname_array);
        if ($couch_docid && $couch_revid) {
            //for couchDB no URL is available in the table, hence using the foreign_id which is clientID
    ...
    function queue_action($client_id = "")
    {
        ...
        if (file_exists($this->_config['repository']) && is_dir($this->_config['repository'])) {
            ...
            while (($file = readdir($dir)) !== false) {
                ...
                if (is_file($file) && strpos(basename_international($file), ".") !== 0) {
                    ...
                    preg_match("/^([0-9]+)_/", basename_international($file), $client_match);
                    $file_info['client_id'] = $client_match[1];
    ...
    function queue_action_process()
    {
        ...
        foreach ($files as $doc_id => $file) {
            ...
            if (!is_numeric($file['client_id']) || !is_numeric($file['category_id']) || !is_numeric($doc_id)) {
                $messages .= "Error processing file '" . $file['name'] ."' the client id must be a number and the category must exist.\n";
                continue;
            }

            //validate that the pod exists
            $d = new Document($doc_id);
            $sql = "SELECT pid from client_data where pubpid = '" . $file['client_id'] . "'";
            $result = $d->_db->Execute($sql);

            if (!$result || $result->EOF) {
                //client id does not exist
                $messages .= "Error processing file '" . $file['name'] ." the specified client id '" . $file['client_id'] . "' could not be found.\n";
                continue;
            }
            ...
            $new_path = $this->_config['repository'] . $file['client_id'] ."/";

            //see if the client dir exists in the repository and create if not
            ...
            //see if client autonumbering is used in this filename, if so strip out the autonumber part
            preg_match("/^([0-9]+)_/", basename_international($fname), $client_match);
            if ($client_match[1] == $file['client_id']) {
            ...
            if (rename($this->_config['repository'].$file['name'], $new_path.$fname)) {
                $messages .= "File " . $fname . " moved to client id '" . $file['client_id'] ."' and category '" . $categories[$file['category_id']]['name'] . "' successfully.\n";
                $d->url = "file://" .$new_path.$fname;
                $d->set_foreign_id($file['client_id']);
    ...
    function move_action_process($client_id = "", $document_id)
    {
        if ($_POST['process'] != "true") {
            return;
        }
        ...
        //move to new client
        if (is_numeric($new_client_id) && is_numeric($document_id)) {
            $d = new Document($document_id);
            // $sql = "SELECT pid from client_data where pubpid = '" . $new_client_id . "'";
            $sql = "SELECT pid from client_data where pid = '" . $new_client_id . "'";
            $result = $d->_db->Execute($sql);

            if (!$result || $result->EOF) {
                //client id does not exist
                $messages .= xl('Document could not be moved to client id', '', '', ' \'') . $new_client_id  . xl('because that id does not exist.', '', '\' ') . "\n";
            } else {
                $couchsavefailed = !$d->change_client($new_client_id);

                $this->_state = false;
                if (!$couchsavefailed) {
                    $messages .= xl('Document moved to client id', '', '', ' \'') . $new_client_id  . xl('successfully.', '', '\' ') . "\n";
                } else {
                    $messages .= xl('Document moved to client id', '', '', ' \'') . $new_client_id  . xl('Failed.', '', '\' ') . "\n";
                }
                $this->assign("messages", $messages);
                return $this->list_action($client_id);
            }
        } elseif (strtolower($new_client_id) == "q" && is_numeric($document_id)) { // in this case return the document
        ...
        $this->_state = false;
        $this->assign("messages", $messages);
        return $this->view_action($client_id, $document_id);
    }

    function validate_action_process($client_id = "", $document_id)
    {
    ...
    //
    function update_action_process($client_id = "", $document_id)
    {
    ...
    function list_action($client_id = "")
    {
        $this->_last_node = null;
        $categories_list = $this->tree->_get_categories_array($client_id);
        ...
        $is_new = isset($_GET['client_name']) ? 1 : false;
        $place_hld = isset($_GET['client_name']) ? filter_input(INPUT_GET, 'client_name') : xl("Client search or select.");
        $cur_pid = isset($_GET['client_id']) ? filter_input(INPUT_GET, 'client_id') : '';
        $used_msg = xl('Current client unavailable here. Use Client Documents');
        if ($cur_pid == '00') {
            $cur_pid = '0';
            $is_new = 1;
        }
    ...
    function document_upload_download_log($clientid, $content)
    {
    ...
    function document_send($email, $body, $attfile, $pname)
    {
        if (empty($email)) {
          ...
          $desc = "Please check the attached client document.\n Content:".$body;
          $mail = new MyMailer();
...
// Function to tag a document to an encounter.
    function tag_action_process($client_id = "", $document_id)
    {
        if ($_POST['process'] != "true") {
            die("process is '" . text($_POST['process']) . "', expected 'true'");
            return;
        ...
        if (is_numeric($document_id)) {
            ...
            if ($encounter_check) {
                ...
                $query = "INSERT INTO form_encounter SET
						...
						pid = ?,
						encounter = ?";
                $bindArray = array($event_date,$file_name,$facility,$_POST['visit_category_id'],(int)$facility_id,(int)$billingFacilityID,(int)$provider_id,$client_id,$encounter);
                $formID = sqlInsert($query, $bindArray);
                addForm($encounter, "New Client Encounter", $formID, "newclient", $client_id, "1", date("Y-m-d H:i:s"), $username);
    ...
    function image_procedure_action($client_id = "", $document_id)
    {
    ...
    function clear_procedure_tag_action($client_id = "", $document_id)
    {
        if (is_numeric($document_id)) {
 ...
 * Also usable for documents that may of been moved to different clients.
 *
...
//clear encounter tag function
    function clear_encounter_tag_action($client_id = "", $document_id)
    {
        if (is_numeric($document_id)) {
            sqlStatement("update documents set encounter_id='0' where foreign_id=? and id = ?", array($client_id,$document_id));
        }
        return $this->view_action($client_id, $document_id);
    }
}
