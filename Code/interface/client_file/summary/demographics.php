...
use OpenEMR\Menu\ClientMenuRole;
...
if ($GLOBALS['enable_cdr']) {
    ...
    if ((!isset($_SESSION['alert_notify_pid']) || ($_SESSION['alert_notify_pid'] != $pid)) && isset($_GET['set_pid']) && $GLOBALS['enable_cdr_crp']) {
        // showing a new client, so check for active reminders and allergy conflicts, which use in active reminder popup
...
function image_widget($doc_id, $doc_catg)
{
    ...
    if (in_array($extension, $viewable_types)) { // extension matches list
        ...
        "/controller.php?document&retrieve&client_id=" . attr_url($pid) . "&document_id=" . attr_url($doc_id) . "&as_file=false&original_file=true&disable_exit=false&show_original=true'" .
        " onclick='top.restoreSession();' class='image_modal'>" .
        " <img src = '$web_root" .
        "/controller.php?document&retrieve&client_id=" . attr_url($pid) . "&document_id=" . attr_url($doc_id) . "&as_file=false'" .
        ...
        $to_url = "<td> <a href='" . $web_root . "/controller.php?document&retrieve" .
            "&client_id=" . attr_url($pid) . "&document_id=" . attr_url($doc_id) . "'" .
...
<script type="text/javascript" language="JavaScript">
 ...
 function deleteme() { // @todo don't think this is used any longer!!
  dlgopen('../deleter.php?client=' + <?php echo js_url($pid); ?> + '&csrf_token_form=' + <?php echo js_url(CsrfUtils::collectCsrfToken()); ?>, '_blank', 500, 450, '', '',{
      allowResize: false,
      allowDrag: false,
      dialogId: 'patdel',
      type: 'iframe'
 ...
 function imdeleted() {
   ...
   top.clearClient();
    <?php } else { ?>
   parent.left_nav.clearClient();
    <?php } ?>
 }

 function newEvt() {
     let title = <?php echo xlj('Appointments'); ?>;
     let url = '../../main/calendar/add_edit_event.php?clientid=' + <?php echo js_url($pid); ?>;
...
function doPublish() {
    let title = <?php echo xlj('Publish Client to FHIR Server'); ?>;
    let url = top.webroot_url + '/phpfhir/providerPublishUI.php?client_id=' + <?php echo js_url($pid); ?>;
...
$(document).ready(function(){
  var msg_updation='';
    ...
    if ($GLOBALS['erx_enable']) {
        //$soap_status=sqlQuery("select soap_import_status from client_data where pid=?",array($pid));
        $soap_status=sqlStatement("select soap_import_status,pid from client_data where pid=? and soap_import_status in ('1','3')", array($pid));
        while ($row_soapstatus=sqlFetchArray($soap_status)) {
            //if($soap_status['soap_import_status']=='1' || $soap_status['soap_import_status']=='3'){ ?>
            top.restoreSession();
            $.ajax({
                type: "POST",
                url: "../../soap_functions/soap_clientfullmedication.php",
                dataType: "html",
                data: {
                    client:<?php echo js_escape($row_soapstatus['pid']); ?>,
                },
                async: false,
    ...
    <?php if ($GLOBALS['enable_cdr'] && $GLOBALS['enable_cdr_prw']) { ?>
      top.restoreSession();
      $("#client_reminders_ps_expand").load("client_reminders_fragment.php",
          {
              csrf_token_form: <?php echo js_escape(CsrfUtils::collectCsrfToken()); ?>
          }
      );
...
<?php if ($vitals_is_registered && acl_check('clients', 'med')) { ?>
...
// modal for dialog boxes
    ...
    $(".rx_modal").on('click', function(e) {
        ...
        var AddAmendment = function () {
            ...
            iam.location.href = "<?php echo $GLOBALS['webroot']?>/interface/client_file/summary/add_edit_amendments.php"
        };
        var ListAmendments = function () {
            var iam = top.tab_mode ? top.frames.editAmendments : window[0];
            iam.location.href = "<?php echo $GLOBALS['webroot']?>/interface/client_file/summary/list_amendments.php"
        };
    ...
    /*Advocay Case Note Modal*/
    $(".acn_modal").on('click', function(e) {
          e.preventDefault();e.stopPropagation();
          var AddAdvocacyCaseNote = function () {
              var iam = top.tab_mode ? top.frames.editAdvocacyCaseNote : window[0];
              iam.location.href = "<?php echo $GLOBALS['webroot']?>/interface/client_file/summary/add_edit_advocacy_notes.php"
          };
          var title = <?php echo xlj('Advocacy Case Note'); ?>;
          dlgopen('', 'editAdvocacyCaseNote', 1000, 300, '', title, {
              buttons: [
                  {text: <?php echo xlj('Save'); ?>, close: false, style: 'primary  btn-sm', click: AddAdvocacyCaseNote},
                  {text: <?php echo xlj('Close'); ?>, close: true, style: 'default btn-sm'}
              ],
              onClosed: 'refreshme',
              allowResize: true,
    ...
    /*Advocacy Case Note Modal End*/

// modal for image viewer
    $(".image_modal").on('click', function(e) {
        e.preventDefault();e.stopPropagation();
        dlgopen('', '', 400, 300, '', <?php echo xlj('Client Images'); ?>, {
            allowResize: true,
            allowDrag: true,
            dialogId: '',
            type: 'iframe',
...
<?php if ($GLOBALS['client_birthday_alert']) {
    // To display the birthday alert:
    //  1. The client is not deceased
...
// JavaScript stuff to do when a new client is set.
//
function setMyClient() {
 // Avoid race conditions with loading of the left_nav or Title frame.
 if (!parent.allFramesLoaded()) {
  setTimeout("setMyClient()", 500);
  return;
 }
<?php
if (isset($_GET['set_pid'])) {
    $date_of_death = is_client_deceased($pid)['date_deceased']; ?>
    parent.left_nav.setClient(<?php echo js_escape($result['fname'] . " " . $result['lname']) .
    "," . js_escape($pid) . "," . js_escape($result['pubpid']) . ",'',";
    if (empty($date_of_death)) {
        echo js_escape(" " . xl('DOB') . ": " . oeFormatShortDate($result['DOB_YMD']) . " " . xl('Age') . ": " . getClientAgeDisplay($result['DOB_YMD']));
    } else {
   ...
   //Encounter details are stored to javacript as array.
    ...
    ?>
    parent.left_nav.setClientEncounter(EncounterIdArray,EncounterDateArray,CalendarCategoryArray);
    <?php
} // end setting new pid ?>
...
<?php if ((isset($_GET['set_pid']) ) && (isset($_GET['set_encounterid'])) && ( intval($_GET['set_encounterid']) > 0 )) {
 ...
 encurl = 'encounter/encounter_top.php?set_encounter=' + <?php echo js_url($encounter);?> + '&pid=' + <?php echo js_url($pid);?>;
  ...
  parent.left_nav.setEncounter(<?php echo js_escape(oeFormatShortDate(date("Y-m-d", strtotime($query_result['date'])))); ?>, <?php echo js_escape($encounter); ?>, 'enc');
    top.restoreSession();
  parent.left_nav.loadFrame('enc2', 'enc', 'client_file/' + encurl);
    <?php } else { ?>
...
<title><?php echo xlt("Dashboard{{client file}}"); ?></title>

<?php
$arrOeUiSettings = array(
    'heading_title' => xl('VALiD Client Record Dashboard'),
...
<body class="body_top client-demographics">
    <div id="container_div" class="<?php echo $oemr_ui->oeContainer();?>">
        ...
        $thisauth = acl_check('clients', 'demo');
        if ($thisauth) {
        ...
        // Create and fire the client demographics view event
        ...
        if (!$thisauth ||
            ...
            if ($thisauth) {?>
                <div class="row">
                    <div class="col-sm-12">
                        <?php require_once("$include_root/client_file/summary/dashboard_header.php"); ?>
                    </div>
                </div>
                <?php
            } // $thisauth
        ...
        <div class="row" >
            <div class="col-sm-12">
                <?php
                    ...
                    // Collect the client menu then build it
                    $menuClient = new ClientMenuRole();
                    $menuClient->displayHorizNavBarMenu();
                     // Get the document ID of the client ID card if access to it is wanted here.
                    $idcard_doc_id = false;
                if ($GLOBALS['client_id_category_name']) {
                    $idcard_doc_id = get_document_by_catg($pid, $GLOBALS['client_id_category_name']);
                }
                ?>
            </div>
        </div>
        ...
        <div style='margin-top:10px' class="main"> <!-- start main content div -->
            <table border="0" cellspacing="0" cellpadding="0" width="100%">
                <tr>
                    <td class="demographics-box" align="left" valign="top">
                        ...
                        <div style='float:left; margin-right:20px'>
                            ...
                            <table cellspacing=0 cellpadding=0>
                                ...
                                <tr >
                                    ...
                                    <br>
                                    <?php
                                //CLIENT BALANCE,INS BALANCE naina@capminds.com
                                    $clientbalance = get_client_balance($pid, false);
                                //Debit the client balance from insurance balance
                                    $insurancebalance = get_client_balance($pid, true) - $clientbalance;
                                    $totalbalance=$clientbalance + $insurancebalance;

                                // Show current balance and billing note, if any.
                                    echo "<table border='0'><tr><td>" .
                                    "<table ><tr><td><span class='bold'><font color='red'>" .
                                    xlt('Client Balance Due') .
                                    " : " . text(oeFormatMoney($clientbalance)) .
                                    "</font></span></td></tr>".
                        ...
                        // Demographics expand collapse widget
                            ...
                            $linkMethod = "html";
                            $bodyClass = "";
                            $widgetAuth = acl_check('clients', 'demo', '', 'write');
                            $fixedWidth = true;
                            expand_collapse_widget(
                                $widgetTitle,
                                $widgetLabel,
                        ...
                        <?php if (acl_check('clients', 'notes')) { ?>
                                <tr>
                                    <td width='650px'>
                            <?php
                        // Notes expand collapse widget
                            ...
                            $linkMethod = "html";
                            $bodyClass = "notab";
                            $widgetAuth = acl_check('clients', 'notes', '', 'write');
                            $fixedWidth = true;
                            expand_collapse_widget(
                                $widgetTitle,
                                $widgetLabel,
                        ...
                        <?php if (acl_check('clients', 'reminder') && $GLOBALS['enable_cdr'] && $GLOBALS['enable_cdr_prw']) {
                                        echo "<tr><td width='650px'>";
                                        // client reminders collapse widget
                                        $widgetTitle = xl("Client Reminders");
                                        $widgetLabel = "client_reminders";
                                        $widgetButtonLabel = xl("Edit");
                                        $widgetButtonLink = "../reminder/client_reminders.php?mode=simple&client_id=" . attr_url($pid);
                                        $widgetButtonClass = "";
                                        $linkMethod = "html";
                                        $bodyClass = "notab";
                                        $widgetAuth = acl_check('clients', 'reminder', '', 'write');
                                        $fixedWidth = true;
                        ...
                        <?php if (acl_check('clients', 'disclosure')) { ?>
                               <tr>
                               <td width='650px'>
                            <?php
                        // disclosures expand collapse widget
                            ...
                            $linkMethod = "html";
                            $bodyClass = "notab";
                            $widgetAuth = acl_check('clients', 'disclosure', '', 'write');
                            $fixedWidth = true;
                            expand_collapse_widget(
                                $widgetTitle,
                                $widgetLabel,
                        ...
                        <?php if ($GLOBALS['amendments'] && acl_check('clients', 'amendment')) { ?>
                          <tr>
                               <td width='650px'>
                                ...
                                $widgetButtonLink = $GLOBALS['webroot'] . "/interface/client_file/summary/list_amendments.php?id=" . attr_url($pid);
                                $widgetButtonClass = "rx_modal";
                                $linkMethod = "html";
                                $bodyClass = "summary_item small";
                                $widgetAuth = acl_check('clients', 'amendment', '', 'write');
                                $fixedWidth = false;
                                expand_collapse_widget($widgetTitle, $widgetLabel, $widgetButtonLabel, $widgetButtonLink, $widgetButtonClass, $linkMethod, $bodyClass, $widgetAuth, $fixedWidth);
                                ...
                                while ($row=sqlFetchArray($result)) {
                                        echo "&nbsp;&nbsp;";
                                        echo "<a class= '" . attr($widgetButtonClass) . "' href='" . $GLOBALS['webroot'] . "/interface/client_file/summary/add_edit_amendments.php?id=" . attr_url($row['amendment_id']) . "' onclick='top.restoreSession()'>" . text($row['amendment_date']);
                                ...
                                <td width="650px">
                                    <div class="section-header-dynamic">
                                        <table>
                                            <tbody>
                                                <tr>
                                                    <td>
                                                        <a class="css_button_small acn_modal" href="<?php echo $GLOBALS['webroot']?>/interface/client_file/summary/add_edit_advocacy_notes.php" onclick="top.restoreSession()"><span>Edit</span></a>
                                                    </td>
                                                    <td>
                                                        <a href="javascript:;" class="small" onclick="toggleIndicator(this,&quot;advocacy_case_notes&quot;)"><span class="text"><b>
                                                        Advocacy Case Notes
                                                            </b></span> (<span class="indicator">expand</span>)</a>
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    <div id="advocacy_case_notes" style="display:none" class="summary_item small">
                                    <center><strong>Advocacy Case Notes Details</strong></center>
                                        <table width="100%">
                                            <tbody >
                                                ...
                                                <tr style="border-bottom:2px solid #000;">
                                                    ...
                                                    <td style="padding: 5px;"><?php echo $advocacy_case_details[fcontact]; ?></td>
                                                </tr>
                                                <tr style="border-bottom:2px solid #000;">
                                                    <td style="padding: 5px;">Service contacts</td>
                                                    <td style="padding: 5px;"><?php echo $advocacy_case_details[scontact]; ?></td>
                                                </tr>
                                                <tr style="border-bottom:2px solid #000;">
                                                    <td style="padding: 5px;">NDIS contacts</td>
                                                    <td style="padding: 5px;"><?php echo $advocacy_case_details[ndiscontact]; ?></td>
                                                </tr>
                                                <tr style="border-bottom:2px solid #000;">
                                                    <td style="padding: 5px;">Alerts (Eg. Violent)</td>
                                                    <td style="padding: 5px;"><?php echo $advocacy_case_details[alert]; ?></td>
                                                </tr>
                                                <tr style="border-bottom:2px solid #000;">
                                                    <td style="padding: 5px;">Advocacy Goal Date</td>
                                                    <td style="padding: 5px;"> <?php if(!(is_null($advocacy_case_details[ad_goal_date])))echo date("d-m-Y", strtotime($advocacy_case_details[ad_goal_date])); ?></td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    <div>
                                    <center style = "margin-top: 10px;"><strong>Advocacy Case Notes </strong></center>
                                        ...
                                        <table width="100%" id="advocacy_case_notes_table">
                                            <tbody>
                                                <tr style="border-bottom:2px solid #000;">
                                                    <th width="15%" style="padding: 5px;">Date</th>
                                                    <th width="15%" style="padding: 5px;">Type</th>
                                                    <th width="40%" style="padding: 5px;">Notes</th>
                                                    <th width="15%" style="padding: 5px;">Actions/Outcomes</th>
                                                    <th width="15%" style="padding: 5px;">Duration(Minutes)</th>
                                                </tr>
                                                <?php while($row = sqlFetchArray($result_acn)){ ?>
                                                    <tr style="border-bottom:2px solid #000;">
                                                        <td style="padding: 5px;"><?php if(!(is_null($row["case_note_date"]))) echo $row["case_note_date"]; ?></td>
                                                        <td style="padding: 5px;"><?php echo $row["case_note_type"]; ?></td>
                                                        <td style="padding: 5px;"><?php echo $row["case_notes"]; ?></td>
                                                        <td style="padding: 5px;"><?php echo $row["case_note_actions"]; ?></td>
                                                        <td style="padding: 5px;"><?php echo $row["case_note_duration"]; ?></td>
                                                    </tr>
                                                <?php } ?>
                                            </tbody>
                                        </table>
                        ...
                        <?php if (acl_check('clients', 'lab')) { ?>
                          ...
                          // check to see if any labdata exist
                            $spruch = "SELECT procedure_report.date_collected AS date " .
                                    ...
                                    "WHERE procedure_order.client_id = ? " .
                        ...
                        <?php } // end if ($vitals_is_registered && acl_check('clients', 'med')) ?>
                        ...
                        while ($gfrow = sqlFetchArray($gfres)) {
                            ...
                            if (!$LBF_ACO || acl_check($LBF_ACO[0], $LBF_ACO[1], '', 'write')) {
                                // check to see if any instances exist for this client
                ...
                <div>
                    ...
                    $photos = pic_array($pid, $GLOBALS['client_photo_category_name']);
                ...
                <br />
                        <?php
                        if ($idcard_doc_id) {
                            image_widget($idcard_doc_id, $GLOBALS['client_id_category_name']);
                        }

                        foreach ($photos as $photo_doc_id) {
                            image_widget($photo_doc_id, $GLOBALS['client_photo_category_name']);
                        }
                    }
                    ?>
                ...
                <div>
                        ...
                        if ($myrow2) {
                            ...
                            while ($myrows3 = sqlFetchArray($resNew1)) {
                                ...
                                // remove time from datetime stamp
                                    ...
                                    $dateDoc = $tempParse[0];
                                    $idDoc = $myrows4['id'];
                                    echo "<a href='$web_root/controller.php?document&retrieve&client_id=" .
                    ...
                    if (!empty($clin_rem_check) && $GLOBALS['enable_cdr'] && $GLOBALS['enable_cdr_crw'] &&
                        acl_check('clients', 'alert')) {
                        // clinical summary expand collapse widget
                        $widgetTitle = xl("Clinical Reminders");
                        $widgetLabel = "clinical_reminders";
                        $widgetButtonLabel = xl("Edit");
                        $widgetButtonLink = "../reminder/clinical_reminders.php?client_id=" . attr_url($pid);
                        ;
                        $widgetButtonClass = "";
                        $linkMethod = "html";
                        $bodyClass = "summary_item small";
                        $widgetAuth = acl_check('clients', 'alert', '', 'write');
                        $fixedWidth = false;
                        expand_collapse_widget($widgetTitle, $widgetLabel, $widgetButtonLabel, $widgetButtonLink, $widgetButtonClass, $linkMethod, $bodyClass, $widgetAuth, $fixedWidth);
                        echo "<br/>";
                      ...
                      //
                    if (isset($pid) && !$GLOBALS['disable_calendar'] && acl_check('clients', 'appt')) {
                      //
                        ...
                        && (acl_check('clients', 'appt', '', 'write') || acl_check('clients', 'appt', '', 'addonly'));
                        $fixedWidth = false;
                        expand_collapse_widget($widgetTitle, $widgetLabel, $widgetButtonLabel, $widgetButtonLink, $widgetButtonClass, $linkMethod, $bodyClass, $widgetAuth, $fixedWidth);
                        $count = 0;
                        //
                    ...
                    if (isset($pid) && !$GLOBALS['disable_calendar'] && $GLOBALS['appt_recurrences_widget'] &&
                        acl_check('clients', 'appt')) {
                         ...
                         //Fetch client's recurrences. Function returns array with recurrence appointments' category, recurrence pattern (interpreted), and end date.
