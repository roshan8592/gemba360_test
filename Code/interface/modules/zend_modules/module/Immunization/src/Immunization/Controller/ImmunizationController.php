...
class ImmunizationController extends AbstractActionController
{
    ...
    */
    public function indexAction()
    {
        ...
        $form_get_hl7    =   '';
        $client_id      =   $request->getPost('client_id', null);
        //pagination
        ...
        if ($new_search) {
            $count  = $this->getImmunizationTable()->immunizedClientDetails($params, 1);
        } else {
        ...
        $details        = $this->getImmunizationTable()->immunizedClientDetails($params);
        $rows = array();
                ...
                foreach ($rows as $r) {
                    ...
                    if ($r['clientid'] != $client_id) {
                        ...
                        $content .= "PID|" . // [[ 3.72 ]]
                            "1|" . // 1. Set id
                        "|" . // 2. (B)Client id
                            $r['pubpid']. "^^^MPI&2.16.840.1.113883.19.3.2.1&ISO^MR" . $r['ss']. "|". // 3. (R) Client indentifier list. TODO: Hard-coded the OID from NIST test.
                        "|" . // 4. (B) Alternate PID
                            $r['clientname']."^^^^L|" . // 5.R. Name
                        ...
                        if ($publicity_code || $protection_indicator || $imm_registry_status_code) {
                            $content .=  "PD1|" . // Client Additional Demographic Segment
                                "|". // 1. Living Dependancy
                                "|". // 2. Living Arrangement
                                $r['fac_name']."|" . // 3. Client Primary Facility
                                $r['primary_care_provider_details']."|" . // 4. Client Primary Care Provider NPI and Provider name
                                "|" . // 5. Student Indicator
                                "|" . // 6. Handicap
                                "|" . // 7. Living Will Code
                                "|" . // 8. Organ Donor Code
                        ...
                        if ($r['guardiansname'] && $r['guardianrelationship']) {
                            $content .=  "NK1|" . // Nearest of kin
                                ...
                                $guardian_relationship_code."^".$r['guardianrelationship']."^HL70063|" . // 3. Relationship of next of kin with client
                    ...
                    $imm_obs_res                  = $this->getImmunizationTable()->getImmunizationObservationResultsData($r['clientid'], $r['immunizationid']);
