...
class NQF_0101_DenominatorException implements CqmFilterIF
{
    ...
    {
        //Risk Category Assessment not done: Medical Reasons and Risk Category Assessment: Client Not Ambulatory with their identifying
        //SNOMEDCT(160685001) code to support a Denominator Exception in the measure when chosen for a client.
        //OR
        ...
        $check = sqlQuery($riskCatAssessQry, array($beginDate, $endDate, $client->id));
        if ($check['cnt'] > 0) {
            return true;
        } else {
            return false;
