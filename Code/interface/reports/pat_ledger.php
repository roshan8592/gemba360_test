<?php
/**
 * This is a report to create a client ledger of charges with payments
 * applied.
 *
 * @package   OpenEMR
...
require_once($GLOBALS['srcdir'].'/client.inc');
...
function GetAllUnapplied($pat = '', $from_dt = '', $to_dt = '')
{
    ...
    $sql = "SELECT ar_session.*, ins.name, " .
      ...
      "LEFT JOIN client_data AS pat on ar_session.client_id = pat.pid " .
      "WHERE " .
      "ar_session.created_time >= ? AND ar_session.created_time <= ? " .
      "AND ar_session.client_id=?";
...
function PrintCreditDetail($detail, $pat, $unassigned = false)
{
    ...
    foreach ($detail as $pmt) {
        ...
        $print .= "<td class='detail' colspan='2'>".
                                      text($description)."&nbsp;</td>";
        $payer = ($pmt['name'] == '') ? xl('Client') : $pmt['name'];
        if ($unassigned) {
...
if ($type_form=='0') {
    if (!isset($_REQUEST['form_client'])) {
        $_REQUEST['form_client'] = '';
    }
...
$form_client   = $_REQUEST['form_client'];
...
<head>
    ...
    <title><?php echo xlt('Client Ledger by Date'); ?></title>
    ...
    <script type="text/javascript">
        ...
        function checkSubmit() {
            var pat = document.forms[0].elements['form_client'].value;
            if(!pat || pat == 0) {
                alert(<?php echo xlj('A Client Must Be Selected to Generate This Report') ?>);
                return false;
            }
        ...
        function setclient(pid, lname, fname, dob) {
          document.forms[0].elements['form_client'].value = lname + ', ' + fname;
    ...
    <script>
    <?php require_once("$include_root/client_file/erx_client_portal_js.php"); // jQuery for popups for eRx and client portal?>
    </script>
    <?php
    if ($type_form == '0') {
        $arrOeUiSettings = array(
        'heading_title' => xl('Report') . " - " . xl('Client Ledger by Date'),
        'include_client_name' => false,
        'expandable' => true,
        'expandable_files' => array("client_ledger_report_xpd"),//all file names need suffix _xpd
        ...
        $arrOeUiSettings = array(
        'heading_title' => xl('Client Ledger'),
        'include_client_name' => true,
        'expandable' => true,
        'expandable_files' => array("client_ledger_client_xpd", "stats_full_client_xpd", "external_data_client_xpd"),//all file names need suffix _xpd
        'action' => "conceal",//conceal, reveal, search, reset, link or back
        'action_title' => "",
        'action_href' => "",//only for actions - reset, link and back
        'show_help_icon' => true,
    ...
    <div id="container_div" class="<?php echo $oemr_ui->oeContainer();?>">
        <div class="row">
            <div class="col-sm-12">
                <?php
                if ($type_form != '0') {
                    require_once("$include_root/client_file/summary/dashboard_header.php");
                } else {
        ...
        <div class="row hideaway" >
            <div class="col-sm-12">
                <form method='post' action='pat_ledger.php?form=<?php echo attr_url($type_form); ?>&client_id=<?php echo attr_url($form_pid); ?>' id='theform' onsubmit='return top.restoreSession()'>
                                ...
                                <div style='float:left'>
                                    <table class='text'>
                                        ...
                                        <tr>
                                            ...
                                            <?php
                                            if ($type_form == '0') { ?>
                                            <td><span class='control-label'><?php echo xlt('Client'); ?>:&nbsp;&nbsp;</span></td>
                                            <td>
                                                <input type='text' size='20' name='form_client' class='form-control' style='width:100%;cursor:pointer;cursor:hand' id='form_client' value='<?php echo ($form_client) ? attr($form_client) : xla('Click To Select'); ?>' onclick='sel_client()' title='<?php echo xla('Click to select client'); ?>' />
                                                <?php
                                            } else { ?>
                                                <input type='hidden' name='form_client' value='<?php echo attr($form_client); ?>' />
                                                <?php
                                            } ?>
                                            ...
                                            <div class="text-center">
                                                <div class="btn-group" role="group">
                                                    ...
                                                    if ($_REQUEST['form_refresh'] || $_REQUEST['form_csvexport']) { ?>
                                                        ...
                                                        <?php
                                                        if ($type_form == '1') { ?>
                                                            <a href="../client_file/summary/demographics.php" class="btn btn-default btn-transmit" onclick="top.restoreSession()">
                                                            <?php echo xlt('Back To Client');?>
                                                            </a>
                                                            <?php
                                                        } ?>
                                                        <?php
...
if ($_REQUEST['form_refresh'] || $_REQUEST['form_csvexport']) {
    ...
    if ($_REQUEST['form_csvexport']) {
      // CSV headers:
        ...
        $client = sqlQuery("SELECT * from client_data WHERE pid=?", array($form_client));
        $pat_dob = $client['DOB'];
        $pat_name = $client['fname']. ' ' . $client['lname'];
        ?>
        <div id="report_header">
            <table width="98%"  border="0" cellspacing="0" cellpadding="0">
                ...
                <tr><td>&nbsp;</td></tr>
                <tr>
                    <td class="title" ><?php echo xlt('Client Ledger'); ?></td>
                </tr>
                <tr>
                <?php
                ...
                ?>
                    ...
                    <td class='bold' ><?php echo xlt('Client')?>:
                    <?php
                    if ($type_form == '1') { ?>
