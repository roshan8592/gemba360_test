...
1. Client-initiated transmission of clinical data from the Report section of the Client 
Portal interface.

2. Provider-initiated transmission of clinical data from the Report section of the Client 
pane in the main OpenEMR interface.

3. Log all data transmissions including date/time, client, and whether transmission 
was initiated by the client through the Client Portal or by an OpenEMR user through the 
main interface.
...
of the Reports section in both the Client Portal and the Client pane of the main provider 
interface. 
...
New Direct messages will be processed as follows. A new "Client Note" will be generated and 
sent to the phiMail notification user specified in the Connectors tab of the Global settings. 
The client note will contain information about the message, including any text at the beginning 
of the message from the sender. Any attachments (and any non-text content) will be automatically 
converted to separate OpenEMR Documents, which will be referenced in the new Client Note.  
The Documents and the Client Note are initially created without an assigned client. 

At this time, the envisioned workflow is that the notification user will review the message text
and any included Documents to determine which client the content belongs to and will then set the 
client using the existing Client Note interface for choosing a client. Once the client is sent, 
the Client Note can be forwarded to another provider or staff member as appropriate using the 
existing forwarding mechanism for Client Notes. The unassigned Documents can be viewed by Selecting 
...
appropriate, assign the document to a specific client using the "Move to Client #" feature in the 
Documents interface.
