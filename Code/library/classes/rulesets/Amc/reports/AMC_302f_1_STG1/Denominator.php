...
class AMC_302f_1_STG1_Denominator implements AmcFilterIF
{
    ...
    {
        //Number of unique clients 2 years of age or older seen by the EP during the EHR reporting period (Effective through 2013 only)
        $options = array( Encounter::OPTION_ENCOUNTER_COUNT => 1 );
        if ((Helper::checkAnyEncounter($client, $beginDate, $endDate, $options)) &&
             ($client->calculateAgeOnDate($endDate) >= 2) ) {
            return true;
        } else {
            return false;
        }
