<?php
/**
 * This script merges two client charts into a single client chart.
 * It is to correct the error of creating a duplicate client.
 *
 * @package   OpenEMR
...
<head>
<title><?php echo xlt('Merge Clients'); ?></title>
...
function sel_client(ename, epid) {
 el_pt_name = ename;
 el_pt_id = epid;
 dlgopen('../main/calendar/find_client_popup.php', '_blank', 600, 400);
}
...
<body class="body_top">
<div class="container">
<h2><?php echo xlt('Merge Clients') ?></h2>
...
if (!empty($_POST['form_submit'])) {
    ...
    $tprow = sqlQuery("SELECT * FROM client_data WHERE pid = ?", array($target_pid));
    $sprow = sqlQuery("SELECT * FROM client_data WHERE pid = ?", array($source_pid));

  // Do some checking to make sure source and target exist and are the same person.
    if (empty($tprow['pid'])) {
        die(xlt('Target client not found'));
    }

    if (empty($sprow['pid'])) {
        die(xlt('Source client not found'));
    }
    ...
    if (empty($tprow['DOB']) || $tprow['DOB'] == '0000-00-00') {
        die(xlt('Target client has no DOB'));
    }

    if (empty($sprow['DOB']) || $sprow['DOB'] == '0000-00-00') {
        die(xlt('Source client has no DOB'));
    }
  ...
  // Change normal documents first as that could fail if CouchDB connection fails.
    ...
    while ($drow = sqlFetchArray($dres)) {
        ...
        echo "<br />" . xlt('Changing client ID for document') . ' ' . text($d->get_url_file());
        if ($PRODUCTION) {
            if (!$d->change_client($target_pid)) {
  ...
  // Move scanned encounter documents and delete their container.
    ...
    while ($trow = sqlFetchArray($tres)) {
        ...
        if ($tblname == 'client_data' || $tblname == 'history_data' || $tblname == 'insurance_data') {
            ...
            "`Field` LIKE 'pid' OR `Field` LIKE 'client_id'");
            if (!empty($crow['Field'])) {
                  $colname = $crow['Field'];
...
<form method='post' action='merge_clients.php'>
<input type="hidden" name="csrf_token_form" value="<?php echo attr(CsrfUtils::collectCsrfToken()); ?>" />
<div class="table-responsive">
<table style='width:100%'>
 <tr>
  <td>
    <?php echo xlt('Target Client') ?>
  </td>
  <td>
   <input type='text' size='30' name='form_target_client'
    value=' (<?php echo xla('Click to select'); ?>)'
    onclick='sel_client(this, this.form.form_target_pid)'
    title='<?php echo xla('Click to select client'); ?>' readonly />
 ...
 <tr>
  <td>
    <?php echo xlt('Source Client') ?>
  </td>
  <td>
   <input type='text' size='30' name='form_source_client'
    value=' (<?php echo xla('Click to select'); ?>)'
    onclick='sel_client(this, this.form.form_source_pid)'
    title='<?php echo xla('Click to select client'); ?>' readonly />
...
<p><?php echo xlt('This will merge two client charts into one.  It is useful when a client has been duplicated by mistake.  If that happens often, fix your office procedures - do not run this routinely!'); ?></p>
