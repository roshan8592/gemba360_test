...
//actual execution is done in the select_query function in Server_side
    ...
    public function query_formation($data)
    {
        ...
        //Global omit employers for building demo.
            ...
            case 'P18':
                $query = "select  portal_username from client_access_offsite where portal_username =? ";
                return array($query,$data[1]);
            break;
            
            case 'P20':
                $x=array($data[1][0]);
                $query="select count(*) AS count from client_data where pubpid = ?";
                return array($query,$x);
            break;
        //getting DOB and SSN for verifying the duplicate client existance
            case 'P21':
                $x=array($data[1][0]);
                $query="select  ss,DOB  from client_data where DOB=?";
                return array($query,$x);
            break;
