...
class CarecoordinationTable extends AbstractTableGateway
{
    ...
    public function document_fetch($data)
    {
        $query = "SELECT am.id as amid, 
                        ...
                        CONCAT_WS(' ',pd.lname, pd.fname) as matched_client
                     FROM documents AS d
                     ...
                     LEFT JOIN audit_details ad ON ad.audit_master_id = am.id AND ad.table_name = 'client_data' AND ad.field_name = 'lname' 
                     LEFT JOIN audit_details ad1 ON ad1.audit_master_id = am.id AND ad1.table_name = 'client_data' AND ad1.field_name = 'fname' 
                     LEFT JOIN audit_details ad2 ON ad2.audit_master_id = am.id AND ad2.table_name = 'client_data' AND ad2.field_name = 'DOB' 
                     LEFT JOIN client_data pd ON pd.lname = ad.field_value AND pd.fname = ad1.field_value AND pd.DOB = DATE(ad2.field_value)
    ...
    public function import($xml, $document_id)
    {
        ...
        //Client Details
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['fname'] = is_array($xml['recordTarget']['clientRole']['client']['name']['given']) ? $xml['recordTarget']['clientRole']['client']['name']['given'][0] : $xml['recordTarget']['clientRole']['client']['name']['given'];
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['lname'] = $xml['recordTarget']['clientRole']['client']['name']['family'];
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['DOB'] = $xml['recordTarget']['clientRole']['client']['birthTime']['value'];
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['sex'] = $xml['recordTarget']['clientRole']['client']['administrativeGenderCode']['displayName'];
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['pubpid'] = $xml['recordTarget']['clientRole']['id'][0]['extension'];
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['ss'] = $xml['recordTarget']['clientRole']['id'][1]['extension'];
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['street'] = $xml['recordTarget']['clientRole']['addr']['streetAddressLine'];
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['city'] = $xml['recordTarget']['clientRole']['addr']['city'];
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['state'] = $xml['recordTarget']['clientRole']['addr']['state'];
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['postal_code'] = $xml['recordTarget']['clientRole']['addr']['postalCode'];
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['country_code'] = $xml['recordTarget']['clientRole']['addr']['country'];
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['phone_home'] = preg_replace('/[^0-9]+/i', '', $xml['recordTarget']['clientRole']['telecom']['value']);
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['status'] = $xml['recordTarget']['clientRole']['client']['maritalStatusCode']['displayName'];
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['religion'] = $xml['recordTarget']['clientRole']['client']['religiousAffiliationCode']['displayName'];
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['race'] = $xml['recordTarget']['clientRole']['client']['raceCode']['displayName'];
        $this->ccda_data_array['field_name_value_array']['client_data'][1]['ethnicity'] = $xml['recordTarget']['clientRole']['client']['ethnicGroupCode']['displayName'];
    ...
    public function getDemographics($data)
    {
        ...
        $query = "SELECT ad.id as adid, 
                   ...
                   WHERE am.id = ? AND ad.table_name = 'client_data' 
                   ORDER BY ad.id";
    ...
    /*
     * Fetch the current demographics data of a client from client_data table
     *
     * @param    pid       Integer   Client ID
     * @return   records   Array     current client data
     */
    ...
    /*
     * Fetch the current Problems of a client from lists table
     *
     * @param    pid       Integer     client id
    ...
    /*
     * Fetch the current Allergies of a client from lists table
     *
     * @param    pid       Integer     client id
    ...
    /*
     * Fetch the current Medications of a client from prescriptions table
     *
     * @param    pid       Integer     client id
    ...
    /*
     * Fetch the current Immunizations of a client from immunizations table
     *
     * @param    pid       Integer     client id
    ...
    public function getImmunizations($data)
    {
        ...
        $query = "SELECT * 
                   FROM immunizations 
                   WHERE client_id = ?"; //removed the field 'added_erroneously' from where condition
        $result = $appTable->zQuery($query, array($data['pid']));
        $records = array();
        foreach ($result as $row) {
            $records[] = $row;
    ...
    /*
     * Fetch the currect Lab Results of a client
     *
     * @param    pid       Integer     client id
    ...
    public function getLabResults($data)
    {
        ...
        $query = "SELECT CONCAT_WS('',po.procedure_order_id,poc.`procedure_order_seq`) AS tcode, 
                   ...
                   WHERE po.client_id = ? AND prs.result NOT IN ('DNR','TNP')";
        $result = $appTable->zQuery($query, array($data['pid']));
        $records = array();
        foreach ($result as $row) {
            $records[] = $row;
    ...
    /*
     * Fetch the current Vitals of a client from form_vitals table
     *
     * @param    pid       Integer     client id
    ...
    /*
     * Fetch the social history of a client from history_data table
     *
     * @param    pid       Integer     client id
    ...
    /*
     * Fetch the encounter data of a client from form_encounter table
     *
     * @param    pid       Integer     client id
    ...
    /*
     * Fetch the billing data of a client from billing table
     *
     * @param    pid       Integer     client id
    ...
    /*
     * Fetch the current Care Plan of a client from form_care_paln table
     *
     * @param    pid       Integer     client id
    ...
    /*
     * Fetch the current Functional Cognitive Status of a client from form_functional_cognitive_status table
     *
     * @param    pid       Integer     client id
    ...
    public function insertApprovedData($data)
    {
        ...
        foreach ($data as $key => $val) {
            if (substr($key, -4) == '-sel') {
                            ...
                            if (substr($key, 0, -4) == 'lists2_exist') {
                                ...
                                $q_upd_pres = "UPDATE prescriptions
                                 ...
                                 $appTable->zQuery($q_upd_pres, array(
                                     ...
                                     $data['pid']));
                                if ($m1_arr[$i] == 1) {
                                    $query6 = "UPDATE prescriptions SET end_date = ?,active = ? WHERE client_id = ? AND id = ?";
                ...
                } else if (substr($key, 0, 12) == 'client_data') {
                    if ($val == 'update') {
                        $var_name = substr($key, 0, -4);
                        $field_name = substr($var_name, 13);
                        $client_data_fields .= $field_name . '=?,';
                        array_push($client_data_values, $data[$var_name]);
                    }
                }
            }
        }

        if (count($client_data_values) > 0) {
            array_push($client_data_values, $data['pid']);
            $client_data_fields = substr($client_data_fields, 0, -1);
            $query = "UPDATE client_data SET $client_data_fields WHERE pid=?";
            $appTable->zQuery($query, $client_data_values);
        }
    ...
    public function InsertLabResults($lab_results, $pid)
    {
        ...
        foreach ($lab_results as $key => $value) {
            ...
            foreach ($value['result'] as $res) {
                ...
                //procedure_order
                $query_insert_po = "INSERT INTO procedure_order(provider_id,client_id,encounter_id,date_collected,date_ordered,order_priority,order_status,activity,lab_id) VALUES (?,?,?,?,?,?,?,?,?)";
    ...
    public function insert_client($audit_master_id, $document_id)
    {
        require_once(dirname(__FILE__) . "/../../../../../../../../library/client.inc");
        $pid = 0;
        $j = 1;
        $k = 1;
        $q = 1;
    ...
    public function InsertEncounter($enc_array, $pid, $revapprove = 1)
    {
        ...
        foreach ($enc_array as $key => $value) {
            ...
            $appTable->zQuery($q_ins_forms, array($encounter_date_value, $encounter_id, 'New Client Encounter', $enc_id, $pid, $_SESSION["authProvider"], 'Default', 0, 'newclient'));
    ...
    /*
     * Fetch the current Referral values of a client from transactions table
     *
     * @param    pid       Integer     client id
