...
class AMC_302d_Numerator implements AmcFilterIF
{
    ...
    public function test(AmcClient $client, $beginDate, $endDate)
    {
        ...
        // client recorded as structured data.
        ...
        //   token.)
        $firstCheck = sqlQuery("SELECT * FROM `lists_touch` WHERE `pid`=? AND (`type`=? OR type=?)", array($client->id,'medication','prescription_erx'));
        $secondCheck = sqlQuery("SELECT * FROM `lists` WHERE `activity`='1' AND `pid`=? AND `type`=?", array($client->id,'medication'));
