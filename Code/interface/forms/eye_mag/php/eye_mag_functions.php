 ...
 * @param string $pid value = client id
...
margin: 2px 0 2px 2px;">
        ...
        // Check authorization.
        if (acl_check('clients', 'med')) {
            $tmp = getClientData($pid);
        }
...
/**
 *  This function builds the complete PMSFH array for a given client, including the ROS for this encounter.
 *
 ...
 *
 * @param string $pid is the client identifier
...
function build_PMSFH($pid)
{
    ...
    //Build the SocHx portion of $PMSFH for this client.
    ...
    */
    ...
    // Build ROS into $PMSFH['ROS'] also for this client.
...
/**
 *  This function uses the complete PMSFH array for a given client, including the ROS for this encounter
...
function display_draw_section($zone, $encounter, $pid, $side = 'OU', $counter = '')
{
    ...
    background-image: none;" />
                ...
                if (file_exists($file_store) && ($doc['id'] > '0')) {
                    $filetoshow = $GLOBALS['web_root']."/controller.php?document&retrieve&client_id=".attr($pid)."&document_id=".attr($doc['id'])."&as_file=false&show_original=true&blahblah=".rand();
                } else {
                    //base image.
...
/**
 *  This function builds an array of documents for this client ($pid).
...
function display($pid, $encounter, $category_value)
{
    ...
    for ($j=0; $j < count($documents['zones'][$category_value]); $j++) {
        ...
        <td>
            <a href='../../../controller.php?document&upload&client_id=".attr($pid)."&parent_id=".attr($documents['zones'][$category_value][$j]['id'])."&'>
        ...
        if ($count_here > '0') {
            $episode .= '<a onclick="openNewForm(\''.$GLOBALS['webroot'].'/controller.php?document&view&client_id='.$pid.'&doc_id='.$id_to_show.'\',\'Documents\');"><img src="../../forms/'.$form_folder.'/images/jpg.png" class="little_image" /></a>';
        }
 ...
 *
 *  @param string $pid is the client id
 *  @param string $encounter is the encounter_id
                ...
                <ul class="navbar-nav">
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            ...
                            <li id="menu_PRINT_narrative" name="menu_PRINT_report"><a id="BUTTON_PRINT_report" target="_new" href="<?php echo $GLOBALS['webroot']; ?>/interface/client_file/report/custom_report.php?printable=1&pdf=0&<?php echo $form_folder."_".$form_id."=".$encounter; ?>"><?php echo xlt("Print Report"); ?></a></li>
                            ...
                            */
                            if ($display !== "fullscreen") { ?>
                                ...
                                <li id="menu_fullscreen" name="menu_fullscreen" <?php echo $fullscreen; ?>>
                                    <a onclick="openNewForm('<?php echo $GLOBALS['webroot']; ?>/interface/client_file/encounter/load_form.php?formname=fee_sheet');top.restoreSession();dopopup('<?php echo $_SERVER['REQUEST_URI']. '&display=fullscreen&encounter='.$encounter; ?>');" href="JavaScript:void(0);" class=""><?php echo xlt('Fullscreen'); ?></a>
                                </li>
                                <?php
                            } ?>
                        </ul>
...
/**
 *  This is currently a floating div top with client demographics and such.
...
function menu_overhaul_left($pid, $encounter)
{
    ...
    /*
     * find out if the client has a photo
     */
    if (!$documents) {
        list($documents) = document_engine($pid);
    }
    ...
    <div class="borderShadow" id="title_bar">
        <div id="left_menu" name="left_menu" class="col-md-4">
            <div style="padding-left: 18px;">
                <table style="text-align:left;">
                    <tr><td class="right" >
                            <?php
                            $age = getClientAgeDisplay($pat_data['DOB'], $encounter_date);
        ...
        <div id="left_menu3" name="left_menu3" class="col-md-3" style="font-size:1.0em;">
            <?php             //if the client has a photograph, use it else use generic avitar thing.
            if ($documents['docs_in_name']['Client Photograph'][0]['id']) {
                ?>
                <object><embed src="<?php echo $GLOBALS['webroot']; ?>/controller.php?document&amp;retrieve&amp;client_id=<?php echo attr($pid); ?>&amp;document_id=<?php echo attr($documents['docs_in_name']['Client Photograph'][0]['id']); ?>&amp;as_file=false" frameborder="0"
                     type="<?php echo attr($documents['docs_in_name']['Client Photograph'][0]['mimetype']); ?>" allowscriptaccess="always" allowfullscreen="false" height="50"></embed></object>
                <?php
            } else {
                ?>
...
function report_header($pid, $direction = 'shell')
{
    ...
    /*******************************************************************
    $titleres = getClientData($pid, "fname,lname,providerID");
    $sql = "SELECT * FROM facility ORDER BY billing_location DESC LIMIT 1";
    *******************************************************************/
    //$titleres = getClientData($pid, "fname,lname,providerID,DATE_FORMAT(DOB,'%m/%d/%Y') as DOB_TS");
    $titleres = getClientData($pid, "fname,lname,providerID,DOB");
    $facility = null;
 ...
 *  when a value is found for a given field in the Eye Form for a given client ($pid)
                    ...
                    foreach ($documents['docs_in_name']['VF'] as $VF) {
                        if ($count < 1) {
                            ...
                            $current_VF = '<tr><td class="GFS_td_1 blue">
                                <a onclick="openNewForm(\''.$GLOBALS['webroot'].'/controller.php?document&view&client_id='.attr($pid).'&doc_id='.attr($VF['id']).'\',\'Documents\');">
                        ...
                        if ($count_OCT > '0') { //need to decide how many to show on open, and hide the rest?  For now show first, hide rest.
                            ...
                            foreach ($documents['docs_in_name']['OCT'] as $OCT) {
                                ...
                                if ($count < 1) {
                                    $current_OCT = '<tr>
                                            <td class="GFS_td_1">
                                            <a onclick="openNewForm(\''.$GLOBALS['webroot'].'/controller.php?document&view&client_id='.attr($pid).'&doc_id='.attr($OCT['id']).'\',\'Documents\');"><img src="../../forms/'.$form_folder.'/images/jpg.png" class="little_image" style="width:15px; height:15px;" /></a>
                                            </td>
...
# gets the provider from the encounter file , or from the logged on user or from the client file
    ...
    if ($providerid < '1') {
       //find the default providerID from the calendar
        ...
        if (($new_providerid < '1')||(!$new_providerid)) {
            ...
            if ($get_authorized ==1) {
                $find_provider2 = sqlQuery("SELECT providerID FROM client_data WHERE pid = ? ", array($pid));
        ...
        sqlStatement("UPDATE client_data set providerID =? WHERE pid = ?", array($providerid,$pid));
    }
...
function in_array_r($needle, $haystack, $strict = false)
{
    ...
    return false;
             ...
             * to discover the target IOPS for this client.
             *
             * @param $pid = client id
             * @param $id = form_id values are needed for
             * @param $provider_id = who is the client's provider is only needed if there is no value anywhere else.
             * @return array (ODIOPTARGET AND OSIOPTARGET to be saved in this encounter
             */
function getIOPTARGETS($pid, $id, $provider_id)
{
    //iterate through this client's encounters to find IOPTARGETS.
