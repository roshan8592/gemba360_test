...
function Client($doc, $r, $pid)
{
    global $msg,$warning_msg,$dem_check;
    $client_data=sqlQuery("select *, DATE_FORMAT(DOB,'%Y%m%d') AS date_of_birth from client_data where pid=?", array($pid));
    $b = $doc->createElement("Client");
    $b->setAttribute('ID', $client_data['pid']);
    $ClientName = $doc->createElement("ClientName");
        $client_data['lname']=stripSpecialCharacter($client_data['lname']);
        $client_data['lname']=trimData($client_data['lname'], 35);
        //$msg = validation(xl('Client Last name'),$client_data['lname'],$msg);
    if ($client_data['lname']=='') {
        $dem_check.=xlt("Client Last name is missing")."<br>";
    }

        $last = $doc->createElement("last");
        $last->appendChild(
            $doc->createTextNode($client_data['lname'])
        );
        $ClientName->appendChild($last);
        $client_data['fname']=stripSpecialCharacter($client_data['fname']);
        $client_data['fname']=trimData($client_data['fname'], 35);
        //$msg = validation(xl('Client First name'),$client_data['fname'],$msg);
    if ($client_data['fname']=='') {
        $dem_check.=xlt("Client First name is missing")."<br>";
    }

        $first = $doc->createElement("first");
        $first->appendChild(
            $doc->createTextNode($client_data['fname'])
        );
        $ClientName->appendChild($first);
        $client_data['mname']=stripSpecialCharacter($client_data['mname']);
        $client_data['mname']=trimData($client_data['mname'], 35);
        $middle = $doc->createElement("middle");
        $middle->appendChild(
            $doc->createTextNode($client_data['mname'])
        );
        $ClientName->appendChild($middle);
    $b->appendChild($ClientName);
    $ClientAddress = $doc->createElement("ClientAddress");
        $client_data['street']=stripSpecialCharacter($client_data['street']);
        $client_data['street']=trimData($client_data['street'], 35);
        $msg = validation(xl('Client Address'), $client_data['street'], $msg);
    if (trim($client_data['street'])=='') {
        $warning_msg .= "<br>".xlt("Client Address is missing");
    }

        $address1 = $doc->createElement("address1");
        $address1->appendChild(
            $doc->createTextNode($client_data['street'])
        );
        $ClientAddress->appendChild($address1);
        //$msg = validation(xl('Client City'),$client_data['city'],$msg);
    if ($client_data['city']=='') {
        $dem_check.=xlt("Client City is missing")."<br>";
    }

        $city = $doc->createElement("city");
        $city->appendChild(
            $doc->createTextNode($client_data['city'])
        );
        $ClientAddress->appendChild($city);
    if ($client_data['state']) {
        $state = $doc->createElement("state");
        $state->appendChild(
            $doc->createTextNode($client_data['state'])
        );
        $ClientAddress->appendChild($state);
    }

    if ($client_data['postal_code']) {
        $zip = $doc->createElement("zip");
        $zip->appendChild(
            $doc->createTextNode($client_data['postal_code'])
        );
        $ClientAddress->appendChild($zip);
    }

        //$msg = validation(xl('Client Country'),$client_data['country_code'],$msg);
    if (trim($client_data['country_code'])=='' && $GLOBALS['erx_default_client_country']=='') {
        $dem_check.=xlt("Client Country is missing. Also you have not set default Client Country in Global Settings")."<br>";
    } elseif (trim($client_data['country_code'])=='') {
        $client_data['country_code'] = $GLOBALS['erx_default_client_country'];
    }

        $county_code = substr($client_data['country_code'], 0, 2);
        $country = $doc->createElement("country");
        $country->appendChild(
            $doc->createTextNode($county_code)
        );
        $ClientAddress->appendChild($country);
    $b->appendChild($ClientAddress);
    $ClientContact = $doc->createElement("ClientContact");
        $client_data['phone_home']=stripPhoneSlashes($client_data['phone_home']);
    if ($client_data['phone_home']) {
        $homeTelephone = $doc->createElement("homeTelephone");
        $homeTelephone->appendChild(
            $doc->createTextNode($client_data['phone_home'])
        );
        $ClientContact->appendChild($homeTelephone);
    }

    $b->appendChild($ClientContact);
    $ClientCharacteristics = $doc->createElement("ClientCharacteristics");
    if (trim($client_data['date_of_birth'])=='' || $client_data['date_of_birth']=='00000000') {
        $warning_msg .= "<br>".xlt("Client Date Of Birth is missing");
    }

    if ($client_data['date_of_birth'] && $client_data['date_of_birth']!='00000000') {
        $dob = $doc->createElement("dob");
        $dob->appendChild(
            $doc->createTextNode($client_data['date_of_birth'])
        );
        $ClientCharacteristics->appendChild($dob);
    }

    if (trim($client_data['sex'])=='') {
        $warning_msg .= "<br>".xlt("Client Gender is missing");
    }

    if ($client_data['sex']) {
        $gender_val=substr($client_data['sex'], 0, 1);
        $gender = $doc->createElement("gender");
        $gender->appendChild(
            $doc->createTextNode($gender_val)
        );
        $ClientCharacteristics->appendChild($gender);
    }

    $b->appendChild($ClientCharacteristics);
    ClientFreeformHealthplans($doc, $b, $pid);
    $allergyId=ClientFreeformAllergy($doc, $b, $pid);
    $r->appendChild($b);
    return $allergyId;
}
...
function ClientMedication($doc, $r, $pid, $med_limit)
{
    global $msg;
    $active='';
...
function ClientFreeformAllergy($doc, $r, $pid)
{
    ...
    while ($row=sqlFetchArray($res)) {
        ...
        $b = $doc->createElement("ClientFreeformAllergy");
...
function ClientFreeformHealthplans($doc, $r, $pid)
{
    ...
    while ($row = sqlFetchArray($resource)) {
        ...
        $clientFreeformHealthplans = $doc->createElement('ClientFreeformHealthplans');
        $clientFreeformHealthplans->appendChild($healthplanName);

        $r->appendChild($clientFreeformHealthplans);
    }
}
