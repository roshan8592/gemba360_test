...
require_once(dirname(__FILE__) . '/client_tracker.inc.php');
...
//
//===============================================================================
// Checks for the client's encounter ID for today, creating it if there is none.
//
function todaysEncounterCheck($client_id, $enc_date = '', $reason = '', $fac_id = '', $billing_fac = '', $provider = '', $cat = '', $return_existing = true)
{
    global $today;
    $encounter = todaysEncounterIf($client_id);
    ...
    addForm(
        ...
        sqlInsert(
            ...
            "encounter = ?," .
            "pc_catid = ?",
            array($dos,$visit_reason,$facility,$facility_id,$billing_facility,$visit_provider,$client_id,$encounter,$visit_cat)
        ),
        "newclient",
        $client_id,
        "1",
...
//===============================================================================
// Get the client's encounter ID for today, if it exists.
// In the case of more than one encounter today, pick the last one.
//
function todaysEncounterIf($client_id)
{
    global $today;
    $tmprow = sqlQuery("SELECT encounter FROM form_encounter WHERE " .
    "pid = ? AND date = ? " .
    "ORDER BY encounter DESC LIMIT 1", array($client_id,"$today 00:00:00"));
...
// Get the client's encounter ID for today, creating it if there is none.
//
function todaysEncounter($client_id, $reason = '')
{
  ...
  *******************************************************************/
    ...
    addForm(
        ...
        sqlInsert(
            ...
            "provider_id = ?",
            array($today, $today, $reason, $facility, $facility_id, $client_id,
