...
if ($_POST['bn_save']) {
 ...
 // Always convert it to jpeg.
    if ($_FILES['form_image']['size']) {
        // If the client's encounter image directory does not yet exist, create it.
        if (! is_dir($imagedir)) {
...
<script language='JavaScript'>
 ...
 function newEvt() {
  dlgopen('../../main/calendar/add_edit_event.php?clientid=' + <?php echo js_url($pid); ?>,
   '_blank', 775, 500);
  return false;
 }
 ...
 function deleteme() {
  dlgopen('../../client_file/deleter.php?formid=' + <?php echo js_url($formrow['id']); ?> + '&csrf_token_form=' + <?php echo js_url(CsrfUtils::collectCsrfToken()); ?>, '_blank', 500, 450);
  return false;
 }
