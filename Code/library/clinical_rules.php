 ...
 */
function clinical_summary_widget($client_id, $mode, $dateTarget = '', $organize_mode = 'default', $user = '')
{
  ...
  // Collect active actions
    $actions = test_rules_clinic('', 'passive_alert', $dateTarget, $mode, $client_id, '', $organize_mode, array(), 'primary', null, null, $user);
  ...
  // Display the actions
    ...
    foreach ($actions as $action) {
        ...
        if ($action['custom_flag']) {
            ...
            $url = "../rules/client_data.php?category=" . attr_url($action['category']);
  ...
  // Only when $mode is reminders-due
    if ($mode == "reminders-due" && $GLOBALS['enable_alert_log']) {
        $new_targets = compare_log_alerts($client_id, $current_targets, 'clinical_reminder_widget', $_SESSION['authId']);
 ...
 */
function active_alert_summary($client_id, $mode, $dateTarget = '', $organize_mode = 'default', $user = '', $test = false)
{
  ...
  // Collect active actions
    $actions = test_rules_clinic('', 'active_alert', $dateTarget, $mode, $client_id, '', $organize_mode, array(), 'primary', null, null, $user);
  ...
  // Only when $mode is reminders-due and $test is FALSE
    if (($mode == "reminders-due") && ($test === false) && ($GLOBALS['enable_alert_log'])) {
        $new_targets = compare_log_alerts($client_id, $current_targets, 'active_reminder_popup', $_SESSION['authId']);
 ...
 */
function allergy_conflict($client_id, $mode, $user, $test = false)
{
  ...
  // Check if allergies conflict with medications or prescriptions
    ...
    if (!empty($sqlParam)) {
        ...
        $res_rx = sqlStatement("SELECT `drug` FROM `prescriptions` WHERE `active`=1 " .
                           "AND `drug` IN (" . $sqlIN . ") AND `client_id`=?", $sqlParam);
  ...
  // If there are conflicts, $test is FALSE, and alert logging is on, then run through compare_log_alerts
    ...
    if ((!empty($conflicts_unique)) && $GLOBALS['enable_alert_log'] && ($test===false)) {
        $new_conflicts = compare_log_alerts($client_id, $conflicts_unique, 'allergy_alert', $_SESSION['authId'], $mode);
    }
 ...
 */
function compare_log_alerts($client_id, $current_targets, $category = 'clinical_reminder_widget', $userid = '', $log_trigger = 'all')
{
  ...
  // Collect most recent action_log
    $prior_targets_sql = sqlQuery("SELECT `value` FROM `clinical_rules_log` " .
                                 ...
                                 "ORDER BY `id` DESC LIMIT 1", array($category,$client_id,$userid));
    $prior_targets = array();
  ...
  //  or If $log_trigger=='new' and there are new items
    if (($log_trigger=='all') || (($log_trigger=='new')  && (!empty($new_targets)))) {
        ...
        sqlStatement("INSERT INTO `clinical_rules_log` " .
              ...
              "VALUES (NOW(),?,?,?,?,?)", array($client_id,$userid,$category,$current_targets_json,$new_targets_json));
    }
 ...
 *
 * Test the clinic rules of entire clinic and create a report or client reminders (can also test
 * on one client or clients of one provider). The structure of the returned results is dependent on the
 ...
 * @param  string       $type          rule filter (active_alert,passive_alert,cqm,cqm_2011,cqm_2014,amc,amc_2011,amc_2014,client_reminder). If blank then will test all rules.
 ...
 * @param  string       $pat_prov_rel  How to choose clients that are related to a chosen provider. 'primary' selects clients that the provider is set as primary provider. 'encounter' selectes clients that the provider has seen. This parameter is only applicable if the $provider parameter is set to a provider or collation setting.
 * @param  integer      $batchSize     number of clients to batch (default is 100; plan to optimize this default setting in the future)
...
function test_rules_clinic_batch_method($provider = '', $type = '', $dateTarget = '', $mode = '', $plan = '', $organize_mode = 'default', $options = array(), $pat_prov_rel = 'primary', $batchSize = '', $report_id = null)
{
  ...
  // Collect total number of pertinent clients (to calculate batching parameters)
    $totalNumClients = buildClientArray('', $provider, $pat_prov_rel, null, null, true);
  ...
  // Set ability to itemize report if this feature is turned on
    ...
    for ($i=0; $i<$totalNumberBatches; $i++) {
        ...
        if ($i == 0) {
            ...
            foreach ($dataSheet_batch as $key => $row) {
                if (!$row['is_sub']) {
                    ...
                    $total_clients = $dataSheet[$key]['total_clients'] + $row['total_clients'];
                    $dataSheet[$key]['total_clients'] = $total_clients;
        ...
        updateReportDatabase($report_id, $total_clients);
    }
 ...
 *
 * @param  integer      $provider      id of a selected provider. If blank, then will test entire clinic. If 'collate_outer' or 'collate_inner', then will test each provider in entire clinic; outer will nest plans  inside collated providers, while inner will nest the providers inside the plans (note inner and outer are only different if organize_mode is set to plans).
 * @param  string       $type          rule filter (active_alert,passive_alert,cqm,cqm_2011,cqm_2104,amc,amc_2011,amc_2014,client_reminder). If blank then will test all rules.
 * @param  string/array $dateTarget    target date (format Y-m-d H:i:s). If blank then will test with current date as target. If an array, then is holding two dates ('dateBegin' and 'dateTarget').
 * @param  string       $mode          choose either 'report' or 'reminders-all' or 'reminders-due' (required)
 ...
 * @param  string       $user          If a user is set, then will only show rules that user has permission to see(only applicable for per client and not when do reports).
 * @return array                       See above for organization structure of the results.
 */
function test_rules_clinic($provider = '', $type = '', $dateTarget = '', $mode = '', $client_id = '', $plan = '', $organize_mode = 'default', $options = array(), $pat_prov_rel = 'primary', $start = null, $batchSize = null, $user = '')
{
  ...
  // then run through this function recursively and return results.
    if (($provider == "collate_outer") || ($provider == "collate_inner" && $organize_mode != 'plans')) {
        ...
        while ($urow = sqlFetchArray($ures)) {
            $newResults = test_rules_clinic($urow['id'], $type, $dateTarget, $mode, $client_id, $plan, $organize_mode, $options, $pat_prov_rel, $start, $batchSize, $user);
            if (!empty($newResults)) {
  ...
  // function recursively and return results.
    if ($organize_mode == "plans") {
        ...
        $plans_resolve = resolve_plans_sql($plan, $client_id);
        ...
        foreach ($plans_resolve as $plan_item) {
            ...
            if ($provider == "collate_inner") {
                ...
                while ($urow = sqlFetchArray($ures)) {
                    $newResults = test_rules_clinic($urow['id'], $type, $dateTarget, $mode, $client_id, $plan_item['id'], 'default', $options, $pat_prov_rel, $start, $batchSize, $user);
                    if (!empty($newResults)) {
                        $provider_item['is_provider'] = true;
                        $provider_item['prov_lname'] = $urow['lname'];
                        $provider_item['prov_fname'] = $urow['fname'];
                ...
                $newResults = test_rules_clinic($provider, $type, $dateTarget, $mode, $client_id, $plan_item['id'], 'default', $options, $pat_prov_rel, $start, $batchSize, $user);
                if (!empty($newResults)) {
                    $plan_item['is_plan'] = true;
                    array_push($results, $plan_item);
                    $results = array_merge($results, $newResults);
  ...
  // Collect applicable client pids
    $clientData = array();
    $clientData = buildClientArray($client_id, $provider, $pat_prov_rel, $start, $batchSize);

  // Go through each client(s)
  //
  //  If in report mode, then tabulate for each rule:
  //    Total Clients
  //    Clients that pass the filter
  //    Clients that pass the target
  ...
  // Note that due to a limitation in the this function, the client_id is explicitly
  //  for grouping items when not being done in real-time or for official reporting.
  //  So for cases such as client reminders on a clinic scale, the calling function
  //  will actually need rather than pass in a explicit client_id for each client in
  //  a separate call to this function.
    if ($mode != "report") {
        // Use per client custom rules (if exist)
        // Note as discussed above, this only works for single client instances.
        $rules = resolve_rules_sql($type, $client_id, false, $plan, $user);
    } else { // $mode = "report"
        // Only use default rules (do not use client custom rules)
        $rules = resolve_rules_sql($type, $client_id, false, $plan, $user);
    }
    ...
    foreach ($rules as $rowRule) {
        ...
        if ($rowRule['cqm_flag'] || $rowRule['amc_flag']) {
            ...
            if ($rowRule['amc_flag']) {
                ...
                $tempResults = $manager->runReport($rowRule, $clientData, $dateArray, $options);
            } else {
                // Send target date
                $tempResults = $manager->runReport($rowRule, $clientData, $dateTarget);
            }
        ...
        if ($mode != "report") {
            ...
            if ($type == "client_reminder") {
                $reminder_interval_type = "client_reminder";
        ...
        if ((count($targetGroups) == 1) || ($mode == "report")) {
            ...
            foreach ($clientData as $rowClient) {
                // First, deal with deceased clients
                //  (for now will simply skip the client)
                // If want to support rules for deceased clients then will need to migrate this below
                // in target_dates foreach(guessing won't ever need to do this, though).
                // Note using the dateTarget rather than dateFocus
                if (is_client_deceased($rowClient['pid'], $dateTarget)) {
                    continue;
                }

                // Count the total clients
                $total_clients++;
                ...
                foreach ($target_dates as $dateFocus) {
                    ...
                    $passFilter = test_filter($rowClient['pid'], $rowRule['id'], $dateFocus);
                    ...
                    $passTarget = test_targets($rowClient['pid'], $rowRule['id'], '', $dateFocus);
                    if ($passTarget) {
                        // increment pass target counter
                        $pass_target++;
                        // If report itemization is turned on, then record the "passed" item and set the flag
                        if ($GLOBALS['report_itemizing_temp_flag_and_id']) {
                            insertItemReportTracker($GLOBALS['report_itemizing_temp_flag_and_id'], $GLOBALS['report_itemized_test_id_iterator'], 1, $rowClient['pid']);
                            $temp_track_pass = 1;
                        }
                ...
                if (!empty($GLOBALS['report_itemizing_temp_flag_and_id']) && !($temp_track_pass)) {
                    insertItemReportTracker($GLOBALS['report_itemizing_temp_flag_and_id'], $GLOBALS['report_itemized_test_id_iterator'], 0, $rowClient['pid']);
                }
            }
        }
        ...
        if ($mode == "report") {
            $newRow=array('is_main'=>true,'total_clients'=>$total_clients,'excluded'=>$exclude_filter,'pass_filter'=>$pass_filter,'pass_target'=>$pass_target,'percentage'=>$percentage);
            ...
            foreach ($targetGroups as $i) {
                ...
                foreach ($clientData as $rowClient) {
                    ...
                    foreach ($target_dates as $dateFocus) {
                        ...
                        //Check if pass target
                        $passTarget = test_targets($rowClient['pid'], $rowRule['id'], $i, $dateFocus);
                        if ($passTarget) {
                            // increment pass target counter
                            $pass_target++;
                            // If report itemization is turned on, then record the "passed" item and set the flag
                ...
                if ($mode == "report") {
                    $newRow=array('is_sub'=>true,'action_category'=>$action['category'],'action_item'=>$action['item'],'total_clients'=>'','excluded'=>'','pass_filter'=>'','pass_target'=>$pass_target,'percentage'=>$percentage);
...
/**
 * Process client array that is to be tested.
 *
 ...
 * @return array/integer                Array of client pid values or number total pertinent clients (if $onlyCount is TRUE)
 */
function buildClientArray($client_id = '', $provider = '', $pat_prov_rel = 'primary', $start = null, $batchSize = null, $onlyCount = false)
{

    if (!empty($client_id)) {
        // only look at the selected client
        if ($onlyCount) {
            $clientNumber = 1;
        } else {
            $clientData[0]['pid'] = $client_id;
        }
    } else {
        if (empty($provider)) {
            // Look at entire practice
            if ($start == null || $batchSize == null || $onlyCount) {
                $rez = sqlStatementCdrEngine("SELECT `pid` FROM `client_data` ORDER BY `pid`");
                if ($onlyCount) {
                    $clientNumber = sqlNumRows($rez);
                }
            } else {
                // batching
                $rez = sqlStatementCdrEngine("SELECT `pid` FROM `client_data` ORDER BY `pid` LIMIT ?,?", array(($start-1),$batchSize));
            }
        } else {
            // Look at an individual physician
            if ($pat_prov_rel == 'encounter') {
                // Choose clients that are related to specific physician by an encounter
                if ($start == null || $batchSize == null || $onlyCount) {
                ...
                // Choose clients that are assigned to the specific physician (primary physician in client demographics)
                if ($start == null || $batchSize == null || $onlyCount) {
                    $rez = sqlStatementCdrEngine("SELECT `pid` FROM `client_data` " .
    ...
    if ($onlyCount) {
        // return the number of applicable clients
        return $clientNumber;
    } else {
        // return array of client pids
        return $clientData;
    }
}

/**
 * Test filter of a selected rule on a selected client
 *
 * @param  integer        $client_id  pid of selected client.
 ...
 */
function test_filter($client_id, $rule, $dateTarget)
{
  ...
  // Collect client information
    $clientData = getClientData($client_id, "sex, DATE_FORMAT(DOB,'%Y %m %d') as DOB_TS");
  ...
  // Calculate client age in years and months
    $clientAgeYears = convertDobtoAgeYearDecimal($clientData['DOB_TS'], $dateTarget);
    $clientAgeMonths = convertDobtoAgeMonthDecimal($clientData['DOB_TS'], $dateTarget);
  ...
  // Min age (year) Filter (assume that there in not more than one of each)
    ...
    if (!empty($filter)) {
        ...
        if ($row ['method_detail'] == "year") {
            if ($row['value'] && ($row['value'] > $clientAgeYears)) {
                return false;
            }
        }

        if ($row ['method_detail'] == "month") {
            if ($row['value'] && ($row['value'] > $clientAgeMonths)) {
                return false;
            }
        }
    }
  ...
  // Gender Filter (assume that there in not more than one of each)
    ...
    if (!empty($filter)) {
        $row = $filter[0];
        if ($row['value'] && ($row['value'] != $clientData['sex'])) {
            return false;
        }
    }
  ...
  // Database Filter
    ...
    if ((!empty($filter)) && !database_check($client_id, $filter, '', $dateTarget)) {
        return false;
    }
  ...
  //   surgeries and allergies.
    ...
    if ((!empty($filter)) && !lists_check($client_id, $filter, $dateTarget)) {
        return false;
    }
  ...
  // Procedure Target (includes) (may need to include an interval in the future)
    ...
    if ((!empty($filter)) && !procedure_check($client_id, $filter, '', $dateTarget)) {
        return false;
    }
...
/**
 * Test targets of a selected rule on a selected client
 *
 * @param  integer  $client_id  pid of selected client.
 ...
 */
function test_targets($client_id, $rule, $group_id = '', $dateTarget)
{
  ...
  // Database Target (includes)
    ...
    if ((!empty($target)) && !database_check($client_id, $target, $interval, $dateTarget)) {
        return false;
    }
  ...
  // Procedure Target (includes)
    ...
    if ((!empty($target)) && !procedure_check($client_id, $target, $interval, $dateTarget)) {
        return false;
    }
  ...
  // Appointment Target (includes) (Specialized functionality for appointment reminders)
    ...
    if ((!empty($target)) && appointment_check($client_id, $dateTarget)) {
        return false;
    }
 ...
 * @param  integer  $client_id       pid of selected client. (if custom plan does not exist then will use the default plan)
 * @param  boolean  $configurableOnly true if only want the configurable (per client) plans (ie. ignore cqm plans)
 * @return array                      active plans
 */
function resolve_plans_sql($type = '', $client_id = '0', $configurableOnly = false)
{

    if ($configurableOnly) {
        // Collect all default, configurable (per client) plans into an array
  ...
  // Need to select rules (use custom if exist)
    foreach ($returnArray as $plan) {
        $customPlan = sqlQueryCdrEngine("SELECT * FROM `clinical_plans` WHERE `id`=? AND `pid`=?", array($plan['id'],$client_id));
 ...
 * @param  integer  $client_id  pid of selected client. (if set to 0, then will return the default rule).
 * @return array                 a plan
 */
function collect_plan($plan, $client_id = '0')
{

    return sqlQueryCdrEngine("SELECT * FROM `clinical_plans` WHERE `id`=? AND `pid`=?", array($plan,$client_id));
}

/**
 * Function to set a specific plan activity for a specific client
 *
 * @param  string   $plan        id(string) of plan
 ...
 * @param  integer  $client_id  pid of selected client.
 */
function set_plan_activity_client($plan, $type, $setting, $client_id)
{
  ...
  // Collect client specific plan, if already exists.
    $query = "SELECT * FROM `clinical_plans` WHERE `id` = ? AND `pid` = ?";
    $client_plan = sqlQueryCdrEngine($query, array($plan,$client_id));

    if (empty($client_plan)) {
        // Create a new client specific plan with flags all set to default
        $query = "INSERT into `clinical_plans` (`id`, `pid`) VALUES (?,?)";
        sqlStatementCdrEngine($query, array($plan, $client_id));
    }

  // Update client specific row
    $query = "UPDATE `clinical_plans` SET `" . escape_sql_column_name($type."_flag", array("clinical_plans")) . "`= ? WHERE id = ? AND pid = ?";
    sqlStatementCdrEngine($query, array($setting,$plan,$client_id));
}

/**
 * Function to return active rules
 *
 * @param  string   $type             rule filter (active_alert,passive_alert,cqm,cqm_2011,cqm_2014,amc_2011,amc_2014,client_reminder)
 * @param  integer  $client_id       pid of selected client. (if custom rule does not exist then will use the default rule)
 * @param  boolean  $configurableOnly true if only want the configurable (per client) rules (ie. ignore cqm and amc rules)
 * @param  string   $plan             collect rules for specific plan
 * @param  string   $user             If a user is set, then will only show rules that user has permission to see
 * @return array                      rules
 */
function resolve_rules_sql($type = '', $client_id = '0', $configurableOnly = false, $plan = '', $user = '')
{

    if ($configurableOnly) {
        // Collect all default, configurable (per client) rules into an array
    ...
    foreach ($returnArray as $rule) {
        ...
        if (!empty($user)) {
            ...
            if (!empty($access_control[0]) && !empty($access_control[1])) {
                ...
                // Section or ACO filters are empty, so use default clients:med aco
                if (!acl_check('clients', 'med', $user)) {
                    // User does not have access to this rule, so skip the rule.
                    continue;
                }
            }
        ...
        $customRule = sqlQueryCdrEngine("SELECT * FROM `clinical_rules` WHERE `id`=? AND `pid`=?", array($rule['id'],$client_id));
 ...
 * @return array                 rule
 */
function collect_rule($rule, $client_id = '0')
{

    return sqlQueryCdrEngine("SELECT * FROM `clinical_rules` WHERE `id`=? AND `pid`=?", array($rule,$client_id));
}

/**
 * Function to set a specific rule activity for a specific client
 *
 * @param  string   $rule        id(string) of rule
 * @param  string   $type        rule filter (active_alert,passive_alert,cqm,amc,client_reminder)
 * @param  string   $setting     activity of rule (yes,no,default)
 * @param  integer  $client_id  pid of selected client.
 */
function set_rule_activity_client($rule, $type, $setting, $client_id)
{
  ...
  //Collect main rule to allow setting of the access_control
    ...
    $client_rule_original = sqlQueryCdrEngine($original_query, array($rule));

  // Collect client specific rule, if already exists.
    $query = "SELECT * FROM `clinical_rules` WHERE `id` = ? AND `pid` = ?";
    $client_rule = sqlQueryCdrEngine($query, array($rule,$client_id));

    if (empty($client_rule)) {
        // Create a new client specific rule with flags all set to default
        $query = "INSERT into `clinical_rules` (`id`, `pid`, `access_control`) VALUES (?,?,?)";
        sqlStatementCdrEngine($query, array($rule, $client_id, $client_rule_original['access_control']));
    }

  // Update client specific row
    $query = "UPDATE `clinical_rules` SET `" . add_escape_custom($type) . "_flag`= ?, `access_control` = ? WHERE id = ? AND pid = ?";
    sqlStatementCdrEngine($query, array($setting,$client_rule_original['access_control'],$rule,$client_id));
}
 ...
 *
 * @param  string  $client_id  pid of selected client.
 ...
 */
function database_check($client_id, $filter, $interval = '', $dateTarget = '')
{
  ...
  // (Assume only one for now and only pertinent for targets)
    ...
    foreach ($filter as $row) {
        ...
        if ($temp_df[0] == "CUSTOM") {
            ...
            if (exist_custom_item($client_id, $temp_df[1], $temp_df[2], $temp_df[3], $temp_df[4], $temp_df[5], $intervalType, $intervalValue, $dateTarget)) {
                // Record the match
                $isMatch = true;
            } else {
            ...
            if (exist_lifestyle_item($client_id, $temp_df[1], $temp_df[2], $dateTarget)) {
                // Record the match
                $isMatch = true;
            } else {
               // If this is a required entry then return false
            ...
            if (exist_database_item($client_id, $temp_df[1], $temp_df[2], $temp_df[3], $temp_df[4], $temp_df[5], $temp_df[6], $intervalType, $intervalValue, $dateTarget)) {
                // Record the match
 ...
 * @return boolean              true if check passed, otherwise false
 */
function procedure_check($client_id, $filter, $interval = '', $dateTarget = '')
{
    $isMatch = false; //matching flag
  ...
  // (Assume only one for now and only pertinent for targets)
    ...
    foreach ($filter as $row) {
        ...
        $temp_df = explode("::", $row['value']);
        if (exist_procedure_item($client_id, $temp_df[0], $temp_df[1], $temp_df[2], $temp_df[3], $temp_df[4], $temp_df[5], $intervalType, $intervalValue, $dateTarget)) {
            // Record the match
            $isMatch = true;
        } else {
            // If this is a required entry then return false
 ...
 */
function appointment_check($client_id, $dateTarget = '')
{
    $isMatch = false; //matching flag
  ...
  //  then return true. (will not send reminders on same day as appointment)
    ...
    "AND openemr_postcalendar_events.pc_pid = ?", array($currentDate,$dateTarget,$client_id));
 ...
 * @return boolean              true if check passed, otherwise false
 */
function lists_check($client_id, $filter, $dateTarget)
{
    $isMatch = false; //matching flag
  ...
  // Set date to current if not set
    ...
    foreach ($filter as $row) {
        if (exist_lists_item($client_id, $row['method_detail'], $row['value'], $dateTarget)) {
            // Record the match
            $isMatch = true;
        } else {
           // If this is a required entry then return false
...
/**
 * Function to check for existance of data in database for a client
 *
 * @param  string   $client_id       pid of selected client.
 ...
 *
 */
function exist_database_item($client_id, $table, $column = '', $data_comp, $data = '', $num_items_comp, $num_items_thres, $intervalType = '', $intervalValue = '', $dateTarget = '')
{

  // Set date to current if not set
    $dateTarget = ($dateTarget) ? $dateTarget : date('Y-m-d H:i:s');

  // Collect the correct column label for client id in the table
    $client_id_label = collect_database_label('pid', $table);
  ...
  // check for items
    if (empty($column)) {
        ...
        " ". $whereTables. " ".
        "WHERE " . add_escape_custom($client_id_label)  . "=? " . $customSQL, array($client_id));
    } else {
        ...
        if ($whereTables=="" && strpos($table, 'form_')!== false) {
            ...
            $sql =sqlStatementCdrEngine(
                ...
                "AND b."  . add_escape_custom($client_id_label)  . "=? " . $customSQL
            ...
            $sql = sqlStatementCdrEngine("SELECT `" . add_escape_custom($column) . "` " .
              ...
              "AND " . add_escape_custom($client_id_label) . "=? " . $customSQL .
              $dateSql, array($data, $client_id));
        }
    }
...
/**
 * Function to check for existence of procedure(s) for a client
 *
 * @param  string   $client_id       pid of selected client.
 ...
 *
 */
function exist_procedure_item($client_id, $proc_title, $proc_code, $result_comp, $result_data = '', $num_items_comp, $num_items_thres, $intervalType = '', $intervalValue = '', $dateTarget = '')
{
  ...
  // mdsupport : Allow trailing '**' in the strings to perform LIKE searches
    ...
    $sql_query .= "(procedure_type.name = ? AND procedure_type.name != '') ) " .
                ...
                "AND " . add_escape_custom($client_id_label) . " = ? " . $dateSql;
    array_push($sqlBindArray, $proc_title, $result_data, $client_id);
...
/**
 * Function to check for existance of data for a client in the rule_client_data table
 *
 * @param  string   $client_id       pid of selected client.
 ...
 * @return boolean                    true if check passed, otherwise false
 */
function exist_custom_item($client_id, $category, $item, $complete, $num_items_comp, $num_items_thres, $intervalType = '', $intervalValue = '', $dateTarget)
{

  // Set the table
    $table = 'rule_client_data';
  ...
  // search for number of specific items
    ...
    "AND `item`=? " .
    "AND `complete`=? " .
    "AND `" . add_escape_custom($client_id_label)  . "`=? " .
    $dateSql, array($category,$item,$complete,$client_id));
...
/**
 * Function to check for existance of data for a client in lifestyle section
 *
 * @param  string  $client_id  pid of selected client.
 * @param  string  $lifestyle   selected label of mysql column of client history
 * @param  string  $status      specific status of selected lifestyle element
 * @param  string  $dateTarget  target date(format Y-m-d H:i:s). blank is current date.
 * @return boolean              true if check passed, otherwise false
 */
function exist_lifestyle_item($client_id, $lifestyle, $status, $dateTarget)
{
  ...
  // Collect pertinent history data
    $history = getHistoryData($client_id, $lifestyle, '', $dateTarget);
...
/**
 * Function to check for lists item of a client. Fully customizable and includes diagnoses, medications,
 ...
 *
 */
function exist_lists_item($client_id, $type, $value, $dateTarget)
{
  ...
  // Attempt to explode the value into a code type and code (if applicable)
    ...
    if (count($value_array) == 2) {
        ...
        if ($code_type=='CUSTOM') {
            ...
            "AND ( (`enddate` IS NULL) OR (`enddate` IS NOT NULL AND `enddate`>=?) )", array($type,$client_id,$code,$dateTarget,$dateTarget,$dateTarget));
            if (!empty($response)) {
                return true;
            }
        } else {
            ...
            "AND `diagnosis` LIKE ? " .
            "AND ( (`begdate` IS NULL AND `date`<=?) OR (`begdate` IS NOT NULL AND `begdate`<=?) ) " .
            "AND ( (`enddate` IS NULL) OR (`enddate` IS NOT NULL AND `enddate`>=?) )", array($type,$client_id,"%".$code_type.":".$code."%",$dateTarget,$dateTarget,$dateTarget));
            if (!empty($response)) {
                return true;
            }
        }
        ...
        "AND `title` $sqloper ".
        "AND ( (`begdate` IS NULL AND `date`<=?) OR (`begdate` IS NOT NULL AND `begdate`<=?) ) " .
        "AND ( (`enddate` IS NULL) OR (`enddate` IS NOT NULL AND `enddate`>=?) )", array($type,$client_id,$value,$dateTarget,$dateTarget,$dateTarget));
        if (!empty($response)) {
            return true;
        }
        ...
        if ($type == 'medication') { // Special case needed for medication as it need to be looked into current medications (prescriptions table) from ccda import
              $response = sqlQueryCdrEngine("SELECT * FROM `prescriptions` where `client_id` = ? and `drug` $sqloper and `date_added` <= ?", array($client_id,$value,$dateTarget));
            if (!empty($response)) {
                return true;
            }
        }
...
function collect_database_label($label, $table)
{
    ...
    if ($table == 'PROCEDURE-EXCEPTION') {
        ...
        if ($label == "pid") {
            $returnedLabel = "procedure_order.client_id";
        } else if ($label == "date") {
 ...
 * @param  string  $rule        id(string) of selected rule
 * @param  string  $dateTarget  target date(format Y-m-d H:i:s).
 * @param  string  $type        either 'client_reminder' or 'clinical_reminder'
 ...
 *
 * @param  integer  $pass_filter     number of clients that pass filter
 * @param  integer  $exclude_filter  number of clients that are excluded
 * @param  integer  $pass_target     number of clients that pass target
