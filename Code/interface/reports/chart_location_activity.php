<?php
/**
 * This reports checkins and checkouts for a specified client's chart.
 *
 * @package   OpenEMR
...
$curr_pid = $pid;
$ptrow = array();
if (!empty($form_client_id)) {
    $query = "SELECT pid, pubpid, fname, mname, lname FROM client_data WHERE " .
    "pubpid = ? ORDER BY pid LIMIT 1";
    $ptrow = sqlQuery($query, array($form_client_id));
    if (empty($ptrow)) {
        $curr_pid = 0;
        echo "<font color='red'>" . xlt('Chart ID') . " '" . text($form_client_id) . "' " . xlt('not found!') . "</font><br />&nbsp;<br />";
    } else {
        $curr_pid = $ptrow['pid'];
    }
  ...
  <td width='200px'>
    ...
    <table class='text'>
        <tr>
            ...
            <td>
               <input type='text' name='form_client_id' class='form-control' size='10' maxlength='31' value='<?php echo attr($form_client_id) ?>'
                title='<?php echo xla('Client ID'); ?>' />
            </td>
        </tr>
    </table>
...
<tbody>
    ...
    $row = array();
    if (!empty($ptrow)) {
        $res = ClientService::getChartTrackerInformationActivity($curr_pid);
