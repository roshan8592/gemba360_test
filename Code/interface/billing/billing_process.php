    ...
    foreach ($ar['claims'] as $claimid => $claim_array) {
        ...
        if (isset($claim_array['bill'])) {
            if (isset($ar['bn_external'])) {
                ...
                $be->addClaim($client_id, $encounter);
            } else {
            ...
            if (!$validate_claim) {
                if ($clear_claim) {
                    $tmp = BillingUtilities::updateClaim(true, $client_id, $encounter, $payer_id, $payer_type, 2); // $sql .= " billed = 1, ";
                }
                if (isset($ar['bn_x12']) || isset($ar['bn_x12_encounter']) && !$clear_claim) {
                    $tmp = BillingUtilities::updateClaim(true, $client_id, $encounter, $payer_id, $payer_type, 1, 1, '', $target, $claim_array['partner']);
                } elseif (isset($ar['bn_ub04_x12'])) {
                    $ub04id = get_ub04_array($client_id, $encounter);
                    $ub_save = json_encode($ub04id);
                    $tmp = BillingUtilities::updateClaim(true, $client_id, $encounter, $payer_id, $payer_type, 1, 1, '', $target, $claim_array['partner'] . '-837I', 0, $ub_save);
                } elseif (isset($ar['bn_process_ub04_form']) || isset($ar['bn_process_ub04'])) {
                    $ub04id = get_ub04_array($client_id, $encounter);
                    $ub_save = json_encode($ub04id);
                    $tmp = BillingUtilities::updateClaim(true, $client_id, $encounter, $payer_id, $payer_type, 1, 1, '', 'ub04', -1, 0, $ub_save);
                } elseif (isset($ar['bn_process_hcfa']) || isset($ar['bn_hcfa_txt_file']) || isset($ar['bn_process_hcfa_form']) && !$clear_claim) {
                    $tmp = BillingUtilities::updateClaim(true, $client_id, $encounter, $payer_id, $payer_type, 1, 1, '', 'hcfa');
                } elseif (isset($ar['bn_mark'])) {
                    // $sql .= " billed = 1, ";
                    $tmp = BillingUtilities::updateClaim(true, $client_id, $encounter, $payer_id, $payer_type, 2);
                } elseif (isset($ar['bn_reopen'])) {
                    $tmp = BillingUtilities::updateClaim(true, $client_id, $encounter, $payer_id, $payer_type, 1, 0);
            ...
            if (! $tmp) {
                ...
                if (isset($ar['bn_mark'])) {
                    ...
                    $log = '';
                    $segs = explode("~\n", X12_5010_837P::gen_x12_837($client_id, $encounter, $log, isset($ar['bn_x12_encounter'])));
                    $hlog .= $log;
                    append_claim($segs);
                    if ($validate_claim || $clear_claim) {
                        continue;
                    }
                    if (! BillingUtilities::updateClaim(false, $client_id, $encounter, - 1, - 1, 2, 2, $bat_filename)) {
                        $bill_info[] = xl("Internal error: claim ") . $claimid . xl(" not found!") . "\n";
                    }
                } elseif (isset($ar['bn_ub04_x12'])) {
                    $log = '';
                    $segs = explode("~\n", generate_x12_837I($client_id, $encounter, $log, $ub04id));
                    $hlog .= $log;
                    append_claim($segs);
                    if ($validate_claim || $clear_claim) {
                        continue;
                    }
                    if (! BillingUtilities::updateClaim(false, $client_id, $encounter, - 1, - 1, 2, 2, $bat_filename, 'X12-837I', - 1, 0, json_encode($ub04id))) {
                        $bill_info[] = xl("Internal error: claim ") . $claimid . xl(" not found!") . "\n";
                    }
                } elseif (isset($ar['bn_process_hcfa'])) {
                    $log = '';
                    $hcfa = new HCFA_1500();
                    $lines = $hcfa->gen_hcfa_1500($client_id, $encounter, $log);
                    $hlog .= $log;
                    ...
                    $claim_count ++;
                    $log = "";
                    $template[] = buildTemplate($client_id, $encounter, "", "", $log);
                    $hlog .= $log;
                    if ($validate_claim || $clear_claim) {
                        continue;
                    }
                    if (! BillingUtilities::updateClaim(false, $client_id, $encounter, - 1, - 1, 2, 2, $bat_filename, 'ub04', - 1, 0, json_encode($ub04id))) {
                        $bill_info[] = xl("Internal error: claim ") . $claimid . xl(" not found!") . "\n";
                    }
