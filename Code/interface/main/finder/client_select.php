...
use OpenEMR\Events\ClientSelect\ClientSelectFilterEvent;
...
<form method='post' action='client_select.php' name='theform' onsubmit='return top.restoreSession()'>
...
//the maximum number of client records to display:
$sqllimit = $MAXSHOW;
$given = "*";
...
if ($popup) {
  ...
  // Construct WHERE clause and save search parameters as form fields.
    ...
    while ($frow = sqlFetchArray($fres)) {
        ...
        if (!empty($_REQUEST[$field_id])) {
            ...
            if ($field_id == 'pid') {
                $where .= " AND " . escape_sql_column_name($field_id, array('client_data')) . " = ?";
                array_push($sqlBindArray, $value);
            } else if ($field_id == 'pubpid') {
                $where .= " AND " . escape_sql_column_name($field_id, array('client_data')) . " LIKE ?";
                array_push($sqlBindArray, $value);
                //for 'date' field
            } else if ($data_type == 4) {
                $where .= " AND " . escape_sql_column_name($field_id, array('client_data')) . " LIKE ?";
  ...
  // If a non-empty service code was given, then restrict to clients who
  ...
  // clause, % and _ wildcards are supported.
    ...
    // Custom filtering which enables module developer to filter clients out of search
    $clientSelectFilterEvent = new ClientSelectFilterEvent(new BoundFilter());
    $clientSelectFilterEvent = $GLOBALS["kernel"]->getEventDispatcher()->dispatch(ClientSelectFilterEvent::EVENT_HANDLE, $clientSelectFilterEvent, 10);
    $boundFilter = $clientSelectFilterEvent->getBoundFilter();
    ...
    $sql = "SELECT $given FROM client_data " .
    ...
    _set_client_inc_count($sqllimit, count($result), "$customWhere AND $where", $sqlBindArray);
  ...
  // Collect setting from cdr report
    ...
    $print_clients = isset($_REQUEST['print_clients'])? $_REQUEST['print_clients'] : 0;
    echo "<input type='hidden' name='print_clients' value='" . attr($print_clients) . "' />\n";

  // Collect client listing from cdr report
    if ($print_clients) {
        // collect entire listing for printing
        $result = collectItemizedClientsCdrReport($report_id, $itemized_test_id, $pass_id, $numerator_label);
        $GLOBALS['CLIENT_INC_COUNT'] = count($result);
        $MAXSHOW = $GLOBALS['CLIENT_INC_COUNT'];
    } else {
        // collect the total listing count
        $GLOBALS['CLIENT_INC_COUNT'] = collectItemizedClientsCdrReport($report_id, $itemized_test_id, $pass_id, $numerator_label, true);
        // then just collect applicable list for pagination
        $result = collectItemizedClientsCdrReport($report_id, $itemized_test_id, $pass_id, $numerator_label, false, $sqllimit, $fstart);
    }
} else {
    ...
    echo "<input type='hidden' name='client' value='" . attr($client) . "' />\n";
    echo "<input type='hidden' name='findBy'  value='" . attr($findBy) . "' />\n";

    if ($findBy == "Last") {
        $result = getClientLnames($client, $given, $orderby, $sqllimit, $fstart);
    } else if ($findBy == "ID") {
        $result = getClientId($client, $given, "id ASC, ".$orderby, $sqllimit, $fstart);
    } else if ($findBy == "DOB") {
        $result = getClientDOB(DateToYYYYMMDD($client), $given, "DOB ASC, ".$orderby, $sqllimit, $fstart);
    } else if ($findBy == "SSN") {
        $result = getClientSSN($client, $given, "ss ASC, ".$orderby, $sqllimit, $fstart);
    } elseif ($findBy == "Phone") {                  //(CHEMED) Search by phone number
        $result = getClientPhone($client, $given, $orderby, $sqllimit, $fstart);
    } else if ($findBy == "Any") {
        $result = getByClientDemographics($client, $given, $orderby, $sqllimit, $fstart);
    } else if ($findBy == "Filter") {
        $result = getByClientDemographicsFilter(
            $searchFields,
            $client,
            $given,
            $orderby,
 ...
 <tr>
  <td class='text'>
   ...
   <a href='../../reports/cqm.php?report_id=<?php echo attr_url($report_id); ?>' class='css_button' onclick='top.restoreSession()'><span><?php echo xlt("Return To Report Results"); ?></span></a>
    <?php } else { ?>
   <a href="./client_select_help.php" target=_new onclick='top.restoreSession()'>[<?php echo xlt('Help'); ?>]&nbsp</a>
    <?php } ?>
  </td>
...
<?php if ($message) {
  ...
  <td>
    <?php if ($from_page == "cdr_report") { ?>
        <?php echo "<a href='client_select.php?from_page=cdr_report&pass_id=" . attr_url($pass_id) . "&report_id=" . attr_url($report_id) . "&itemized_test_id=" . attr_url($itemized_test_id) . "&numerator_label=" . attr_url($row['numerator_label']) . "&print_clients=1&csrf_token_form=" . attr_url(CsrfUtils::collectCsrfToken()) . "' class='css_button' onclick='top.restoreSession()'><span>" . xlt("Print Entire Listing") . "</span></a>"; ?>
    <?php } ?> &nbsp;
  </td>
...
//
// $count = $fstart + $GLOBALS['CLIENT_INC_COUNT']; // Why did I do that???
$count = $GLOBALS['CLIENT_INC_COUNT'];
$fend = $fstart + $MAXSHOW;
if ($fend > $count) {
    $fend = $count;
}
...
<?php } ?>
 ...
 <tr>
    <?php if ($from_page == "cdr_report") {
        ...
        echo "<b>";
        if ($pass_id == "fail") {
             echo xlt("Failed Clients");
        } else if ($pass_id == "pass") {
             echo xlt("Passed Clients");
        } else if ($pass_id == "exclude") {
             echo xlt("Excluded Clients");
        } else { // $pass_id == "all"
             echo xlt("All Clients");
        }
...
<?php if (empty($GLOBALS['client_search_results_style'])) { ?>
...
<th class="srDateNext">
    <?php
    $add_days = 90;
    if (!$popup && preg_match('/^(\d+)\s*(.*)/', $client, $matches) > 0) {
        $add_days = $matches[1];
        $client = $matches[2];
    }
    ?>
[<?php echo attr($add_days);?> <?php echo xlt('Days From Last Encounter'); ?>]
  ...
  // Alternate client search results style; this gets address plus other
...
if ($result) {
    foreach ($result as $iter) {
        ...
        if (empty($GLOBALS['client_search_results_style'])) {
...
$(function (){
    ...
    $(".oneresult").click(function() { SelectClient(this); });
    // $(".event").dblclick(function() { EditEvent(this); });
    <?php if ($print_clients) { ?>
      var win = top.printLogPrint ? top : opener.top;
      win.printLogPrint(window);
    <?php } ?>
});

var SelectClient = function (eObj) {
...
// will set the pid and load all the other frames.
    ...
    <?php if (!$popup) { ?>
        top.restoreSession();
        document.location.href = "../../client_file/summary/demographics.php?set_pid=" + parts[0];
    <?php } else if ($popup) { ?>
