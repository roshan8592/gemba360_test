...
function getQRDAStratumInfo($patArr, $begin_date)
{
    ...
    if (count($patArr) > 0) {
        ...
        $stratumOneQry = "SELECT FLOOR( DATEDIFF( '".add_escape_custom($begin_date)."' , DOB ) /365 ) as pt_age FROM client_data WHERE pid IN (".add_escape_custom(implode(",", $patArr)).") HAVING  (pt_age BETWEEN 1 AND 10) ";
        $stratumOneRes = sqlStatement($stratumOneQry);
        $stratumOneRows = sqlNumRows($stratumOneRes);

        //Age Between 12 and 17
        $stratumTwoQry = "SELECT FLOOR( DATEDIFF( '".add_escape_custom($begin_date)."' , DOB ) /365 ) as pt_age FROM client_data WHERE pid IN (".add_escape_custom(implode(",", $patArr)).") HAVING  (pt_age BETWEEN 11 AND 16) ";
...
function getQRDAClientNeedInfo($patArr)
{
    ...
    $mainArr = array();
    if (count($patArr) > 0) {
        $patRes = sqlStatement("SELECT pid, sex, race, ethnicity FROM client_data WHERE pid IN (".add_escape_custom(implode(",", $patArr)).")");
...
function payerClient($client_id)
{
    $payer = 'Other';
    $insQry = "SELECT insd.*, ic.ins_type_code FROM (SELECT pid, provider FROM insurance_data WHERE type = 'primary' ORDER BY id DESC) insd ".
              "INNER JOIN  insurance_companies ic ON insd.provider = ic.id ".
              "WHERE insd.pid = ?";
    $insRes = sqlStatement($insQry, array($client_id));
    while ($insRow = sqlFetchArray($insRes)) {
        if ($insRow['ins_type_code'] == 8) {//Self Pay (Private Insurance)
...
function allEncPat($client_id, $from_date, $to_date)
{
    $encArr = array();
    ...
    $patRes = sqlStatement($patQry, array($client_id, $from_date, $to_date));
    while ($patRow = sqlFetchArray($patRes)) {
        $encArr[] = $patRow;
    }
...
function allListsPat($type, $client_id, $from_date, $to_date)
{
    $diagArr = array();
    $diagQry = "SELECT * FROM lists WHERE TYPE = ? AND pid = ? AND (DATE(date) BETWEEN ? AND ?)";
    $diagRes = sqlStatement($diagQry, array($type, $client_id, $from_date, $to_date));
...
function allOrderMedsPat($client_id, $from_date, $to_date)
{
    $medArr = array();
    $medQry = "SELECT * FROM prescriptions where client_id = ? AND active = 0 AND (DATE(date_added) BETWEEN ? AND ?)";
    $medRes = sqlStatement($medQry, array($client_id, $from_date, $to_date));
...
function allActiveMedsPat($client_id, $from_date, $to_date)
{
    $medArr = array();
    $medQry = "SELECT * FROM prescriptions where client_id = ? AND active = 1 AND (DATE(date_added) BETWEEN ? AND ?)";
    $medRes = sqlStatement($medQry, array($client_id, $from_date, $to_date));
    while ($medRow = sqlFetchArray($medRes)) {
        $medArr[] = $medRow;
    }
...
function allProcPat($proc_type = "Procedure", $client_id, $from_date, $to_date)
{
    $procArr = array();
    $procQry = "SELECT poc.procedure_code, poc.procedure_name, po.date_ordered, fe.encounter,fe.date FROM form_encounter fe ".
            ...
            "WHERE poc.procedure_order_title = ? AND po.client_id = ? ".
            "AND (po.date_ordered BETWEEN ? AND ?)";
    $procRes = sqlStatement($procQry, array($proc_type, $client_id, $from_date, $to_date));
...
function allVitalsPat($client_id, $from_date, $to_date)
{
    $vitArr = array();
    ...
    $vitRes = sqlStatement($vitQry, array($client_id, $from_date, $to_date));
...
function allImmuPat($client_id, $from_date, $to_date)
{
    $immArr = array();
    $immQry =   "SELECT * FROM immunizations ".
            "WHERE client_id = ? ".
            "AND (administered_date BETWEEN ? AND ?)";
    $immRes = sqlStatement($immQry, array($client_id, $from_date, $to_date));
...
function getPatData($client_id)
{
    $clientRow = sqlQuery("SELECT * FROM client_data WHERE pid= ?", array($client_id));
    return $clientRow;
}
...
function clientQRDAHistory($client_id)
{
    $clientHistRow = sqlQuery("SELECT tobacco, date FROM history_data WHERE pid= ? ORDER BY id DESC LIMIT 1", array($client_id));
    return $clientHistRow;
}
