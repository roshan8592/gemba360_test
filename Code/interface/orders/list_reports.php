...
// Check authorization.
$thisauth = acl_check('clients', 'med');
if (!$thisauth) {
...
var dlgtitle = <?php echo xlj("Match Client") ?>;
...
// see the results concurrently with other stuff like related client notes.
// Opening in the other frame is not good enough because if they then do related
// clients notes it will wipe out this script's list. We need 3 viewports.
...
// Invokes the client matching dialog.
// args is a PHP-serialized array of client attributes.
...
function openPtMatch(args) {
    top.restoreSession();
    dlgopen('client_match_dialog.php?key=' + encodeURIComponent(args), '_blank', 850, 400, '', dlgtitle);
}
...
<form class="form-inline" method='post' action='list_reports.php' enctype='multipart/form-data'>
    ...
    if (empty($_POST['form_external_refresh'])) {
        // Get client matching selections from this form if there are any.
    ...
    // Display a row for each required client matching decision or message.
    $s = '';
    $matchreqs = false;
    $errors = false;
    $orphan_orders = false;

    // Generate HTML to request client matching.
    if (is_array($info['match'])) {
        foreach ($info['match'] as $matchkey => $matchval) {
            ...
            $s .= xlt('Click to match client') . ' "' . text($tmp['lname']) . ', ' . text($tmp['fname']) . '"';
            $s .= "</a>";
            $s .= "</td>\n";
    ...
    if ($s) {
        ...
        if ($matchreqs || $errors) {
            echo "<p class='bold' style='color:#008800'>";
            if ($matchreqs) {
                echo xlt('Click where indicated above to match the client.') . ' ';
                echo xlt('After that the Match column will show the selected client ID, or 0 to create.') . ' ';
                echo xlt('If you do not select a match the client will be created.') . ' ';
            }
    ...
    $form_client = !empty($_POST['form_client']);
    ...
    <table>
        <tr>
            <td class='text' align='center'>
                ...
                <input class="form-control" type='checkbox' name='form_client' id="ck_client" value='1'
                    <?php if ($form_client) {
                        echo 'checked ';
                    } ?>/>
                <label for="ck_client"><?php echo xlt('Current Pt Only'); ?></label>
    ...
    <table class="table table-bordered table-condensed table-striped table-hover">
        <thead>
        <tr class='head'>
            <th colspan='2'><?php echo xlt('Client'); ?></th>
        ...
        <?php
        $selects =
            "po.client_id, po.procedure_order_id, po.date_ordered, po.date_transmitted, " .
        ...
        $query = "SELECT " .
            ...
            "LEFT JOIN client_data AS pd ON pd.pid = po.client_id $joins " .
            "WHERE $where " .
            "ORDER BY pd.lname, pd.fname, pd.mname, po.client_id, $orderby";
        ...
        while ($row = sqlFetchArray($res)) {
            $client_id = empty($row['client_id']) ? 0 : ($row['client_id'] + 0);
...
// Generate client columns.
            if ($lastptid != $client_id) {
                $lastpoid = -1;
                echo "  <td onclick='openClient(" . attr_js($client_id) . ")' style='cursor:pointer;color:blue'>";
                echo text($ptname);
                echo "</td>\n";
