...
if (isset($_POST["privatemode"]) && $_POST["privatemode"] =="user_admin") {
    if ($_POST["mode"] == "update") {
      ...
      //Display message when Emergency Login user was activated
        ...
        if ($_POST["client_menu_role"]) {
            $clientMenuRole = filter_input(INPUT_POST, 'client_menu_role');
            sqlStatement("update `users` set `client_menu_role` = ? where `id` = ? ", array($clientMenuRole, $_POST["id"]));
        }
    ...
    if ($_POST["mode"] == "new_user") {
        ...
        if ($doit == true) {
          ...
          //if password expiration option is enabled,  calculate the expiration date of the password
            ...
            "', client_menu_role = '"  . add_escape_custom(trim((isset($_POST['client_menu_role']) ? $_POST['client_menu_role'] : ''))) .
