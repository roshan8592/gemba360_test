...
if (substr($_POST['hidden_mode'], 0, 3) == 'add') {
  ...
  //or, CAMOS has been started freshly, therefore the last entry of the current client should be selected.
  $preselect_mode = '';
if ($preselect_category == '' && !$out_of_encounter) {
    ...
    //so let's get the most recent values from form_CAMOS for this client's pid
...
//if (!$out_of_encounter) { //do not do stuff that is encounter specific if not in an encounter
  ...
  //as above
    ...
    if (substr($_POST['hidden_mode'], 0, 5) == 'clone') {
        ...
        $name_data_flag = false; //flag to see if we are going to use client names in search result of clone others.
        $show_phone_flag = false; //if we do show client names, flag to see if we show phone numbers too
            ...
            (preg_match('/^(codes)(.*)/', $clone_search, $matches))) {
                ...
                while ($result = sqlFetchArray($statement)) {
                    ...
                    if ($name_data_flag === true) {
                            $tmp = getMyClientData($result['id'], $show_phone_flag)."\n$break\n".$tmp;
                    }
...
if (!$out_of_encounter) {
    ?>
    f2.action = '<?php print $GLOBALS['webroot'] ?>/interface/client_file/encounter/load_form.php?formname=CAMOS';
    <?php
} else {
    ?>
...
function searchName($string)
{
 //match one or more names and return clause for query of pids
    ...
    $query = sqlStatement("select pid from client_data where fname like ? or fname like ? or " .
...
function getMyClientData($form_id, $show_phone_flag)
{
//return a string of client data and encounter data based on the form_CAMOS id
    $ret = '';
    $name = '';
    $dob = '';
    $enc_date = '';
    ...
    "from client_data as t1 join forms as t2 on (t1.pid = t2.pid) where t2.form_id=? " .
