...
abstract class AbstractAmcReport implements RsReportIF
{
    ...
    public function __construct(array $rowRule, array $clientIdArray, $dateTarget, $options)
    {
        ...
        $this->_amcPopulation = new AmcPopulation($clientIdArray);
    ...
    public function execute()
    {
        ...
        //
        // Note that when AMC rules supports different client populations and
        ...
        $totalClients = count($this->_amcPopulation);

        // Figure out object to be counted
        //   (clients, labs, transitions, visits, or prescriptions)
        $object_to_count = $this->getObjectToCount();
        if (empty($object_to_count)) {
            $object_to_count="clients";
        }
        
        $numeratorObjects = 0;
        $denominatorObjects = 0;
        foreach ($this->_amcPopulation as $client) {
            // If begin measurement is empty, then make the begin
            //  measurement the client dob.
            $tempBeginMeasurement = "";
            if (empty($this->_beginMeasurement)) {
                $tempBeginMeasurement = $client->dob;
            } else {
                $tempBeginMeasurement = $this->_beginMeasurement;
            }

            // Count Denominators
            if ($object_to_count == "clients") {
                // Counting clients
                if (!$denominator->test($client, $tempBeginMeasurement, $this->_endMeasurement)) {
                    continue;
                }

                $denominatorObjects++;
            } else {
                // Counting objects other than clients
                //   First, collect the pertinent objects
                $objects = $this->collectObjects($client, $object_to_count, $tempBeginMeasurement, $this->_endMeasurement);
            ...
            if ($object_to_count == "clients") {
                // Counting clients
                if (!$numerator->test($client, $tempBeginMeasurement, $this->_endMeasurement)) {
                    // If itemization is turned on, then record the "failed" item
                    if ($GLOBALS['report_itemizing_temp_flag_and_id']) {
                        insertItemReportTracker($GLOBALS['report_itemizing_temp_flag_and_id'], $GLOBALS['report_itemized_test_id_iterator'], 0, $client->id);
                    }
                    ...
                    if ($GLOBALS['report_itemizing_temp_flag_and_id']) {
                        insertItemReportTracker($GLOBALS['report_itemizing_temp_flag_and_id'], $GLOBALS['report_itemized_test_id_iterator'], 1, $client->id);
                    }
                }
            } else {
                // Counting objects other than clients
        ...
        $result = new AmcResult($this->_rowRule, $totalClients, $denominatorObjects, 0, $numeratorObjects, $percentage);
        $this->_resultsArray[]= $result;
    }

    private function collectObjects($client, $object_label, $begin, $end)
    {
        ...
        switch ($object_label) {
            case "transitions-in":
                 $sql = "SELECT amc_misc_data.map_id as `encounter`, amc_misc_data.date_completed as `completed`, form_encounter.date as `date` " .
                        ...
                        "AND ((opc.pc_catname = 'New Client') OR (opc.pc_catname = 'Established Client' AND amc_misc_data.soc_provided is not null)) ";
                array_push($sqlBindArray, $client->id, $client->id, $begin, $end);
                break;
            case "transitions-out":
                ...
                array_push($sqlBindArray, $client->id, $begin, $end);
                break;
            case "encounters":
                $sql = "SELECT * " .
                       "FROM `form_encounter` " .
