...
# to its category, and its name must begin with the client's pubpid
...
# This is the user from whom client notes are addressed:
#
my $SCANNER_OPERATOR = 'ksears';

# This person gets client notes if the doctor cannot be determined or is
# out of the office:
#
...
# Generate a client note if appropriate for this top-level category.
#
sub generate_note($$$) {
  ...
  # this client.  That's who we'll send the note to.
  #
  ...
  if (@frow) {
    ...
    &tolog("Client $pid has no clinical forms, using default practitioner", 0);
  }
  ...
  &tolog("Client note assigned to $assigned_to", 0);
}
...
sub process_file($$) {
  ...
  # Get the chart number and look up the client's pid.
  #
  my $pubpid = '';
  if ($fname =~ /^([A-Za-z0-9]+)/) {
    $pubpid = $1;
  ...
  #
  my $psth = $dbh->prepare("SELECT pid FROM client_data " .
  ...
  #
  if (! @prow) {
    &tolog("$path: there is no client with chart id '$pubpid'", 1);
    rename "$INPATH/$path", "$INPATH/$dname/ERR-$fname" if (! $DEBUG);
    return;
  }
  if ($psth->fetchrow_array()) {
    &tolog("$path: there are multiple clients with chart id '$pubpid'", 1);
    rename "$INPATH/$path", "$INPATH/$dname/ERR-$fname" if (! $DEBUG);
    return;
  }
  my $pid = $prow[0];
  ...
  # Generate the client note if appropriate for this category.
  #
