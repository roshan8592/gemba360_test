<?php
/**
 * Client selector for insurance gui
 *
 * @package   OpenEMR
...
<form border='0' method='post' name="find_client" action="browse.php?browsenum=<?php echo attr_url($browsenum); ?>">
<input type="hidden" name="csrf_token_form" value="<?php echo attr(CsrfUtils::collectCsrfToken()); ?>" />

<input type='entry' size='10' name='client' id='searchparm'>
...
<a href="javascript:document.find_client.submit();" class="link"><?php echo xlt('Find'); ?></a>&nbsp;&nbsp;
...
if (isset($_GET{'set_pid'})) {
    ...
    $result = getClientData($_GET{'set_pid'});
...
<span class=bold><?php echo xlt('ID'); ?></span>
    ...
    $findby = $_POST['findBy'];
    $client = $_POST['client'];
    if ($findby == "Last" && $result = getClientLnames("$client", "*")) {
    ...
    if ($findby == "ID" && $result = getClientId("$client", "*")) {
        foreach ($result as $iter) {
            if ($total >= $M) {
                break;
            }
    ...
    if ($findby == "DOB" && $result = getClientDOB(DateToYYYYMMDD($client), "*")) {
        foreach ($result as $iter) {
            if ($total >= $M) {
                break;
            }
    ...
    if ($findby == "SSN" && $result = getClientSSN("$client", "*")) {
        foreach ($result as $iter) {
            if ($total >= $M) {
                break;
            }
