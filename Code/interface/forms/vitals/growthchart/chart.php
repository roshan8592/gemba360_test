...
if (isset($pid) && is_numeric($pid)) {
    $client_data = getClientData($pid, "fname, lname, sex, DATE_FORMAT(DOB,'%Y%m%d') as DOB");
    $nowAge = getClientAge($client_data['DOB']);
    $dob = $client_data['DOB'];
...
if (isset($datapoints) && $datapoints != "") {
    ...
    $tmpAge = getClientAgeInDays($client_data['DOB'], $date);
...
if ($charttype == 'birth') {
    ...
    if (preg_match('/^male/i', $client_data['sex'])) {
    ...
    } elseif (preg_match('/^female/i', $client_data['sex'])) {
    ...
    // current client age between 2 and 20 years
...
if ($_GET['html'] == 1) {
    ...
    foreach ($datapoints as $data) {
        ...
        // get age of client at this data-point
        // to get data from function getClientAgeYMD including $age,$age_in_months, $ageinYMD
        extract(getClientAgeYMD($dob, $date));
        if ($charttype=='birth') {
        ...
        // from that client's 2-20 yr chart
        $daysold = getClientAgeInDays($dob, $date);
...
foreach ($datapoints as $data) {
    ...
    // get age of client at this data-point
    // to get data from function getClientAgeYMD including $age, $ageinYMD, $age_in_months
    extract(getClientAgeYMD($dob, $date));
    if ($charttype=='birth') {
        // for birth style chart, we use the age in months
        $age=$age_in_months;
