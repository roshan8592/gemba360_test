...
$result = getClientData($pid, "fname,lname,pid,pubpid,phone_home,sex,pharmacy_id,DOB,DATE_FORMAT(DOB,'%Y%m%d') as DOB_YMD");
$provider_results = sqlQuery("select * from users where username= ?", array($_SESSION{"authUser"}));

////////////////////////////////////////////////////////////////////
// Function:    getClientDateOfLastEncounter
function getClientDateOfLastEncounter($nPid)
{
...
$m_strEventDate = getClientDateOfLastEncounter($result['pid']);
...
if ($obj_bav['introductie'] != '') {
    ...
    $obj['introductie'] = xl("Since", "", "", " ") . $m_strEventDate . xl("we have seen your above client for evaluation and treatment at our outclient psychiatry clinic. Thank you for this referral.", "", " ");
}
...
<?php
// here we fill in the header above with client name etc ? ??? - move above
