...
/* Use this code to identify duplicate clients in OpenEMR
 *
 */
...
if ($parameters['go'] == "Go") {
    ...
    // for EACH client in OpenEMR find potential matches
    $sqlstmt = "select id, pid, fname, lname, dob, sex, ss from client_data";
...
$(function(){
    ...
    // pop up an OpenEMR window directly to the client info
    var moreinfoWin = null;
    $(".moreinfo").on("click", function(evt) {
        if (moreinfoWin) { moreinfoWin.close(); }
        moreinfoWin = window.open("<?php echo $GLOBALS['webroot']; ?>/interface/client_file/client_file.php?set_pid=" + encodeURIComponent($(this).attr("oemrid")), "moreinfo");
        evt.stopPropagation();
    });
