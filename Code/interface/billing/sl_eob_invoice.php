...
$ferow = sqlQuery("SELECT e.*, p.fname, p.mname, p.lname FROM form_encounter AS e, client_data AS p WHERE e.id = ? AND p.pid = e.pid", array($trans_id));
if (empty($ferow)) {
    die("There is no encounter with form_encounter.id = '" . text($trans_id) . "'.");
}
$client_id = 0 + $ferow['pid'];
        ...
        if ($ALLOW_DELETE && !$debug) {
            if (is_array($_POST['form_del'])) {
                foreach ($_POST['form_del'] as $arseq => $dummy) {
                    row_delete("ar_activity", "pid = '" . add_escape_custom($client_id) . "' AND " . "encounter = '" . add_escape_custom($encounter_id) . "' AND sequence_no = '" . add_escape_custom($arseq) . "'");
                }
            }
        }
...
// This should not happen but if it does, apply old logic.
            ...
            if (0.0 + $thispay) {
                SLEOB::arPostPayment($client_id, $encounter_id, $session_id, $thispay, $code, $payer_type, '', $debug, '', $thiscodetype);
                $paytotal += $thispay;
            }
...
// Ins1, Ins2 or Ins3.
                ...
                SLEOB::arPostAdjustment($client_id, $encounter_id, $session_id, $thisadj, $code, $payer_type, $reason, $debug, '', $thiscodetype);
            }
        }
...
// Maintain which insurances are marked as finished.
        ...
        sqlStatement("UPDATE form_encounter SET last_level_closed = ?, stmt_count = ? WHERE pid = ? AND encounter = ?", array($form_done, $form_stmt_count, $client_id, $encounter_id));

        if ($_POST['form_secondary']) {
            SLEOB::arSetupSecondary($client_id, $encounter_id, $debug);
        }
...
$codes = ar_get_invoice_summary($client_id, $encounter_id, true);
$pdrow = sqlQuery("select billing_note from client_data where pid = ? limit 1", array($client_id));
?>
        ...
        <form action='sl_eob_invoice.php?id=<?php echo attr_url($trans_id); ?>' method='post' onsubmit='return validate(this)'>
            ...
            <fieldset>
                ...
                <div class="col-xs-12 oe-custom-line">
                    <div class="col-xs-3">
                        <label class="control-label" for="form_name"><?php echo xlt('Client'); ?>:</label>
                    ...
                    <div class="col-xs-3">
                        ...
                        $tmp = sqlQuery("SELECT bill_date FROM billing WHERE " .
                            ...
                            "activity = 1 ORDER BY fee DESC, id ASC LIMIT 1", array($client_id, $encounter_id));
                    ...
                    <div class="col-xs-2">
                        ...
                        <input type="text" class="form-control" class="form-control" id='form_provider'
                               name='form_provider' value='<?php echo attr($client_id) . "." . attr($encounter_id); ?>'
                               disabled>
                    </div>
                    <div class="col-xs-2">
                    ...
                    <div class="col-xs-2">
                        ...
                        <?php
                        for ($i = 1; $i <= 3; ++$i) {
                            $payerid = SLEOB::arGetPayerID($client_id, $svcdate, $i);
                            if ($payerid) {
                    ...
                    <div class="col-xs-4">
                        ...
                        <div style="padding-left:15px">
                            ...
                            foreach (array(0 => 'None', 1 => 'Ins1', 2 => 'Ins2', 3 => 'Ins3') as $key => $value) {
                                if ($key && !SLEOB::arGetPayerID($client_id, $svcdate, $key)) {
                                    continue;
                                }
                ...
                <div class="col-sm-12 text-left position-override" id="search-btn">
                    ...
                    <?php if ($GLOBALS['new_tabs_layout'] && $from_posting) { ?>
                        <button type='button' class="btn btn-default btn-view pull-right" name='form_goto' id="btn-goto"
                            onclick="goEncounterSummary(<?php echo attr_js($client_id) ?>)"><?php echo xlt("Past Encounters"); ?></button>
                    <?php } ?>
                </div>
            </div>
        </form>
...
<script language="JavaScript">
    ...
    if (f1.form_source) {
        ...
        // not match the payer ID for the client's insurance.  This is
