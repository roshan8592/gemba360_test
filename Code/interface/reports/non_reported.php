<?php
/**
 * This report lists non reported client diagnoses for a given date range.
...
if ($_POST['form_get_hl7']==='true') {
    ...
    "concat(p.fname, '^',p.mname,'^', p.lname) as clientname, ";
} else {
    $query .= "concat(p.fname, ' ',p.mname,' ', p.lname) as clientname, ".
    "l.date as issuedate, "  ;
}

  $query .=
  "l.id as issueid, l.title as issuetitle, DATE_FORMAT(l.begdate,'%Y%m%d%H%i') as begin_date ". // Ensoftek: Jul-2015: Get begin date
  "from lists l, client_data p, codes c ".
  "where ".
  "c.reportable=1 and ".
...
if ($_POST['form_get_hl7']==='true') {
    ...
    while ($r = sqlFetchArray($res)) {
        ...
        "|||$now||".
        "ADT^A01^ADT_A01" . // Hard-code to A01: Client visits provider/facility
        ...
        substr($now . "_" . $r['clientid'], 0, 15) . "^^^^VN" . // Supposed to be visit number. Since, we don't have any encounter, we'll use the format 'date_pid' to make it unique
        "|||||||||||||||||||||||||" .
        $r['begin_date'] .
        "$D" ;
...
<tr>
<td>
        <?php echo text($row['clientid']) ?>
</td>
<td>
        <?php echo text($row['clientname']) ?>
</td>
<td>
