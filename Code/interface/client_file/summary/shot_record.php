...
//collect client data
$res2 = sqlQuery("select concat(p.lname,', ',p.fname,' ',p.mname) client_name ".
                ",date_format(p.DOB,'%c/%e/%Y') as client_DOB ".
                ",concat(p.street,'\n',p.city,', ',p.state,' ',p.postal_code) as client_address".
                " from client_data p where p.pid = ?", array($pid));
...
function printPDF($res, $res2, $data)
{
    ...
    $pdf->ezText("\n" . $res2['client_name'] . "\n" . xl('Date of Birth') . ": " . $res2['client_DOB'] . "\n" . $res2['client_address']);
    $pdf->ezText("\n");
...
//print html css
  ...
  //convert end of line characters to html (escape for html output first)
    ...
    $res2['client_address'] = text($res2['client_address']);
    $res2['client_address'] = preg_replace($patterns, $replace, $res2['client_address']);
  ...
  //plan 15 lines per page
    ...
    for ($i=0; $i<$countTotalPages; $i++) {
        ...
        //display client information (Note client address is already escaped)
        echo "<div class='clientAddress'>" . text($res2['client_name']) . "<br>" .
        text(xl('Date of Birth') . ": " . $res2['client_DOB']) . "<br>" .
        $res2['client_address'] . "</div>\n";
