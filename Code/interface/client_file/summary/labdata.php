 ...
 *
 * the script will list all available client's 'result_codes'
 * from table 'procedure_results'. Check those you wish to view.
 * If you see nothing to select, then
 *    a) there is actually no lab data of this client available
...
$path_to_this_script = $rootdir . "/client_file/summary/labdata.php";
...
$main_spell .= "AND procedure_order.client_id = ? ";
...
if ($printable) {
    echo "<table border=0>";
    echo "<tr><td>" . xlt('Client') . ": </td><td><b>" . text($lastname) . ", " . text($firstname) . "</b></td></tr>";
    echo "<tr><td>" . xlt('Client ID') . ": </td><td>" . text($pid) . "</td></tr>";
...
if (!$printable) {
    ...
    // What items are there for client $pid?
    // -----------------------------------------------
    $value_list = array();
    ...
    $spell .= "WHERE procedure_order.client_id = ? ";
...
if ($mode != 'list') {
    ...
    echo "<span>" . xlt('Back to Client') . "</span></a>";
...
// print results of client's items
//-------------------------------------------
$mode = $_POST['mode'];
...
if ($value_select) {
    ...
    if ($mode=='list') {
        ...
        foreach ($value_select as $this_value) {
            ...
            while ($myrow = sqlFetchArray($query)) {
                ...
                if (!$printable) {
                    echo "<a href='../../client_file/encounter/encounter_top.php?set_encounter=". attr_url($myrow['encounter_id']) . "' target='RBot'>";
    ...
    if ($mode=='matrix') {
        ...
        $datelist = array();
        $i = 0;
        // get all data of client's items
        foreach ($value_select as $this_value) {
            $spell  = $main_spell;
            $query  = sqlStatement($spell, array($this_value,$pid));
