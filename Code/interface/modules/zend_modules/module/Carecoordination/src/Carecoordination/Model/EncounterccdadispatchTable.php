...
class EncounterccdadispatchTable extends AbstractTableGateway
{
    ...
    public function validcredential($credentials)
    {
        ...
        if ($credentials[6]) {
            $pres = $appTable->zQuery("SELECT * FROM client_access_offsite WHERE portal_username=?", array($credentials[6]));
            $prow = $pres->current();

            $newclient_to_approve = 0;
            if (!$prow['portal_pwd']) {
                $newclient_to_approve = 1;
    ...
    public function getClientId($portalUserName = '')
    {
        $appTable   = new ApplicationTable();
        $pres = $appTable->zQuery("SELECT * FROM client_access_offsite WHERE portal_username=?", array($portalUserName));
        $prow = $pres->current();
    ...
    public function valid($credentials)
    {
        ...
        if (($credentials[1]==$GLOBALS['portal_offsite_username'] && $ok==1 && $GLOBALS['portal_offsite_enable']==1)||$GLOBALS['validated_offsite_portal']==true) {
            $pres = $appTable->zQuery("SELECT * FROM client_access_offsite WHERE portal_username=?", array($credentials[6]));
            $prow = $pres->current();
            if ($credentials[4] == 'existingclient') {
                ...
                $prow = $pres->current();
                $okO = 1;
            } elseif ($credentials[4] == 'newclient') {
                if ($this->validcredential($credentials) === 2) {
                    $okN = 3;
                    ...
                    $okN = 1;
                    $pres = $appTable->zQuery("SELECT IFNULL(MAX(pid)+1,1) AS pid FROM client_data");
                    $prow = $pres->current();
                }
            }
                ...
                $pres = $appTable->zQuery("
                    ...
                    FROM
                        ...
                        JOIN audit_details ad2 
                            ...
                            AND ad2.table_name = 'client_access_offsite' 
                            AND ad2.field_name = 'portal_pwd' 
                    WHERE ad.table_name = 'client_access_offsite' 
                        AND ad.field_name = 'portal_username' 
                        AND ad.field_value = ?
                ", array($credentials[6]));
                $arow = $pres->current();
            ...
            if ($okE) {
                if ($okE == 1) {
                    $pres = $appTable->zQuery("SELECT allow_client_portal FROM client_data WHERE pid=?", array($pid));
                    $portal = $pres->current();
                } elseif ($okE == 2) {
                    $pres = $appTable->zQuery("
                        SELECT 
                            IF(COUNT(*),'yes','no') AS allow_client_portal
                        FROM
                            audit_master am 
                            JOIN audit_details ad 
                                ON ad.audit_master_id = am.id 
                ...
                if (strtolower($portal['allow_client_portal'])!='yes') {
                    return false;
                }
            }
    ...
    /*Fetch Client data from EMR

    * @param    $pid
    * @param    $encounter
    * @return   $client_data   Client Data in XML format
    */
    public function getClientdata($pid, $encounter)
    {
        $query      = "select client_data.*, l1.notes AS race_code, l1.title as race_title, l2.notes AS ethnicity_code, l2.title as ethnicity_title, l3.title as religion, l3.notes as religion_code, l4.notes as language_code, l4.title as language_title
                        from client_data
    ...
    public function getInformationRecipient($pid, $encounter, $recipients, $params)
    {
        ...
        if ($recipients == 'hie') {
            ...
            $field_name[]   = $params;
        } elseif ($recipients == 'client') {
            $query          = "select fname, lname from client_data WHERE pid = ?";
            $field_name[]   = $params;
        } else {
            if (!$params) {
    ...
    */
    ...
    public function getMedications($pid, $encounter)
    {
        ...
        $query      = "select l.id, l.date_added, l.drug, l.dosage, l.quantity, l.size, l.substitute, l.drug_info_erx, l.active, SUBSTRING(l3.codes, LOCATE(':',l3.codes)+1, LENGTH(l3.codes)) AS route_code,
                       ...
                       where l.client_id = ? and l.active = 1";
        $appTable   = new ApplicationTable();
    ...
    public function getResults($pid, $encounter)
    {
        ...
        WHERE po.client_id = ? AND prs.result NOT IN ('DNR','TNP') $wherCon";
        $appTable   = new ApplicationTable();
        $res        = $appTable->zQuery($query, array($pid));
    ...
    * @param    int     $pid           Client Internal Identifier.
    * @param    int     $encounter     Current selected encounter.

    * return    string  $complications  XML which contains the details collected from the client.
    */
    ...
    * return    string  $procedure_diag  XML which contains the details collected from the client.
    */
    ...
    * return    string  $procedure_description  XML which contains the details collected from the client.
    */
    ...
    * return    string  $procedure_indications  XML which contains the details collected from the client.
    */
    ...
    * return    string  $anesthesia  XML which contains the details collected from the client.
    */
    ...
    * return    string  $post_operative_diag  XML which contains the details collected from the client.
    */
    ...
    * return    string  $pre_operative_diag  XML which contains the details collected from the client.
    */
    ...
    * return    string  $hp  XML which contains the details collected from the client.
    */
    ...
    * return    string  $physical_exam  XML which contains the details collected from the client.
    */
    ...
    * return    string  $chief_complaint  XML which contains the details collected from the client.
    */
    ...
    * return    string  $general_status  XML which contains the details collected from the client.
    */
    ...
    * return    string  $history_past_illness  XML which contains the details collected from the client.
    */
    ...
    * return    string  $review_of_systems  XML which contains the details collected from the client.
    */
    ...
    * return    string  $vitals  XML which contains the details collected from the client.
    */
    ...
    * return    string  $social_history  XML which contains the details collected from the client.
    */
    ...
    */
    ...
    /*
    Get the Age of a client
    * @param    int     $pid    Client Internal Identifier.

    * return    int     $age    Age of a client will be returned
    */
    public function getAge($pid, $date = null)
    {
        ...
        $age = 0;
        $query      = "select ROUND(DATEDIFF('$date',DOB)/365.25) AS age from client_data where pid= ? ";
        $appTable   = new ApplicationTable();
        $res        = $appTable->zQuery($query, array($pid));
    ...
    */
    public function fetchFormValues($pid, $encounter, $formTables)
    {
        ...
        foreach ($formTables as $formTables_details) {
            /***************Fetching the form id for the client***************/
    ...
    */
    public function signOff($pid, $encounter)
    {
        ...
        $query_client_data     = "SELECT * FROM client_data WHERE pid = ?";
        $appTable               = new ApplicationTable();
        $result_client_data    = $appTable->zQuery($query_client_data, array($pid));
        foreach ($result_client_data as $row_client_data) {
        }
        ...
        foreach ($result_dem as $row_dem) {
            $query_insert_client_data  = "INSERT INTO combination_form_locked_data SET pid = ?, encounter = ?, form_dir = ?, field_name = ?, field_value = ?";
            $appTable                   = new ApplicationTable();
            $result_dem                 = $appTable->zQuery($query_insert_client_data, array($pid, $encounter, 'DEM', $row_dem['field_id'], $row_client_data[$row_dem['field_id']]));
        }
    ...
    */
    ...
    {
        $appTable   = new ApplicationTable();
        $query      = "SELECT providerID FROM client_data WHERE `pid`  = ?";
    ...
    * return    string  $planofcare  XML which contains the details collected from the client.
    */
    ...
    * return    string  $functional_cognitive  XML which contains the details collected from the client.
    */
    ...
    {
        $appTable   = new ApplicationTable();
        $query      = "SELECT care_team FROM client_data WHERE `pid`  = ?";
        $result     = $appTable->zQuery($query, array($pid));
        $row        = $result->current();
