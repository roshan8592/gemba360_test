...
//-----------------------------------------------------------------------+
//===============================================================================
//Client ajax section and listing of charges..Used in New Payment and Edit Payment screen.
//===============================================================================
if (isset($_POST["mode"])) {
    if (($_POST["mode"] == "search" || $_POST["default_search_client"] == "default_search_client") && $_REQUEST['hidden_client_code']*1>0) {
        $hidden_client_code=$_REQUEST['hidden_client_code'];
        ...
        $ResultSearchNew = sqlStatement("SELECT b.id,last_level_closed,b.encounter,fe.`date`,b.code_type,b.code,b.modifier,fee
            ...
            $StringForQuery ORDER BY fe.`date`, fe.encounter,b.code,b.modifier", array($hidden_client_code, $hidden_client_code));
        $res = sqlStatement("SELECT fname,lname,mname FROM client_data
                         where pid =?", array($hidden_client_code));
...
?>
                    ...
                    <div class="col-xs-12" style="padding-bottom:5px">
                        <div class="col-xs-3">
                            <label class="control-label" for="client_name"><?php echo xlt('Client'); ?>:</label>
                            <input id="hidden_ajax_client_close_value" type="hidden" value="<?php echo $Message=='' ? attr($NameNew) : '' ;?>">
                            <!--<input autocomplete="off" class="form-control" type="text" id='client_name' name='client_name' onkeydown="PreventIt(event)" value="<?php echo $Message=='' ? attr($NameNew) : '' ;?>">-->
                            <input name='client_code'  class="form-control"   id='client_code' class="text"  onKeyDown="PreventIt(event)" value="<?php echo $Message=='' ? attr($NameNew) : '' ;?>"  autocomplete="off" />
                        </div>
                        <div class="col-xs-2">
                            <label class="control-label" for="client_name"><?php echo xlt('Client Id'); ?>:</label>
                            <div class="form-control" name="client_name" id="client_name"" >
                                <?php echo ($Message=='') ? text($hidden_client_code) : '' ;?>
                            </div>
                        </div>
                        <div class="col-xs-7">
                        ...
                        <div id='ajax_div_client_section'>
                            <div id='ajax_div_client_error'>
                            </div>
                ...
                <table class="table-condensed"   id="TableDistributePortion">
                  <thead class="" bgcolor="#dddddd">
                    ...
                    do {
                        ...
                        $ferow = sqlQuery("SELECT date, last_level_closed " .
                          ...
                          "pid = ? AND encounter = ?", array($hidden_client_code, $RowSearch['encounter']));
                        ...
                        $new_payer_id = SLEOB::arGetPayerID($hidden_client_code, $date_of_service, $new_payer_type);
                        ...
                        $resId = sqlStatement("SELECT b.id FROM billing AS b, code_types AS ct 
                                               ...
                                               AND b.activity!=0 ORDER BY id", array($hidden_client_code, $Encounter));
                        ...
                        if ($BillingId!=$Id) {//multiple cpt in single encounter
                            ...
                            pid =? and  encounter  =? and billing.activity!=0", array($hidden_client_code, $Encounter));
                            $rowCopay = sqlFetchArray($resCopay);
                            $Copay=$rowCopay['copay']*-1;

                            $resMoneyGot = sqlStatement("SELECT sum(pay_amount) as ClientPay FROM ar_activity where
                            pid =?  and  encounter =? and  payer_type=0 and 
                            account_code='PCP'", array($hidden_client_code, $Encounter));//new fees screen copay gives account_code='PCP'
                            $rowMoneyGot = sqlFetchArray($resMoneyGot);
                            $ClientPay=$rowMoneyGot['ClientPay'];

                            $Copay=$Copay+$ClientPay;
                        }
                            ...
                            account_code='PCP')", array($hidden_client_code, $Codetype, $Code, $Modifier, $Encounter));//new fees screen copay gives account_code='PCP'
                            $rowMoneyGot = sqlFetchArray($resMoneyGot);
                            ...
                            pid =? and (code_type=? or code_type='') and code=? and modifier=? and encounter =?", array($hidden_client_code, $Codetype, $Code, $Modifier, $Encounter));
                  ...
                  <tr class="text"  bgcolor='<?php echo attr($bgcolor); ?>' id="trCharges<?php echo attr($CountIndex); ?>">
                    <td align="left" class="<?php echo attr($StringClass); ?>" ><input name="HiddenIns<?php echo attr($CountIndex); ?>" style="width:70px;text-align:right; font-size:12px" id="HiddenIns<?php echo attr($CountIndex); ?>"
                     value="<?php echo attr($Ins); ?>" type="hidden"/><?php echo generate_select_list("payment_ins$CountIndex", "payment_ins", "$Ins", "Insurance/Client", '', 'oe-payment-select class3', 'ActionOnInsPat("'.$CountIndex.'")');?></td>
