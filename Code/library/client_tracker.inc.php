<?php
/**
* library/client_tracker.inc.php Functions used in the Client Flow Board.
*
* Functions for use in the Client Flow Board and Client Flow Board Reports.
*
*
...
function fetch_Client_Tracker_Events($from_date, $to_date, $provider_id = null, $facility_id = null, $form_apptstatus = null, $form_apptcat = null, $form_client_name = null, $form_client_id = null)
{
    # used to determine which providers to display in the Client Tracker
    if ($provider_id == 'ALL') {
      //set null to $provider id if it's 'all'
        $provider_id = null;
    }

    $events = fetchAppointments($from_date, $to_date, $form_client_id, $provider_id, $facility_id, $form_apptstatus, null, null, $form_apptcat, true, 0, null, $form_client_name);
    return $events;
}
...
{
    #Check to see if there is an encounter in the client_tracker table.
    $enc_yn = sqlQuery("SELECT encounter from client_tracker WHERE `apptdate` = ? AND encounter > 0 " .
...
function manage_tracker_status($apptdate, $appttime, $eid, $pid, $user, $status = '', $room = '', $enc_id = '')
{
  ...
  #Check to see if there is an entry in the client_tracker table.
    $tracker = sqlQuery("SELECT id, apptdate, appttime, eid, pid, original_user, encounter, lastseq,".
                       "client_tracker_element.room AS lastroom,client_tracker_element.status AS laststatus ".
                       "from `client_tracker`".
                       "LEFT JOIN client_tracker_element " .
                       "ON client_tracker.id = client_tracker_element.pt_tracker_id " .
                       "AND client_tracker.lastseq = client_tracker_element.seq " .
    ...
    if (empty($tracker)) {
        ...
        if (($status != $tracker['laststatus']) || ($room != $tracker['lastroom'])) {
            ...
            #Update lastseq in tracker.
             sqlStatement(
                 "UPDATE `client_tracker` SET  `lastseq` = ? WHERE `id` = ?",
        ...
        if (!empty($enc_id)) {
            ...
            sqlStatement("UPDATE `client_tracker` SET `encounter` = ? WHERE `id` = ?", array($enc_id,$tracker_id));
        }
    }
...
# This is used to collect the tracker elements for the Client Flow Board Report
# returns the elements in an array
function collect_Tracker_Elements($trackerid)
{
    $res = sqlStatement("SELECT * FROM client_tracker_element WHERE pt_tracker_id = ? ORDER BY LENGTH(seq), seq ", array($trackerid));
...
{
    $tracker = sqlQuery(
        "SELECT client_tracker_element.start_datetime " .
                                   "FROM client_tracker_element " .
                                   "INNER JOIN list_options " .
                                   "ON client_tracker_element.status = list_options.option_id " .
                                   "WHERE  list_options.list_id = 'apptstat' " .
                                   "AND list_options.toggle_setting_1 = '1' AND list_options.activity = 1 " .
                                   "AND client_tracker_element.pt_tracker_id = ?",
        array($trackerid)
    );
...
# Check if randomization has not yet been done (is random_drug_test NULL). If already done, then exit.
      $drug_test_done = sqlQuery("SELECT `random_drug_test`, pid from client_tracker " .
                                     "WHERE id =? ", array($tracker_id));
      $Client_id = $drug_test_done['pid'];

    if (is_null($drug_test_done['random_drug_test'])) {
        # get a count of the number of times the client has been screened.
        if ($yearly_limit >0) {
            # check to see if screens are within the current year.
            $lastyear = date("Y-m-d", strtotime("-1 year", strtotime(date("Y-m-d H:i:s"))));
            $drug_test_count = sqlQuery("SELECT COUNT(*) from client_tracker " .
                                 "WHERE drug_screen_completed = '1' AND apptdate >= ? AND pid =? ", array($lastyear,$Client_id));
        }

        # check that the client is not at the yearly limit.
       ...
       #Update the tracker file.
        sqlStatement("UPDATE client_tracker SET " .
                 "random_drug_test = ? " .
