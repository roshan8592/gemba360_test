...
$parameterArray ['recipients'] = 'client'; // emr_direct or hie else if not set $_SESSION['authUserID']
...
function portalccdafetching($pid, $server_url, $parameterArray)
{
    ...
    try {
        $ch = curl_init();
        $url = $server_url . "/interface/modules/zend_modules/public/encounterccdadispatch/index?site=" . urlencode($site_id) . "&me=" . urlencode(session_id()) . "&param=1&view=1&combination=" . urlencode($pid) . "&recipient=client";
