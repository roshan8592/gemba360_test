...
class OEMRUser
{
    ...
    public function query_formation($data)
    {
        ...
        // Entries pending  for approval for Existing Client and New Client.
            case 'U4':
                $pid = $this->getPid($data[1]);
            ...
            case 'P1':
                $query= "select MAX(pid)+1 AS pid  from client_data ";
            
                return array($query);
            break;
            
            //for building client Demo
            case 'P2':
            ...
            //for building client Demo   Date of Birth
            case 'P3':
                $pid = $this->getPid($data[1]);
                $query="select  *, DATE_FORMAT(DOB,'%Y-%m-%d') as DOB_YMD from client_data where pid=? order by date DESC limit 0,1 ";
                return array($query,array($pid));
            break;
            
           //for building client Demo   Employer Data
            case 'P4':
                $pid = $this->getPid($data[1]);
           ...
           //for building client Demo   Insurance company details for Client
            case 'P5':
        ...
        //Global omit employers for building demo.
            ...
            //list of approvals
						...
						pd.lname,(SELECT field_value FROM audit_details WHERE audit_master_id = am.id AND table_name = 'client_data' AND field_name = 'lname'),
						(SELECT ad2.field_value FROM audit_details ad JOIN audit_details ad1 ON ad1.table_name = 'client_access_offsite' AND
						ad1.field_name = 'portal_username' AND ad1.field_value = ad.field_value JOIN audit_details ad2 ON ad2.table_name = 'client_data'
						AND ad2.field_name = 'lname' AND ad2.audit_master_id = ad1.audit_master_id WHERE ad.audit_master_id = am.id AND
						ad.table_name = 'client_access_offsite' AND ad.field_name = 'portal_username')) AS lname,COALESCE(pd.fname,(SELECT field_value
						FROM audit_details WHERE audit_master_id = am.id AND table_name = 'client_data' AND field_name = 'fname'),(SELECT ad2.field_value
						FROM audit_details ad JOIN audit_details ad1 ON ad1.table_name = 'client_access_offsite' AND ad1.field_name = 'portal_username' AND
						ad1.field_value = ad.field_value JOIN audit_details ad2 ON ad2.table_name = 'client_data' AND ad2.field_name = 'fname' AND
						ad2.audit_master_id = ad1.audit_master_id WHERE ad.audit_master_id = am.id AND ad.table_name = 'client_access_offsite' AND
						...
						AND table_name = 'client_data' AND field_name = 'mname'),(SELECT ad2.field_value FROM audit_details ad JOIN audit_details ad1 ON
						ad1.table_name = 'client_access_offsite' AND ad1.field_name = 'portal_username' AND ad1.field_value = ad.field_value 
						JOIN audit_details ad2 ON ad2.table_name = 'client_data' AND ad2.field_name = 'mname' AND ad2.audit_master_id = ad1.audit_master_id 
						WHERE ad.audit_master_id = am.id AND ad.table_name = 'client_access_offsite' AND ad.field_name = 'portal_username')) AS mname,
						COALESCE(pd.dob,(SELECT field_value FROM audit_details WHERE audit_master_id = am.id AND table_name = 'client_data' AND
						field_name = 'dob'),(SELECT ad2.field_value FROM audit_details ad JOIN audit_details ad1 ON ad1.table_name = 'client_access_offsite' 
						AND ad1.field_name = 'portal_username' AND ad1.field_value = ad.field_value JOIN audit_details ad2 ON ad2.table_name = 'client_data' 
						AND ad2.field_name = 'dob' AND ad2.audit_master_id = ad1.audit_master_id WHERE ad.audit_master_id = am.id AND
						ad.table_name = 'client_access_offsite' AND ad.field_name = 'portal_username')) AS DOB FROM audit_master am LEFT JOIN client_data pd 
            ...
            case 'E4':
            //list of approvals
                $query="select * from audit_master,client_data,audit_details where audit_master.pid=client_data.pid and
            ...
            case 'F12':
            //Selection from master document for showing to client
            ...
            case 'F13':
                $query = "SELECT pid ,fname,lname,mname,DOB FROM client_data
