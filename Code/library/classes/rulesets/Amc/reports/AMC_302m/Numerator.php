...
class AMC_302m_Numerator implements AmcFilterIF
{
    ...
    {
        // Is client provided client specific education during the report period.

        // Check for any client specific education instances.
        $item = sqlQuery("SELECT * FROM `amc_misc_data` as amc, `form_encounter` as enc " .
                         ...
                         "AND `amc_id` = 'client_edu_amc' " .
                         "AND enc.date >= ? " .
                         "AND enc.date <= ?", array($client->id,$beginDate,$endDate));
