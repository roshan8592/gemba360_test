<?php
/**
 * This section handles ajax functions for insurance,client and for encounters.
 *
 * @package   OpenEMR
...
$(function(){
  $("#type_code").keyup(function(e){
      if (e.which == 9 || e.which == 13)
         ...
         {//Both insurance or client can come.The drop down value in 'type_name' decides which one to process.
           ajaxFunction('non','Simple',document.getElementById('type_code'));
           return;
         }
  });
  $("#client_code").keyup(function(e){
      if (e.which == 9 || e.which == 13)
         {//tab key,enter key.Prevent ajax activity.
          return false;
         }
         ...
         {
           ajaxFunction('client','Submit',document.getElementById('client_code'));
           return;
         }
  });
  $("#form_pt_name").keyup(function(e){
      if (e.which == 9 || e.which == 13)
         ...
         {
           ajaxFunction('client','Simple',document.getElementById('form_pt_name'));
           return;
         }
  });
  ...
  if(Source=='encounter')
  ...
  else if(Source=='client')
   {
      if(SourceObject.value.length<3)
       return false;
      document.getElementById('ajax_mode').value='set_client';
   }
   //For the below two cases, same text box is used for both insurance and client.
  else if(document.getElementById('type_name') && document.getElementById('type_name').options[document.getElementById('type_name').selectedIndex].value=='client')
   {//Client
...
//Send ajax request
   $.ajax({
    ...
    data: {
     ...
     client_code: Source=='client' ? SourceObject.value : '',
    insurance_text_ajax: document.getElementById('type_code') ? document.getElementById('type_code').value : '',
    encounter_client_code:Source=='encounter' ? document.getElementById('hidden_client_code').value : '',
    ...
    document.getElementById('ajax_mode').value='';
      if(Source=='encounter')
      ...
      else if(Source=='client')
       {
         ...
         $("#ajax_div_client_error").empty();
         $("#ajax_div_client").empty();
         $("#ajax_div_insurance_error").empty();
         $("#ajax_div_insurance").empty();
         $("#ajax_div_client").html(thedata);
         $("#ajax_div_client").show();
       }
      else
       {//Client or Insurance
         $("#ajax_div_client_error").empty();
...
//Html retured from the ajax above, contains list of either insurance,client or encounter.
...
function PutTheValuesClick(Code,Name)
 {//Used while -->CLICK<-- over list in the insurance/client portion.
  ...
  document.getElementById('div_insurance_or_client').innerHTML=Code;
...
function PutTheValuesClickDistribute(Code,Name)
 {//Used while -->CLICK<-- over list in the client portion before the start of distribution of amount.
 if(document.getElementById('SelFacility'))
  {
    document.getElementById('SelFacility').style.display='';//In Internet explorer this drop down comes over the ajax listing.
  }
  document.getElementById('client_code').value=Name;
  document.getElementById('hidden_ajax_client_close_value').value=Name;
  document.getElementById('hidden_client_code').value=Code;
  document.getElementById('client_name').innerHTML=Code;
  document.getElementById('ajax_div_client').style.display='none';
  document.getElementById('client_name').focus();
    document.getElementById('mode').value='search';
    top.restoreSession();
    document.forms[0].submit();
 }
function PutTheValuesClickClient(Code,Name)//Non submission client ajax.
 {
...
function PlaceValues(evt,Code,Name)
 {//Used while -->KEY PRESS<-- over list in the insurance/client portion.
...
function PlaceValuesDistribute(evt,Code,Name)
 {//Used while -->KEY PRESS<-- over list in the client portion before the start of distribution of amount.
    evt = (evt) ? evt : window.event;
    var charCode = (evt.which) ? evt.which : evt.keyCode;
    if (charCode == 13)//enter key
     {//Vanish the list and populate the parent text box
...
function PlaceValuesClient(evt,Code,Name)
 {
    evt = (evt) ? evt : window.event;
    var charCode = (evt.which) ? evt.which : evt.keyCode;
    if (charCode == 13)//enter key
     {//Vanish the list and populate the parent text box
      PutTheValuesClickClient(Code,Name);
      PreventIt(evt)  //For browser chorome.It gets submitted,to prevent it the PreventIt(evt) is written
     }
    else if(!((charCode == 38) || (charCode == 40)))
     {//if non arrow keys, focus on the parent text box(ie he again types and wants ajax to activate)
...
function HideTheAjaxDivs()
 {//Starts working when clicking on the body.Hides the ajax and restores the codes back, as he may have changed it in the text box.
  ...
  if(document.getElementById('ajax_div_client'))
   {
      if(document.getElementById('ajax_div_client').style.display!='none')
       {
         if(document.getElementById('SelFacility'))
          {
            document.getElementById('SelFacility').style.display='';//In Internet explorer this drop down comes over the ajax listing.
          }
          if(document.getElementById('client_code'))
            document.getElementById('client_code').value=document.getElementById('hidden_ajax_client_close_value').value;
          else if(document.getElementById('form_pt_name'))
            document.getElementById('form_pt_name').value=document.getElementById('hidden_ajax_client_close_value').value;
         $("#ajax_div_client_error").empty();
         $("#ajax_div_client").empty();
         $("#ajax_div_insurance_error").empty();
         $("#ajax_div_insurance").empty();
         $("#ajax_div_client").hide();
       }
   }
