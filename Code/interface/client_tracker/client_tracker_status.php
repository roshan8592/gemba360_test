<?php
/**
 * Client Tracker Status Editor
 *
 * This allows entry and editing of current status for the client from within client tracker and updates the status on the calendar.
 * Contains a drop down for the Room information driven by the list Client Flow Board Rooms.
 *
 * @package   OpenEMR
...
$trow = sqlQuery("SELECT apptdate, appttime, client_tracker_element.room AS lastroom, " .
                        "client_tracker_element.status AS laststatus, eid, random_drug_test, encounter, pid " .
                        "FROM client_tracker " .
                        ...
                        "WHERE client_tracker.id =?", array($_GET['tracker_id']));
...
"from client_data where pid =? limit 1", array($tkpid));
?>
...
<body class="body_top">
    <div class="container">
        ...
        <form id="form_note" method="post" action="client_tracker_status.php?tracker_id=<?php echo attr_url($tracker_id) ?>&csrf_token_form=<?php echo attr_url(CsrfUtils::collectCsrfToken()); ?>" enctype="multipart/form-data" >
            ...
            <div class="form-group">
                ...
                <?php echo generate_select_list('roomnum', 'client_flow_board_rooms', $trow['lastroom'], xl('Exam Room Number')); ?>
            </div>
