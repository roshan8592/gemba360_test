...
class AMC_314g_1_2_20_Numerator implements AmcFilterIF
{
    ...
    {
        if (!in_array($client->id, $this->patArr)) {
            $this->patArr[] = $client->id;
                        //
                        // Still TODO
                        // AMC MU2 TODO :
            ...
            $docLabQry = "SELECT count(*) as cnt FROM documents d ".
                         ...
                         "INNER JOIN client_data pd ON pd.pid = d.foreign_id ".
                         "WHERE d.foreign_id = ? AND (d.date BETWEEN ? AND ?) ";
            $check = sqlQuery($docLabQry, array($client->id, $beginDate, $endDate));
            if ($check['cnt'] > 0) {
                return true;
            } else {
                return false;
