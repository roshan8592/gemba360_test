...
// Update price level in client demographics if it's changed.
    ...
    if ($_POST['running_as_ajax']) {
        ...
        if (isset($_POST['ippfconmeth'])) {
            ...
            if ($tmp_form_id) {
                ...
                formJump("{$GLOBALS['rootdir']}/client_file/encounter/view_form.php" .
        ...
        if ($rapid_data_entry || ($_POST['bn_save_close'] && $_POST['form_has_charges'])) {
            ...
            formJump("{$GLOBALS['rootdir']}/client_file/pos_checkout.php?framed=1" .
...
function newEvt() {
 var f = document.forms[0];
 var url = '../../main/calendar/add_edit_event.php?clientid=<?php echo urlencode($fs->pid); ?>';
            ...
            <div class="col-sm-12">
                ...
                onsubmit="return validate(this)">
                    ...
                    if ($isBilled) {
                        ...
                        <?php
                        // Allow the client price level to be fixed here.
                        echo "<fieldset>";
                            ...
                            <table name='selected_codes' id='selected_codes' class="table" cellspacing='5'>
                                ...
                                if ($billresult) {
                                    ...
                                    $resMoneyGot = sqlStatement(
                                        "SELECT pay_amount as ClientPay,session_id as id,date(post_time) as date ".
                                    ...
                                    if ($_POST['newcodes'] && !$alertmsg) {
                                        ...
                                        foreach ($arrcodes as $codestring) {
                                            ...
                                            if ($newtype == 'MA') {
                                                ...
                                                if ($tmp['sex'] == '1' && $fs->client_male || $tmp['sex'] == '2' && !$fs->client_male) {
                                ...
                                <?php } else { // visit is billed ?>
                                    <?php if ($fs->hasCharges) { // billed with charges ?>
                                        <button type='button' class='btn btn-default btn-show'
                                            onclick="top.restoreSession();location='../../client_file/pos_checkout.php?framed=1<?php
