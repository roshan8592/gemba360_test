...
class CdrAlertManager
{
    ...
    function populate()
    {
        ...
        foreach ($rules as $rowRule) {
            $rule_id = $rowRule['id'];
            $cdra[] = new CdrResults($rule_id, $rowRule['active_alert_flag'], $rowRule['passive_alert_flag'], $rowRule['client_reminder_flag'], $rowRule['access_control']);
        }
                
        return $cdra;
    }
        
    function update($rule_ids, $active_alert_flags, $passive_alert_flags, $client_reminder_flags, $access_controls)
    {
        ...
        for ($index=0; $index < count($rule_ids); $index++) {
            ...
            $client_reminder_flag = $client_reminder_flags[$index];
                  $access_control = $access_controls[$index];
            $cdra = new CdrResults($rule_id, $active_alert_flag, $passive_alert_flag, $client_reminder_flag, $access_control);
            $cdra->update_table();
        }
    }
