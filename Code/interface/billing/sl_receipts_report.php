...
<head>
    ...
    <script language="JavaScript">
        ...
        function sel_procedure() {
            dlgopen('../client_file/encounter/find_code_popup.php?target_element=form_proc_codefull&codetype=' + <?php echo js_url(collect_codetypes("procedure", "csv")); ?>, '_blank', 500, 400);
        }

        // This invokes the find-code (diagnosis codes) popup.
        function sel_diagnosis() {
            dlgopen('../client_file/encounter/find_code_popup.php?target_element=form_dx_codefull&codetype=' + <?php echo js_url(collect_codetypes("diagnosis", "csv")); ?>, '_blank', 500, 400);
        }
  ...
  <th align="right">
    ...
    if ($_POST['form_refresh']) {
        ...
        if (!$form_proc_code || !$form_proc_codetype) {
            ...
            "JOIN forms AS f ON f.pid = b.pid AND f.encounter = b.encounter AND f.formdir = 'newclient' " .
            ...
            while ($row = sqlFetchArray($res)) {
              ...
              // that diagnosis.
                if ($form_dx_code && $form_dx_codetype) {
                    ...
                    "activity = 1", array($client_id,$encounter_id,$form_dx_codetype,$form_dx_code));
                    if (empty($tmp['count'])) {
              ...
              //
                ...
                $arows[$key]['memo'] = '';
                if ($GLOBALS['cash_receipts_report_invoice'] == '0') {
                    $arows[$key]['invnumber'] = "$client_id.$encounter_id";
                } else {
                    $arows[$key]['invnumber'] = "$client_name";
                }
        ...
        $query = "SELECT a.pid, a.encounter, a.post_time, a.code, a.modifier, a.pay_amount, " .
          ...
          "JOIN forms AS f ON f.pid = a.pid AND f.encounter = a.encounter AND f.formdir = 'newclient' " .
          "LEFT OUTER JOIN users AS u ON u.username = f.user " .
        ...
        ***************************************************************/
        ...
        $query = "SELECT a.pid, a.encounter, a.post_time, a.code, a.modifier, a.pay_amount, " .
         ...
         "JOIN form_encounter AS fe ON fe.pid = a.pid AND fe.encounter = a.encounter " .
         "LEFT OUTER JOIN ar_session AS s ON s.session_id = a.session_id " .
         "LEFT OUTER JOIN client_data AS p ON p.pid = a.pid " .
  ...
  <td class="detail">
    ...
    <?php } ?>
                ...
                if ($form_proc_code && $form_proc_codetype) {
                        ...
                        list($client_id, $encounter_id) = explode(".", $row['invnumber']);
                        $tmp = sqlQuery("SELECT SUM(fee) AS sum FROM billing WHERE " .
                        "pid = ? AND encounter = ? AND " .
                        "code_type = ? AND code = ? AND activity = 1", array($client_id,$encounter_id,$form_proc_codetype,$form_proc_code));
                        bucks($tmp['sum']);
                        echo "  </td>\n";
                }
                ?>
