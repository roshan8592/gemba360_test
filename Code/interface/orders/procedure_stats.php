...
function loadColumnData($key, $row)
{
  ...
  // For each client attribute to report, this increments the array item
...
if ($_POST['form_submit']) {
    ...
    // This gets us all results, with encounter and client
    // info attached and grouped by client and encounter.

    $sqlBindArray = array();

    $query = "SELECT " .
    "po.client_id, po.encounter_id, po.date_ordered, " .
    ...
    "JOIN form_encounter AS fe ON fe.pid = po.client_id AND fe.encounter = po.encounter_id " .
    "JOIN client_data AS pd ON pd.pid = po.client_id $sexcond" .
