<?php
/**
 * Display, enter, modify and manage client notes.
 *
 * @package   OpenEMR
...
if ($docid) {
    ...
    $client_id = intval($row['foreign_id']);
} else if ($orderid) {
    $row = sqlQuery("SELECT client_id FROM procedure_order WHERE procedure_order_id = ?", array($orderid));
    $client_id = intval($row['client_id']);
}

// Check authorization.
if (!acl_check('clients', 'notes', '', array('write','addonly'))) {
    die(xlt('Not authorized'));
}

$tmp = getClientData($client_id, "squad");
...
$pres = getClientData($client_id, "lname, fname");
...
$title = text(getClientName($client_id));
?>
    <title><?php echo $title; ?></title>
    <div class="row">
        <div class="col-md-12">
            <div class="page-header">
                <h3><?php echo xlt('Client Messages') . text($title_docname) . " " . xlt('for');
                if (!$orderid) {
                    ?>&nbsp;
    ...
    <div class="row oe-margin-b-10">
        <div class="col-md-12">
            ...
            <?php if (!$orderid) { ?>
                <a href="demographics.php" class="btn btn-default" onclick="top.restoreSession()"><span><?php echo xlt('Back to Client'); ?></span></a>
            <?php } ?>
        </div>
    ...
    <div class="row oe-margin-b-10">
        <div class="col-md-12">
            ...
            $colorbeg = "";
            $colorend = "";
            $resnote = getClientData($client_id, "billing_note");
...
if ($result != "") {
    ...
    foreach ($result as $iter) {
        ...
        $body = $iter['body'];
        $body = preg_replace('/(\sto\s)-client-(\))/', '${1}'.$clientname.'${2}', $body);
        ...
        if (preg_match('/^\d\d\d\d-\d\d-\d\d \d\d\:\d\d /', $body)) {
            ...
            ' (' . text($iter['user']) . ') ' . nl2br(text(oeFormatClientNote($body)));
        }
        ...
        // Modified 6/2009 by BM to incorporate the client notes into the list_options listings
...
if ($result_sent != "") {
    ...
    foreach ($result_sent as $iter) {
        ...
        $body = preg_replace('/(:\d{2}\s\()' . $client_id . '(\sto\s)/', '${1}' . $clientname . '${2}', $body);
        if (($iter{"activity"}) && ($iter['message_status'] != "Done")) {
            $checked = "checked";
        } else {
            $checked = "";
...
<?php
if ($_GET['set_pid']) {
    $ndata = getClientData($client_id, "fname, lname, pubpid");
    ?>
 parent.left_nav.setClient(<?php echo js_escape($ndata['fname']." ".$ndata['lname']) . "," .
     js_escape($client_id) . "," . js_escape($ndata['pubpid']) . ",window.name"; ?>);
    <?php
}

// If this note references a new client document, pop up a display
// of that document.
//
if ($noteid /* && $title == 'New Document' */) {
    ...
    if (preg_match('/New scanned document (\d+): [^\n]+\/([^\n]+)/', $prow['body'], $matches)) {
        ...
        $docname = $matches[2];
        ?>
     window.open('../../../controller.php?document&retrieve&client_id=<?php echo attr_url($client_id); ?>&document_id=<?php echo attr_url($docid); ?>&<?php echo attr_url($docname);?>&as_file=true',
