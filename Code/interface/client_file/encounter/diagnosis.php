...
if (isset($mode)) {
    ...
    if ($mode == "add") {
        ...
        $tmp = sqlQuery("SELECT users.id FROM forms, users WHERE " .
            ...
            "forms.formdir='newclient' AND users.username = forms.user AND " .
      ...
      // If HCPCS, get and save default NDC data.
        ...
        if (!empty($procs) && !empty($diags)) {
            ...
            foreach ($procs as $proc) {
                ...
                $sql[] = "UPDATE billing set justify = concat(justify,'" . add_escape_custom($justify_string)  ."') where encounter = '" . add_escape_custom($_POST['encounter_id']) . "' and pid = '" . add_escape_custom($_POST['client_id']) . "' and code = '" . add_escape_custom($proc) . "'";
            }
        }
        ...
        for ($lino = 1; !empty($ndcarr["$lino"]['code']); ++$lino) {
            ...
            if ($ndc['ndcnum']) {
              ...
              sqlStatement("UPDATE billing SET ndc_info = ? WHERE " .
                "encounter = ? AND " .
                "pid = ? AND " .
                "code = ?", array($ndc_info, $_POST['encounter_id'], $_POST['client_id'], $ndc['code']));
        }
    }
}
...
if (!$thisauth) {
    ...
    "encounter = ? AND formdir = 'newclient' LIMIT 1", array($encounter));
...
<input type="hidden" name="client_id" value="<?php echo attr($pid); ?>">
</form>
</table>
