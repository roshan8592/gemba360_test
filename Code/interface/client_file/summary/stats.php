...
foreach ($ISSUE_TYPES as $key => $arr) {
  // Skip if user has no access to this issue type.
    ...
    if ($old_key=="medication" && $GLOBALS['erx_enable'] && $erx_upload_complete == 1) {
        ...
        ?>
    <div>
        <table id="client_stats_prescriptions">
        <?php
        ...
        <tr><td>
            ...
            $res=sqlStatement("select * from prescriptions where client_id=? and active='1'", array($pid));
            ?>
        <table>
            <?php
            if (sqlNumRows($res) == 0) {
    ...
    if (sqlNumRows($pres) > 0 || $arr[4] == 1) {
        ...
        if ($_POST['embeddedScreen']) {
            ...
            if (($key == "allergy" || $key == "medication") && $GLOBALS['erx_enable']) {
                ...
                $widgetButtonLink = "load_location(\"${GLOBALS['webroot']}/interface/client_file/summary/stats_full.php?active=all&category=" . attr_url($key) . "\")";
            }
...
?>
</table> <!-- end client_stats_issues -->

<table id="client_stats_spreadsheets">
<?php
...
<tr>
    <?php if ($_POST['embeddedScreen']) {
        ...
        $widgetButtonLabel = xl("Edit");
        $widgetButtonLink = "javascript:load_location(\"${GLOBALS['webroot']}/interface/client_file/summary/immunizations.php\")";
        $widgetButtonClass = "";
        $linkMethod = "javascript";
        $bodyClass = "summary_item small";
        $widgetAuth = true;
...
</table> <!-- end client_stats_imm-->
</div>
<?php } ?>

<?php if (!$GLOBALS['disable_prescriptions'] && acl_check('clients', 'rx')) { ?>
<div>
<table id="client_stats_prescriptions">
...
<tr><td colspan='<?php echo $numcols ?>' class='issuetitle'>
    ...
    <?php if ($_POST['embeddedScreen']) {
        ...
        $bodyClass = "summary_item small";
        $widgetAuth=acl_check('clients', 'rx', '', array('write','addonly'));
        $fixedWidth = false;
        expand_collapse_widget($widgetTitle, $widgetLabel, $widgetButtonLabel, $widgetButtonLink, $widgetButtonClass, $linkMethod, $bodyClass, $widgetAuth, $fixedWidth);
    } else { ?>
    ...
    chdir("../../../");
    $c = new Controller();
    echo $c->act(array("prescription" => "", "fragment" => "", "client_id" => $pid));
    ?>
...
if ($erx_upload_complete == 1) {
    ...
    $widgetButtonLabel = xl("Edit");
    $widgetButtonLink = "load_location(\"${GLOBALS['webroot']}/interface/client_file/summary/stats_full.php?active=all&category=medication\")";
    $widgetButtonClass = "";
    $linkMethod = "javascript";
    $bodyClass = "summary_item small";
    $widgetAuth = true;
...
?>
</table> <!-- end client_stats_prescriptions -->
</div>
</div> <!-- end client_stats_summary -->
