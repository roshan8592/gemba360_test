<?php
/**
 * Client Tracker (Client Flow Board)
 *
...
$form_client_name = !is_null($_POST['form_client_name']) ? $_POST['form_client_name'] : null;
$form_client_id = !is_null($_POST['form_client_id']) ? $_POST['form_client_id'] : null;
                    ...
                    <div class=" text-center row divTable" style="width: 85%;padding: 10px 10px 0;margin: 10px auto;">
                        <div class="col-sm-<?php echo attr($col_width); ?> text-center" style="margin-top:15px;">
                            ...
                            <input type="text"
                                   ...
                                   class="form-control input-sm" id="form_client_name" name="form_client_name"
                        ...
                        <div class="col-sm-<?php echo attr($col_width); ?> text-center" style="margin-top:15px;">
                            ...
                            <input placeholder="<?php echo xla('Client ID'); ?>"
                                   class="form-control input-sm" type="text"
                                   id="form_client_id" name="form_client_id"
...
// get all appts for date range and refine view client side.  very fast...
    ...
    $appointments = fetch_Client_Tracker_Events($from_date, $to_date, '', '', '', '', $form_client_name, $form_client_id);
    ...
    ?>
                ...
                <span class="hidden-xs" id="status_summary">
                    <?php
                    $statuses_output = "<span style='margin:0 10px;'><em>" . xlt('Total clients') . ':</em> <span class="badge">' . text($appointments_status['count_all']) . "</span></span>";
                ...
                <span id="pull_kiosk_right" class="pull-right">
                  ...
                  <label for='setting_new_window' id='settings'>
                    <input type='checkbox' name='setting_new_window' id='setting_new_window'
                        ...
                        <?php echo xlt('Open Client in New Window'); ?>
                  </label>
                ...
                <div class="col-sm-12 textclear" >
                    ...
                    foreach ($appointments as $appointment) {
                        ...
                        if (empty($appointment['room']) && ($logged_in) && ($setting_bootstrap_submenu != 'hide')) {
                            //Client has not arrived yet, display MedEx Reminder info
                        ...
                        if ($appt_pid == 0) {
                            continue; // skip when $appt_pid = 0, since this means it is not a client specific appt slot
                        }
                        ...
                        <td class="detail text-center hidden-xs" name="kiosk_hide">
                            <a href="#"
                               onclick="return toclient(<?php echo attr_js($appt_pid); ?>,<?php echo attr_js($appt_enc); ?>)">
                        ...
                        <td class="detail hidden-xs text-center small">
                            ...
                            <?php } else { ?>
                                <a onclick="return bpopup(<?php echo attr_js($tracker_id); // calls popup for client tracker status?>)">
                            <?php } ?>
                            <?php
                            if ($appointment['room'] > '') {
                                echo text(getListItemTitle('client_flow_board_rooms', $appt_room));
                            } else {
...
if (!$_REQUEST['flb_table']) { ?>
    ...
    <!-- form used to open a new top level window when a client row is clicked -->
    ...
    <script type="text/javascript">
        ...
        function toggleSelectors() {
            if ($("#flb_selectors").css('display') === 'none') {
                $.post("<?php echo $GLOBALS['webroot'] . "/interface/client_tracker/client_tracker.php"; ?>", {
        ...
        function refreshMe(fromTimer) {
            ...
            top.restoreSession();
            var posting = $.post('../client_tracker/client_tracker.php', {
                flb_table: '1',
                ...
                form_client_name: $("#form_client_name").val(),
                form_client_id: $("#form_client_id").val(),
        ...
        function refineMe() {
            ...
            var pidV = String($("#form_client_id").val());
            var pidRE = new RegExp(pidV, 'g');
            var pnameV = $("#form_client_name").val();
        ...
        // popup for client tracker status
        function bpopup(tkid) {
            top.restoreSession();
            dlgopen('../client_tracker/client_tracker_status.php?tracker_id=' + encodeURIComponent(tkid) + '&csrf_token_form=' + <?php echo js_url(CsrfUtils::collectCsrfToken()); ?>, '_blank', 500, 250);
            return false;
        }
        ...
        // used to display the client demographic and encounter screens
        function toclient(newpid, enc) {
            if ($('#setting_new_window').val() === 'checked') {
                ...
                top.restoreSession();
                if (enc > 0) {
                    top.RTop.location = "<?php echo $GLOBALS['webroot']; ?>/interface/client_file/summary/demographics.php?set_pid=" + encodeURIComponent(newpid) + "&set_encounterid=" + encodeURIComponent(enc);
                }
                else {
                    top.RTop.location = "<?php echo $GLOBALS['webroot']; ?>/interface/client_file/summary/demographics.php?set_pid=" + encodeURIComponent(newpid);
                }
            }
        }
        ...
        function openNewTopWindow(newpid, newencounterid) {
            document.fnew.clientID.value = newpid;
