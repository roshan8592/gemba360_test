...
class AMC_304f_Numerator implements AmcFilterIF
{
    ...
    public function test(AmcClient $client, $beginDate, $endDate)
    {
        ...
        $amcResults = sqlStatement("SELECT * FROM `amc_misc_data` WHERE `amc_id`=? AND `pid`=? AND `date_created`>=? AND `date_created`<=?", array('provide_rec_pat_amc',$client->id,$beginDate,$endDate));
        ...
        // All requested records for client were given within 3 business days
        return true;
    }
}
