                ...
                <?php if ($type_report == "standard") { ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <option value='client_reminder' <?php echo ($rule_filter == "client_reminder") ? "selected" : ""; ?>>
                            <?php echo xlt('Client Reminder Rules'); ?></option>
                            </select>
                        </td>
                    </tr>
                <?php } ?>
                ...
                <tr>
                        ...
                        <td>
                                <?php
                                 ...
                                 // Build a drop-down list of of client provider relationships.
                                 //
                                 echo "   <select ". $dis_text ." id='form_pat_prov_rel' name='form_pat_prov_rel' class='form-control' title='" . xla('Only applicable if a provider or collated list was chosen above. PRIMARY only selects clients that the provider is the primary provider. ENCOUNTER selects all clients that the provider has seen.') . "'>\n";
...
<thead>
 ...
 <th>
    <?php
    if ($type_report == 'cqm' || $type_report == 'cqm_2011' || $type_report == 'cqm_2014') {
        echo xlt('Initial Client Population');
    } else {
        echo xlt('Total Clients');
    }
    ?>
  </th>
  ...
  <th>
    ...
    <?php } else { ?>
        <?php echo xlt('Applicable Clients') .' (' . xlt('Denominator') . ')'; ?></a>
    <?php } ?>
  </th>
  ...
  <th>
    ...
    <?php } else { ?>
        <?php echo xlt('Passed Clients') . ' (' . xlt('Numerator') . ')'; ?></a>
    <?php } ?>
  </th>
...
<tr bgcolor='<?php echo attr($bgcolor); ?>'>
        ...
        if (isset($row['is_main']) || isset($row['is_sub'])) {
            ...
            if (isset($row['is_main'])) {
                // is_sub is a special case of is_main whereas total clients, denominator, and excluded clients are taken
                // from is_main prior to it. So, need to store denominator clients from is_main for subsequent is_sub
                // to calculate the number of clients that failed.
            ...
            if ($type_report != "amc") {
                ...
                if (($type_report != "standard") && isset($row['itemized_test_id']) && ($row['excluded'] > 0)) {
                    ...
                    echo "<td align='center'><a href='../main/finder/client_select.php?from_page=cdr_report&pass_id=exclude&report_id=" . attr_url($report_id) . "&itemized_test_id=" . attr_url($row['itemized_test_id']) . "&numerator_label=" . attr_url($row['numerator_label']) . "&csrf_token_form=" . attr_url(CsrfUtils::collectCsrfToken()) . "' onclick='top.restoreSession()'>" . text($row['excluded']) . "</a></td>";
                } else {
            ...
            if ($type_report == 'cqm' || $type_report == 'cqm_2011' || $type_report == 'cqm_2014') {
                ...
                if (isset($row['itemized_test_id']) && ($row['exception'] > 0)) {
                   // Note standard reporting exluded is different than cqm/amc and will not support itemization
                    echo "<td align='center'><a href='../main/finder/client_select.php?from_page=cdr_report&pass_id=exception&report_id=" . attr_url($report_id) . "&itemized_test_id=" . attr_url($row['itemized_test_id']) . "&numerator_label=" . attr_url($row['numerator_label']) . "&csrf_token_form=" . attr_url(CsrfUtils::collectCsrfToken()) . "' onclick='top.restoreSession()'>" . text($row['exception']) . "</a></td>";
                } else {
            ...
            if (isset($row['itemized_test_id']) && ($row['pass_target'] > 0)) {
                echo "<td align='center'><a href='../main/finder/client_select.php?from_page=cdr_report&pass_id=pass&report_id=" . attr_url($report_id) . "&itemized_test_id=" . attr_url($row['itemized_test_id']) . "&numerator_label=" . attr_url($row['numerator_label']) . "&csrf_token_form=" . attr_url(CsrfUtils::collectCsrfToken()) . "' onclick='top.restoreSession()'>" . text($row['pass_target']) . "</a></td>";
            } else {
            ...
            if (isset($row['itemized_test_id']) && ($failed_items > 0)) {
                echo "<td align='center'><a href='../main/finder/client_select.php?from_page=cdr_report&pass_id=fail&report_id=" . attr_url($report_id) . "&itemized_test_id=" . attr_url($row['itemized_test_id']) . "&numerator_label=" . attr_url($row['numerator_label']) . "&csrf_token_form=" . attr_url(CsrfUtils::collectCsrfToken()) . "' onclick='top.restoreSession()'>" . text($failed_items) . "</a></td>";
            } else {
