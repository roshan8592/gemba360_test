...
{
    $sql = "SELECT * FROM claims WHERE client_id = ? AND encounter_id = ? AND status > 0 AND status < 4 ";
...
function get_ub04_array($pid, $encounter, &$log = "")
{
    ...
    $ub04id[3] = $pid . ' ' . $encounter; /* 3a. CLIENT CONTROL NUMBER */
    ...
    // $ub04id[16] = ''; /* 8a. CLIENT IDENTIFIER */
    $tmp = $claim->clientLastName() . ', ' . $claim->clientFirstName();
    if ($claim->clientMiddleName()) {
        $tmp .= ', ' . substr($claim->clientMiddleName(), 0, 1);
    }
    $ub04id[18] = $tmp; /* 8b. CLIENT NAME */
    $ub04id[17] = $claim->clientStreet(); /* 9a. CLIENT STREET ADDRESS */
    $ub04id[19] = $claim->clientCity(); /* 9b. CLIENT CITY */
    $ub04id[20] = $claim->clientState(); /* 9c. CLIENT STATE */
    $ub04id[21] = $claim->clientZip(); /* 9d. CLIENT ZIP CODE */
    $ub04id[22] = ''; /* 9e. CLIENT COUNTRY CODE */
    $tmp = $claim->clientDOB();
    $ub04id[23] = substr($tmp, 4, 2) . substr($tmp, 6, 2) . substr($tmp, 0, 4); /* 10. CLIENT BIRTH DATE (MMDDYYYY) */
    $ub04id[24] = $claim->clientSex(); /* 11. CLIENT SEX */
    $tmp = $claim->onsetDate();
    $ub04id[25] = substr($tmp, 4, 2) . substr($tmp, 6, 2) . substr($tmp, 2, 2); /* 12. CLIENT ADMISSION/START OF CARE DATE (MMDDYY) */

    $ub04_proc_index = 0; // Test for second page of charges.
    $proccount = $claim->procCount();
    $clm_total_charges = 0;
    $clm_amount_adjusted = 0;
    $clm_amount_paid = $ub04_proc_index ? 0 : $claim->clientPaidAmount();
    ...
    for ($svccount = 0; $svccount < 22 && $ub04_proc_index < $proccount; ++ $ub04_proc_index) {
        ...
        // @todo need if inclient or out client for box 74 - 74e
    ...
    $ub04id[368] = $diagnosis[1] ? substr($diagnosis[1], 0, 7) : ''; /* 70a. CLIENT'S REASON FOR VISIT */
    $ub04id[369] = $diagnosis[2] ? substr($diagnosis[2], 0, 7) : ''; /* 70b. CLIENT'S REASON FOR VISIT */
    $ub04id[370] = $diagnosis[3] ? substr($diagnosis[3], 0, 7) : ''; /* 70c. CLIENT'S REASON FOR VISIT */
    ...
    $ub04id[305] = $claim->insuredRelationship(); /* 59a. CLIENT'S RELATIONSHIP TO INSURED - PRIMARY PLAN */
