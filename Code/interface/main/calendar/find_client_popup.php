...
include_once("$srcdir/client.inc");
...
if ($_REQUEST['searchby'] && $_REQUEST['searchparm']) {
    ...
    if ($searchby == "Last") {
        $result = getClientLnames("$searchparm", "*");
    } elseif ($searchby == "Phone") {                  //(CHEMED) Search by phone number
        $result = getClientPhone("$searchparm", "*");
    } elseif ($searchby == "ID") {
        $result = getClientId("$searchparm", "*");
    } elseif ($searchby == "DOB") {
        $result = getClientDOB(DateToYYYYMMDD($searchparm), "*");
    } elseif ($searchby == "SSN") {
        $result = getClientSSN("$searchparm", "*");
    }
}
?>
<!DOCTYPE html>
...
<title><?php echo htmlspecialchars(xl('Client Finder'), ENT_NOQUOTES); ?></title>
...
<form class="form-inline" method='post' name='theform' id="theform" action='find_client_popup.php?<?php if (isset($_GET['pflag'])) {
    echo "pflag=0";
                                                                                                   } ?>'>
...
<br>
<!--VicarePlus :: If pflag is set the new client create link will not be displayed -->
<a class="noresult" href='find_client_popup.php?res=noresult' 
    <?php
    if (isset($_GET['pflag']) || (!acl_check('clients', 'demo', '', array('write','addonly')))) {
        ?> style="display:none;" 
        <?php
    }
    ?>  >
    <?php echo htmlspecialchars(xl('Click Here to add a new client.'), ENT_NOQUOTES); ?></a>
</div>
...
// another way to select a client from the list of results
