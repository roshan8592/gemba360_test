...
class BirthdayReminder
{
    ...
    public function birthdayAlertResponse($turnOff)
    {
        ...
        $sql = "REPLACE INTO `client_birthday_alert` (`pid`, `user_id`, `turned_off_on`) VALUES (?,?,?)";
        $res = sqlStatement($sql, array($this->pid, $this->user_id, $date));
    }

    public function isDisplayBirthdayAlert()
    {
        //Collect dob and if deceased for the client
        $sql = "SELECT `deceased_date`, `DOB` FROM `client_data` WHERE `pid` = ?";
        ...
        if ((($GLOBALS['client_birthday_alert'] == 3) && ($today >= date('Y-m-d', $dobStr)) && ($today <= date('Y').'-'.date('m-d', strtotime('+28 days', strtotime($res['DOB'])))  ))||
            (($GLOBALS['client_birthday_alert'] == 2) && ($today >= date('Y-m-d', $dobStr))) ||
            (($GLOBALS['client_birthday_alert'] == 1) && (date('m-d') == date('m-d', $dobStr)))) {
    ...
    {
        $sql = "SELECT `turned_off_on` FROM `client_birthday_alert` WHERE pid = ? AND user_id = ?";
