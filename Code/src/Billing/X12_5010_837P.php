...
class X12_5010_837P
{
    public static function gen_x12_837($pid, $encounter, &$log, $encounter_claim = false)
    {
        ...
        $ClientHL = $claim->isSelfOfInsured() ? 0 : 1;
        ...
        //        HIPAA Individual Client Identifier is mandated use."
        ...
        // For 5010, further subscriber info is sent only if they are the client.
        ...
        if (!$claim->isSelfOfInsured()) {
            ++$edicount;
            $out .= "HL" .        // Loop 2000C Client Information
            "*" . $HLcount .
            "*" . $HLSubscriber .
            "*" . "23" .
            "*" . "0" .
            ...
            ++$edicount;
            $out .= "NM1" .       // Loop 2010CA Client
            "*" . "QC" .
            "*" . "1" .
            "*";
            if ($claim->clientLastName()) {
                $out .= $claim->clientLastName();
            } else {
                $log .= "*** Missing client last name.\n";
            }
            $out .=  "*";
            if ($claim->clientFirstName) {
                $out .= $claim->clientFirstName();
            } else {
                $log .= "*** Missing client first name.\n";
            }
            if ($claim->clientMiddleName() !== '') {
                $out .= "*" . $claim->clientMiddleName();
            }
            $out .= "~\n";
            ...
            $out .= "N3" .
            "*";
            if ($claim->clientStreet()) {
                $out .= $claim->clientStreet();
            } else {
                $log .= "*** Missing client street.\n";
            }
            $out .= "~\n";
            ...
            "*";
            if ($claim->clientCity()) {
                $out .= $claim->clientCity();
            } else {
                $log .= "*** Missing client city.\n";
            }
            $out .= "*";
            if ($claim->clientState()) {
                $out .= $claim->clientState();
            } else {
                $log .= "*** Missing client state.\n";
            }
            $out .= "*";
            if ($claim->x12Zip($claim->clientZip())) {
                $out .= $claim->x12Zip($claim->clientZip());
            } else {
                $log .= "*** Missing client zip.\n";
            }
            $out .= "~\n";
            ...
            if ($claim->clientDOB()) {
                ...
                $log .= "*** Missing client DOB.\n";
            }
            $out .= "*";
            if ($claim->clientSex()) {
                $out .= $claim->clientSex();
            } else {
                $log .= "*** Missing client sex.\n";
            }
            $out .= "~\n";

            // Segment REF*Y4 (Property and Casualty Claim Number) omitted.
            // Segment REF (Property and Casualty Client Identifier) omitted.
            // Segment PER (Property and Casualty Client Contact Information) omitted.
        } // end of client different from insured
        ...
        $clientpaid = $claim->clientPaidAmount();
        if ($clientpaid != 0) {
            ++$edicount;
            $out .= "AMT" .     // Client paid amount. Page 190/220.
            "*" . "F5" .
            "*" . $clientpaid .
            "~\n";
        }
        ...
        // Segment CRC (Client Condition Information: Vision) omitted.
        ...
        for ($ins = 1; $ins < $claim->payerCount(); ++$ins) {
            ...
            if ($claim->payerSequence($ins) < $claim->payerSequence()) {
                ...
                // Segment AMT*EAF (Remaining Client Liability) omitted.
            ...
            // Segment MOA (Medicare Outclient Adjudication) omitted.
            ++$edicount;
        ...
        for ($prockey = 0; $prockey < $proccount; ++$prockey) {
            ...
            // Segment QTY (Ambulance Client Count) omitted.
