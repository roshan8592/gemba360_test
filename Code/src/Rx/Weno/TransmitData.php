...
class TransmitData
{
    ...
    public function getDrugList($pid, $date)
    {
        ...
        $sql = "SELECT * FROM prescriptions WHERE client_id = ? AND ntx = 1 AND txDate = ?";
    ...
    public function clientPharmacyInfo($pid)
    {
        $sql = "SELECT a.pharmacy_id, b.name, b.npi, b.ncpdp FROM client_data AS a, pharmacies AS b WHERE a.pid = ? AND a.pharmacy_id = b.id";
    ...
    public function validateClient($pid)
    {
         $clientInfo = "SELECT DOB, street, postal_code, city, state, sex FROM client_data WHERE pid = ?";
         $val = array($pid);
         $clientRes = sqlQuery($clientInfo, $val);
         return $clientRes;
    }
