...
$sigline['signed'] =
  ...
  . "<img src='./sig.jpg'>"
  . "</div>\n";
$query = sqlStatement("select fname,lname,street,city,state,postal_code,phone_home,DATE_FORMAT(DOB,'%m/%d/%y') as DOB from client_data where pid =?", array($_SESSION['pid']));
if ($result = sqlFetchArray($query)) {
    $client_name = $result['fname'] . ' ' . $result['lname'];
    $client_address = $result['street'];
    $client_city = $result['city'];
    $client_state = $result['state'];
    $client_zip = $result['postal_code'];
    $client_phone = $result['phone_home'];
    $client_dob = $result['DOB'];
}
...
<img src='./vline.jpg' id='vline'>
        ...
        if ($camos_content[0]) { //decide if we are printing this rx
            ...
            {
                global $client_name,$client_address,$client_city,$client_state,$client_zip,$client_phone,$client_dob;
                print "<span class='mytagname'> " . xlt('Name') . ":</span>\n";
                print "<span class='mydata'> " . text($client_name) . " </span>\n";
                print "<span class='mytagname'> " . xlt('Address') . ": </span>\n";
                print "<span class='mydata'> " . text($client_address) . ", " . text($client_city) . ", " .
                text($client_state) . " " . text($client_zip) . " </span><br/>\n";
                print "<span class='mytagname'>" . xlt('Phone') . ":</span>\n";
                print "<span class='mydata'>" . text($client_phone) . "</span>\n";
                print "<span class='mytagname'>" . xlt('DOB') . ":</span>\n";
                print "<span class='mydata'> " . text($client_dob) . " </span>\n";
...
<div class='topheader'>
  ...
  <div class='content'>
        ...
        $content = preg_replace('/CLIENTNAME/i', $client_name, $camos_content[0]);
