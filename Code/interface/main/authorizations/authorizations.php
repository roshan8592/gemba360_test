...
// MAR 20041008 the full authorizations screen sucks... no links to the client charts
...
// This authorizes everything for the specified client.
...
<td valign='top'>
    ...
    if ($authorize) {
        ...
        while (list($ppid,$client) = each($authorize)) {
            $name = getClientData($ppid);

            // If I want to see mine only and this client is not mine, skip it.
            ...
            // Clicking the client name will load both frames for that client,
            // as demographics.php takes care of loading the bottom frame.
            echo "<a href='$rootdir/client_file/summary/demographics.php?set_pid=" .
          ...
          $providerID = sqlFetchArray(sqlStatement(
            "select providerID from client_data where pid=?", array($ppid) ));
          ...
          $providerName = sqlFetchArray(sqlStatement(
            ...
            ****/
            ...
            echo "<td valign=top><span class=bold>".xlt('Transactions').":</span><span class=text><br>" .
              $client{"transaction"} . "</td>\n";
            echo "<td valign=top><span class=bold>".xlt('Client Notes').":</span><span class=text><br>" .
              $client{"pnotes"} . "</td>\n";
...
<?php if (true) : ?>
    top.restoreSession();
    location.href = "<?php echo $GLOBALS['webroot']; ?>/interface/client_file/summary/pnotes_full.php?noteid=" + encodeURIComponent(parts[1]) + "&set_pid=" + encodeURIComponent(parts[0]) + "&active=1";
<?php else : ?>
    // no-op
