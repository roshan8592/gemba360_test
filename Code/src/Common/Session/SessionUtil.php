<?php
/**
 * start/destroy session/cookie for OpenEMR or OpenEMR (client) portal
 *
 * OpenEMR session/cookie strategy:
 *  1. The vital difference between the OpenEMR and OpenEMR (client) portal session/cookie is the
 ...
 *        separate clients via separate logins by same users. This is done via custom
 *        restore_session() javascript function; session IDs are effectively saved in the
 *        top level browser window.
 *     b. For (client) portal OpenEMR, setting cookie_httponly to true. Since only one client will
 *        be logging into the client portal, can set this to true, which will help to prevent XSS
 *        vulnerabilities.
 ...
 *     b.  For api OpenEMR and (client) portal OpenEMR, destroy the session and cookie.
 *
 * @package   OpenEMR
