...
// for Integrated A/R.
//
function ar_get_invoice_summary($client_id, $encounter_id, $with_detail = false)
{
    $codes = array();
    $keysuff1 = 1000;
    $keysuff2 = 5000;
  ...
  // Get charges from services.
    ...
    "activity = 1 AND fee != 0.00 ORDER BY id", array($client_id,$encounter_id));
  ...
  // Get charges from product sales.
    ...
    "ORDER BY s.sale_id";
    $res = sqlStatement($query, array($client_id,$encounter_id));
    while ($row = sqlFetchArray($res)) {
        $amount = sprintf('%01.2f', $row['fee']);
    ...
    $res = sqlStatement("SELECT insurance_data.type as type, insurance_companies.name as name " .
        ...
        "WHERE insurance_data.pid = ?", array($client_id));
    while ($row = sqlFetchArray($res)) {
  ...
  // Get payments and adjustments. (includes copays)
    ...
    "ORDER BY s.check_date, a.sequence_no", array($client_id,$encounter_id));
    while ($row = sqlFetchArray($res)) {
        $code = $row['code'];
        if (! $code) {
            $code = "Unknown";
...
// Returns: -1=Nobody, 0=Client, 1=Ins1, 2=Ins2, 3=Ins3.
// for Integrated A/R.
//
function ar_responsible_party($client_id, $encounter_id)
{
    $row = sqlQuery("SELECT date, last_level_billed, last_level_closed " .
    "FROM form_encounter WHERE " .
    "pid = ? AND encounter = ? " .
    "ORDER BY id DESC LIMIT 1", array($client_id,$encounter_id));
    if (empty($row)) {
        return -1;
    }
    ...
    if (SLEOB::arGetPayerID($client_id, substr($row['date'], 0, 10), $next_level)) {
        return $next_level;
    }
  ...
  // Currently hoping that form_encounter.balance_due can be discarded.
    $balance = 0;
    $codes = ar_get_invoice_summary($client_id, $encounter_id);
