...
<head>
    ...
    <script language="JavaScript">
        ...
        function sel_diagnosis(e) {
            ...
            dlgopen('../client_file/encounter/find_code_popup.php?codetype=<?php echo attr_url(collect_codetypes("diagnosis", "csv")); ?>', '_blank', 500, 400);
        }

        //This invokes the find-code popup.
        function sel_procedure(e) {
            current_sel_name = e.name;
            dlgopen('../client_file/encounter/find_code_popup.php?codetype=<?php echo attr_url(collect_codetypes("procedure", "csv")); ?>', '_blank', 500, 400);
        }
    </script>
...
if ($_POST['form_refresh']) {
    $sqlstmt = "select
                ...
                pd.pid AS client_id,
                DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),pd.dob)), '%Y')+0 AS client_age,
    ...
    if ($type == 'Procedure') {
        $sqlstmt = $sqlstmt.",po.date_ordered AS procedure_order_date_ordered,
            ...
            po.client_instructions AS procedure_order_client_instructions,
...
//from
    ...
    if (strlen($form_immunization)!=0) {
        $sqlstmt = $sqlstmt." LEFT OUTER JOIN immunizations as imm ON imm.client_id = pd.pid
						  LEFT OUTER JOIN codes as immc ON imm.cvx_code = immc.id ";
    }

    if (strlen($form_drug_name)!=0 || $_POST['form_drug'] == true) {
           $sqlstmt=$sqlstmt." left outer join prescriptions AS r on r.client_id=pd.pid
...
// order by
    if ($_POST['form_pt_name'] == true) {
        $odrstmt=$odrstmt.",client_name";
    }

    if ($_POST['form_pt_age'] == true) {
        $odrstmt=$odrstmt.",client_age";
    }
    ...
    if ($odrstmt == '') {
        $odrstmt = " ORDER BY client_id";
    } else {
        $odrstmt = " ORDER BY ".ltrim($odrstmt, ",");
    }

    if ($type == 'Medical History') {
        $sqlstmt="select * from (".$sqlstmt." ".$whr_stmt." ".$odrstmt.",history_data_date desc) a group by client_id";
    } else {
    ...
    <table width=90% align="center" cellpadding="5" cellspacing="0" style="font-family:tahoma;color:black;" border="0">
        <tr bgcolor = "#CCCCCC" style="font-size:15px;">
            ...
            echo " "; ?> <?php echo text($row['client_name']); ?></b></td>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
            <td align="center">
            ...
            $img_id++; ?>" title="<?php echo xla('Click here to view client details'); ?>" /></span>
            </td></tr>
            ...
            $row_id++;?>" class="border1" style="display:none; font-size:13px;" cellpadding=5>
                ...
                <tr bgcolor="#FFFFFF">
                <td><?php echo text($row['client_name']); ?>&nbsp;</td>
                <td> <?php echo text($row['client_id']); ?>&nbsp;</td>
                <td> <?php echo text($row['client_age']); ?>&nbsp;</td>
                                <td> <?php echo generate_display_field(array('data_type'=>'1','list_id'=>'sex'), $row['client_sex']); ?>&nbsp;</td>
                <td> <?php echo generate_display_field(array('data_type'=>'1','list_id'=>'race'), $row['client_race']); ?>&nbsp;</td>
                               <td> <?php echo generate_display_field(array('data_type'=>'1','list_id'=>'ethnicity'), $row['client_ethinic']); ?>&nbsp;</td>
                               <td <?php
                                if (strlen($communication) == 0 || ($_POST['communication_check'] == true)) {
                                    ?> colspan=5 <?php
...
<!-- Procedures Report Start-->
                ...
                <td colspan=3><b><?php echo xlt('Control ID');?></b></td>
                    ...
                    ?>
                                  ...
                                  <td> <?php echo text($row['procedure_order_client_instructions']); ?>&nbsp;</td>
