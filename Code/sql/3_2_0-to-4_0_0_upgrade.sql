...
#IfMissingColumn log client_id
ALTER TABLE log ADD client_id bigint(20) DEFAULT NULL;
#EndIf
...
#IfMissingColumn client_data vfc
...
#IfNotRow categories name Client Photograph
 INSERT INTO categories select (select MAX(id) from categories) + 1, 'Client Photograph', '', 1, rght, rght + 1 from categories where name = 'Categories';
 UPDATE categories SET rght = rght + 2 WHERE name = 'Categories';
 UPDATE categories_seq SET id = (select MAX(id) from categories);
#Endif

#IfMissingColumn client_data completed_ad
ALTER TABLE client_data
...
INSERT INTO list_options ( list_id, option_id, title, seq, is_default ) VALUES ('transactions', 'Client Request', 'Client Request', 20, 0);
...
UPDATE registry SET category = 'Administrative' WHERE category = 'category' AND directory = 'newclient';
...
#EndIf
#IfNotRow2D list_options list_id note_type option_id Client Reminders
...
#IfMissingColumn client_data race
ALTER TABLE `client_data` ADD `race` varchar(255) NOT NULL default '';
#EndIf

#IfMissingColumn client_data ethnicity
ALTER TABLE `client_data` ADD `ethnicity` varchar(255) NOT NULL default '';
#EndIf
...
UPDATE client_data SET ethnicity= case WHEN ethnoracial IN ('Hispanic','othr_us','othr_non_us') THEN ethnoracial ELSE '' END, race = case WHEN ethnoracial NOT IN ('Hispanic','othr_us','othr_non_us') THEN ethnoracial ELSE '' END;
