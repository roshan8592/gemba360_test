...
class OnsiteActivityViewController extends AppBaseController
{
    ...
    public function Query()
    {
        ...
        try {
            ...
            if ($filter) {
                $criteria->AddFilter(new CriteriaFilter('Id,Date,ClientId,Activity,RequireAudit,PendingAction,ActionTaken,Status,Narrative,TableAction,TableArgs,ActionUser,ActionTakenTime,Checksum,Title,Fname,Lname,Mname,Dob,Ss,Street,PostalCode,City,State,Referrerid,Providerid,RefProviderid,Pubpid,CareTeam,Username,Authorized,Ufname,Umname,Ulname,Facility,Active,Utitle,PhysicianType', '%' . $filter . '%'));
            }
  ...
  client_data.city,
  client_data.state,
  client_data.referrerID,
  client_data.providerID,
  client_data.ref_providerID,
  client_data.pubpid,
  client_data.care_team,
  users.username,
  users.authorized,
  ...
  onsite_portal_activity.client_id ";
        $sql .= "From onsite_portal_activity Left Join
  client_data On onsite_portal_activity.client_id = client_data.pid Left Join
  users On client_data.providerID = users.id ";
    ...
    public function Update()
    {
        try {
            ...
            $onsiteactivityview->ClientId = $this->SafeGetVal($json, 'clientId', $onsiteactivityview->ClientId);
