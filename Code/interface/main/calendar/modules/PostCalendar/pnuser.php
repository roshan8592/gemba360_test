...
function postcalendar_user_search()
{
    ...
    $ClientID = pnVarCleanFromInput("client_id");
    // limit the number of results returned by getClientPID
    // this helps to prevent the server from stalling on a request with
    // no PID and thousands of PIDs in the database -- JRM
    // the function getClientPID($pid, $given, $orderby, $limit, $start) <-- defined in library/client.inc
    $plistlimit = 500;
    if (is_numeric($ClientID)) {
        $tpl->assign('ClientList', getClientPID(array('pid'=>$ClientID, 'limit'=>$plistlimit)));
    } else {
        $tpl->assign('ClientList', getClientPID(array('limit' =>$plistlimit)));
    }
    ...
    if (!empty($submit) && strtolower($submit) == "listapps") {
        ...
        $searchargs['client_id'] = pnVarCleanFromInput("client_id");
        $searchargs['listappsFlag'] = true;

        $sqlKeywords .= "(a.pc_pid = '" . add_escape_custom(pnVarCleanFromInput("client_id")) . "' )";
