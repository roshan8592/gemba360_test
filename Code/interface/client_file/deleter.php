<?php
/**
 * delete tool, for logging and removing client data.
 *
...
// Delete and undo product sales for a given client or visit.
// This is special because it has to replace the inventory.
//
function delete_drug_sales($client_id, $encounter_id = 0)
{
    $where = $encounter_id ? "ds.encounter = '" . add_escape_custom($encounter_id) . "'" :
    "ds.pid = '" . add_escape_custom($client_id) . "' AND ds.encounter != 0";
    ...
    if ($encounter_id) {
        ...
        row_delete("drug_sales", "pid = '" . add_escape_custom($client_id) . "'");
    }
}

// Delete a form's data that is specific to that form.
//
function form_delete($formdir, $formid, $client_id, $encounter_id)
{
    $formdir = ($formdir == 'newclient') ? 'encounter' : $formdir;
...
<title><?php echo xlt('Delete Client, Encounter, Form, Issue, Document, Payment, Billing or Transaction'); ?></title>
...
if ($_POST['form_submit']) {
    ...
    if ($client) {
        ...
        row_modify("billing", "activity = 0", "pid = '" . add_escape_custom($client) . "'");
        row_modify("pnotes", "deleted = 1", "pid = '" . add_escape_custom($client) . "'");
       // row_modify("prescriptions" , "active = 0"  , "client_id = '$client'");
        row_delete("prescriptions", "client_id = '" . add_escape_custom($client) . "'");
        row_delete("claims", "client_id = '" . add_escape_custom($client) . "'");
        delete_drug_sales($client);
        row_delete("payments", "pid = '" . add_escape_custom($client) . "'");
        row_delete("ar_activity", "pid = '" . add_escape_custom($client) . "'");
        row_delete("openemr_postcalendar_events", "pc_pid = '" . add_escape_custom($client) . "'");
        row_delete("immunizations", "client_id = '" . add_escape_custom($client) . "'");
        row_delete("issue_encounter", "pid = '" . add_escape_custom($client) . "'");
        row_delete("lists", "pid = '" . add_escape_custom($client) . "'");
        row_delete("transactions", "pid = '" . add_escape_custom($client) . "'");
        row_delete("employer_data", "pid = '" . add_escape_custom($client) . "'");
        row_delete("history_data", "pid = '" . add_escape_custom($client) . "'");
        row_delete("insurance_data", "pid = '" . add_escape_custom($client) . "'");

        $res = sqlStatement("SELECT * FROM forms WHERE pid = ?", array($client));
        while ($row = sqlFetchArray($res)) {
            form_delete($row['formdir'], $row['form_id'], $row['pid'], $row['encounter']);
        }

        row_delete("forms", "pid = '" . add_escape_custom($client) . "'");

       // Delete all documents for the client.
        $res = sqlStatement("SELECT id FROM documents WHERE foreign_id = ?", array($client));
        while ($row = sqlFetchArray($res)) {
            delete_document($row['id']);
        }

        row_delete("client_data", "pid = '" . add_escape_custom($client) . "'");
    } else if ($encounterid) {
        ...
        while ($payrow = sqlFetchArray($payres)) {
            if ($payrow['encounter']) {
                ...
                "payer_type = 0 AND " .
                "adj_amount = 0.00 " .
                "GROUP BY session_id ORDER BY session_id DESC", array($client_id, $payrow['encounter']));
                ...
                row_delete(
                    "ar_activity",
                    "pid = '" . add_escape_custom($client_id) . "' AND " .
                ...
                if ($ref_id) {
                        row_delete(
                            "ar_session",
                            "client_id = '" . add_escape_custom($client_id) ."' AND " .
                            "session_id = '" . add_escape_custom($ref_id) . "'"
                        );
                }
            } else {
        ...
        list($client_id, $encounter_id) = explode(".", $billing);
        sqlStatement("DELETE FROM ar_activity WHERE " .
        "pid = ? AND encounter = ? ", array($client_id, $encounter_id));
        ...
        "WHERE pid = ? AND encounter = ?", array($client_id, $encounter_id));
        sqlStatement("UPDATE drug_sales SET billed = 0 WHERE " .
        "pid = ? AND encounter = ?", array($client_id, $encounter_id));
        BillingUtilities::updateClaim(true, $client_id, $encounter_id, -1, -1, 1, 0, ''); // clears for rebilling
    } else if ($transaction) {
        if (!acl_check('admin', 'super')) {
            die("Not authorized!");
        }
...
<form method='post' name="deletefrm" action='deleter.php?client=<?php echo attr_url($client) ?>&encounterid=<?php echo attr_url($encounterid) ?>&formid=<?php echo attr_url($formid) ?>&issue=<?php echo attr_url($issue) ?>&document=<?php echo attr_url($document) ?>&payment=<?php echo attr_url($payment) ?>&billing=<?php echo attr_url($billing) ?>&transaction=<?php echo attr_url($transaction); ?>&csrf_token_form=<?php echo attr_url(CsrfUtils::collectCsrfToken()); ?>' onsubmit="javascript:alert('1');document.deleform.submit();">
<input type="hidden" name="csrf_token_form" value="<?php echo attr(CsrfUtils::collectCsrfToken()); ?>" />
<p class="lead">&nbsp;<br><?php echo xlt('Do you really want to delete'); ?>

<?php
if ($client) {
    echo xlt('client') . " " . text($client);
} else if ($encounterid) {
