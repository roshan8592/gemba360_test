...
/*   This script is for de-identifying an OpenEMR database for the purpose of creating a live demo or for
    development using real data but keepinng client identities a secret.  The script does the following

        * removing client information from the client_data table and replacing client demographics with a randomly generated name
 ...
 * @link    http://www.open-emr.org
    ...
    MUST MAKE SURE THAT ALL TEXT AREAS IN FORMS DO NOT INCLUDE THE CLIENTS REAL NAME.
*/
...
/* This function replaces the first and last name of the client with a auto generated name
    removes data from the encounter, removes personal information from the client_data, notes, and form_encounter
    tables. This should be the first function called.

    INPUT: connection, last name array, firstname arrays, debug
    OUTPUT: Number of clients DEID.
*/
function deIdClientData($con, $lnames, $male, $female, $DEBUG = false)
{

    removeColumn($con, "client_data", "ss", "0000-00-00");
    ...
    $removeLname = ("Select lname, pid, id, ss, street, sex from client_data ");
    ...
    while ($result = mysqli_fetch_array($query)) {
        ...
        $i++;
        $string = '';
        //Give the user a new last name in client_data.lname
        ...
        mysqli_query($con, $string) or die("Failed Client Replacement");
        $string = '';
...
function deIdInsuranceDataTable($con, $pid)
{
    ...
    //get the client demographics: fname, middlename, lname, street, state, city, zip, dob, subscriber phone,
    $query = "Select fname, lname, mname, DOB, street, state, city, postal_code, ss, phone_home from client_data where pid = $pid";
...
function deIdForms($con)
{
    ...
    removeColumn($con, "form_soap", "subjective", "Client Hurts");
    ...
    removeColumn($con, "pnotes", "body", "DATETIME (FROMUSER to USER) Note about Client posted here");
...
$clients = deIdClientData($con, $lnames, $male, $female, $DEBUG);
...
mysqli_close($con);
echo " \n successfully updated $clients clients \n\n";
