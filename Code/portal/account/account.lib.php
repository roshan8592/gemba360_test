...
function notifyAdmin($pid, $provider)
{
    ...
    $note = xl("New client registration received from client portal. Reminder to check for possible new appointment");
    $title = xl("New Client");
...
function isNew($dob = '', $lname = '', $fname = '', $email = '')
{
    ...
    $dob = '%' . trim($dob) . '%';
    $semail = '%' . trim($email) . '%';
    $sql = "select pid from client_data Where client_data.lname LIKE ? And client_data.fname LIKE ? And client_data.DOB LIKE ? order by date limit 0,1";
    $data = array(
        $last,
        $first,
        $dob
    ...
    if ($email) {
        $sql = "select pid from client_data Where client_data.lname LIKE ? And client_data.fname LIKE ? And client_data.DOB LIKE ? And client_data.email LIKE ? order by date limit 0,1";
        $data = array(
            $last,
            $first,
            $dob,
...
function doCredentials($pid)
{
    ...
    $newpd = sqlQuery("SELECT * FROM `client_data` WHERE `pid`=?", array(
        $pid
    ));
    ...
    $res = sqlStatement("SELECT * FROM client_access_onsite WHERE pid=?", array(
        $pid
    ));
    ...
    if (sqlNumRows($res)) {
        sqlStatement("UPDATE client_access_onsite SET portal_username=?,portal_pwd=?,portal_pwd_status=0 " . $salt_clause . " WHERE pid=?", $query_parameters);
    } else {
        sqlStatement("INSERT INTO client_access_onsite SET portal_username=?,portal_pwd=?,portal_pwd_status=0" . $salt_clause . " ,pid=?", $query_parameters);
    }
