...
class Model extends SMA_Common\Model
{
    public function getAuthUsers()
    {
        ...
        $result = array();
        if (!IS_PORTAL) {
            $query = "SELECT client_data.pid as recip_id, Concat_Ws(' ', client_data.fname, client_data.lname) as username FROM client_data " .
                "LEFT JOIN client_access_onsite pao ON pao.pid = client_data.pid " .
                "WHERE client_data.pid = pao.pid AND pao.portal_pwd_status = 1";
        ...
        while ($row = sqlFetchArray($response)) {
            if (IS_PORTAL || IS_DASHBOARD) {
                ...
                if ((in_array(C_USER, $u)) || $row['sender_id'] == C_USER) {
                     $result[] = $row; // only current client messages
                }
            } else {
...
<head>
    ...
    <title><?php echo xlt('Secure Client Chat'); ?></title>
                ...
                <div class="panel direct-chat direct-chat-warning">
                    <div class="panel-heading">
                        <div class="clearfix">
                            ...
                            <a class="btn btn-sm btn-success pull-left ml10" href="./../client/provider" ng-show="!isPortal"><?php echo xlt('Home'); ?></a>
