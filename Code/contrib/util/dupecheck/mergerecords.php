...
$sqlstmt = "select pid from client_data where id=?";
...
foreach ($parameters['otherid'] as $otherID) {
    ...
    $sqlstmt = "select lname, pid from client_data where id=?";
    ...
    UpdateTable("batchcom", "client_id", $otherPID, $masterPID);
    UpdateTable("immunizations", "client_id", $otherPID, $masterPID);
    UpdateTable("prescriptions", "client_id", $otherPID, $masterPID);
    UpdateTable("claims", "client_id", $otherPID, $masterPID);
    ...
    // How to handle the data that should be unique to each client:
    //  Demographics, Employment, Insurance, and History ??
    //
    //UpdateTable("client_data", "pid", $otherID, $$parameters['masterid']);
    ...
    // alter the client's last name to indicate they have been merged into another record
    $newlname = "~~~MERGED~~~".$orow['lname'];
    $sqlstmt = "update client_data set lname=? where pid=?";
    ...
    // add client notes regarding the merged data
    $notetext = "All related client data has been merged into client record PID# ".$masterPID;
    echo "<li>Added note about the merge to the PID " . text($otherPID) . "</li>";
    if ($commitchanges == true) {
        addPnote($otherPID, $notetext);
    }

    $notetext = "All related client data has been merged from client record PID# ".$otherPID;
