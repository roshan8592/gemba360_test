...
<form method=post name=choose_clients>
...
if ($_POST['submit_pdf'] || $_POST['submit_html'] || ($_GET['pid'] && $_GET['encounter'])) {
    ...
    <title><?php xl('Client Notes', 'e'); ?></title>
    </head>
        <body>
            ...
            if ($results2 = sqlFetchArray($query2)) {
                if ($results2['category'] == 'exam') {
                    ...
                    array_push(
                        ...
                        preg_replace(array("/\n+/","/clientname/i"), array(' ',$results1['fname'].' '.$results1['lname']), $results2['content'])
                    );
                }
            }
        }
