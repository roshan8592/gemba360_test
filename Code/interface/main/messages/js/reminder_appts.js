...
/**
 * Function to find a client in the DB
 * This pop-up is the standard openEMR file find_client_popup.php
 * It returns pid, lname, fname, dob to function "setclient" below
 * which then populates the form with the select client data
 */
...
/**
 * Function to insert client data into addRecall fields
...
function show_client(newpid) {
    if (newpid.length === 0) {
        return;
    }
    top.restoreSession();
    top.RTop.location = "../../client_file/summary/demographics.php?set_pid=" + newpid;
}
...
function process_this(material, id, eid='') {
    ...
    $.ajax({
        ...
        if (material === 'labels') window.open("../../client_file/addr_appt_label.php", "_blank");
...
function newEvt(pid, pc_eid) {
    ...
    var url = '../../main/calendar/add_edit_event.php?clientid=' + pid + '&eid=' + pc_eid;
...
function doRecallclick_edit(goHere) {
    top.restoreSession();
    if (window.location.pathname.match(/client_tracker/)) {
        zone ='main/';
    } else {
        zone = '';
    }
