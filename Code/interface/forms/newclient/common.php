...
<title><?php echo xlt('Client Encounter'); ?></title>
...
<script language="JavaScript">
    ...
    function newissue() {
        dlgopen('../../client_file/summary/add_edit_issue.php', '_blank', 700, 535, '', '', {
            buttons: [
    ...
    $collectthis = collectValidationPageRules("/interface/forms/newclient/common.php");
    ...
    function cancelClickedNew() {
        if (top.tab_mode) {
            window.parent.left_nav.loadFrame('ens1', window.name, 'client_file/history/encounters.php');
        }
        var target = window;
        while (target != top) {
            if (target.name == 'RBot') {
                target.parent.left_nav.loadFrame('ens1', window.name, 'client_file/history/encounters.php');
                break;
            }
            else if (target.name == 'RTop') {
                target.parent.left_nav.loadFrame('dem1', window.name, 'client_file/summary/demographics.php');
                break;
            }
    ...
    function cancelClickedOld() {
        location.href = '<?php echo "$rootdir/client_file/encounter/forms.php"; ?>';
        return false;
    }
...
if ($viewmode) {
    $body_javascript = '';
    $heading_caption = xl('Client Encounter Form');
} else {
...
<body class="body_top" <?php echo $body_javascript;?>>
    <div id="container_div" class="<?php echo attr($oemr_ui->oeContainer()); ?>">
        ...
        <div class="row">
            <div class="col-sm-12">
                <form id="new-encounter-form" method='post' action="<?php echo $rootdir ?>/forms/newclient/save.php" name='new_encounter'>
                <?php if ($viewmode) { ?>
                            ...
                            <div class="form-group ">
                                    ...
                                    <div class="col-sm-3">
                                        <select  name='pc_catid' id='pc_catid' class='form-control col-sm-12'>
                                            ...
                                            <?php
                                            //Bring only client ang group categories
                                            ...
                                            <div class="col-sm-12">
                                                <div class="col-sm-12 pull-left" style="padding-bottom:5px">
                                                    <div class="btn-group" role="group">
                                                        <?php if (acl_check('clients', 'med', '', 'write')) { ?>
                                                       <a href="../../client_file/summary/add_edit_issue.php" class="css_button_small link_submit enc_issue"
...
if (!$viewmode) { ?>
 function duplicateVisit(enc, datestr) {
    if (!confirm(<?php echo xlj("A visit already exists for this client today. Click Cancel to open it, or OK to proceed with creating a new one.") ?>)) {
            // User pressed the cancel button, so re-direct to today's encounter
            top.restoreSession();
            parent.left_nav.setEncounter(datestr, enc, window.name);
            parent.left_nav.loadFrame('enc2', window.name, 'client_file/encounter/encounter_top.php?set_encounter=' + encodeURIComponent(enc));
            return;
        }
  ...
  // Search for an encounter from today
    ...
    " AND fe.date <= ? " .
    " AND " .
    "f.formdir = 'newclient' AND f.form_id = fe.id AND f.deleted = 0 " .
