...
class CarecoordinationController extends AbstractActionController
{
    ...
    public function uploadAction()
    {
        ...
        if ($action == 'add_new_client') {
            $this->getCarecoordinationTable()->insert_client($am_id, $document_id);
        }
    ...
    public function importAction()
    {
        ...
        $client_role = $array['recordTarget']['clientRole'];
        $client_pub_pid = $client_role['id'][0]['extension'];
        $client_ssn = $client_role['id'][1]['extension'];
        $client_address = $client_role['addr']['streetAddressLine'];
        $client_city = $client_role['addr']['city'];
        $client_state = $client_role['addr']['state'];
        $client_postalcode = $client_role['addr']['postalCode'];
        $client_country = $client_role['addr']['country'];
        $client_phone_type = $client_role['telecom']['use'];
        $client_phone_no = $client_role['telecom']['value'];
        $client_fname = $client_role['client']['name']['given'][0];
        $client_lname = $client_role['client']['name']['given'][1];
        $client_family_name = $client_role['client']['name']['family'];
        $client_gender_code = $client_role['client']['administrativeGenderCode']['code'];
        $client_gender_name = $client_role['client']['administrativeGenderCode']['displayName'];
        $client_dob = $client_role['client']['birthTime']['value'];
        $client_marital_status = $client_role['client']['religiousAffiliationCode']['code'];
        $client_marital_status_display = $client_role['client']['religiousAffiliationCode']['displayName'];
        $client_race = $client_role['client']['raceCode']['code'];
        $client_race_display = $client_role['client']['raceCode']['displayName'];
        $client_ethnicity = $client_role['client']['ethnicGroupCode']['code'];
        $client_ethnicity_display = $client_role['client']['ethnicGroupCode']['displayName'];
        $client_language = $client_role['client']['languageCommunication']['languageCode']['code'];
