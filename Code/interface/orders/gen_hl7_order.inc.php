...
/**
 * Get array of insurance payers for the specified client as of the specified
 * date. If no date is passed then the current date is used.
 *
 * @param  integer $pid             Client ID.
...
function gen_hl7_order($orderid, &$out)
{
  ...
  // Delimiters
    ...
    $porow = sqlQuery(
        ...
        "forms AS f, client_data AS pd, users AS u " .
        "WHERE " .
  ...
  // Client Identification
    ...
    $d1 . $porow['pid'] .            // Client ID (not required)
    $d1 . $porow['pid'] .            // Client ID (required)
    $d1 .                            // Alternate Client ID (not required)
  ...
  // Client Visit.
    $out .= "PV1" .
    $d1 . "1" .                           // Set ID (always just 1 of these)
    $d1 .                                 // Client Class (if required, O for Outclient)
    $d1 .                                 // Client Location (for inclient only?)
    $d1 . $d1 . $d1 .
  ...
  // Guarantor. OpenEMR doesn't have these so use the client.
    $out .= "GT1" .
    $d1 . "1" .                      // Set ID (always just 1 of these)
    $d1 .
    $d1 . hl7Text($porow['lname']) .
