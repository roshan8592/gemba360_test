...
#superbill_clientdata {
}
#superbill_clientdata h1 {
    font-weight: bold;
    font-size: 1.2em;
    margin: 0px;
    padding: 5px;
  ...
  <td width='650px'>
    ...
    <table class='text'>
        <tr>
            ...
            <td>
            <input type='text' size='20' name='form_client' style='width:100%;cursor:pointer;cursor:hand' value='<?php echo ($form_client) ? attr($form_client) : xla('Click To Select'); ?>' onclick='sel_client()' title='<?php echo xla('Click to select client'); ?>' />
...
<h2><?php echo text($facility['name'])?></h2>
    ...
    <?php
        ...
        $res_query =    "select * from forms where " .
                        "form_name = 'New Client Encounter' and " .
                        "date between ? and ? " ;
    ...
    while ($result = sqlFetchArray($res)) {
        if ($result{"form_name"} == "New Client Encounter") {
            $newclient[] = $result{"form_id"}.":".$result{"encounter"};
            $pids[] = $result{"pid"};
        }
    }
    ...
    function postToGet($newclient, $pids)
    {
        $getstring="";
        $serialnewclient = serialize($newclient);
        $serialpids = serialize($pids);
        $getstring = "newclient=".urlencode($serialnewclient)."&pids=".urlencode($serialpids);
    ...
    foreach ($newclient as $client) {
        ...
        */
        ...
        print "<div id='superbill_clientdata'>";
        print "<h1>".xlt('Client Data').":</h1>";
        printRecDataOne($client_data_array, getRecClientData($pids[$iCounter]), $N);
        print "</div>";
        ...
        if (count($client) > 0) {
            ...
            $total = 0.00;
            $copays = 0.00;
            //foreach ($client as $be) {

            $ta = explode(":", $client);
            $billing = getClientBillingEncounter($pids[$iCounter], $ta[1]);
            ...
            $copays = BillingUtilities::getClientCopay($pids[$iCounter], $ta[1]);
            //}
            echo "<tr><td>&nbsp;</td></tr>";
