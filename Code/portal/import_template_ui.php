...
{
    $response = sqlStatement("SELECT client_data.pid, Concat_Ws(' ', client_data.fname, client_data.lname) as ptname FROM client_data WHERE allow_client_portal = 'YES'");
    $resultpd = array ();
        ...
        success: function (templateHtml, textStatus, jqXHR) {
            if (mode == 'get') {
                ...
                $('#templatecontent').summernote({
                    ...
                    nugget: {
                        list: [
                            '{ParseAsHTML}', '{TextInput}', '{smTextInput}', '{CheckMark}', '{ynRadioGroup}', '{DOS}', '{ReferringDOC}', '{ClientID}','{ClientName}', '{ClientSex}', '{ClientDOB}', '{ClientPhone}', '{ClientSignature}', '{Address}', '{City}', '{State}', '{Zip}', '{AdminSignature}', '{Medications}', '{ProblemList}', '{Allergies}', '{ChiefComplaint}'
                        ],
...
<hr>
<h3><?php echo xlt('Client Document Template Upload'); ?></h3>
...
<?php echo xlt('Example Privacy_Agreement.txt becomes Privacy Agreement button in Client Documents'); ?>.</em></h4>
...
<br>
<button class="btn btn-primary" type="button" onclick="location.href='./client/provider'"><?php echo xlt('Home'); ?></button>
...
<form id = "edit_form" name = "edit_form" class="form-inline" action="" method="post">
 ...
 <label for="sel_pt"><?php echo xlt('Client'); ?></label>
 <select class="form-control" id="sel_pt" name="sel_pt">
<option value='0'><?php echo xlt("Global All Clients")?></option>
<?PHP
$ppt = getAuthUsers();
global $getdir;
foreach ($ppt as $pt) {
