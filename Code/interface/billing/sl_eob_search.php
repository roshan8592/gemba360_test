...
if (!empty($GLOBALS['portal_onsite_two_enable'])) {
    /* Addition of onsite portal client notify of invoice and reformated invoice - sjpadgett 01/2017 */
    require_once("../../portal/lib/portal_mail.inc");
    require_once("../../portal/lib/appsql.class.php");

    function is_auth_portal($pid = 0)
    {
        if ($pData = sqlQuery("SELECT id, allow_client_portal, fname, lname FROM `client_data` WHERE `pid` = ?", array($pid))) {
            if ($pData['allow_client_portal'] != "YES") {
                return false;
            } else {
    ...
    function notify_portal($thispid, array $invoices, $template, $invid)
    {
        ...
        if (SaveClientAudit($thispid, $invoices) != true) {
            return false;
        } // this is all the invoice data for portal auditing
        $note = xl('You have an invoice due for payment in your Client Documents. There you may pay, download or print the invoice. Thank you.');
        if (sendMail($_SESSION['authUser'], $note, xlt('Bill/Collect'), '', '0', $_SESSION['authUser'], $_SESSION['authUser'], $_SESSION['portalUser'], $invoices[0]['client'], "New", '0') == 1) { // remind admin this was sent
            sendMail($_SESSION['portalUser'], $note, xlt('Bill/Collect'), '', '0', $_SESSION['authUser'], $_SESSION['authUser'], $_SESSION['portalUser'], $invoices[0]['client'], "New", '0'); // notify client
        } else {
            return false;
        }
    ...
    function SaveClientAudit($pid, $invs)
    {
        ...
        try {
            ...
            $audit['narrative'] = "Request client online payment.";
            $audit['table_action'] = '';
...
function emailLogin($client_id, $message)
{
    $clientData = sqlQuery("SELECT * FROM `client_data` WHERE `pid`=?", array($client_id));
    if ($clientData['hipaa_allowemail'] != "YES" || empty($clientData['email']) || empty($GLOBALS['client_reminder_sender_email'])) {
        return false;
    }

    if (!(validEmail($clientData['email']))) {
        return false;
    }

    if (!(validEmail($GLOBALS['client_reminder_sender_email']))) {
        return false;
    }
    ...
    $mail = new MyMailer();
    $pt_name = $clientData['fname'] . ' ' . $clientData['lname'];
    $pt_email = $clientData['email'];
    $email_subject = ($facility['name'] . ' ' . xl('Client Statement Bill'));
    $email_sender = $GLOBALS['client_reminder_sender_email'];
...
if (($_REQUEST['form_print'] || $_REQUEST['form_download'] || $_REQUEST['form_email'] || $_REQUEST['form_pdf']) || $_REQUEST['form_portalnotify'] && $_REQUEST['form_cb']) {
    ...
    $res = sqlStatement("SELECT " .
        ...
        "FROM form_encounter AS f, client_data AS p " .
        "WHERE $where " .
        "p.pid = f.pid " .
    ...
    while ($row = $rows[$rcnt++]) {
        ...
        if ($flagT) {
            $flagT = false;
            $aClientFirstName = $row['fname'];
            $aClientID = $row['pid'];
            $useClientNamePdf = true;
        } elseif (!$multipleClients) {
            if ($aClientID != $row['pid']) {
        ...
        // If this is a new client then print the pending statement
        // and start a new one.  This is an associative array:
        //
        //  cid     = same as pid
        //  pid     = OpenEMR client ID
        //  client = client name
        ...
        if ($stmt['cid'] != $row['pid']) {
            ...
            $stmt['client'] = $row['fname'] . ' ' . $row['lname'];
        ...
        foreach ($invlines as $key => $value) {
            ...
            if ($GLOBALS['use_custom_statement']) {
                $line['desc'] = ($key == 'CO-PAY') ? "Client Payment" : $value['code_text'];
            } else {
                $line['desc'] = ($key == 'CO-PAY') ? "Client Payment" : "Procedure $key";
            }
    ...
    if ($_REQUEST['form_download']) {
        ...
        upload_file_to_client_pdf($STMT_TEMP_FILE, $aClientFirstName, $aClientID, $useClientNamePdf);
    } elseif ($_REQUEST['form_email']) {
        upload_file_to_client_email($stmt['pid'], $STMT_TEMP_FILE);
    } elseif ($_REQUEST['form_portalnotify']) {
        if ($alertmsg == "") {
            $alertmsg = xl('Sending Invoice to Client Portal Completed');
        }
    } else { // Must be print!
        if ($DEBUG) {
...
<head>
    ...
    <script language="JavaScript">
        ...
        function npopup(pid) {
            window.open('sl_eob_client_note.php?client_id=' + encodeURIComponent(pid), '_blank', 'width=500,height=250,resizable=1');
            return false;
        }
        ...
        function toEncSummary(pid) {
            // Tabs only
            top.restoreSession();
            let encurl = 'client_file/history/encounters.php?billing=1&issue=0&pagesize=20&pagestart=0';
            let paturl = "client_file/summary/demographics.php?set_pid=" + encodeURIComponent(pid);
                ...
                <fieldset id="payment-allocate" class="oe-show-hide">
                    ...
                    <div class="col-xs-12 oe-custom-line">
                        <div class="col-xs-3">
                            ...
                            echo "    <option value='0'>-- " . xlt('Client') . " --</option>\n";
                    ...
                    <div class="col-xs-12 .oe-custom-line oe-show-hide" id='inv-search'>
                        <div class="col-xs-3">
                            ...
                            <input type='text' name='form_name' id='form_name' class='form-control'
                                   ...
                                   title='<?php echo xla("Any part of the client name, or \"last,first\", or \"X-Y\""); ?>'
                        ...
                        <div class="col-xs-2">
                            ...
                            <input type='text' name='form_pid' id='form_pid' class='form-control'
                                   ...
                                   title='<?php echo xla("Client chart ID"); ?>'>
                        </div>
                        <div class="col-xs-2">
                ...
                <fieldset id="search-results" class="oe-show-hide">
                    ...
                    <div class="table-responsive">
                        ...
                        if ($_REQUEST['form_search'] || $_REQUEST['form_print']) {
                            ...
                            "JOIN client_data AS p ON p.pid = f.pid " .
                            "WHERE $where " .
                        ...
                        <table class="table table-striped table-condensed">
                            ...
                            <tr>
                                ...
                                <th class="dehead">&nbsp;<?php echo xlt('Client'); ?></th>
                            ...
                            while ($row = sqlFetchArray($t_res)) {
                                ...
                                //new filter only clients with debt.
                                ...
                                // the client was sent a statement for this invoice.
                                //
                                $duncount = $row['stmt_count'];

                                // But if we have not yet billed the client, then compute $duncount as a
                                ...
                                // An invoice is now due from the client if money is owed and we are
                                ...
                                <tr>
                                    ...
                                    <td align="left" class="detail">
                                        <?php
                                        $clientData = sqlQuery("SELECT * FROM `client_data` WHERE `pid`=?", array($row['pid']));
                                        if ($clientData['hipaa_allowemail'] == "YES" && $clientData['allow_client_portal'] == "YES" && $clientData['hipaa_notice'] == "YES" && validEmail($clientData['email'])) {
                                            echo xlt("YES");
                                        } else {
                                            echo xlt("NO");
                                        }
                            ...
                            if ($eracount) { ?>
                                ...
                                if ($is_portal) { ?>
                                    <button type="submit" class="btn btn-default btn-save" name='form_portalnotify'
                                            value="<?php echo xla('Notify via Client Portal'); ?>">
                                        <?php echo xlt('Notify via Client Portal'); ?></button>
                                    <?php
                                }
                            }
                            ?>
