...
// For those who care that this is the client report.
$GLOBALS['CLIENT_REPORT_ACTIVE'] = true;
...
$auth_med      = acl_check('clients', 'med');
$auth_demo     = acl_check('clients', 'demo');
...
if ($printable) {
  ...
  *******************************************************************/
  ...
  // in HTML view it's just one line at the top of page 1
    echo '<page_header style="text-align:right;" class="custom-tag"> ' . xlt("CLIENT") . ':' . text($titleres['lname']) . ', ' . text($titleres['fname']) . ' - ' . text($titleres['DOB_TS']) . '</page_header>    ';
...
<a href="javascript:window.close();"><span class='title'><?php echo xlt('Client') . ": " . text($titleres['fname']) . " " . text($titleres['lname']); ?></span></a><br>
<span class='text'><?php echo xlt('Generated on'); ?>: <?php echo text(oeFormatShortDate()); ?></span>
    ...
    <a href="client_report.php" onclick='top.restoreSession()'>
 <span class='title'><?php echo xlt('Client Report'); ?></span>
...
// For each form field from client_report.php...
//
foreach ($ar as $key => $val) {
    ...
    if (stristr($key, "include_")) {
        if ($val == "recurring_days") {
            ...
            if (!empty($ar['newclient']) && count($ar['newclient']) > 0) {
                $billings = array();
                echo "<table>";
    ...
    /****
            ...
            print "<span class=bold>Client Allergies:</span><br>";
            printListData($pid, "allergy", "1");

        } elseif ($val == "medications") {

            print "<span class=bold>Client Medications:</span><br>";
            printListData($pid, "medication", "1");

        } elseif ($val == "medical_problems") {

            print "<span class=bold>Client Medical Problems:</span><br>";
    ...
    ****/
            ...
            if (acl_check('clients', 'med')) {
                ...
                print "<h1>" . xlt('Client Immunization') . ":</h1>";
                $sql = "select i1.immunization_id, i1.administered_date, substring(i1.note,1,20) as immunization_note, c.code_text_short ".
                   ...
                   " where i1.client_id = ? and i1.added_erroneously = 0 ".
        ...
        // communication report
            ...
            print "<h1>" . xlt('Client Communication sent') . ":</h1>";
            ...
            print "<h1>" . xlt('Client Notes') . ":</h1>";
            printClientNotes($pid);
            echo "</div>";
        } elseif ($val == "transactions") {
            echo "<hr />";
            echo "<div class='text transactions'>\n";
            print "<h1>" . xlt('Client Transactions') . ":</h1>";
            ...
            foreach ($val as $valkey => $valvalue) {
                ...
                if ($extension == ".png" || $extension == ".jpg" || $extension == ".jpeg" || $extension == ".gif") {
                    if ($PDF_OUTPUT) {
                        ...
                        "/controller.php?document&retrieve&client_id=&document_id=" .
                    ...
                    if ($PDF_OUTPUT && $extension == ".pdf") {
                        ...
                        if ($PDF_OUTPUT) {
                            ...
                            echo "<img src='" . $GLOBALS['webroot'] . "/controller.php?document&retrieve&client_id=&document_id=" . attr_url($document_id) . "&as_file=false&original_file=false'><br><br>";
                        }
                    }
                } // end if-else
            } // end Documents loop
...
if ($PDF_OUTPUT) {
    $content = getContent();
    $ptd = getClientData($pid, "fname,lname");
    ...
    if ($PDF_OUTPUT == 1) {
        ...
        $ptdata = getClientData($pid, 'cmsportal_login');
        ...
        echo "<p>" . xlt('Report has been sent to the client.') . "</p>\n";
        echo "</body></html>\n";
    }
...
<script type="text/javascript" src="<?php echo $GLOBALS['web_root']?>/interface/client_file/report/custom_report.js?v=<?php echo $v_js_includes; ?>"></script>
<?php } ?>
</body>
</html>
<?php } ?>
