...
function openClient(ptid) {
 // This works whether we are a popup or in the OpenEMR frameset.
 myRestoreSession();
 document.location.href = 'client_form.php?postid=<?php echo xls($postid); ?>&ptid=' + ptid;
}
...
<body class="body_top">
<center>
<form method='post' action='client_select.php' onsubmit='return myRestoreSession()'>
...
if ($login_matches == 1) {
  ...
  // leave them onscreen and let the user choose.
    ...
    echo "openClient('" . addslashes($login_pid) . "');\n";
    echo "</script>\n";
}
?>

<p>
<input type='button' value='<?php echo xla('Add New Client'); ?>' onclick="openClient(0)" />
&nbsp;
