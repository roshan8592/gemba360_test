...
class CategoryTree extends Tree
{
    ...
    */
    ...
    function _get_categories_array($client_id, $user = '')
    {
        $categories = array();
        $sqlArray = array();
        ...
        if (is_numeric($client_id)) {
            if ($client_id == "00") {
      // Collect documents that are not assigned to a client
                $sql .= " AND (d.foreign_id = 0 OR d.foreign_id IS NULL) ";
            } else {
      // Collect documents for a specific client
                $sql .= " AND d.foreign_id = ? ";
                array_push($sqlArray, $client_id);
            }
        }
