...
var actpage = {
	...
	fetchParams: { filter: '', orderBy: 'clientId', orderDesc: 'DESC', page: 1, status: 'waiting' },
	...
	init: function() {
		...
		function showPaymentModal(cpid,recid){
			 var title = 'Client Online Payment';
			 var params = {
		                buttons: [
		...
		function showProfileModal(cpid){
			 var title = 'Demographics Legend Red: Chart Values. Blue: Client Edits.';
			 var params = {
		                buttons: [
					   	   { text: 'Help', close: false, style: 'info btn-sm',id: 'formHelp'},
					   	   { text: 'Cancel', close: true, style: 'default btn-sm'},
		                   { text: 'Revert Edits', close: false, style: 'success btn-sm',id:'replaceAllButton'},
		                   { text: 'Commit to Chart', style: 'danger btn-sm', close: false,id:'saveClientButton'}],
		                size: eModal.size.xl,
		                subtitle: 'Provider Audit.',
		                title: title,
		                useBin: false,
		                url: './clientdata?pid='+cpid+'&user='+cuser
		            };
		        return eModal.ajax(params)
		            .then(function () { });
		 };
        ...
        /*function showProfileModal(cpid) {
            var title = 'Demographics Legend Red: Charted Values. Blue: Client Edits Provider Audit.';
