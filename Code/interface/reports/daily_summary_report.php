 ...
 *  New Clients, Visited clients, Total Charges, Total Co-pay and Balance amount for the selected facility & providers wise.
 *
 * @package   OpenEMR
...
<html>
    ...
    <body class="body_top">
        ...
        $facilities = $totalAppointment = $totalNewClient = $totalVisit = $totalPayment = $dailySummaryReport = $totalPaid = array();

        // define all the where condition variable as initial value set 1=1
        $whereTotalVisitConditions = $whereTotalPaymentConditions = $wherePaidConditions = $whereNewClientConditions = '1 = 1 ';
        ...
        $sqlBindArrayAppointment = $sqlBindArrayTotalVisit = $sqlBindArrayTotalPayment = $sqlBindArrayPaid = $sqlBindArrayNewClient = array();

        // make all condition on by default today's date
        if ($dateSet != 1 && $facilitySet != 1) {
            $whereNewClientConditions .= ' AND DATE(`OPE`.`pc_eventDate`) = ? ';
            array_push($sqlBindArrayNewClient, date("Y-m-d"));
        ...
        if (1 === $facilitySet) {
            ...
            $whereNewClientConditions .= ' AND `f`.`id` = ?';
            array_push($sqlBindArrayNewClient, $selectedFacility);
        ...
        if (1 === $dateSet) {
            $whereNewClientConditions .= ' AND DATE(`OPE`.`pc_eventDate`) BETWEEN ? AND ?';
            array_push($sqlBindArrayNewClient, $from_date, $to_date);
        ...
        if (isset($selectedProvider) && !empty($selectedProvider)) {
            ...
            $whereNewClientConditions .= ' AND `OPE`.`pc_aid` = ?';
            array_push($sqlBindArrayNewClient, $selectedProvider);
        ...
        //Count Total New Client
        $newClientSql = sqlStatement("SELECT `OPE`.`pc_eventDate` , `f`.`name` AS facility_Name , count( * ) AS totalNewClient, `PD`.`providerID`, CONCAT( `u`.`fname`, ' ', `u`.`lname` ) AS provider_name
                                        FROM `client_data` AS PD
                                        ...
                                        WHERE `OPE`.`pc_title` = 'New Client'
                                        AND  $whereNewClientConditions
                                        GROUP BY `f`.`id` , `OPE`.`pc_eventDate`,provider_name
                                        ORDER BY `OPE`.`pc_eventDate` ASC", $sqlBindArrayNewClient);



        while ($totalNewClientRecord = sqlFetchArray($newClientSql)) {
            $totalNewClient[$totalNewClientRecord['pc_eventDate']][$totalNewClientRecord['facility_Name']][$totalNewClientRecord['provider_name']]['newClient'] = $totalNewClientRecord['totalNewClient'];
        }
        ...
        $dailySummaryReport = array_merge_recursive($totalAppointment, $totalNewClient, $totalVisit, $totalPayment, $totalPaid);
        ?>
        ...
        <div id="report_results" style="font-size: 12px">
            ...
            <table class="flowboard" cellpadding='5' cellspacing='2' id="ds_report">
                <tr class="head">
                    ...
                    <td><?php echo xlt('New Clients'); ?></td>
                    <td><?php echo xlt('Visited Clients'); ?></td>
                ...
                if (count($dailySummaryReport) > 0) { // check if daily summary array has value
                            ...
                            if (isset($dataValue[$facility])) {
                                foreach ($dataValue[$facility] as $provider => $information) { // array which consists different/dynamic values
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo isset($information['newClient']) ? text($information['newClient']) : 0; ?></td>
                                    ...
                                    <?php
                                    if (count($dailySummaryReport) > 0) { // calculate the total count of the appointments, new client,visits, payments, paid amount and due amount
                                        $totalAppointments += $information['appointments'];
                                        $totalNewRegisterClient += $information['newClient'];
                    ...
                    <tr class="totalrow">
                        ...
                        <td><?php echo text($totalNewRegisterClient); ?></td>
