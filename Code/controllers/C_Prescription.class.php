...
class C_Prescription extends Controller
{
    ...
    function edit_action($id = "", $client_id = "", $p_obj = null)
    {
        ...
        if (!empty($client_id)) {
            $this->prescriptions[0]->set_client_id($client_id);
        }
    ...
    function list_action($id, $sort = "")
    {
        ...
        if ($GLOBALS['rx_show_drug_drug']) {
            ...
            if ($rxn == false) {
                ...
                $pid = $GLOBALS['pid'];
                $medList = sqlStatement("SELECT drug FROM prescriptions WHERE active = 1 AND client_id = ?", array($pid));
                $nameList = array();
    ...
    // are set as e-prescriptions)
        if (!(empty($_POST['escribe_flag']))) {
              ...
              processAmcCall('e_prescribe_amc', true, 'add', $this->prescriptions[0]->get_client_id(), 'prescriptions', $this->prescriptions[0]->id);
        } else {
              // remove the e-prescribe flag
              processAmcCall('e_prescribe_amc', true, 'remove', $this->prescriptions[0]->get_client_id(), 'prescriptions', $this->prescriptions[0]->id);
        }

    // Set the AMC reporting flag (to record prescriptions that checked drug formulary)
        if (!(empty($_POST['checked_formulary_flag']))) {
              // add the e-prescribe flag
              processAmcCall('e_prescribe_chk_formulary_amc', true, 'add', $this->prescriptions[0]->get_client_id(), 'prescriptions', $this->prescriptions[0]->id);
        } else {
              // remove the e-prescribe flag
              processAmcCall('e_prescribe_chk_formulary_amc', true, 'remove', $this->prescriptions[0]->get_client_id(), 'prescriptions', $this->prescriptions[0]->id);
        }
    ...
    // Set the AMC reporting flag (to record prescriptions that are controlled substances)
        if (!(empty($_POST['controlled_substance_flag']))) {
              // add the e-prescribe flag
              processAmcCall('e_prescribe_cont_subst_amc', true, 'add', $this->prescriptions[0]->get_client_id(), 'prescriptions', $this->prescriptions[0]->id);
        } else {
              // remove the e-prescribe flag
              processAmcCall('e_prescribe_cont_subst_amc', true, 'remove', $this->prescriptions[0]->get_client_id(), 'prescriptions', $this->prescriptions[0]->id);
        }
...
//     }
        $this->list_action($this->prescriptions[0]->get_client_id());
        exit;
    }
    ...
    function send_action($id)
    {
        ...
        // Populate pharmacy info if the client has a default pharmacy.
        ...
        $prow = sqlQuery("SELECT pt.pharmacy_id FROM prescriptions AS rx, " .
            "client_data AS pt WHERE rx.id = '$id' AND pt.pid = rx.client_id");
        if ($prow['pharmacy_id']) {
    ...
    // A client had a bad experience with a client misusing a DEA number, so
    ...
    //     Administration->Globals->Rx
        ...
        $pdf->ezText('<b>' . xl('Client Name & Address') . '</b>', 6);
        $pdf->ezText($p->client->get_name_display(), 10);
        $res = sqlQuery("SELECT  concat(street,'\n',city,', ',state,' ',postal_code,'\n',if(phone_home!='',phone_home,if(phone_cell!='',phone_cell,if(phone_biz!='',phone_biz,'')))) addr from client_data where pid =". add_escape_custom($p->client->id));
        $pdf->ezText($res['addr']);
        $my_y = $pdf->y;
        $pdf->ezNewPage();
        $pdf->line($pdf->ez['leftMargin'], $pdf->y, $pdf->ez['pageWidth']-$pdf->ez['rightMargin'], $pdf->y);
        $pdf->ezText('<b>' . xl('Date of Birth') . '</b>', 6);
        $pdf->ezText($p->client->date_of_birth, 10);
        $pdf->ezText('');
        $pdf->line($pdf->ez['leftMargin'], $pdf->y, $pdf->ez['pageWidth']-$pdf->ez['rightMargin'], $pdf->y);
        $pdf->ezText('<b>' . xl('Medical Record #') . '</b>', 6);
        $pdf->ezText(str_pad($p->client->get_pubpid(), 10, "0", STR_PAD_LEFT), 10);
        $pdf->ezColumnsStop();
        if ($my_y < $pdf->y) {
            $pdf->ezSetY($my_y);
        }
    ...
    function multiprintcss_header($p)
    {
        ...
        echo ('<b><span class="small">' . xl('Client Name & Address') . '</span></b>'. '<br>');
        echo ($p->client->get_name_display() . '<br>');
        $res = sqlQuery("SELECT  concat(street,'\n',city,', ',state,' ',postal_code,'\n',if(phone_home!='',phone_home,if(phone_cell!='',phone_cell,if(phone_biz!='',phone_biz,'')))) addr from client_data where pid =". add_escape_custom($p->client->id));
        if (!empty($res)) {
            $patterns = array ('/\n/');
            $replace = array ('<br>');
        ...
        echo ($p->client->date_of_birth );
        echo ("</td>\n");
        echo ("</tr>\n");
        echo ("<tr>\n");
        echo ("<td class='bordered'>\n");
        echo ('<b><span class="small">' . xl('Medical Record #') . '</span></b>' . '<br>');
        echo (str_pad($p->client->get_pubpid(), 10, "0", STR_PAD_LEFT));
        echo ("</td>\n");
        echo ("</tr>\n");
        echo ("<tr>\n");
    ...
    function multiprint_action($id = "")
    {
        ...
        $this->multiprint_footer($pdf);
            ...
            $pFirstName = $p->client->fname; //modified by epsdky for prescription title change to include client name and ID
            $pFName = convert_safe_file_dir_name($pFirstName);
            $modedFileName = "Rx_{$pFName}_{$p->client->id}.pdf";
    ...
    function _email_prescription($p, $email)
    {
        ...
        $mail->Body = $text_body;
        $mail->Subject = "Prescription for: " . $p->client->get_name_display();
