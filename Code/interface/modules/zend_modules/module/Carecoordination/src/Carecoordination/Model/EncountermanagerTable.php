...
class EncountermanagerTable extends AbstractTableGateway
{
    public function getEncounters($data, $getCount = null)
    {
        ...
        $query  =   "SELECT pd.fname, pd.lname, pd.mname, date(fe.date) as date, fe.pid, fe.encounter,
                        ...
                        JOIN client_data AS pd ON pd.pid=fe.pid
    ...
    public function transmitCCD($data = array())
    {
        ...
        foreach ($rec_arr as $recipient) {
            ...
            if ($requested_by=="client") {
                $text_out = "Delivery of the attached clinical document was requested by the client";
            } else {
            ...
            foreach ($arr as $value) {
               ...
               //get client name in Last_First format (used for CCDA filename)
                $sql    = "SELECT pid, id, lname, fname, mname, providerID, DATE_FORMAT(DOB,'%m/%d/%Y') as DOB_TS FROM client_data WHERE pid = ?";
                ...
                if (empty($clientData[0]['lname'])) {
                    ...
                    $att_filename = " " . str_replace(" ", "_", $xml_type . "_" . $clientData[0]['lname']  . "_" . $clientData[0]['fname']) . "." . $extension;
...
//              return("$config_err 8");
    ...
    public function getFileID($pid)
    {
        ...
        $query    = "SELECT cc.id, pd.fname, pd.lname, pd.pid FROM ccda AS cc
		    LEFT JOIN client_data AS pd ON pd.pid=cc.pid
		    WHERE cc.pid = ?
