...
class Document extends ORDataObject
{
    ...
    */
    ...
    // Function added by Rod to change the client associated with a document.
    ...
    //
    function change_client($new_client_id)
    {
        $couch_docid = $this->get_couch_docid();
        $couch_revid = $this->get_couch_revid();

        // Set the new client in CouchDB.
        if ($couch_docid && $couch_revid) {
            ...
            list ($db, $docid, $revid, $client_id, $encounter, $type, $json) = $data;
            $data = array($db, $couch_docid, $couch_revid, $new_client_id, $couchresp->encounter,
            ...
            if (!$resp->_id || !$resp->_rev) {
                    $data = array($db, $couch_docid, $new_client_id, $couchresp->encounter);
        ...
        // Set the new client in mysql.
        $this->set_foreign_id($new_client_id);
        $this->persist();
   ...
   *
   * @param  string  $client_id   Client pid; if not known then this may be a simple directory name
   ...
   */
    function createDocument(
        ...
        if ($this->storagemethod == 1) {
            ...
            $couch = new CouchDB();
            $docname = $_SESSION['authId'] . $filename . $client_id . $encounter_id . date("%Y-%m-%d H:i:s");
            ...
            $db = $GLOBALS['couchdb_dbase'];
            $couchdata = array($db, $docid, $client_id, $encounter_id, $mimetype, $json, $th_json);
            ...
            if (!$resp->id || !$resp->_rev) {
                ...
                $couchdata = array($db, $docid, $client_id, $encounter_id);
            ...
            if ((!empty($higher_level_path)) && (is_numeric($client_id) && $client_id > 0)) {
                // Allow higher level directory structure in documents directory and a client is mapped.
                $filepath = $repository . $higher_level_path . "/";
            } else if (!empty($higher_level_path)) {
                // Allow higher level directory structure in documents directory and there is no client mapping
                ...
                ++$path_depth;
            } else if (!(is_numeric($client_id)) || !($client_id > 0)) {
                // This is the default action except there is no client mapping (when client_id is 00 or direct)
                // (will create up to 10000 random directories and set the path_depth to 2).
                $filepath = $repository . $client_id . '/' . rand(1, 10000)  . '/';
                $path_depth = 2;
                $client_id = 0;
            } else {
                // This is the default action where the client is used as one level directory structure in documents directory.
                $filepath = $repository . $client_id . '/';
                $path_depth = 1;
            }

            if (!file_exists($filepath)) {
                if (!mkdir($filepath, 0700, true)) {
                    return xl('Unable to create client document subdirectory');
                }
            }
  ...
  /**
   * Post a client note that is linked to this document.
   *
   ...
   */
    function postClientNote($provider, $category_id, $message = '')
    {
