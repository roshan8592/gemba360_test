...
class AMC_304i_STG2_Numerator implements AmcFilterIF
{
    ...
    public function test(AmcClient $client, $beginDate, $endDate)
    {
        ...
        $amcElement_elec = amcCollect('send_sum_elec_amc', $client->id, 'transactions', $client->object['id']);
        $amc_elec_check  = sqlQuery('select count(*) as cnt from ccda where pid = ? and emr_transfer = 1', array($client->id));
