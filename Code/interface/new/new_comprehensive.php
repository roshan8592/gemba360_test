...
$WITH_SEARCH = ($GLOBALS['full_new_client_form'] == '1' || $GLOBALS['full_new_client_form'] == '2' );
$SHORT_FORM  = ($GLOBALS['full_new_client_form'] == '2' || $GLOBALS['full_new_client_form'] == '3' || $GLOBALS['full_new_client_form'] == '4');
...
<title><?php echo xlt("Search or Add Client"); ?></title>
...
<?php } ?>
 if (force_submit) {
  force_submit = false;
  f.create.value = <?php echo xlj('Create New Client'); ?>;
 }
 return true;
}
...
// This invokes the client search dialog.
function searchme() {
 var f = document.forms[0];
 var url = '../main/finder/client_select.php?popup=1&csrf_token_form=<?php echo attr_url(CsrfUtils::collectCsrfToken()); ?>';
...
function srchDone(pid){
    top.restoreSession();
    document.location.href = "./../../client_file/summary/demographics.php?set_pid=" + encodeURIComponent(pid);
}

$(document).ready(function(){
    var file_element = "<input type='file' name='file[]' class='form-control' style='margin-bottom: 8px;' id='form_profile_pic'><input type='hidden' name='MAX_FILE_SIZE' value='64000000'><input type='hidden' name='category_id' value='10'><input type='hidden' name='process' value='true'><input type='hidden' name='destination' title='Leave Blank To Keep Original Filename' id='destination-name'><input type='hidden' name='encrypted' title='Check the box if this is an encrypted file' id='encrypted'><input type='hidden' name='passphrase' title='Pass phrase to decrypt document' id='passphrase'>";
    $("#form_profile_pic").parent().html(file_element);
        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="page-header">
                    <h2><?php echo xlt('Search or Add Client');?></h2>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-xs-12">
                <div class="btn-group">
                    ...
                    <?php } ?>
                    <button type="button" class="btn btn-default btn-save" name='create' id="create" value="<?php echo xla('Create New Client'); ?>">
                        <?php echo xlt('Create New Client'); ?>
                    </button>
                </div>
            </div>
        </div>
                        ...
                        <table border="0">
                            ...
                            <tr>
                                ...
                                <td valign=top>
                                    ...
                                    ?>
                                    <a href="javascript:popUp('../../interface/client_file/summary/browse.php?browsenum=<?php echo attr_url($i); ?>')" class=text>(<?php echo xlt('Browse'); ?>)</a><br />
...
// var matches = 0; // number of clients that match the demographic information being entered
...
$(function () {
    ...
    var check = function(e) {
        ...
        ?>
        <?php if (($GLOBALS['full_new_client_form'] == '4') && (checkIfClientValidationHookIsActive())) :?>
            // Use zend module client validation hook to open the controller and send the dup-checker fields.
...
// callback for new client save confirm from new_search_popup.php
