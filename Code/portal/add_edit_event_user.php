...
if ($_POST['form_action'] != "") {
  // Leave
    ...
    $note = $type . " " . xl("request was received from portal client") . " ";
    ...
    $note .=  ". " . xl("Use Portal Dashboard to confirm with client.");
    $title = xl("Client Reminders");
...
if ($clientid) {
    ...
    if ($prow['phone_home']) {
        $clienttitle .= " H=" . $prow['phone_home'];
    }

    if ($prow['phone_biz']) {
        $clienttitle  .= " W=" . $prow['phone_biz'];
    }
}
...
<table border='0' width='100%'>
 ...
 <tr>
  ...
  <td style='padding:0px 5px 5px 0' nowrap>
   <input class="form-control" type='text' id='form_client' name='form_client' value='<?php echo attr($clientname); ?>' title='Client' readonly />
   <input type='hidden' name='form_pid' value='<?php echo attr($clientid); ?>' />
  </td>
  <td nowrap>
   &nbsp;
  </td>
...
?>
 ...
 function sel_client() {
  dlgopen('find_client_popup.php', '_blank', 500, 400);
 }
 ...
 function categoryChanged() {
    ...
    document.getElementById("form_client").disabled=false;
 ...
 function find_available() {
     ...
     var url = 'find_appt_popup_user.php?byclient&providerid=' + encodeURIComponent(s) + '&catid=5' + '&startdate=' + encodeURIComponent(formDate.value);
     var params = {
         buttons: [
...
//  in lunch outofoffice reserved vacation
 // f.form_category.value='2';
  if (f.form_client.value=='Click to select' && (!(
