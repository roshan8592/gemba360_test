...
// D = Check for duplicates in New Client form
...
function generate_form_field($frow, $currvalue)
{
  ...
  // These data types support backup lists.
    if ($data_type == 1 || $data_type == 33) {
        ...
        $tmp = htmlspecialchars($GLOBALS['gbl_mask_client_id'], ENT_QUOTES);
        ...
        echo "</table>";
    } elseif ($data_type == 24) { // the list of active allergies for the current client
        // this is read-only!
...
// Currently used by interface/client_file/download_template.php and interface/main/finder/dynamic_finder_ajax.php.
...
function display_layout_rows($formtype, $result1, $result2 = '')
{
    ...
    while ($frow = sqlFetchArray($fres)) {
        ...
        // filter out all the empty field data from the client report.
...
function expand_collapse_widget($title, $label, $buttonLabel, $buttonLink, $buttonClass, $linkMethod, $bodyClass, $auth, $fixedWidth, $forceExpandAlways = false)
{
    ...
    if ($auth) {
        ...
        if ($linkMethod == "javascript") {
            ...
            if (!isset($_SESSION['client_portal_onsite_two'])) {
                // prevent an error from occuring when calling the function from the client portal
    ...
    if (isset($_SESSION['client_portal_onsite_two'])) {
        // collapse all entries in the client portal
        $text = xl('expand');
    ...
    if ($forceExpandAlways) {
        // Special case to force the widget to always be expanded
        $styling = "";
    } elseif (isset($_SESSION['client_portal_onsite_two'])) {
        // collapse all entries in the client portal
...
//function to get the translated title value in Client Transactions
...
// Depending on options this might come from lbf_data, client_data,
...
function lbf_current_value($frow, $formid, $encounter)
{
    ...
    if ($source == 'D' || $source == 'H') {
        // Get from client_data, employer_data or history_data.
        if ($source == 'H') {
            $table = 'history_data';
