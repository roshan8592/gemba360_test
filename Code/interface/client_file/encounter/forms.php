...
if (!empty($_GET['attachid'])) {
    ...
    foreach ($attachid as $aid) {
        ...
        $tmp = sqlQuery(
            ...
            "procedure_order_id = ? AND client_id = ? AND encounter_id = 0 AND activity = 1",
            array($aid, $pid)
        );
        if (!empty($tmp['count'])) {
              sqlStatement(
...
jQuery(document).ready( function($) {
    ...
    $("#prov_edu_res").click(function() {
        ...
        $.post( "../../../library/ajax/amc_misc_data.php",
            { amc_id: "client_edu_amc",
              complete: true,
              mode: mode,
              client_id: <?php echo js_escape($pid); ?>,
...
if (!isset($_GET['attachid'])) {
    $ares = sqlStatement(
        ...
        "client_id = ? AND encounter_id = 0 AND activity = 1 " .
    ...
    while ($arow = sqlFetchArray($ares)) {
        ...
        xlj('for this client not yet assigned to any encounter.') . " + ' ' + " .
...
function imdeleted(EncounterId) {
    ...
    if (top.tab_mode) {
        ...
        top.window.parent.left_nav.loadFrame('ens1', window.parent.name, 'client_file/history/encounters.php');
    }
}

// Called to open the data entry form a specified encounter form instance.
function openEncounterForm(formdir, formname, formid) {
  var url = <?php echo js_escape($rootdir); ?> + '/client_file/encounter/view_form.php?formname=' +
      encodeURIComponent(formdir) + '&id=' + encodeURIComponent(formid);
  if (formdir == 'newclient' || !parent.twAddFrameTab) {
    top.restoreSession();
    location.href = url;
  }
  else {
...
function refreshVisitDisplay() {
  location.href = <?php echo js_escape($rootdir); ?> + '/client_file/encounter/forms.php';
}
        ...
        foreach ($reg as $entry) {
          // Check permission to create forms of this type.
            ...
            $StringEcho .= "<tr><td style='border-top: 1px solid #000000;padding:0px;'><a onclick=\"openNewForm(" .
                attr_js($rootdir."/client_file/encounter/load_form.php?formname=".urlencode($entry['directory'])) .
...
if ($encounterLocked === false) {
    ...
    if (sqlNumRows($lres)) {
        ...
        while ($lrow = sqlFetchArray($lres)) {
            ...
            $StringEcho .= "<tr><td style='border-top: 1px solid #000000;padding:0px;'><a onclick=\"openNewForm(" .
                attr_js($rootdir."/client_file/encounter/load_form.php?formname=".urlencode($option_id)) .
...
if ($attendant_type == 'pid' && is_numeric($pid)) {
    ...
    // Check for no access to the client's squad.
    $result = getClientData($pid, "fname,lname,squad");
    ...
    <div style='float:right;margin-right:25px;border-style:solid;border-width:1px;'>
        <div style='float:left;margin:5px 5px 5px 5px;'>
            ...
            <?php // Display the education resource checkbox (AMC prompting)
                $itemAMC = amcCollect("client_edu_amc", $pid, 'form_encounter', $encounter);
            ?>
...
<span class="bold"><?php echo xlt("Document(s)"); ?>:</span>
    ...
    foreach ($docs_list as $doc_iter) {
        $doc_url = $doc->_tpl_vars['CURRENT_ACTION']. "&view&client_id=" . attr_url($pid) . "&document_id=" . attr_url($doc_iter['id']) . "&";
...
if ($pass_sens_squad &&
    ($result = getFormByEncounter(
        ...
        "",
        "FIND_IN_SET(formdir,'newclient') DESC, form_name, date DESC"
    ))) {
    ...
    foreach ($result as $iter) {
        ...
        $form_name = ($formdir == 'newclient') ? xl('Visit Summary') : xl_form_title($iter['form_name']);
