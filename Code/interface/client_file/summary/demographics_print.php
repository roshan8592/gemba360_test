<?php
/**
 * This will print a blank form, and if "clientid" is specified then
 * any existing data for the specified client is included.
 *
 * @package   OpenEMR
...
$clientid = empty($_REQUEST['clientid']) ? 0 : 0 + $_REQUEST['clientid'];
if ($clientid < 0) {
    $clientid = 0 + $pid; // -1 means current pid
}
...
// if we are doing a client-specific complete form.
// TODO - now use mPDF, so should test if still need this fix
$PDF_OUTPUT = ($clientid && $isform) ? false : true;
...
if ($clientid) {
    $prow = getClientData($pid, "*, DATE_FORMAT(DOB,'%Y-%m-%d') as DOB_YMD");
