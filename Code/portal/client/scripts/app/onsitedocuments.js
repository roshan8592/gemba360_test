...
var page = {
    ...
    isSaved: true,
    fetchParams: { filter: '', orderBy: '', orderDesc: '', page: 1,clientId: cpid,recid: recid },
...
// template rendered ready -------------------------------------------------------//
        this.modelView.on('rendered',function () {
            ...
            if ( cuser != '-client-') {
                ...
                {
                    // not used
                  h = webRoot+'/controller.php?document&view&client_id=30&doc_id=' + docid;
                  location.href = h;
                }
                $("#signTemplate").hide();
                $("#printTemplate").hide();
            ...
            if ( cuser != '-client-') {
                // disable admin signature in client view
                $('#clientSignature').prop("onclick", null);
            } else {
    ...
    updateModel: function () {
        ...
        var isLink = $('#clientSignature').attr('src') ? $('#clientSignature').attr('src').indexOf('signhere') : -1
        if (isLink != -1 ) {
            $('#clientSignature').attr('src',signhere);
        }
        var ptsignature = $('#clientSignature').attr('src');
        ...
        page.onsiteDocument.save({
            ...
            'clientSignedStatus': ptsignature?'1':'0',
            'clientSignedTime': ptsignature? new Date() : '0000-00-00', //page.onsiteDocument.get('clientSignedTime'),
            ...
            success: function () {
                ...
                pageAudit.onsitePortalActivity.set('clientId',cpid)
    ...
    deleteModel: function () {
        ...
        page.onsiteDocument.destroy({
            ...
            success: function () {
                ...
                pageAudit.onsitePortalActivity.set('tableArgs',page.onsiteDocument.get('id'))
                pageAudit.onsitePortalActivity.set('narrative','Client deleted un-charted template')
                pageAudit.updateModel()
