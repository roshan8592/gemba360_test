...
<div class='tabContainer' >
  <div class='tab current' >
    ...
    <?php
     ...
     $has_note = 0;
     $thisauth = acl_check('clients', 'notes');
    if ($thisauth) {
    ...
    if (!$thisauth) {
        ...
        $pres = getClientData($pid, "lname, fname");
        ...
        if ($has_note < 1) { ?>
            <span class='text'>
            <?php
                echo xlt("There are no messages on file for this client.");
        ...
        <div class='tab'>
            ...
            if (!$thisauth) {
                        ...
                        foreach ($result_sent as $iter) {
                            ...
                            if (preg_match('/^\d\d\d\d-\d\d-\d\d \d\d\:\d\d /', $body)) {
                                ...
                                $body = text(oeFormatSDFT(strtotime($iter['date'])) . date(' H:i', strtotime($iter['date'])) .
                                        ' (' . $iter['user'] . ') ') .
                                    nl2br(text(oeFormatClientNote($body)));
                            }

                            $body = preg_replace('/(:\d{2}\s\()'.$iter['pid'].'(\sto\s)/', '${1}'.$clientname.'${2}', $body);
                ...
                if ($has_sent_note < 1) { ?>
                    <span class='text'>
                    <?php
                    echo xlt("There are no notes on file for this client.");
                    if (acl_check('clients', 'notes', '', array('write', 'addonly'))) {
                        echo " ";
                        echo "<a href='pnotes_full.php' onclick='top.restoreSession()'>";
