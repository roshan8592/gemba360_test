 ...
 * sorting by client, practitioner, appointment type, and date.
 *
 * @package   OpenEMR
...
# This session will hold array of clients that are listed in this
            ...
            <tr>
                ...
                <td>
                    <div class="checkbox">
                        ...
                        <?php echo ($incl_reminders ? ' checked':''); # This will include the reminder for the clients on the report ?>>
                    ...
                    <div class="text-center">
                        <div class="btn-group" role="group">
                            ...
                            <?php if ($_POST['form_refresh'] || $_POST['form_orderby']) { ?>
                                ...
                                <a href='#' class='btn btn-default btn-transmit' onclick='window.open("../client_file/printed_fee_sheet.php?fill=2", "_blank").opener = null' onsubmit='return top.restoreSession()'>
                                    <?php echo xlt('Superbills'); ?>
                                </a>
                                <a href='#' class='btn btn-default btn-transmit' onclick='window.open("../client_file/addr_appt_label.php", "_blank").opener = null' onsubmit='return top.restoreSession()'>
...
<table>
    ...
    $appointments = fetchAppointments($from_date, $to_date, $client, $provider, $facility, $form_apptstatus, $with_out_provider, $with_out_facility, $form_apptcat);
    ...
    foreach ($appointments as $appointment) {
        ...
        <tr valign='top' id='p1.<?php echo attr($client_id) ?>' bgcolor='<?php echo attr($bgcolor); ?>'>
        ...
        <?php
        if ($client_id && $incl_reminders) {
            // collect reminders first, so can skip it if empty
            $rems = fetch_reminders($client_id, $appointment['pc_eventDate']);
        }
        ?>
        <?php
        if ($client_id && (!empty($rems) || !empty($appointment['pc_hometext']))) { // Not display of available slot or not showing reminders and comments empty ?>
    <tr valign='top' id='p2.<?php echo attr($client_id) ?>' >
