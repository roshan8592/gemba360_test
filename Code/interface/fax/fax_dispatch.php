...
if ($_POST['form_save']) {
    ...
    if ($_POST['form_cb_copy']) {
        $client_id = (int) $_POST['form_pid'];
        if (!$client_id) {
            die(xlt('Internal error - client ID was not provided!'));
        }

        // Compute the name of the target directory and make sure it exists.
        $docdir = $GLOBALS['OE_SITE_DIR'] . "/documents/" . check_file_dir_name($client_id);
        exec("mkdir -p " . escapeshellarg($docdir));

        // If copying to client documents...
        //
        if ($_POST['form_cb_copy_type'] == 1) {
            ...
            if ($tmp2) {
                ...
                "'application/pdf', ?, ? " .
                ")";
                sqlStatement($query, array($newid, $fsize, 'file://'.$target, $client_id, $docdate));
            ...
            if ($_POST['form_cb_note'] && !$info_msg) {
                ...
                // Link the new client note to the document.
                setGpRelation(1, $newid, 6, $noteid);
            } // end post client note
            ...
            if (empty($_POST['form_copy_sn_visit'])) {
                $info_msg .= "This client has no visits! ";
            } else {
            ...
            if (!$info_msg) {
                ...
                //
                $imagedir = $GLOBALS['OE_SITE_DIR'] . "/documents/" . check_file_dir_name($client_id) . "/encounters";
            ...
            // If we are posting a client note...
            if ($_POST['form_cb_note'] && !$info_msg) {
...
<?php if ($using_scanned_notes) { ?>
  // This loads the client's list of recent encounters:
...
<?php } ?>
 ...
 function sel_client() {
  dlgopen('../main/calendar/find_client_popup.php', '_blank', 750, 550, false, 'Select Client');
 }
...
<b><?php echo xlt('Copy Pages to Client Chart'); ?></b></p>

<div id='div_copy' class='section' style='display:none;'>
 <table>
  <tr>
   <td class='itemtitle' width='1%' nowrap><?php echo xlt('Client'); ?></td>
   <td>
    <input type='text' size='10' name='form_client' style='width:100%'
     value=' (<?php echo xla('Click to select'); ?>)' onclick='sel_client()'
  ...
  <tr>
   <td colspan='2' style='padding-top:0.5em;'>
    ...
    <b><?php echo xlt('Client Document'); ?></b>&nbsp;
<?php if ($using_scanned_notes) { ?>
...
?>
      ...
      <tr>
       ...
       <td>
        ...
        title='Name for this document in the client chart' />
       </td>
      </tr>
      <tr>
  ...
  <tr>
   <td colspan='2' style='padding-top:0.5em;'>
    ...
    <b><?php echo xlt('Create Client Note'); ?></b>
