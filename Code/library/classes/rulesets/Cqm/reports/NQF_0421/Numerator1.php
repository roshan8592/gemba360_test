    ...
    public function test(CqmClient $client, $beginDate, $endDate)
    {
        ...
        do {
            ...
            $query = "SELECT form_vitals.BMI " .
                     ...
                     "AND ( enc_category_map.rule_enc_id = 'enc_outclient' )";
            $res = sqlStatement($query, array( $client->id ));
            ...
            $number = sqlNumRows($res);
            if ($number >= 1 &&
                ( Helper::check(ClinicalType::CARE_GOAL, CareGoal::FOLLOW_UP_PLAN_BMI_MGMT, $client) ||
                  Helper::check(ClinicalType::COMMUNICATION, Communication::DIET_CNSLT, $client) ) ) {
                $return = true;
                break;
            }
