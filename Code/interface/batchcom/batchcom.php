<?php
/**
 * Batch Communication Tool for selecting/communicating with subsets of clients
 *
 * @package OpenEMR
...
$sort_by_choices = array(xl('Zip Code')=>'client_data.postal_code', xl('Last Name')=>'client_data.lname', xl('Appointment Date')=>'last_appt');
...
if ($_POST['form_action']=='process') {
    ...
    //process sql
    if (!$form_err) {
        $sql = "select client_data.*, cal_events.pc_eventDate as next_appt, cal_events.pc_startTime
                    as appt_start_time,cal_date.last_appt,forms.last_visit from client_data
                    left outer join openemr_postcalendar_events as cal_events on client_data.pid=cal_events.pc_pid
                    ...
                    as cal_date on cal_date.pc_pid=client_data.pid left outer join (select pid,max(date)
                    as last_visit from forms where curdate() >= date group by pid)
                    as forms on forms.pid=client_data.pid where 1=1";
        $params = array();
        ...
        if ($_POST['age_from']!=0 and $_POST['age_from']!='') {
            $sql .= " and DATEDIFF( CURDATE( ), client_data.DOB )/ 365.25 >= ?";
