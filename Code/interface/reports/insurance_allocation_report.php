<?php
/**
 * This module shows relative insurance usage by unique clients
 * that are seen within a given time period.  Each client that had
...
<title><?php echo xlt('Client Insurance Distribution'); ?></title>
...
<span class='title'><?php echo xlt('Report'); ?> - <?php echo xlt('Client Insurance Distribution'); ?></span>
...
<table>
 ...
 <thead>
  ...
  <th align='right'> <?php echo xlt('Clients'); ?> </th>
...
if ($_POST['form_refresh'] || $_POST['form_csvexport']) {
    ...
    while ($row = sqlFetchArray($res)) {
        ...
        "ORDER BY insurance_data.date DESC LIMIT 1", array($client_id, $encounter_date));
        $plan = $irow['name'] ? $irow['name'] : '-- No Insurance --';
        $insarr[$plan]['visits'] += 1;
        $insarr[$plan]['charges'] += sprintf('%0.2f', $row['charges']);
        if ($client_id != $prev_pid) {
              ++$patcount;
              $insarr[$plan]['clients'] += 1;
              $prev_pid = $client_id;
        }
    }
    ...
    foreach ($insarr as $key => $val) {
        if ($_POST['form_csvexport']) {
            ...
            echo '"' . $val['visits']                                      . '",';
            echo '"' . $val['clients']                                    . '",';
            echo '"' . sprintf("%.1f", $val['clients'] * 100 / $patcount) . '"' . "\n";
        } else {
            ?>
     <tr>
      <td>
  ...
  <td align='right'>
            <?php echo text($val['clients']); ?>
  </td>
  <td align='right'>
            <?php printf("%.1f", $val['clients'] * 100 / $patcount) ?>
  </td>
 </tr>
            <?php
        } // end not export
