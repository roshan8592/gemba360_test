...
$uspfx = 'client_finder.'; //substr(__FILE__, strlen($webserver_root)) . '.';
$client_finder_exact_search = prevSetting($uspfx, 'client_finder_exact_search', 'client_finder_exact_search', ' ');
...
<script language="JavaScript">
    ...
    $(function () {
        ...
        ?> /><?php echo xlt('Open in New Window'); ?></label><label for='setting_search_type' id='setting_search_type_label'><input type='checkbox' name='setting_search_type'  id='setting_search_type' onchange='persistCriteria(this, event)' value='<?php echo attr($client_finder_exact_search); ?>'<?php echo text($client_finder_exact_search); ?>/><?php echo xlt('Search with exact method'); ?></label></form>");
        ...
        $('#pt_table').on('click', 'tbody tr', function () {
            ...
            if (document.myform.form_new_window.checked) {
                ...
                top.restoreSession();
                top.RTop.location = "../../client_file/summary/demographics.php?set_pid=" + encodeURIComponent(newpid);
            }
        });
    });

    function openNewTopWindow(pid) {
        document.fnew.clientID.value = pid;
        top.restoreSession();
    ...
    function persistCriteria(el, e) {
        e.preventDefault();
        let target = uspfx + "client_finder_exact_search";
...
<?php
    ...
    'heading_title' => xl('Client Finder'),
...
<body class="body_top">
    <div id="container_div" class="<?php echo attr($oemr_ui->oeContainer()); ?>">
        ...
        <div class="row">
            <div class="col-sm-12">
                <?php if (acl_check('clients', 'demo', '', array('write','addonly'))) { ?>
                    <button id="create_client_btn1" class="btn btn-default btn-add" onclick="top.restoreSession();top.RTop.location = '<?php echo $web_root ?>/interface/new/new.php'"><?php echo xlt('Add New Client'); ?></button>
                <?php } ?>
            </div>
            </div>
        <br>
