<?php
/**
 * This is the Indigent Clients Report.  It displays a summary of
 * encounters within the specified time period for clients without
 * insurance.
 *
 * @package   OpenEMR
...
<title><?php echo xlt('Indigent Clients Report')?></title>
...
<span class='title'><?php echo xlt('Report'); ?> - <?php echo xlt('Indigent Clients'); ?></span>

<form method='post' action='indigent_clients_report.php' id='theform' onsubmit='return top.restoreSession()'>
...
if ($_POST['form_refresh']) {
    ...
    "FROM form_encounter AS e, client_data AS p, insurance_data AS i " .
    ...
    for ($irow = 0; $row = sqlFetchArray($rez); ++$irow) {
        ...
        $arow = sqlQuery("SELECT SUM(fee) AS amount FROM billing WHERE " .
          ...
          "activity = 1 AND code_type != 'COPAY'", array($client_id, $encounter_id));
        $inv_amount += $arow['amount'];
        $arow = sqlQuery("SELECT SUM(fee) AS amount FROM billing WHERE " .
          "pid = ? AND encounter = ? AND " .
          "activity = 1 AND code_type = 'COPAY'", array($client_id, $encounter_id));
