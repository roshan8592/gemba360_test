...
<html>
    <head>
        ...
        <title>
            <?php echo xlt('Client List Creation'); ?>
        </title>
    ...
    <body class="body_top">
        ...
        <span class='title'>
        <?php echo xlt('Report - Client List Creation');?>
        </span>
        ...
        <form name='theform' id='theform' method='post' action='client_list_creation.php' onSubmit="return Form_Validate();">
                ...
                <table>
                    ...
                    <td width='640px'>
                        ...
                        <table class='text'>
                            ...
                            <tr>
                                <td class='control-label'><?php echo xlt('Client ID'); ?>:</td>
                                <td><input name='client_id' class="numeric_only form-control" type='text' id="client_id" title='<?php echo xla('Optional numeric client ID'); ?>' value='<?php echo attr($client_id); ?>' size='10' maxlength='20' /></td>
        ...
        if ($_POST['form_refresh']) {
            $sqlstmt = "select
						pd.date as client_date,
						concat(pd.lname, ', ', pd.fname) AS client_name,
						pd.pid AS client_id,
						DATE_FORMAT(FROM_DAYS(DATEDIFF('".date('Y-m-d H:i:s')."',pd.dob)), '%Y')+0 AS client_age,
						pd.sex AS client_sex,
						pd.race AS client_race,pd.ethnicity AS client_ethinic,
            ...
            //from
            $sqlstmt=$sqlstmt." from client_data as pd left outer join users as u on u.id = pd.providerid";
            //JOINS
            switch ($srch_option) {
                case "Problems":
                ...
                case "Allergies":
                    ...
                    break;
                case "Lab results":
                    $sqlstmt = $sqlstmt." left outer join procedure_order as po on po.client_id = pd.pid
            ...
            switch ($srch_option) {
                ...
                case "Communication":
                    ...
                    $sort = array("client_date","client_name","client_id","client_age","client_sex","users_provider", "communications");
                    if ($sortby == "") {
                        $sortby = $sort[6];
                    }
                    ...
                    break;
                case "Demographics":
                    $sort = array("client_date","client_name","client_id","client_age","client_sex","client_race","client_ethinic","users_provider");
                    break;
            }
            ...
            switch ($srch_option) {
                ...
                case "Communication":
                    ...
                    break;
                case "Demographics":
                    $odrstmt = " ORDER BY client_date asc";
                    //$odrstmt = " ROUND((LENGTH(communications) - LENGTH(REPLACE(communications, ',', '')))/LENGTH(',')) , communications";
                    break;
            }
            ...
            if (sqlNumRows($result) > 0) {
                ...
                while ($row = sqlFetchArray($result)) {
                        $patArr[] = $row['client_id'];
                        $patInfoArr = array();
                        $patInfoArr['client_id'] = $row['client_id'];
                        //Diagnosis Check
                    if ($srch_option == "Medications" || $srch_option == "Allergies" || $srch_option == "Problems") {
                        ...
                        $patInfoArr['client_name'] = $row['client_name'];
                        $patInfoArr['client_age'] = $row['client_age'];
                        $patInfoArr['client_sex'] = $row['client_sex'];
                        $patInfoArr['client_race'] = $row['client_race'];
                        $patInfoArr['client_ethinic'] = $row['client_ethinic'];
                        ...
                        $patInfoArr['client_date'] = $row['client_date'];
                        $patInfoArr['client_name'] = $row['client_name'];
                        $patInfoArr['client_age'] = $row['client_age'];
                        $patInfoArr['client_sex'] = $row['client_sex'];
                        $patInfoArr['users_provider'] = $row['users_provider'];
                ...
                <div id = "report_results">
                    <table>
                        <tr>
                            <td class="text"><strong><?php echo xlt('Total Number of Clients')?>:</strong>&nbsp;<span id="total_clients"><?php echo text(count(array_unique($patArr))); ?></span></td>
                        </tr>
                    </table>
                    ...
                    if ($srch_option == "Medications" || $srch_option == "Allergies" || $srch_option == "Problems") { ?>
                        <tr style="font-size:15px;">
                            ...
                            <td width="15%"><b><?php echo xlt('Client Name'); ?></b></td>
                        ...
                        <?php foreach ($patFinalDataArr as $patKey => $patDetailVal) { ?>
                                <tr bgcolor = "#CCCCCC" style="font-size:15px;">
                                    ...
                                    <td ><?php echo text($patDetailVal['client_name']); ?></td>
                                    <td ><?php echo text($patDetailVal['client_id']); ?></td>
                                    <td ><?php echo text($patDetailVal['client_age']);?></td>
                                    <td ><?php echo text($patDetailVal['client_sex']);?></td>
                        ...
                        foreach ($patFinalDataArr as $patKey => $labResInsideArr) {?>
                                <tr bgcolor = "#CCCCCC" >
                                                    ...
                                                    <td colspan="3"> <?php echo text($labResInsideArr['client_id']); ?>&nbsp;</td>
                               </tr>
                                        <?php
                        }
                    } elseif ($srch_option == "Communication") { ?>
                        <tr style="font-size:15px;">
                            ...
                            <td width="20%"><b><?php echo xlt('Client Name'); ?></b><?php echo $sortlink[1]; ?></td>
                        ...
                        <?php foreach ($patFinalDataArr as $patKey => $patDetailVal) { ?>
                                <tr bgcolor = "#CCCCCC" >
                                    <td ><?php echo ($patDetailVal['client_date'] != '') ? text(oeFormatDateTime($patDetailVal['client_date'], "global", true)) : ""; ?></td>
                                    <td ><?php echo text($patDetailVal['client_name']); ?></td>
                                    <td ><?php echo text($patDetailVal['client_id']); ?></td>
                                    <td ><?php echo text($patDetailVal['client_age']);?></td>
                                    <td ><?php echo text($patDetailVal['client_sex']);?></td>
                        ...
                        <tr style="font-size:15px;">
                            ...
                            <?php foreach ($patFinalDataArr as $patKey => $patDetailVal) { ?>
                                <tr bgcolor = "#CCCCCC" style="font-size:15px;">
                                    ...
                                    <td ><?php echo text($patDetailVal['client_age']);?></td>
                                    <td ><?php echo text($patDetailVal['client_sex']);?></td>
                                    <td ><?php echo generate_display_field(array('data_type'=>'36','list_id'=>'race'), $patDetailVal['client_race']); ?></td>
