...
if ($_REQUEST['recipient'] === 'client' && $_REQUEST['site'] && $controllerName) {
    session_id($_REQUEST['me']);
    session_start();
    $ignoreAuth_onsite_portal_two = false; // eval'ed in globals but why not...
    if ($_SESSION['pid'] && $_SESSION['sessionUser']=='-client-' && $_SESSION['portal_init']) {
        // Onsite portal was validated and client authorized and re-validated via forwarded session.
