...
function mainQrdaCatOneGenerate($xml, $client_id, $rule_id, $provider_id)
{
    //Open Main Clinical Document
    $xml->open_clinicaldocument();

    //Header Function
    getHeaderQRDA1($xml, $client_id, $provider_id);

    //Component Function
    getComponentQRDA1($xml, $client_id, $rule_id);

    //Close Main Clinical Document
    $xml->close_clinicaldocument();

    //Downloaded XML
    $xmlDynFileName = downloadQRDACat1($xml, $client_id, $rule_id);

    return $xmlDynFileName;
}

    //Main Header Function
function getHeaderQRDA1($xml, $client_id, $provider_id)
{
    global $mainQrdaRaceCodeArr, $mainEthiCodeArr, $from_date, $to_date;

    //Client Info
    if ($client_id != "") {
        $clientRow = getPatData($client_id);
    }
    ...
    //clientRole Open
    $xml->open_customTag('clientRole');

    $xml->self_customTag('id', array('root' => '2.16.840.1.113883.4.572', 'extension' =>'112233'));

    $xml->add_clientAddress($clientRow);

    if ($clientRow['phone_home'] != "") {
        $xml->self_customTag('telecom', array('value' => $clientRow['phone_home'], 'use'=>'HP'));
    } else {
        $xml->self_customTag('telecom', array('nullFlavor' => "UNK"));
    }

    $xml->open_customTag('client');

    $patNameArr = array('fname' =>$clientRow['fname'], 'lname'=>$clientRow['lname']);
    $xml->add_patName($patNameArr);

    if ($clientRow['sex'] == "Male") {
        $gender = "M";
    } else if ($clientRow['sex'] == "Female") {
        $gender = "F";
    }

    $xml->self_customTag('administrativeGenderCode', array('codeSystem' => '2.16.840.1.113883.18.2', 'code' =>$gender));

    $xml->self_customTag('birthTime', array('value' =>date('Ymd', strtotime($clientRow['DOB']))));

    if ($mainQrdaRaceCodeArr[$clientRow['race']] == "") {
        $mainQrdaRaceCodeArr[$clientRow['race']] = "2131-1";
    }

    $xml->self_customTag('raceCode', array('codeSystem' =>'2.16.840.1.113883.6.238', 'code' => $mainQrdaRaceCodeArr[$clientRow['race']]));
    $xml->self_customTag('ethnicGroupCode', array('codeSystem' =>'2.16.840.1.113883.6.238', 'code' => $mainEthiCodeArr[$clientRow['ethnicity']]));
    ...
    $xml->add_clientAddress($facilResRow);
    ...
    //Component Function
function getComponentQRDA1($xml, $client_id, $rule_id)
{
    //Component Open
    ...
    //Client Data
    getQRDACat1ClientData($xml, $client_id);
    ...
    //Client Data
function getQRDACat1ClientData($xml, $client_id)
{
    ...
    $xml->element('text', "Client Data");

    //Insurance(Payer) Info
    payerQRDA($xml, $client_id);

    //Client History Info
    patCharactersticQRDA($xml, $client_id);

    //Encounter Section
    getAllClientEncounters($xml, $client_id);

    //Physical Exams(vitals)
    getAllPhysicalExams($xml, $client_id);

    //Diagnosis (Medical Problems)
    getAllMedicalProbs($xml, $client_id);

    //Ordered Medications
    getAllOrderMedications($xml, $client_id);

    // Active Medications
    getAllActiveMedications($xml, $client_id);

    //Immunization
    getAllImmunization($xml, $client_id);

    //Procedures
    getAllProcedures($xml, $client_id);

    //Lab Tests
    getAllLabTests($xml, $client_id);

    //Interventions
    getAllInterventionProcedures($xml, $client_id);

    //Risk Category Assessment
    getAllRiskCatAssessment($xml, $client_id);
    ...
    //Immunization
function getAllImmunization($xml, $client_id)
{
    global $from_date, $to_date;
    $medArr = allImmuPat($client_id, $from_date, $to_date);
...
function getAllPhysicalExams($xml, $client_id)
{
    global $encCheckUniqId, $from_date, $to_date;

    $vitArr   = allVitalsPat($client_id, $from_date, $to_date);
...
function getAllRiskCatAssessment($xml, $client_id)
{
    global $encCheckUniqId, $from_date, $to_date;
    $procArr = allProcPat("risk_category", $client_id, $from_date, $to_date);
...
function getAllProcedures($xml, $client_id)
{
    global $encCheckUniqId, $from_date, $to_date;
    $procArr = allProcPat("Procedure", $client_id, $from_date, $to_date);
    foreach ($procArr as $procRow) {
        $vset = sqlQuery("select * from valueset where code = ? and nqf_code = ? ", array($procRow['procedure_code'],$xml->nqf_code));
        if (!empty($vset['valueset'])) {
            //Entry open
...
function getAllLabTests($xml, $client_id)
{
    global $encCheckUniqId, $from_date, $to_date;
    $procArr = allProcPat("laboratory_test", $client_id, $from_date, $to_date);
    foreach ($procArr as $procRow) {
        $vset = sqlQuery("select * from valueset where code = ? and nqf_code = ? ", array($procRow['procedure_code'],$xml->nqf_code));
        if (!empty($vset['valueset'])) {
            //Entry open
...
function getAllInterventionProcedures($xml, $client_id)
{
    global $encCheckUniqId, $from_date, $to_date;
    $procArr = allProcPat("intervention", $client_id, $from_date, $to_date);
    foreach ($procArr as $procRow) {
        $vset = sqlQuery("select * from valueset where code = ? and nqf_code = ? ", array($procRow['procedure_code'],$xml->nqf_code));
        if (!empty($vset['valueset'])) {
            //Entry open
...
function getAllOrderMedications($xml, $client_id)
{
    global $from_date, $to_date;
    $medArr = allOrderMedsPat($client_id, $from_date, $to_date);
...
function getAllActiveMedications($xml, $client_id)
{
    global $from_date, $to_date;
    $medArr = allActiveMedsPat($client_id, $from_date, $to_date);
    ...
    //Medical problems
function getAllMedicalProbs($xml, $client_id)
{
    global $from_date, $to_date;
    $diagArr = allListsPat('medical_problem', $client_id, $from_date, $to_date);
    ...
    //Encounters function
function getAllClientEncounters($xml, $client_id)
{
    global $encCheckUniqId, $from_date, $to_date,$EncounterCptCodes;
    $encArr = allEncPat($client_id, $from_date, $to_date);
    ...
    $encArr = allProcPat("enc_checkup_procedure", $client_id, $from_date, $to_date);
    foreach ($encArr as $encRow) {
        $encRow['encounter'];
    ...
    //Client Data Sub Function for Payer Data
function payerQRDA($xml, $client_id)
{
    global $mainQrdaPayerCodeSendArr, $from_date, $to_date;

    //Insurance getting
    $payer = payerClient($client_id);
...
function downloadQRDACat1($xml, $client_id, $rule_id)
{
    //Client Info
    if ($client_id != "") {
        $clientRow = getPatData($client_id);
        $patFname = $clientRow['fname'];
        $patLname = $clientRow['lname'];
    }
    ...
    //Client History Info
function patCharactersticQRDA($xml, $client_id)
{

    //Client History
    $patHist = clientQRDAHistory($client_id);
