...
class NQF_0013_Exclusion implements CqmFilterIF
{
    ...
    public function test(CqmClient $client, $beginDate, $endDate)
    {
        ...
        if (Helper::check(ClinicalType::DIAGNOSIS, Diagnosis::PREGNANCY, $client, $beginDate, $beginDate)  || Helper::check(ClinicalType::DIAGNOSIS, Diagnosis::END_STAGE_RENAL_DISEASE, $client, $beginDate, $beginDate) || Helper::check(ClinicalType::DIAGNOSIS, Diagnosis::CHRONIC_KIDNEY_DISEASE, $client, $beginDate, $beginDate)) {
            return true;
        }
        
        $procedure_code = implode(',', Codes::lookup(Procedure::DIALYSIS_SERVICE, 'SNOMED'));
        //Dialysis procedure exists exclude the client
        ...
        //echo $sql;
        $check = sqlQuery($sql, array($client->id, $beginDate, $endDate));
        if ($check['cnt'] > 0) {
            return true;
        }
