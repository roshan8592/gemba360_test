...
// MAIN Body
//
// Build output to handle multiple pids and and superbill for each client.
...
// This tells us if client/encounter data is to be filled in.
...
// This tracks our position in the $SBCODES array.
    ...
    while (--$pages >= 0) {
        ...
        $html .= '<table style="width: 100%"><tr>' .
            '<td>' . xlt('Client') . ': <span style="font-weight: bold;">' . text($patdata['fname']) . ' ' . text($patdata['mname']) . ' ' . text($patdata['lname']) . '</span></td>' .
...
<td colspan='3' valign='top' class='fshead' style='height:${lheight}pt'>";
            ...
            if ($form_fill && $encounter) {
                $query = "SELECT fe.reason, fe.date, u.fname, u.mname, u.lname, u.username " .
                        ...
                        "WHERE f.pid = ? AND f.encounter = ? AND f.formdir = 'newclient' AND f.deleted = 0 " .
