...
$TEMPLATE_LABELS = array(
  ...
  'label_subhead_client'       => xlt('Client Copy'),
...
if ($transid) {
    ...
    if (empty($_REQUEST['client_id'])) {
        // If no transaction ID or client ID, this will be a totally blank form.
        $client_id = 0;
        $refer_date = '';
    } else {
    ...
    $trow = array('id' => '', 'pid' => $client_id, 'refer_date' => $refer_date);
}

if ($client_id) {
    $patdata = getClientData($client_id);
    $client_age = getClientAge(str_replace('-', '', $patdata['DOB']));
} else {
    $patdata = array('DOB' => '');
    $client_age = '';
}
...
$vrow = sqlQuery("SELECT * FROM form_vitals WHERE " .
  "pid = ? AND date <= ? " .
  "ORDER BY date DESC LIMIT 1", array($client_id, $refer_date." 23:59:59"));
if (empty($vrow)) {
    $vrow = array(
    'bps' => '',
    'bpd' => '',
...
$s = str_replace("{ref_pid}", text($client_id), $s);
$s = str_replace("{pt_age}", text($client_age), $s);
