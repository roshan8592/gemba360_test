...
class Claim
{
    ...
    public $client_data;      // row from client_data table
  ...
  //
    public function __construct($pid, $encounter_id)
    {
        ...
        $resMoneyGot = sqlStatement("SELECT pay_amount as ClientPay,session_id as id,".
        ...
        while ($rowMoneyGot = sqlFetchArray($resMoneyGot)) {
              $ClientPay=$rowMoneyGot['ClientPay']*-1;
              $this->copay -= $ClientPay;
        }
        ...
        *****************************************************************/
        ...
        $sql = "SELECT * FROM client_data WHERE pid = ? ORDER BY id LIMIT 1";
        $this->client_data = sqlQuery($sql, array($this->pid));
        ...
        $this->insurance_numbers['provider_number_type'] != '1C') ?
          $this->client_data['ref_providerID'] : $provider_id;
        $sql = "SELECT * FROM users WHERE id = ?";
  ...
  // Note this will include "client responsibility" adjustments which are
  ...
  //
    public function payerAdjustments($ins, $code = 'Claim')
    {
        ...
        if (!empty($this->invoice[$code])) {
            ...
            // Compute this procedure's client responsibility amount as of this
            ...
            // help us allocate any unknown client responsibility amounts.
            $thispaidanything = 0;
            ...
            // Allocate any unknown client responsibility by guessing if the
  ...
  // Return the amount already paid by the client.
  //
    public function clientPaidAmount()
    {
        ...
        foreach ($this->invoice as $codekey => $codeval) {
            foreach ($codeval['dtl'] as $key => $value) {
                // plv exists to indicate the payer level.
                if ($value['plv'] == 0) { // 0 indicates client
                    $amount += $value['pmt'];
                }
            }
        }
...
/////////
  ...
  # The billing facility and the client must both accept for this to return true.
  ...
  // Returns the HIPAA code of the client-to-subscriber relationship.
  //
  ...
  // Is the client also the subscriber?
  //
 ...
 // "ID"
  ...
  //
    ...
    public function clientLastName()
    {
        return $this->x12Clean(trim($this->client_data['lname']));
    }

    public function clientFirstName()
    {
        return $this->x12Clean(trim($this->client_data['fname']));
    }

    public function clientMiddleName()
    {
        return $this->x12Clean(trim($this->client_data['mname']));
    }

    public function clientStreet()
    {
        return $this->x12Clean(trim($this->client_data['street']));
    }

    public function clientCity()
    {
        return $this->x12Clean(trim($this->client_data['city']));
    }

    public function clientState()
    {
        return $this->x12Clean(trim($this->client_data['state']));
    }

    public function clientZip()
    {
        return $this->x12Clean(trim($this->client_data['postal_code']));
    }

    public function clientPhone()
    {
        $ptphone = $this->client_data['phone_home'];
        if (!$ptphone) {
            $ptphone = $this->client_data['phone_biz'];
        }

        if (!$ptphone) {
            $ptphone = $this->client_data['phone_cell'];
        }
    ...
    {
        return str_replace('-', '', $this->client_data['DOB']);
    }

    public function clientSex()
    {
        return strtoupper(substr($this->client_data['sex'], 0, 1));
    }

  // Client Marital Status: M = Married, S = Single, or something else.
    public function clientStatus()
    {
        return strtoupper(substr($this->client_data['status'], 0, 1));
    }
  ...
  // indicate employed.
    ...
    {
        return strtoupper($this->x12Clean(trim($this->client_data['occupation'])));
    }
