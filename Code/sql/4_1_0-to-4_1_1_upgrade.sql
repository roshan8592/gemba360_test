...
#IfNotIndex immunizations client_id
CREATE INDEX `client_id` ON `immunizations` (`client_id`);
#EndIf

#IfNotIndex procedure_order client_id
CREATE INDEX `client_id` ON `procedure_order` (`client_id`);
#EndIf
...
#IfNotIndex extended_log client_id
CREATE INDEX `client_id` ON `extended_log` (`client_id`);
#EndIf

#IfNotIndex prescriptions client_id
CREATE INDEX `client_id` ON `prescriptions` (`client_id`);
#EndIf
...
#IfMissingColumn client_access_offsite authorize_net_id
ALTER TABLE `client_access_offsite` ADD COLUMN `authorize_net_id` VARCHAR(20) COMMENT 'authorize.net profile id';
#EndIf
...
CREATE TABLE `temp_table_one` (
  ...
  payment_type varchar( 50 ) NOT NULL DEFAULT 'client',
  description text NOT NULL,
  adjustment_code varchar( 50 ) NOT NULL DEFAULT 'client_payment',
...
INSERT INTO `temp_table_one` (`user_id`, `pay_total`, `client_id`, `post_to_date`, `pid`, `encounter`, `post_time`, `post_user`, `pay_amount`, `description`) SELECT `user`, (`fee`*-1), `pid`, `date`, `pid`, `encounter`, `date`, `user`, (`fee`*-1), 'COPAY' FROM `billing` WHERE `code_type`='COPAY' AND `activity`!=0;
...
INSERT INTO `ar_session` (`payer_id`, `user_id`, `pay_total`, `payment_type`, `description`, `client_id`, `payment_method`, `adjustment_code`, `post_to_date`) SELECT `payer_id`, `user_id`, `pay_total`, `payment_type`, `description`, `client_id`, `payment_method`, `adjustment_code`, `post_to_date` FROM `temp_table_one`;
...
#IfMissingColumn client_data ref_providerID
ALTER TABLE `client_data` ADD COLUMN `ref_providerID` int(11) default NULL;
UPDATE `client_data` SET `ref_providerID`=`providerID`;
