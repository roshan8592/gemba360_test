...
$this->assign('title', xlt("Client Portal") . " | " . xlt("Client Documents"));
...
<script type="text/javascript" src="<?php echo $GLOBALS['web_root']; ?>/portal/client/scripts/libs/LAB.min.js"></script>
<script type="text/javascript">
    ...
    $LAB.script("<?php echo $GLOBALS['assets_static_relative']; ?>/underscore/underscore-min.js")
        ...
        .script("<?php echo $GLOBALS['web_root']; ?>/portal/client/scripts/app.js?v=<?php echo $GLOBALS['v_js_includes']; ?>")
        .script("<?php echo $GLOBALS['web_root']; ?>/portal/client/scripts/model.js?v=<?php echo $GLOBALS['v_js_includes']; ?>").wait()
        .script("<?php echo $GLOBALS['web_root']; ?>/portal/client/scripts/view.js?v=<?php echo $GLOBALS['v_js_includes']; ?>").wait()
</script>
</head>
<script type="text/javascript">
    $LAB.script("<?php echo $GLOBALS['web_root']; ?>/portal/client/scripts/app/onsitedocuments.js?v=<?php echo $GLOBALS['v_js_includes']; ?>").wait()
        .script("<?php echo $GLOBALS['web_root']; ?>/portal/client/scripts/app/onsiteportalactivities.js?v=<?php echo $GLOBALS['v_js_includes']; ?>").wait(
        function () {
            $(function () {
                page.init();
                pageAudit.init();
...
<script type="text/template" id="onsiteDocumentModelTemplate">
    <aside class="col-sm-2 col-xs-3" id="sidebar-pills">
        <ul class="nav nav-pills  nav-stacked" id="sidebar">
            <li data-toggle="pill" class="bg-info"><a id="signTemplate" href="#openSignModal"
                data-toggle="modal" data-backdrop="true" data-target="#openSignModal"data-type="client-signature"><span><?php echo xlt('Signature');?></span></a></li>
    ...
    <div class="col-md-8 col-sm-8 col-xs-8 nopadding">
        ...
        <div class="panel panel-primary" id="docpanel">
            <header class="panel-heading" id='docPanelHeader'><?php echo xlt('Client Document');?></header>
    ...
    <div class="container-fluid">
        ...
        <thead>
            <tr class='bg-primary' style='cursor:pointer'>
                ...
                <th id="header_ClientSignedStatus"><?php echo xlt('Client Signed');?><% if (page.orderBy == 'ClientSignedStatus') { %> <i class='icon-arrow-<%= page.orderDesc ? 'up' : 'down' %>' /><% } %></th>
                <th id="header_ClientSignedTime"><?php echo xlt('Client Signed Date');?><% if (page.orderBy == 'ClientSignedTime') { %> <i class='icon-arrow-<%= page.orderDesc ? 'up' : 'down' %>' /><% } %></th>
            </tr>
        </thead>
        <tbody>
        <% items.each(function(item) { %>
            <tr  style='background:white' id="<%= _.escape(item.get('id')) %>">
                ...
                <td><%if (item.get('clientSignedStatus')=='1') { %><%= 'Yes' %><% } else { %>No<% } %></td>
                <td><%if (item.get('clientSignedTime')) { %><%= item.get('clientSignedTime') %><% } else { %>NULL<% } %></td>
            </tr>
        <% }); %>
        </tbody>
        </table>
