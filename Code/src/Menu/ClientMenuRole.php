...
class ClientMenuRole extends MenuRole
{
    ...
    public function getMenu()
    {
        ...
        $clientMenuRole = $this->getMenuRole();

        // Load the selected menu
        if (preg_match("/.json$/", $clientMenuRole)) {
            // load custom menu (includes .json in id)
            $menu_parsed = json_decode(file_get_contents($GLOBALS['OE_SITE_DIR'] . "/documents/custom_menus/client_menus/" . $clientMenuRole));
        } else {
            // load a standardized menu (does not include .json in id)
            $menu_parsed = json_decode(file_get_contents($GLOBALS['fileroot'] . "/interface/main/tabs/menu/menus/client_menus/" . $clientMenuRole . ".json"));
        }
    ...
    /**
     * Build the html select element to list the ClientMenuRole options.
     *
     * @var string $selected Current ClientMenuRole for current users.
     * @return string Html select element to list the ClientMenuRole options.
     */
    public function displayMenuRoleSelector($selected = "")
    {
        $output = "<select name='client_menu_role' id='client_menu_role' class='form-control'>";
        $output .= "<option value='standard' " . (($selected == "standard") ? "selected" : "") . ">" . xlt("Standard") . "</option>";

        $customMenuDir = $GLOBALS['OE_SITE_DIR'] . "/documents/custom_menus/client_menus/";
    ...
    /**
     * Collect the clientMenuRole for logged in user.
     *
     * @return string Identifier for the clientMenuRole
     */
    private function getMenuRole()
    {
        $userService = new UserService();
        $user = $userService->getCurrentlyLoggedInUser();
        $clientMenuRole = $user->getClientMenuRole();
        if (empty($clientMenuRole)) {
            $clientMenuRole = "standard";
        }

        return $clientMenuRole;
    }
    ...
    public function displayHorizNavBarMenu()
    {
        ...
        $li_id = 1;
        $str_top = <<<EOT
        <nav class="navbar navbar-default navbar-color navbar-static-top client-menu">
