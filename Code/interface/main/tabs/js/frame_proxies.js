...
var attendant_type = 'client';

var left_nav = {

};

left_nav.setClient = function(pname, pid, pubpid, frname, str_dob)
{
    if(
        (app_view_model.application_data.client()!==null)
        && (pid===app_view_model.application_data.client().pid()))
    {
        app_view_model.application_data.client().pname(pname);
        app_view_model.application_data.client().pubpid(pubpid);
        app_view_model.application_data.client().str_dob(str_dob);

        return;
    }
    var new_client=new client_data_view_model(pname,pid,pubpid,str_dob);
    app_view_model.application_data.client(new_client);
    app_view_model.application_data.therapy_group(null)
    navigateTab(webroot_url+"/interface/client_file/history/encounters.php","enc", function () {
        tabCloseByName('rev');
    });

    /* close therapy group tabs */
    tabCloseByName('gdg');
    attendant_type = 'client';
    app_view_model.attendant_template_type('client-data-template');
};
...
left_nav.setTherapyGroup = function(group_id, group_name){
    ...
    navigateTab(webroot_url+"/interface/client_file/history/encounters.php","enc");
    tabCloseByName('gng');
    /* close client tab */
    tabCloseByName('pat');
...
left_nav.setClientEncounter = function(EncounterIdArray,EncounterDateArray,CalendarCategoryArray)
{
