...
class SLEOB
{
    ...
    public static function slInvoiceNumber(&$out)
    {
        ...
        if ($acount == 2) {
            ...
            $pres = sqlStatement("SELECT pid FROM client_data WHERE " .
                "lname LIKE ? AND " .
                "fname LIKE ? " .
                "ORDER BY pid DESC", array($out['client_lname'], $out['client_fname']));
    ...
    public static function arPostSession($payer_id, $check_number, $check_date, $pay_total, $post_to_date, $deposit_date, $debug)
    {
        $query = "INSERT INTO ar_session( " .
            "payer_id,user_id,closed,reference,check_date,pay_total,post_to_date,deposit_date,client_id,payment_type,adjustment_code,payment_method " .
    ...
    //
    public static function arPostPayment($client_id, $encounter_id, $session_id, $amount, $code, $payer_type, $memo, $debug, $time = '', $codetype = '')
    {
        $codeonly = $code;
        $modifier = '';
        $tmp = strpos($code, ':');
        ...
        sqlBeginTrans();
        $sequence_no = sqlQuery("SELECT IFNULL(MAX(sequence_no),0) + 1 AS increment FROM ar_activity WHERE pid = ? AND encounter = ?", array($client_id, $encounter_id));
        $query = "INSERT INTO ar_activity ( " .
            "pid, encounter, sequence_no, code_type, code, modifier, payer_type, post_time, post_user, " .
            "session_id, memo, pay_amount " .
            ") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        sqlStatement($query, array($client_id, $encounter_id, $sequence_no['increment'], $codetype, $codeonly, $modifier, $payer_type, $time, $_SESSION['authUserID'], $session_id, $memo, $amount));
        sqlCommitTrans();
        return;
    }
    ...
    //
    public static function arPostCharge($client_id, $encounter_id, $session_id, $amount, $units, $thisdate, $code, $description, $debug, $codetype = '')
    {
        /*****************************************************************
         * // Select an existing billing item as a template.
         * $row= sqlQuery("SELECT * FROM billing WHERE " .
         * "pid = '$client_id' AND encounter = '$encounter_id' AND " .
    ...
    //
    public static function arPostAdjustment($client_id, $encounter_id, $session_id, $amount, $code, $payer_type, $reason, $debug, $time = '', $codetype = '')
    {
        $codeonly = $code;
        $modifier = '';
        $tmp = strpos($code, ':');
        ...
        $query = "INSERT INTO ar_activity ( " .
            ...
            ") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        sqlStatement($query, array($client_id, $encounter_id, $sequence_no['increment'], $codetype, $codeonly, $modifier, $payer_type, $_SESSION['authUserID'], $time, $session_id, $reason, $amount));
        sqlCommitTrans();
        return;
    }

    public static function arGetPayerID($client_id, $date_of_service, $payer_type)
    {
        ...
        $nprow = sqlQuery($query, array($client_id, $value, $date_of_service));
        if (empty($nprow)) {
            return 0;
        }
    ...
    //
    public static function arSetupSecondary($client_id, $encounter_id, $debug, $crossover = 0)
    {
        if ($crossover == 1) {
        ...
        $new_payer_id = self::arGetPayerID($client_id, $date_of_service, $new_payer_type);

        if ($new_payer_id) {
            // Queue up the claim.
            if (!$debug) {
                BillingUtilities::updateClaim(true, $client_id, $encounter_id, $new_payer_id, $new_payer_type, $status, 5, '', 'hcfa', '', $crossover);
            }
        } else {
            // Just reopen the claim.
            if (!$debug) {
                BillingUtilities::updateClaim(true, $client_id, $encounter_id, -1, -1, $status, 0, '', '', '', $crossover);
            }
        }
