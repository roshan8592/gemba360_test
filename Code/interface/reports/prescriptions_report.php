...
$form_client_id = trim($_POST['form_client_id']);
...
<script language="JavaScript">
    ...
    function show_receipt(payid) {
        // dlgopen('../client_file/front_payment.php?receipt=1&payid=' + payid, '_blank', 550, 400);
        return false;
    }
  ...
  <td width='640px'>
    ...
    <table class='text'>
        ...
        <tr>
            ...
            <td>
               <input type='text' class='form-control' name='form_client_id' size='10' maxlength='20' value='<?php echo attr($form_client_id); ?>'
                title='<?php echo xla('Optional numeric client ID'); ?>' />
            </td>
            <td class='control-label'>
                <?php echo xlt('Drug'); ?>:
            </td>
...
<tbody>
    ...
    if ($_POST['form_refresh']) {
        ...
        if ($form_client_id) {
            $where .= " AND p.pubpid = ?";
            array_push($sqlBindArray, $form_client_id);
        }
        ...
        "LEFT OUTER JOIN client_data AS p ON p.pid = r.client_id " .
        ...
        while ($row = sqlFetchArray($res)) {
            ...
            if (strcmp($row['pubpid'], $last_client_id) == 0) {
                $client_name = '&nbsp;';
                $client_id   = '&nbsp;';
   ...
   <tr>
    <td>
            <?php echo text($client_name); ?>
  </td>
  <td>
            <?php echo text($client_id); ?>
  </td>
  <td>
