...
<table>
 <tr>
  ...
  <th id="sortby_pid" class="text" title="<?php echo xla('Sort by ClientID'); ?>"><?php echo xlt('ClientID'); ?></th>
...
<input type="hidden" name="event" value="<?php echo attr($eventname)."-".attr($type_event) ?>">
    ...
    $dispArr = array();
    $icnt = 1;
    if ($ret = EventAuditLogger::instance()->getEvents(array('sdate' => $start_date,'edate' => $end_date, 'user' => $form_user, 'client' => $form_pid, 'sortby' => $_GET['sortby'], 'levent' =>$gev, 'tevent' =>$tevent))) {
        ...
        foreach ($ret as $iter) {
            ...
            if (count($logEncryptData) > 0) {
                ...
                $concatLogColumns = $iter['date'].$iter['event'].$iter['user'].$iter['groupname'].$iter['comments'].$iter['client_id'].$iter['success'].$iter['checksum'].$iter['crt_user'];
     ...
     <TR class="oneresult">
          ...
          <TD class="text tamperColor"><?php echo text($iter["client_id"]);?></TD>
...
$(function(){
    ...
    $("#sortby_pid").click(function() { $("#sortby").val("client_id"); $("#theform").submit(); });
