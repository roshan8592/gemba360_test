...
// Will start the (client) portal OpenEMR session/cookie.
require_once(dirname(__FILE__) . "/../src/Common/Session/SessionUtil.php");
OpenEMR\Common\Session\SessionUtil::portalSessionStart();

$isPortal = false;
if (isset($_SESSION['pid']) && isset($_SESSION['client_portal_onsite_two'])) {
    $pid = $_SESSION['pid'];
    $ignoreAuth = true;
    $isPortal = true;
...
$pid = $_REQUEST['hidden_client_code'] > 0 ? $_REQUEST['hidden_client_code'] : $pid;
$recid = isset($_REQUEST['recid']) ? (int) $_REQUEST['recid'] : 0;
$adminUser = '';
$portalClient = '';

$query = "SELECT pao.portal_username as recip_id, Concat_Ws(' ', client_data.fname, client_data.lname) as username FROM client_data " .
    "LEFT JOIN client_access_onsite pao ON pao.pid = client_data.pid " .
    "WHERE client_data.pid = ? AND pao.portal_pwd_status = 1";
$portalClient = sqlQueryNoLog($query, $pid);
...
function echoLine($iname, $date, $charges, $ptpaid, $inspaid, $duept, $encounter = 0, $copay = 0, $patcopay = 0)
{
    ...
    $balance = (round($duept, 2) != 0) ? 0 : $balance; // if balance is due from client, then insurance balance is displayed as zero
    ...
    echo "  <td class='detail' align='center' id='td_client_copay_$var_index' >" . text(bucks($patcopay)) . "</td>\n";
...
$patdata = sqlQuery("SELECT " . "p.fname, p.mname, p.lname, p.postal_code, p.pubpid,p.pid, i.copay " . "FROM client_data AS p " . "LEFT OUTER JOIN insurance_data AS i ON " . "i.pid = p.pid AND i.type = 'primary' " . "WHERE p.pid = ? ORDER BY i.date DESC LIMIT 1", array($pid
));
...
if ($_POST['form_save']) {
    ...
    $patdata = getClientData($form_pid, 'fname,mname,lname,pubpid');
            ...
            if ($amount = 0 + $payment) {
                ...
                if ($_REQUEST['radio_type_of_payment'] == 'copay') {//copay saving to ar_session and ar_activity tables
                    $session_id = sqlInsert(
                        ...
                        " global_amount,payment_type,description,client_id,payment_method,adjustment_code,post_to_date) " .
                        " VALUES ('0',?,?,now(),now(),?,'','client','COPAY',?,?,'client_payment',now())",
                ...
                if ($_REQUEST['radio_type_of_payment'] == 'invoice_balance' || $_REQUEST['radio_type_of_payment'] == 'cash') {                //Payment by client after insurance paid, cash clients similar to do not bill insurance in feesheet.
                    ...
                    $adjustment_code = 'client_payment';
                    $payment_id = sqlInsert(
                        "insert into ar_session set " .
                        "payer_id = ?" .
                        ", client_id = ?" .
                    ...
                    $resMoneyGot = sqlStatement(
                        "SELECT sum(pay_amount) as ClientPay FROM ar_activity where pid =? and " .
...
// Get the client's name and chart number.
    $patdata = getClientData($form_pid, 'fname,mname,lname,pubpid');
...
// Now proceed with printing the receipt.
    ...
    <script>
        ...
        function goHome() {
            window.location.replace("./client/onsiteactivityviews");
        }
        ...
        function notifyClient() {
            ...
            let rid = <?php echo js_escape($portalClient['recip_id']); ?>;
            let rn = <?php echo js_escape($portalClient['username']); ?>;
            $.ajax({
                url: formURL,
                type: "POST",
                data: {
                ...
                success: function (data, textStatus, jqXHR) {
                    alert('Receipt sent to client via Messages.')
                },
    ...
    <button class='btn btn-sm' type='button' onclick="notifyClient()"><?php echo xla('Notify Client'); ?></button>
    </body></html>
    <?php
    ob_end_flush();
} else {
        ...
        function coloring() {
            for (var i = 1; ; ++i) {
                if (document.getElementById('paying_' + i)) {
                    ...
                    client_balance = document.getElementById('duept_' + i).innerHTML * 1;
                    if (client_balance > 0 && paying > 0) {
                        if (paying > client_balance) {
                            document.getElementById('paying_' + i).style.background = '#FF0000';
                        }
                        else if (paying < client_balance) {
...
//no checks taken here....
        ...
        function make_it_hide_enc_pay() {
            ...
            document.getElementById('td_head_client_co_pay').style.display = "none";
            ...
            for (var i = 1; ; ++i) {
                ...
                var td_client_copay_elem = document.getElementById('td_client_copay_' + i)
                ...
                if (td_inspaid_elem) {
                    ...
                    td_client_copay_elem.style.display = "none";
        ...
        function make_visible() {
            ...
            document.getElementById('td_head_insurance_payment').style.display = "none";
            document.getElementById('td_head_client_payment').style.display = "none";
            document.getElementById('td_head_client_co_pay').style.display = "none";
            document.getElementById('td_head_co_pay').style.display = "none";
            document.getElementById('td_head_insurance_balance').style.display = "none";
            document.getElementById('td_head_client_balance').style.display = "none";
            for (var i = 1; ; ++i) {
        ...
        function make_it_hide() {
            ...
            document.getElementById('td_head_client_payment').style.display = "";
            document.getElementById('td_head_client_co_pay').style.display = "";
            document.getElementById('td_head_co_pay').style.display = "";
            document.getElementById('td_head_insurance_balance').style.display = "";
            document.getElementById('td_head_client_balance').style.display = "";
            for (var i = 1; ; ++i) {
                var td_charges_elem = document.getElementById('td_charges_' + i)
                var td_inspaid_elem = document.getElementById('td_inspaid_' + i)
                var td_ptpaid_elem = document.getElementById('td_ptpaid_' + i)
                ...
                if (td_charges_elem) {
                    ...
                    td_client_copay_elem.style.display = "";
    ...
    <form id="invoiceForm" method='post' action='<?php echo $GLOBALS["webroot"] ?>/portal/portal_payment.php'>
        ...
        <table id="table_display" style="width: 100%; background: #eee;" class="table table-condensed table-striped table-bordered">
            ...
            <tr bgcolor="#cccccc" id="tr_head">
                ...
                <td class="dehead" align="center" width="70" id="td_head_client_payment">
                    <?php echo xlt('Client Payment') ?>
                </td>
                <td class="dehead" align="center" width="55" id="td_head_client_co_pay">
                ...
                <td class="dehead" align="center" width="70" id="td_head_client_balance">
                    <?php echo xlt('Client Balance') ?>
                </td>
            ...
            <?php
            $encs = array();
            // Get the unbilled service charges and payments by encounter for this client.
            //
            ...
            foreach ($encs as $key => $value) {
                ...
                $patcopay = BillingUtilities::getClientCopay($pid, $enc);
                // Insurance Payment
                //
                ...
                $duept = 0;
                if ((($NumberOfInsurance == 0 || $value['last_level_closed'] == 4 || $NumberOfInsurance == $value['last_level_closed']))) { // Client balance
        ...
        <input type="hidden" name="hidden_client_code" id="hidden_client_code" value="<?php echo attr($pid); ?>"/>
