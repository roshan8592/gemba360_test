<?php
/**
 * Client matching and selection for the WordPress Client Portal.
 *
 * @package   OpenEMR
...
if ($_POST['bn_save']) {
  ...
  // configured as unused.
    ...
    if (empty($ptid)) {
        ...
        if (empty($newdata['client_data']['pubpid'])) {
            // pubpid for new client defaults to pid.
            $newdata['client_data']['pubpid'] = "$ptid";
        }

        updateClientData($ptid, $newdata['client_data' ], true);
        updateEmployerData($ptid, $newdata['employer_data'], true);
        newHistoryData($ptid);
    } else {
        $newdata['client_data']['id'] = $_POST['db_id'];
        updateClientData($ptid, $newdata['client_data']);
    }
...
// If needed, this creates the new client in the CMS. It executes as an AJAX script
 ...
 if (f.form_cmsportal_login_pass) {
  ...
  if (login) {
   ...
   $.ajax({
    type: "GET",
    dataType: "text",
    url: 'client_form_ajax.php?login=' + login + '&pass=' + pass + '&email=' + email,
    async: false,
...
<form method='post' action='client_form.php' onsubmit='return validate()'>
...
while ($lorow = sqlFetchArray($lores)) {
  ...
  // or are mandatory for a new client.
