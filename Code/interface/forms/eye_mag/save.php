...
if ($_REQUEST["mode"] == "new") {
    ...
    if ($_REQUEST['action'] == 'docs') {
        $query = "update client_data set ref_providerID=?,referrerID=? where pid =?";
        ...
        if ($deletion == 1) {
            ...
            if ($form_type == 'ROS') { //ROS
                ...
                if ($_REQUEST['marital_status'] > '') {
                    ...
                    while ($frow = sqlFetchArray($fres)) {
                        if (($_REQUEST['marital_status'] == $frow['option_id']) || ($_REQUEST['marital_status'] == $frow['title'])) {
                            ...
                            $query = "UPDATE client_data set status=? where pid=?";
                ...
                if ($_REQUEST['occupation'] > '') {
                    $query = "UPDATE client_data set occupation=? where pid=?";
    ...
    if ($_REQUEST['action'] == 'code_visit') {
        ...
        foreach ($CODING as $item) { //need toremove duplicate codes
            ...
            $sql = "SELECT codes.*, prices.pr_price FROM codes " .
                "LEFT OUTER JOIN client_data ON client_data.pid = '$pid' " .
                "LEFT OUTER JOIN prices ON prices.pr_id = codes.id AND " .
                "prices.pr_selector = '' AND " .
                "prices.pr_level = client_data.pricelevel " .
                "WHERE code =?" .
                " LIMIT 1";
    ...
    if ($_REQUEST['action'] == 'new_pharmacy') {
        $query = "UPDATE client_data set pharmacy_id=? where pid=?";
    ...
    if ($_REQUEST['action'] == 'new_appt_status') {
        if ($_POST['new_status']) {
            ...
            //get eid
            $sql = "select * from client_tracker where  `pid` = ? and `apptdate`=?";
            $tracker = sqlFetchArray(sqlStatement($sql, array($_POST['pid'], $Vdate)));
            sqlStatement("UPDATE `client_tracker` SET  `lastseq` = ? WHERE `id` = ?", array(($tracker['lastseq'] + 1), $tracker['id']));
            #Add a tracker item.
            $sql = "INSERT INTO `client_tracker_element` " .
        ...
        echo "Failed to update Client Tracker.";
        exit;
    }
