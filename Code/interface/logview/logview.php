...
<head>
    ...
    <script>
        ...
        // VicarePlus :: This invokes the find-client popup.
        function sel_client() {
            dlgopen('../main/calendar/find_client_popup.php?pflag=0', '_blank', 500, 400);
        }

        // VicarePlus :: This is for callback by the find-client popup.
...
if ($_GET["form_client"]) {
    $form_client = isset($_GET["form_client"]) ? $_GET["form_client"] : "";
}
...
<div class="row col-xs-12">
    <div class="well col-lg-11">
        ...
        <form method="GET" name="theform" id="theform" class="form-horizontal">
            ...
            <div class="row form-group">
                ...
                <label class="control-label col-sm-1" for="end_date"><?php echo xlt('Client'); ?>:</label>
                <div class="col-sm-3">
                    <input type='text' size='20' class='form-control' name='form_client' id='form_client' style='width:100%;cursor:pointer;cursor:hand' value='<?php echo $form_client ? attr($form_client) : xla('Click To Select'); ?>' onclick='sel_client()' title='<?php echo xla('Click to select client'); ?>' />
    ...
    <table class="table table-striped">
     <tr>
      ...
      <th id="sortby_pid" class="sortby" title="<?php echo xla('Sort by ClientID'); ?>"><?php echo xlt('Client ID'); ?></th>
    ...
    <input type="hidden" name="event" value="<?php echo attr($eventname) . "-" . attr($type_event) ?>">
        ...
        if ($ret = EventAuditLogger::instance()->getEvents(array('sdate' => $start_date,'edate' => $end_date, 'user' => $form_user, 'client' => $form_pid, 'sortby' => $_GET['sortby'], 'levent' =>$gev, 'tevent' =>$tevent,'direction' => $_GET['direction']))) {
        ...
        <TD><?php echo text($iter["client_id"]); ?></TD>
      ...
      <TD><?php echo text($iter["checksum"]); ?></TD>
        ...
        if (($eventname=="disclosure") || ($gev == "")) {
            ...
            if ($ret = EventAuditLogger::instance()->getEvents(array('sdate' => $start_date,'edate' => $end_date, 'user' => $form_user, 'client' => $form_pid, 'sortby' => $_GET['sortby'], 'event' =>$eventname))) {
                foreach ($ret as $iter) {
...
$(function(){
    ...
    $("#sortby_pid").click(function() { set_sort_direction(); $("#sortby").val("client_id"); $("#theform").submit(); });
