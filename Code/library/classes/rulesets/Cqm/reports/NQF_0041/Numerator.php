...
class NQF_0041_Numerator implements CqmFilterIF
{
    ...
    public function test(CqmClient $client, $beginDate, $endDate)
    {
        ...
        $query = "select count(*) as cnt from form_encounter fe ".
               "INNER JOIN procedure_order po on po.client_id = fe.pid ".
        ...
        $sql = sqlQuery($query, array($client->id,$beginDate,$endDate,$periodMinus153Days,$periodPlus89Days));
        if ($sql['cnt'] > 0) {
            return true;
        }
        
        $query = "select count(*) as cnt from form_encounter fe ".
               "INNER JOIN immunizations imm on imm.client_id = fe.pid ".
               "WHERE pid = ? and fe.date between ? and ? ".
