...
function make_task($ajax_req)
{
    ...
    $sql = "SELECT * from form_taskman where FROM_ID=? and TO_ID=? and CLIENT_ID=? and ENC_ID=?";
    $task = sqlQuery($sql, array($from_id,$to_id,$client_id,$enc));
    ...
    if (!$doc['ID'] && $task['ID'] && (strtotime($task['REQ_DATE']) < (time() - 60))) {
        ...
        $sql = "DELETE from form_taskman where FROM_ID=? and TO_ID=? and CLIENT_ID=? and ENC_ID=?";
        $task = sqlQuery($sql, array($from_id,$to_id,$client_id,$enc));
    }
    ...
    if ($task['ID'] && $task['COMPLETED'] =='2') {
        ...
        if ($task['DOC_TYPE'] == 'Fax') {
            $send['DOC_link'] = "<a href='".$webroot."/openemr/controller.php?document&view&client_id=".attr($task['CLIENT_ID'])."&doc_id=".attr($task['DOC_ID'])."'
            ...
            $sql = "INSERT into form_taskman
				(REQ_DATE, FROM_ID,  TO_ID,  CLIENT_ID,  DOC_TYPE,  DOC_ID,  ENC_ID) VALUES
				(NOW(), ?, ?, ?, ?, ?, ?)";
            sqlQuery($sql, array($from_id, $to_id, $client_id, $doc_type, $doc_id, $enc));
...
function deliver_document($task)
{
    ...
    $clientData    =  sqlQuery($query, array($task['CLIENT_ID']));
...
function make_document($task)
{
    ...
    $encounter_data =sqlQuery($query, array($encounter,$task['CLIENT_ID']));
    ...
    $filepath = $GLOBALS['oer_config']['documents']['repository'] . $task['CLIENT_ID'];
    ...
    if (($task['DOC_TYPE'] == 'Fax') || ($task['DOC_TYPE'] == 'Fax-resend')) {
        ?>
        <div class='wrapper'>
        <?php echo report_header($task['CLIENT_ID'], 'PDF'); ?>
            <br />
            <br />
            <br />
            <br />
    ...
    $return = addNewDocument($filename, $type, $temp_filename, 0, $size, $task['FROM_ID'], $task['CLIENT_ID'], $category_id);
