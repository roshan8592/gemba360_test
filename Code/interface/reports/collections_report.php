 ...
 * (TLH) Added ability to download selected invoices only or all for client
 *
 * @package   OpenEMR
...
function endClient($ptrow)
{
    global $export_client_count, $export_dollars, $bgcolor;
    ...
    if ($_POST['form_export']) {
        ...
        echo "1";                      // service code
        echo sprintf("%010.0f", $ptrow['pid']); // transmittal number = client id
        echo " ";                      // filler
        ...
        if ($ptrow['count'] > 1) {
          ...
          echo "&nbsp;</td>\n";
          echo "  <td class='detotal' colspan='$final_colspan'>&nbsp;Total Client Balance:</td>\n";
            ***************************************************************/
            echo "  <td class='detotal' colspan='" . attr(($initial_colspan + $final_colspan)) .
            "'>&nbsp;" . xlt('Total Client Balance') . ":</td>\n";
            /**************************************************************/
            if ($form_age_cols) {
                for ($c = 0; $c < $form_age_cols; ++$c) {
            ...
            <td>
              ...
              <div class="checkbox">
                           <label><input type='checkbox' name='form_cb_with_debt'<?php echo ($form_cb_with_debt) ? ' checked' : ''; ?>>
                            <?php echo xlt('Clients with debt') ?></label>
              </div>
                        </td>
                    </tr>
...
if ($_POST['form_refresh'] || $_POST['form_export'] || $_POST['form_csvexport']) {
    ...
    while ($erow = sqlFetchArray($eres)) {
      ...
      // If we have not yet billed the client, then compute $duncount as a
      ...
      // number $insposition (1-3).
        ...
        if (! $duncount) {
            for ($i = 1; $i <= 3; ++$i) {
                $tmp = SLEOB::arGetPayerID($client_id, $svcdate, $i);
                if (empty($tmp)) {
                    break;
                }
                $payerids[] = $tmp;
      ...
      // An invoice is due from the client if money is owed and we are
      // not waiting for insurance to pay.
        ...
        $row['id']        = $erow['id'];
        $row['invnumber'] = "$client_id.$encounter_id";
        $row['custid']    = $client_id;
      ...
      // Also get the primary insurance company name whenever there is one.
        ...
        if ($insposition == 1) {
            ...
            if (empty($payerids)) {
                $tmp = SLEOB::arGetPayerID($client_id, $svcdate, 1);
                if (!empty($tmp)) {
                    $payerids[] = $tmp;
                }
            }
      ...
      // all billing items.
        $invlines = ar_get_invoice_summary($client_id, $encounter_id, true);
      ...
      // Look up insurance policy number if we need it.
        if ($form_cb_policy) {
            ...
            "ORDER BY date DESC LIMIT 1", array($client_id, $instype, $svcdate));
...
<?php } ?>
    ...
    foreach ($rows as $key => $row) {
        ...
        if ($is_ins_summary && $insname != $ptrow['insname']) {
            ...
            // For the report, this will write the client totals.  For the
            // collections export this writes everything for the client:
            endClient($ptrow);
            $bgcolor = ((++$orow & 1) ? "#ffdddd" : "#ddddff");
            $ptrow = array('insname' => $insname, 'ptname' => $ptname, 'pid' => $pid, 'count' => 1);
            foreach ($row as $key => $value) {
...
<td class="detail" align="left"><?php echo text(bucks($balance)); ?>&nbsp;</td>
  ...
  <td class="detail" align="center">
    ...
    if ($_POST['form_export']) {
        echo "</textarea>\n";
        $alertmsg .= "$export_client_count clients with total of " .
        ...
        // echo "</textarea>\n";
        // $alertmsg .= "$export_client_count clients representing $" .
