    ...
    if (isset($_REQUEST['final_this_page_criteria'])) {
        foreach ($_REQUEST['final_this_page_criteria'] as $criteria_key => $criteria_value) {
          ...
          //---------------------------------------------------------
            ...
            } elseif (strpos($criteria_value, "client_data.fname")!== false) {
                $SplitArray=explode(' like ', $criteria_value);
                $query_part .= " AND ($criteria_value or client_data.lname like ".$SplitArray[1].")";
...
function getBillsBetweendayReport(
    ...
    "billing.activity = 1 " .
    "LEFT OUTER JOIN client_data ON client_data.pid = form_encounter.pid " .
    "LEFT OUTER JOIN claims on claims.client_id = form_encounter.pid and claims.encounter_id = form_encounter.encounter " .
    ...
    $query = sqlStatement("SELECT ar_activity.pid as pid, 'Client Payment' AS code_type, ar_activity.pay_amount AS pat_code, date(ar_activity.post_time) AS date,".
    ...
    "FROM ar_activity LEFT OUTER JOIN client_data ON client_data.pid = ar_activity.pid " .
