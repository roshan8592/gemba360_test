...
class EncounterccdadispatchController extends AbstractActionController
{
    ...
    public function ccdaFetching($parameterArray = array())
    {
        ...
        if ($validResult == 'existingclient') {
            ...
            $this->client_id   = $this->getEncounterccdadispatchTable()->getClientId($parameterArray[0][6]);//$this->getRequest()->getQuery('pid');
            $this->client_username   = $parameterArray[0][6];
            ...
            if ($this->recipients == 'client') {
                $this->params   = $this->client_id;
            } else {
            ...
            if ($combination != '') {
                ...
                foreach ($arr as $row) {
                    ...
                    $this->create_data($this->client_id, $this->encounter_id, $this->sections, $send, $this->components);
                    ...
                    $ccdaDocumentId = $this->getEncounterccdadispatchTable()->logCCDA($this->client_id, $this->encounter_id, base64_encode($content), $this->createdtime, 0, $_SESSION['authId'], $view, $send, $emr_transfer);
                    try {
                        $event = isset($parameterArray['event']) ? $parameterArray['event'] : 'client-record';
                        $menu_item = isset($parameterArray['menu_item']) ? $parameterArray['menu_item'] : 'Dashboard';

                        EventAuditLogger::instance()->newEvent($event, $this->client_username, '', 1, '', $this->client_id, $log_from = 'client-portal', $menu_item, $ccdaDocumentId);
                    } catch (Exception $e) {
                    }
                }
                ...
                $practice_filename  = "CCDA_{$this->client_id}.xml";
                $this->create_data($this->client_id, $this->encounter_id, $this->sections, $send, $this->components);
                $content            = $this->socket_get("$mirth_ip", "6661", $this->data);
                $to_replace = '<?xml version="1.0" encoding="UTF-8"?>
				<?xml-stylesheet type="text/xsl" href="CDA.xsl"?>
    ...
    public function downloadCcdaLogDoc($parameterArray = array())
    {
        ...
        // validate credentials
        if ($validResult == 'existingclient' && isset($parameterArray['docid']) && $parameterArray['docid'] > 0) {
    ...
    public function indexAction()
    {
        ...
        $this->client_id   = $this->getRequest()->getQuery('pid');
        ...
        if ($combination != '') {
            ...
            foreach ($arr as $row) {
                ...
                $this->encounter_id = ($arr[1] > 0 ? $arr[1] : null);
                if ($this->latest_ccda) {
                    $this->encounter_id = $this->getEncounterccdadispatchTable()->getLatestEncounter($this->client_id);
                }
		...
		<!--';
                $content = preg_replace('/<ClinicalDocument.*><!--/', $to_replace, trim($content));
                $this->getEncounterccdadispatchTable()->logCCDA($this->client_id, $this->encounter_id, base64_encode($content), $this->createdtime, 0, $_SESSION['authId'], $view, $send, $emr_transfer);
                if (!$view) {
                    if ($hie_hook) {
                        echo $content;
                    } else {
            ...
            $practice_filename  = "CCDA_{$this->client_id}.xml";
            $this->create_data($this->client_id, $this->encounter_id, $this->sections, $send);
            $content            = $this->socket_get("$mirth_ip", "6661", $this->data);
            $to_replace = '<?xml version="1.0" encoding="UTF-8"?>
            <?xml-stylesheet type="text/xsl" href="CDA.xsl"?>
            <ClinicalDocument xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    public function create_data($pid, $encounter, $sections, $send = 0, $components)
    {
        ...
        $this->data .= $this->getEncounterccdadispatchTable()->getClientdata($pid, $encounter);
...
//            $ccd .= $this->getEncounterccdadispatchTable()->getRefferals($pid,$encounter);
    ...
    * $return   string  $procedure_notes      XML which contains the details collected from the client.
    */
    ...
    * $return   string  $operative_notes      XML which contains the details collected from the client.
    */
    ...
    * $return   string  $consultation_notes      XML which contains the details collected from the client.
    */
    ...
    * $return   string  $history_and_physical_notes      XML which contains the details collected from the client.
    */
