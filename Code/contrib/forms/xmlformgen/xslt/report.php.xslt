...
<xsl:text disable-output-escaping="yes"><![CDATA[    if ($data) {
        ...
        if (isset($GLOBALS['CLIENT_REPORT_ACTIVE']) && ! empty($_POST['pdf'])) { // PDF Print
            $td_style = "<td style='width:24%'><span class='bold'>";
            echo '<table style="width:775px;"><tr>';
        } elseif (isset($GLOBALS['CLIENT_REPORT_ACTIVE']) && empty($_POST['pdf'])) { // Client report view/search and printable
            $cols = 4;
