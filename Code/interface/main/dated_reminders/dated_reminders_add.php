...
// ------- check if ClientID is set and in numeric
    isset($_POST['ClientID']) and is_numeric($_POST['ClientID'])) {
        $dueDate = DateToYYYYMMDD($_POST['dueDate']);
        $priority = intval($_POST['priority']);
        $message = $_POST['message'];
        $fromID = $_SESSION['authId'];
        $patID = $_POST['ClientID'];
...
// get current client, first check if this is a forwarded message, if it is use the original pid
if (isset($this_message['pid'])) {
    $clientID = (isset($this_message['pid']) ? $this_message['pid'] : 0);
    $reminder_title = xl("Forward this Reminder");
} else {
    $clientID = (isset($pid) ? $pid : 0);
    ...
    <script language="JavaScript">
      $(function (){
        ...
        $("#sendButton").click(function(){
          ...
          // todo : check if ClientID is numeric , no rush as this is checked in the php after the post
        ...
        $("#removeClient").click(function(){
          $("#ClientID").val("0");
          $("#clientName").val("<?php echo xla('Click to select client'); ?>");
          $(this).hide();
          return false;
        })
          // update word counter
        ...
        function sel_client(){
           dlgopen('../../main/calendar/find_client_popup.php', '_newDRPat', 650, 400, '', '');
        }

        function setclient(pid, lname, fname, dob){
              $("#clientName").val(fname +' '+ lname)
              $("#ClientID").val(pid);
              $("#removeClient").show();
              return false;
        }
...
<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000;"></div>
    ...
    <input type="hidden" name="csrf_token_form" value="<?php echo attr(CsrfUtils::collectCsrfToken()); ?>" />
     ...
     <div class="col-xs-12" style="margin-top:20px;">
        <div class="form-group">
            <label class="control-label col-xs-3 text-right" for="clientName"><?php echo xlt('Link To Client') ?>: <i id="link-tooltip" class="fa fa-info-circle text-primary" aria-hidden="true" data-original-title="" title=""></i></label>
            <div class="col-xs-5">
                <input type='text' id='clientName' name='clientName' class='form-control'
                       value='<?php echo ($clientID > 0 ? attr(getPatName($clientID)) : xla('Click to select client')); ?>' onclick='sel_client()'
                       title='<?php xla('Click to select client'); ?>' readonly />
                <input type="hidden" name="ClientID" id="ClientID" value="<?php echo (isset($clientID) ? attr($clientID) : 0) ?>" />
            </div>
            <div class="col-xs-2">
                <button type="button" class="btn btn-default btn-undo" <?php echo ($clientID > 0 ? '' : 'style="display:none"') ?> id="removeClient"><?php echo xlt('unlink client') ?></button>
            </div>
        </div>
    </div>
    </fieldset>
    <fieldset>
        <div class="col-xs-12" style="margin-top:20px;">
            <div class="form-group">
                <label class="control-label col-xs-3 text-right" for="clientName"><?php echo xlt('Send to') ?> :  <br />
    ...
    foreach ($TempRemindersArray as $RA) {
        ...
        $remindersArray[$RA['messageID']]['ClientName'] = $RA['ClientName'];
  ...
  <script>
    $( document ).ready(function() {
        $('#link-tooltip').tooltip({title: "<?php echo xla('This message need not necessarily be linked to a client'); ?>"});
