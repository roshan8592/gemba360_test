...
class EventAuditLogger
{
    ...
    private const LOG_TABLES = [
        "billing" => "client-record",
        "claims" => "client-record",
        "employer_data" => "client-record",
        "forms" => "client-record",
        "form_encounter" => "client-record",
        "form_dictation" => "client-record",
        "form_misc_billing_options" => "client-record",
        "form_reviewofs" => "client-record",
        "form_ros" => "client-record",
        "form_soap" => "client-record",
        "form_vitals" => "client-record",
        "history_data" => "client-record",
        "immunizations" => "client-record",
        "insurance_data" => "client-record",
        "issue_encounter" => "client-record",
        "lists" => "client-record",
        "client_data" => "client-record",
        "payments" => "client-record",
        "pnotes" => "client-record",
        "onotes" => "client-record",
        "prescriptions" => "order",
        "transactions" => "client-record",
        "amendments" => "client-record",
        "amendments_history" => "client-record",
...
MSG;
    ...
    private const RFC3881_MSG_CLIENT_TEMPLATE = <<<MSG
<ParticipantObjectIdentification ParticipantObjectID="%s" ParticipantObjectTypeCode="1" ParticipantObjectTypeCodeRole="1">
 <ParticipantObjectIDTypeCode code="2" displayName="Client Number" codeSystemName="RFC-3881" />
</ParticipantObjectIdentification>
MSG;
     ...
     */
    public function newEvent($event, $user, $groupname, $success, $comments = "", $client_id = null, $log_from = 'open-emr', $menu_item = 'dashboard', $ccda_doc_id = 0)
    {
        ...
        if ($log_from == 'client-portal') {
            $sqlMenuItems = "SELECT * FROM client_portal_menu";

            $resMenuItems = sqlStatement($sqlMenuItems);
            for ($iter=0; $rowMenuItem=sqlFetchArray($resMenuItems); $iter++) {
                $menuItems[$rowMenuItem['client_portal_menu_id']] = $rowMenuItem['menu_name'];
            }

            $menuItemId = array_search($menu_item, $menuItems);
            $sql = "insert into log ( date, event,category, user, client_id, groupname, success, comments,
                log_from, menu_item_id, crt_user, ccda_doc_id) values ( NOW(), ?,'Client Portal', ?, ?, ?, ?, ?, ?, ?,?, ?)";
            $ret = sqlStatementNoLog($sql, array($event, $user, $client_id, $groupname, $success, $comments,$log_from, $menuItemId,$crt_user, $ccda_doc_id));
        } else {
            /* More details added to the log */
            $sql = "insert into log ( date, event,category, user, groupname, success, comments, crt_user, client_id) " .
                "values ( NOW(), " . $adodb->qstr($event) . ",". $adodb->qstr($category) . "," . $adodb->qstr($user) .
                "," . $adodb->qstr($groupname) . "," . $adodb->qstr($success) . "," .
                $adodb->qstr($comments) ."," .
                $adodb->qstr($crt_user) ."," . $adodb->qstr($client_id). ")";
        ...
        $this->send_atna_audit_msg($user, $groupname, $event, $client_id, $success, $comments);
    }
    ...
    public function getEvents($params)
    {
        ...
        $cols = "DISTINCT date, event, category, user, groupname, client_id, success, comments,checksum,crt_user, id ";
        ...
        //VicarePlus :: For Generating log with client id.
        $client = "";
        ...
        if ($event!="") {
            ...
            $sqlBindArray = array();
            $columns = "DISTINCT date, event, user, recipient,client_id,description";
            ...
            if ($client != "") {
                ...
                array_push($sqlBindArray, $client);
            }
    ...
    private function determineRFC3881EventIdDisplayName($event)
    {
        ...
        if (strpos($event, 'client-record') !== false) {
            $eventIdDisplayName = 'Client Record';
            ...
            $eventIdDisplayName = 'Client Care Assignment';
     ...
     * @return string
     */
    private function create_rfc3881_msg($user, $group, $event, $client_id, $outcome, $comments)
    {
        ...
        $clientRecordForMsg = ($eventIdDisplayName == 'Client Record' && $client_id != 0) ? sprintf(self::RFC3881_MSG_CLIENT_TEMPLATE, $client_id) : '';
        /* Add the syslog header  with $eventDateTime and $_SERVER['SERVER_NAME'] */
        return sprintf(self::RFC3881_MSG_PRIMARY_TEMPLATE, $eventDateTime, $_SERVER['SERVER_NAME'], $eventActionCode, $eventDateTime, $eventOutcome, $eventIdDisplayName, $srcUserID, $srcNetwork, $destUserID, $destNetwork, $srcUserID, $user, $clientRecordForMsg);
    }
     ...
     * @param $comments
     */
    public function send_atna_audit_msg($user, $group, $event, $client_id, $outcome, $comments)
    {
        ...
        if ($conn !== false) {
            $msg = $this->create_rfc3881_msg($user, $group, $event, $client_id, $outcome, $comments);
            fwrite($conn, $msg);
            fclose($conn);
        }
    }
    ...
    public function auditSQLEvent($statement, $outcome, $binds = null)
    {
        ...
        /* If the event is a client-record, then note the client id */
        $pid = 0;
        if ($event == "client-record") {
        ...
        $sql = "insert into log (date, event,category, user, groupname, comments, client_id, success, checksum,crt_user) " .
            "values ( ".
    ...
    public function auditSQLAuditTamper($enable)
    {
        ...
        $sql = "insert into log (date, event, user, groupname, comments, client_id, success, checksum,crt_user) " .
            "values ( NOW(), " .
            $adodb->qstr($event) . ", " .
            $adodb->qstr($user) . "," .
            $adodb->qstr($group) . "," .
    ...
    public function recordDisclosure($dates, $event, $pid, $recipient, $description, $user)
    {
        ...
        $success=1;
        $sql = "insert into extended_log ( date, event, user, recipient, client_id, description) " .
    ...
    private function eventCategoryFinder($sql, $event, $table)
    {
        ...
        } else if ($table == 'client_data' || $table == 'employer_data') {
