...
//   2. For the _set_client_inc_count function - $sqlBindArraySpecial
...
_set_client_inc_count($MAXSHOW, count($result), $where, $sqlBindArraySpecial);
?>
...
<input type='button' value='<?php echo xla('Cancel'); ?>'
 onclick='dlgclose();' />
<?php } else { ?>
<input type='button' value='<?php echo xla('Confirm Create New Client'); ?>'
...
// will set the pid and load all the other frames.
  $newPage = "../client_file/summary/demographics.php?set_pid=";
  $target = "document";
...
<?php if ($pubpid_matched) { ?>
alert(<?php echo xlj('A client with this ID already exists.'); ?>);
<?php } else { ?>
opener.force_submit = true;
f.create.value = <?php echo xlj('Confirm Create New Client'); ?>;
<?php } ?>
