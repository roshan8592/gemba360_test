...
class CheckCreateUserTest extends PHPUnit_Extensions_Selenium2TestCase
{
    ...
    */
    public function testAddClient()
    {
        ...
        /*Move to frame left nav and click on new client*/
        $this->frame("left_nav");
        $newClientLink=$this->byId('new0');
        $newClientLink->click();
        $this->frame(null);
        $this->frame("RTop");
       ...
       /*Move to the popup and click on create client*/
       ...
       /*Accept the alert when creating a new client*/
    ...
    /**
     * Check that the new client exist in the database
     *
     * @depends testAddClient
     */
    public function testFindClient($testset)
    {
        $this->database_connection();
        $sql=sprintf("SELECT * FROM client_data where fname='%s' and lname='%s' and DOB = '%s' ", $testset['name'], $testset['lname'], $testset['dob']);
        $res=$this->dbconn->query($sql);
        $numRows=mysqli_num_rows($res);
        $this->assertEquals(1, $numRows, "Client doesn't exists in the database");
    }
