...
function &postcalendar_userapi_pcQueryEventsFA($args)
{
  ...
  // link to the events tables
    ...
    "concat(u2.fname, ' ', u2.lname) as owner_name, pd.DOB as client_dob, " .
    "a.pc_facility" .
    "FROM  ( $table AS a ) " .
    ...
    "LEFT JOIN client_data as pd ON a.pc_pid=pd.pid " .
  ...
  // return an empty array if we don't have any results
    ...
    for ($i=0; !$result->EOF; $result->MoveNext()) {
        ...
        list($tmp['eid'],          $tmp['uname'],         $tmp['catid'],
         ...
         $tmp['aid'],          $tmp['provider_name'], $tmp['client_name'],
         $tmp['owner_name'],   $tmp['client_dob'],   $tmp['facility'])   = $result->fields;
        ...
        $events[$i]['client_name']= $tmp['client_name'];
        $events[$i]['provider_name'] = $tmp['provider_name'];
        $events[$i]['owner_name']  = $tmp['owner_name'];
        $events[$i]['client_dob'] = $tmp['client_dob'];
        $events[$i]['client_age'] = date("Y") - substr(($tmp['client_dob']), 0, 4);
...
function &postcalendar_userapi_pcQueryEvents($args)
{
  ...
  // link to the events tables
    ...
    "DOB as client_dob, a.pc_facility, pd.pubpid, a.pc_gid, " .
  ...
  // return an empty array if we don't have any results
    ...
    for ($i=0; !$result->EOF; $result->MoveNext()) {
        ...
        list($tmp['eid'],          $tmp['uname'],       $tmp['catid'],
         ...
         $tmp['client_name'], $tmp['owner_name'],  $tmp['client_dob'],
        ...
        $events[$i]['owner_name']  = $tmp['owner_name'];
        $events[$i]['client_dob'] = $tmp['client_dob'];
        $events[$i]['client_age'] = getClientAge($tmp['client_dob']);
        $events[$i]['facility']    = getFacility($tmp['facility']);
        $events[$i]['sharing']     = $tmp['sharing'];
        $events[$i]['prefcatid']   = $tmp['prefcatid'];
        $events[$i]['aid']         = $tmp['aid'];
...
function &postcalendar_userapi_pcGetEvents($args)
{
    ...
    if ($faFlag && !isset($events)) {
        ...
        $a = array('listappsFlag' => true,'start'=>$start_date,'end'=>$end_date, 'client_id' => $client_id, 's_keywords' => $s_keywords);
