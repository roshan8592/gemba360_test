...
class CcrTable extends AbstractTableGateway
{
  ...
  */
    public function getDemographics($data)
    {
        ...
        $query      = "SELECT ad.id as adid, table_name, field_name, field_value FROM audit_master am JOIN audit_details ad ON ad.audit_master_id = am.id
                  WHERE am.id = ? AND ad.table_name = 'client_data' ORDER BY ad.id";
  ...
  */
    ...
    {
        $appTable   = new ApplicationTable();
        $query      = "SELECT * FROM client_data WHERE pid = ?";
  ...
  */
    ...
    {
        $appTable   = new ApplicationTable();
        $query      = "SELECT * FROM prescriptions WHERE client_id = ?";
        $result     = $appTable->zQuery($query, array($data['pid']));
        $records    = array();
        foreach ($result as $row) {
            $records[] = $row;
  ...
  */
    ...
    {
        $appTable   = new ApplicationTable();
        $query      = "SELECT * FROM immunizations WHERE client_id = ?";//removed the field 'added_erroneously' from where condition
        $result     = $appTable->zQuery($query, array($data['pid']));
        $records    = array();
        foreach ($result as $row) {
            $records[] = $row;
  ...
  */
    public function getLabResults($data)
    {
        ...
        $query      = "SELECT * FROM procedure_order AS po LEFT JOIN procedure_order_code AS poc
                  ...
                  LEFT JOIN procedure_result AS prs ON prs.procedure_report_id = pr.procedure_report_id WHERE client_id = ?";
        $result     = $appTable->zQuery($query, array($data['pid']));
        $records    = array();
        foreach ($result as $row) {
            $records[] = $row;
  ...
  *
  * @param    data    Array     POST values from the approval screen which contains the client id
        ...
        foreach ($data as $key => $val) {
            if (substr($key, -4) == '-sel') {
                if (is_array($val)) {
                    for ($i=0; $i<count($val); $i++) {
                        if ($val[$i] == 'insert') {
                            if (substr($key, 0, -4) == 'lists1') {
                                ...
                                $query = "INSERT INTO prescriptions (client_id, date_added, active, drug, size, form, quantity) VALUES (?,?,?,?,?,?,?)";
                                ...
                                $query = "INSERT INTO immunizations (client_id, administered_date, note) VALUES (?,?,?)";
                    ...
                    if (substr($key, 0, 12) == 'client_data') {
                        if ($val == 'update') {
  ...
  */
  ...
  /*
  * Fetch the client data from audit tables and update to client_data table
  *
  * @param    audit_master_id     Integer     id from audit master table
  */
    public function insert_client($audit_master_id)
    {
        $pid = 0;
        $appTable   = new ApplicationTable();
        $pres       = $appTable->zQuery("SELECT IFNULL(MAX(pid)+1,1) AS pid FROM client_data", array());
        ...
        foreach ($res as $row) {
            ...
            foreach ($resfield as $rowfield) {
                if ($table == 'client_data') {
                    if ($rowfield['field_name'] == 'DOB') {
                        $newdata['client_data'][$rowfield['field_name']] = substr($rowfield['field_value'], 0, 10);
                    } else {
            ...
            if ($table == 'client_data') {
                ...
                $query_insert = "INSERT INTO prescriptions(client_id,date_added,active,drug,size,form,quantity) VALUES (?,?,?,?,?,?,?)";
                ...
                $query_insert = "INSERT INTO immunizations(client_id,administered_date,note) VALUES (?,?,?)";
