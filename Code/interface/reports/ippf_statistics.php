...
function getGcacData($row, $what, $morejoins="") {
  ...
  $query = "SELECT $what " .
    ...
    "LEFT JOIN issue_encounter AS ie ON ie.pid = '$client_id' AND " .
    "ie.encounter = '$encounter_id' AND ie.list_id = l.id " .
    "$morejoins " .
    "WHERE l.pid = '$client_id' AND " .
...
function loadColumnData($key, $row, $quantity = 1)
{
  ...
  // Skip this key if we are counting unique clients and the key
  // has already seen this client.
    if ($form_content == '2' && $row['pid'] == $areport[$key]['.prp']) {
        return;
    }

  // If we are counting new acceptors, then require a unique client
  ...
  // If we are counting new clients, then require a unique client
  ...
  // Flag this client as having been encountered for this report row.
...
function process_ippf_code($row, $code, $quantity = 1)
{
  ...
  //
    if ($form_by === '1') {
        ...
        $query = "SELECT COUNT(*) AS count " .
        "FROM lists AS l " .
        "JOIN issue_encounter AS ie ON ie.pid = '$client_id' AND " .
        "ie.encounter = '$encounter_id' AND ie.list_id = l.id " .
        "WHERE l.pid = '$client_id' AND " .
        "l.activity = 1 AND l.type = 'ippf_gcac'";
        // echo "<!-- $key: $query -->\n"; // debugging
        ...
        if (!hadRecentAbService($client_id, $encdate)) {
            $query = "SELECT COUNT(*) AS count " .
            ...
            $irow = sqlQuery($query, array($client_id, $encdate, $encdate));
            if (empty($irow['count'])) {
                return;
            }
        }
    ...
    } else if ($form_by === '17') { // Client Name.
...
if ($_POST['form_submit']) {
    ...
    foreach ($arr_show as $askey => $asval) {
        ...
        $pd_fields .= ', pd.' . escape_sql_column_name($askey, array('client_data'));
    }
    ...
    if ($form_content == 5) { // sales of contraceptive products
        ...
        "JOIN client_data AS pd ON pd.pid = ds.pid $sexcond" .
    ...
    // Get referrals and related client data.
    if ($form_content != 5 && ($form_by === '9' || $form_by === '10' || $form_by === '20' || $form_by === '1')) {
        ...
        "FROM transactions AS t " .
        "JOIN client_data AS pd ON pd.pid = t.pid $sexcond" .
    ...
    $query = "SELECT " .
      ...
      "FROM lists AS l " .
      "JOIN client_data AS pd ON pd.pid = l.pid $sexcond" .
    ...
    *****************************************************************/
    ...
    if ($form_content != 5 && $form_by !== '9' && $form_by !== '10' && $form_by !== '20') {
        ...
        // This gets us all MA codes, with encounter and client
        // info attached and grouped by client and encounter.
        $query = "SELECT " .
        ...
        "JOIN client_data AS pd ON pd.pid = fe.pid $sexcond" .
