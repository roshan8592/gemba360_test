...
function submitme() {
 var f = document.forms['client_data'];
 if (validate(f)) {
  top.restoreSession();
  f.submit();
 }
...
if ($_POST['form_complete']) {
    ...
    // Save that form as a row in rule_client_data table
    ...
    if (!isset($form_entryID)) {
        // Insert new row of data into rule_client_data table
        sqlStatement("INSERT INTO `rule_client_data` (`date`, `pid`, `category`, `item`, `complete`, `result`) " .
        "VALUES (?,?,?,?,?,?)", array($form_date, $pid, $form_category, $form_item, $form_complete, $form_result));
    } else { // $form_mode == "edit"
        // Modify selected row in rule_client_data table
        sqlStatement("UPDATE `rule_client_data` " .
        "SET `date`=?, `complete`=?, `result`=? " .
        "WHERE `id`=?", array($form_date,$form_complete,$form_result,$form_entryID));
    }

    // Close this window and refresh the client summary display.
    echo "<html>\n<body>\n<script language='JavaScript'>\n";
    echo " dlgclose();\n";
...
<br>
<form action='client_data.php' name='client_data' method='post' onsubmit='return top.restoreSession()'>
  ...
  <table class="showborder" cellspacing="0px" cellpadding="2px">
    ...
    while ($row = sqlFetchArray($res)) {
        ...
        if (isset($entryID) && ($entryID == $row['id'])) {
            ...
            echo "<td><a href='client_data.php?category=" .
