...
DEFINE("TBL_PAT_ACC_ON", "client_access_onsite");
DEFINE("COL_PID", "pid");
...
$sql = "SELECT * FROM `client_data` WHERE `pid` = ?";

if ($userData = sqlQuery($sql, array(
    $auth['pid']
))) { // if query gets executed
    if (empty($userData)) {
        $logit->portalLog('login attempt', '', ($_POST['uname'] . ':not active client'), '', '0');
        OpenEMR\Common\Session\SessionUtil::portalSessionCookieDestroy();
        header('Location: ' . $landingpage . '&w');
        exit();
    }
    ...
    if ($userData['allow_client_portal'] != "YES") {
        // Client has not authorized portal, so escape
    ...
    if ($password_update) {
        ...
        if (! (empty($_POST['pass_new'])) && ! (empty($_POST['pass_new_confirm'])) && ($code_new == $code_new_confirm)) {
            ...
            // Update the password and continue (client is authorized)
            privStatement(
