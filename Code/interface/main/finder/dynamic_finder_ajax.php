...
use OpenEMR\Events\ClientFinder\ColumnFilter;
...
//
$searchMethodInClientList = isset($_GET['searchType' ]) && $_GET['searchType' ]==="true" ?  true : false;
    ...
    for ($i = 0; $i < intval($_GET['iSortingCols']); ++$i) {
        ...
        if ($_GET["bSortable_$iSortCol"] == "true") {
            ...
            if ($aColumns[$iSortCol] == 'name') {
                ...
                $orderby .= "`" . escape_sql_column_name($aColumns[$iSortCol], array('client_data')) . "` $sSortDir";
            }
        }
    }
}
    ...
    foreach ($aColumns as $colname) {
        ...
        if ($colname == 'name') {
            $where .=
                ...
                "fname LIKE ? OR " .
                "mname LIKE ? ";
            if ($searchMethodInClientList) { // exact search
                array_push($srch_bind, $sSearch, $sSearch, $sSearch);
            } else {// like search
                array_push($srch_bind, ($sSearch . "%"), ($sSearch . "%"), ($sSearch . "%"));
            }
        } elseif ($searchMethodInClientList) { // exact search
            $where .= "`" . escape_sql_column_name($colname, array('client_data')) . "` LIKE ? ";
            array_push($srch_bind, $sSearch);
        } elseif ($searchAny) {
            $where .= " `" . escape_sql_column_name($colname, array('client_data')) . "` LIKE ?"; // any search
            array_push($srch_bind, ('%' . $sSearch . '%'));
        } else {
            $where .= "`" . escape_sql_column_name($colname, array('client_data')) . "` LIKE ? ";
            array_push($srch_bind, ($sSearch . '%'));
...
// This allows a module to subscribe to a 'client-finder.filter' event and
// add filtering before data ever gets to the user
$clientFinderFilterEvent = new ClientFinderFilterEvent(new BoundFilter(), $aColumns, $columnFilters);
$clientFinderFilterEvent = $GLOBALS["kernel"]->getEventDispatcher()->dispatch(ClientFinderFilterEvent::EVENT_HANDLE, $clientFinderFilterEvent, 10);
$boundFilter = $clientFinderFilterEvent->getBoundFilter();
...
foreach ($aColumns as $colname) {
    ...
    if ($colname == 'name') {
        ...
        $sellist .= "`" . escape_sql_column_name($colname, array('client_data')) . "`";
    }
}

// Get total number of rows in the table.
// Include the custom filter clause and bound values, if any
$row = sqlQuery("SELECT COUNT(id) AS count FROM client_data WHERE $customWhere", $boundFilter->getBoundValues());
$iTotal = $row['count'];
...
$row = sqlQuery("SELECT COUNT(id) AS count FROM client_data WHERE $where", $srch_bind);
...
// save into variable data about fields of 'client_data' from 'layout_options'
$fieldsInfo = array();
...
$query = "SELECT $sellist FROM client_data WHERE $where $orderby $limit";
$res = sqlStatement($query, $srch_bind);
while ($row = sqlFetchArray($res)) {
    // Each <tr> will have an ID identifying the client.
