<?php
/**
 * Client Flow Board (Client Tracker) (Report Based on the appointment report)
 *
 * This program used to select and print the information captured in the Client Flow Board program,
...
require_once("$srcdir/client_tracker.inc.php");
...
if ($client && !isset($_POST['form_from_date'])) {
    // If a specific client, default to 2 years ago.
    $tmp = date('Y') - 2;
...
if ($_POST["form_client"]) {
    $form_client = isset($_POST['form_client']) ? $_POST['form_client'] : '';
}
...
<head>
    <title><?php echo xlt('Client Flow Board Report'); ?></title>
    ...
    <script type="text/javascript">
        ...
        // CapMinds :: invokes  find-client popup.
        function sel_client() {
            dlgopen('../main/calendar/find_client_popup.php?pflag=0', '_blank', 500, 400);
        }

        // CapMinds :: callback by the find-client popup.
...
<span class='title'><?php echo xlt('Client Flow Board'); ?> - <?php echo xlt('Drug Screen Report'); ?></span>
<?php } else { ?>
<span class='title'><?php echo xlt('Client Flow Board Report'); ?></span>
<?php } ?>
...
<form method='post' name='theform' id='theform' action='client_flow_board_report.php' onsubmit='return top.restoreSession()'>
...
<table>
    <tr>
        ...
        <table class='text'>
            ...
            <tr>
            <td>
            &nbsp;&nbsp;<span class='text'><?php echo xlt('Client'); ?>: </span>
            </td>
            <td>
            <input type='text' size='20' name='form_client' class='form-control' style='cursor:pointer;cursor:hand' value='<?php echo ($form_client) ? attr($form_client) : xla('Click To Select'); ?>' onclick='sel_client()' title='<?php echo xla('Click to select client'); ?>' />
...
<table>
    ...
    <thead>
    <?php if (!$chk_show_drug_screens && !$chk_show_completed_drug_screens) { # the first part of this block is for the Client Flow Board report ?>
        ...
        <th><a href="nojs.php" onclick="return dosort('client')"
        <?php echo ($form_orderby == "client") ? " style=\"color:#00cc00\"" : ""; ?>>&nbsp;&nbsp;&nbsp;<?php  echo xlt('Client'); ?></a>
        </th>
    ...
    $appointments = fetchAppointments($from_date, $to_date, $client, $provider, $facility, $form_apptstatus, $with_out_provider, $with_out_facility, $form_apptcat, true);
    # sort the appointments by the appointment time
    $appointments = sortAppointments($appointments, $form_orderby);
    # $j is used to count the number of clients that match the selected criteria.
    $j=0;
    //print_r2($appointments);
    foreach ($appointments as $appointment) {
        $client_id = $appointment['pid'];
        ...
        #if a client id is entered just get that client.
        ...
        # $j is incremented for a client that made it for display.
        $j=$j+1;
        ?>

    <tr bgcolor='<?php echo attr($bgcolor); ?>'>
        <?php
        if (!$chk_show_drug_screens && !$chk_show_completed_drug_screens) { # the first part of this block is for the Client Flow Board report ?>
 ...
 <td class="detail" bgcolor='<?php echo attr($alert_color) ?>'><b>&nbsp;<?php echo text($tracker_time); ?></b></td><?php
    ...
    ?>
    <tr>
        <?php if (!$chk_show_drug_screens && !$chk_show_completed_drug_screens) { # is it Client Flow Board or Drug screen ?>
        <td colspan="10" align="left"><?php echo xlt('Total number of Client Flow Board entries'); ?>&nbsp;<?php echo text($j);?>&nbsp;<?php echo xlt('Clients'); ?></td>
        <?php } else { ?>
        <td colspan="10" align="left"><?php echo xlt('Total number of Drug Screen entries'); ?>&nbsp;<?php echo text($j);?>&nbsp;<?php echo xlt('Clients'); ?></td>
        <?php } ?>
    </tr>
    </tbody>
</table>
...
name="client" value="<?php echo attr($client) ?>" /> <input type='hidden'
