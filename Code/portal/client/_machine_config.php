...
if (isset($_SESSION['pid']) && (isset($_SESSION['client_portal_onsite_two']) || $_SESSION['register'] === true)) {
    $pid = $_SESSION['pid'];
    $ignoreAuth = true;
...
/**
 * the root url of the application with trailing slash, for example http://localhost/client/
 * default is relative base address
 */
if ($GLOBALS['portal_onsite_two_basepath']) {
    GlobalConfig::$ROOT_URL = RequestUtil::GetServerRootUrl() . preg_replace('/^\//', '', $GLOBALS['web_root']) . '/portal/client/';
} else {
    GlobalConfig::$ROOT_URL = $GLOBALS['web_root'] . '/portal/client/';
}
