 ...
 *
 * Originally culled from /interface/client_file/summary and adapted...
 *
 * @package   OpenEMR
...
if ($issue && !acl_check('clients', 'med', '', 'write')) {
    die(xlt("Edit is not authorized!"));
}

if (!acl_check('clients', 'med', '', array('write','addonly'))) {
    ...
    function sel_diagnosis() {
      ...
      term = f.form_title.value;
        ...
        if (($irow['type'] == 'PMH') ||($irow['type'] == 'POH')) {
            ?>
          dlgopen('../../client_file/encounter/find_code_popup.php?codetype=<?php echo attr(collect_codetypes("medical_problem", "csv")) ?>&search_term='+term, '_blank', 500, 400);
            <?php
        } else {
            ?>
          dlgopen('../../client_file/encounter/find_code_popup.php?codetype=<?php echo attr(collect_codetypes("diagnosis", "csv")) ?>&search_term='+term, '_blank', 500, 400);
            <?php
        }
        ?>
      f.form_save.focus();
        ...
        <table id="row_social" width="100%">
              ...
              <tbody>
                  <tr>
                    ...
                    <td colspan="3"><input type="text" style="width:75px;" name="marital_status" id="marital_status" value="<?php echo attr($client['status']); ?>">
                    &nbsp;<?php echo xlt('Occupation'); ?>:&nbsp;<input type="text" style="width:175px;" name="occupation" id="occupation" value="<?php echo attr($client['occupation']); ?>"></td>
                  </tr>
                  <tr>
                    <td></td>
                    <td>
