  ...
  var comp = '';
	...
	if(send_to == "printer" || send_to == "fax"){
		...
		if(send_to == "printer"){
			...
			window.open(WEB_ROOT+"/interface/client_file/encounter/print_report.php?"+url,"Print",'width=1000,height=800,resizable=yes,scrollbars=yes');
		...
		if(send_to == "fax"){
            ...
            if(combination == ''){
                $('.ap-st-st-12').fadeToggle();
                $('.activity_indicator').css({"display" :"none"});
				var resultTranslated = js_xl("Please select at least one client.");
                alert(resultTranslated.msg);
                return false;
            }
