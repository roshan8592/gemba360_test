...
// get client data for send alert
$db_client = cron_getPhoneAlertclientData($type, $before_trigger_hours);
echo "<br>" . htmlspecialchars(xl("Total Records Found") . ": " . count($db_client), ENT_QUOTES);

//Create a new instance of the phone service client
$client = new MaviqClient($phone_id, $phone_token, $phone_url);

for ($p=0; $p<count($db_client); $p++) {
    $prow =$db_client[$p];
...
// Purpose: update status yes if alert send to client
////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////
// Function:    cron_getPhoneAlertclientData
// Purpose: get client data for send to alert
////////////////////////////////////////////////////////////////////
function cron_getPhoneAlertclientData($type, $trigger_hours)
{

    //Added by Yijin 1/12/10 to handle phone reminders. Client needs to have hipaa Voice flag set to yes and a home phone
    if ($type=='Phone') {
    ...
    $client_field = "pd.pid,pd.title,pd.fname,pd.lname,pd.mname,pd.phone_cell,pd.email,pd.hipaa_allowsms,pd.hipaa_allowemail,pd.phone_home,pd.hipaa_voice,";
    $ssql .= " and (ope.pc_eventDate=?)";

    $query = "select $client_field pd.pid,ope.pc_eid,ope.pc_pid,ope.pc_title,
		...
		from
			openemr_postcalendar_events as ope ,client_data as pd
		where
			ope.pc_pid=pd.pid $ssql
		order by
			ope.pc_eventDate,ope.pc_endDate,pd.pid";

    $db_client = (sqlStatement($query, array($check_date)));
    $client_array = array();
    $cnt=0;
    while ($prow = sqlFetchArray($db_client)) {
        $client_array[$cnt] = $prow;
        $cnt++;
    }
...
{
    $client_info = $prow['title']." ".$prow['fname']." ".$prow['mname']." ".$prow['lname']."|||".$prow['phone_home'];

    $message = $phone_msg;

    $sql_loginsert = "INSERT INTO `notification_log` ( `iLogId` , `pid` , `pc_eid` , `message`, `type` , `client_info` , `smsgateway_info` , `pc_eventDate` , `pc_endDate` , `pc_startTime` , `pc_endTime` , `dSentDateTime` ) VALUES ";
    $sql_loginsert .= "(NULL , ?, ?, ?, 'Phone', ?, ?, ?, ?, ?, ?, ?)";
    $db_loginsert = ( sqlStatement($sql_loginsert, array($prow['pid'], $prow['pc_eid'], $message, $client_info, $phone_gateway, $prow['pc_eventDate'], $prow['pc_endDate'], $prow['pc_startTime'], $prow['pc_endTime'], date("Y-m-d H:i:s"))));
}
