...
#IfNotRow globals gl_name full_new_client_form
INSERT INTO globals ( gl_name, gl_index, gl_value ) VALUES ( 'full_new_client_form'       , 0, '3' );
#EndIf
#IfNotRow globals gl_name client_search_results_style
...
#IfMissingColumn client_data usertext11
ALTER TABLE client_data ADD usertext11 varchar(255) NOT NULL DEFAULT '';
#endIf
#IfMissingColumn client_data usertext12
ALTER TABLE client_data ADD usertext12 varchar(255) NOT NULL DEFAULT '';
#endIf
#IfMissingColumn client_data usertext13
ALTER TABLE client_data ADD usertext13 varchar(255) NOT NULL DEFAULT '';
#endIf
#IfMissingColumn client_data usertext14
ALTER TABLE client_data ADD usertext14 varchar(255) NOT NULL DEFAULT '';
#endIf
#IfMissingColumn client_data usertext15
ALTER TABLE client_data ADD usertext15 varchar(255) NOT NULL DEFAULT '';
#endIf
#IfMissingColumn client_data usertext16
ALTER TABLE client_data ADD usertext16 varchar(255) NOT NULL DEFAULT '';
#endIf
#IfMissingColumn client_data usertext17
ALTER TABLE client_data ADD usertext17 varchar(255) NOT NULL DEFAULT '';
#endIf
#IfMissingColumn client_data usertext18
ALTER TABLE client_data ADD usertext18 varchar(255) NOT NULL DEFAULT '';
#endIf
#IfMissingColumn client_data usertext19
ALTER TABLE client_data ADD usertext19 varchar(255) NOT NULL DEFAULT '';
#endIf
#IfMissingColumn client_data usertext20
ALTER TABLE client_data ADD usertext20 varchar(255) NOT NULL DEFAULT '';
#endIf
...
#IfMissingColumn client_data contrastart
ALTER TABLE client_data ADD contrastart DATE DEFAULT NULL;
#EndIf

#IfMissingColumn client_data ippfconmeth
ALTER TABLE client_data ADD ippfconmeth varchar(255) NOT NULL DEFAULT '';
#EndIf
...
#
# Create newmauser data item for each client contrastart that assigns the form_id and
...
INSERT INTO lbf_data (field_id, field_value)
  SELECT 'newmauser', pd.pid FROM client_data AS pd
  WHERE pd.contrastart IS NOT NULL AND pd.contrastart NOT LIKE '0%';
#
# Create newmethod data item for each such client that also has a starting method defined.
INSERT INTO lbf_data (form_id, field_id, field_value)
  SELECT MAX(ld.form_id), 'newmethod', pd.ippfconmeth
  FROM client_data AS pd, lbf_data AS ld
  WHERE pd.contrastart IS NOT NULL AND pd.contrastart NOT LIKE '0%'
  AND pd.ippfconmeth != ''
...
INSERT INTO forms (date, encounter, form_name, form_id, pid, user, groupname, authorized, formdir)
  SELECT CURRENT_DATE, fe.encounter, 'New Client Encounter', fe.id, pd.pid, 'admin', 'Default', '1', 'newclient'
  FROM client_data AS pd, form_encounter AS fe
  WHERE pd.contrastart IS NOT NULL AND pd.contrastart NOT LIKE '0%'
  AND fe.pid = pd.pid
  AND fe.date = pd.contrastart
  AND fe.reason = 'PreOpenEMR Data'
  AND (SELECT COUNT(*) FROM forms WHERE pid = pd.pid AND encounter = fe.encounter AND formdir = 'newclient' and deleted = 0) = 0;
#
...
INSERT INTO forms (date, encounter, form_name, form_id, pid, user, groupname, authorized, formdir)
  ...
  FROM client_data AS pd, form_encounter AS fe, lbf_data AS ld
  WHERE pd.contrastart IS NOT NULL AND pd.contrastart NOT LIKE '0%'
  AND fe.pid = pd.pid
  AND fe.date = pd.contrastart
  AND ld.field_id = 'newmauser'
...
ALTER TABLE client_data DROP ippfconmeth;
