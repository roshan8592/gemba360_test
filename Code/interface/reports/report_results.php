...
while ($row = sqlFetchArray($res)) {
  // Figure out the title and link
    if ($row['type'] == "cqm") {
        ...
        $type_title = xl('Processing Client Reminders');
        ...
        $type_title = xl('Processing and Sending Client Reminders');
        $link="../batchcom/batch_reminders.php?report_id=" . attr_url($row["report_id"]);
        ...
        $link="cqm.php?report_id=" . attr_url($row["report_id"]) . "&back=list";
    } else if ($row['type'] == "client_reminder") {
        if (!$GLOBALS['enable_cdr']) {
            continue;
        }

        $type_title = xl('Standard Measures (Client Reminders)');
        $link="cqm.php?report_id=" . attr_url($row["report_id"]) . "&back=list";
    } else {
...
<tr>
  ...
  <td align='center'><?php echo text(oeFormatDateTime($row["date_report"], "global", true)); ?></td>
    ...
    <?php } else { ?>
      <td align='center'><?php echo xlt("Pending") . " (" . text($row["progress_items"]) . " / " . text($row["total_items"]) . " " . xlt("Clients Processed") . ")"; ?></td>
    <?php } ?>
