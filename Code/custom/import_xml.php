<?php
/**
 * Imports client demographics from our custom XML format.
 *
 * @package OpenEMR
  ...
  // container in the tree structure.  $probeix is the current level.
    ...
    if (xml_parse_into_struct($parser, $_POST['form_import_data'], $xml)) {
        foreach ($xml as $taginfo) {
            ...
            if ($probeix == 1 && $probearr[$probeix] == 'client') {
                $aclient[$tag] = $tagval;
    ...
    newClientData(
        ...
        $aclient['phone_home'],
        $aclient['phone_biz'],
        $aclient['phone_contact'],
        $aclient['status'],
        $aclient['contact_relationship'],
        $aclient['referrer'],
        $aclient['referrerID'],
        $aclient['email'],
        $aclient['language'],
        $aclient['ethnoracial'],
        $aclient['interpreter'],
        $aclient['migrantseasonal'],
        $aclient['family_size'],
        $aclient['monthly_income'],
        $aclient['homeless'],
        fixDate($aclient['financial_review']),
        $aclient['pubpid'],
        $pid,
        $olddata['providerID'],
        $aclient['genericname1'],
        $aclient['genericval1'],
        $aclient['genericname2'],
        $aclient['genericval2'],
        $aclient['billing_note'],
        $aclient['phone_cell'],
        $aclient['hipaa_mail'],
        $aclient['hipaa_voice'],
        $olddata['squad']
...
<head>
    ...
    <title><?php echo xlt('Import Client Demographics XML'); ?></title>
</head>
