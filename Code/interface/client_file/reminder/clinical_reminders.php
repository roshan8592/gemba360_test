...
<div id='namecontainer_creminders' class='namecontainer_creminders' style='float:left;margin-right:10px'>
    <?php echo xlt('for');?>&nbsp;
  <span class="title">
    <a href="../summary/demographics.php" onclick="top.restoreSession()"><?php echo text(getClientName($pid)); ?></a>
  </span>
</div>
<div>
  <a href="../summary/demographics.php" class="css_button" onclick="top.restoreSession()">
    <span><?php echo xlt('Back To Client');?></span>
  </a>
</div>
        ...
        <?php foreach ($plans_default as $plan) { ?>
          ...
          <tr>
            ...
            <td align="center">
                ...
                $client_plan = collect_plan($plan['id'], $client_id);

              // Set the client specific setting for gui
                if (empty($client_plan)) {
                    $select = "default";
                } else {
                    if ($client_plan['normal_flag'] == "1") {
                        $select = "on";
                    } else if ($client_plan['normal_flag'] == "0") {
                        $select = "off";
                    } else { // $client_rule['normal_flag'] == NULL
                        $select = "default";
                    }
                } ?>
            ...
            <td align="center">
              ...
              // Set the client specific setting for gui
                if (empty($client_rule)) {
                    ...
                    if ($client_rule['passive_alert_flag'] == "1") {
                        $select = "on";
                    } else if ($client_rule['passive_alert_flag'] == "0") {
                        $select = "off";
                    } else { // $client_rule['passive_alert_flag'] == NULL
                        $select = "default";
                    }
                } ?>
            ...
            <td align="center">
              ...
              // Set the client specific setting for gui
                if (empty($client_rule)) {
                    ...
                    if ($client_rule['active_alert_flag'] == "1") {
                        $select = "on";
                    } else if ($client_rule['active_alert_flag'] == "0") {
                        $select = "off";
                    } else { // $client_rule['active_alert_flag'] == NULL
                        $select = "default";
                    }
                } ?>
