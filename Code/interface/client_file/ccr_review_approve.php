<?php
/**
 * interface/client_file/ccr_review_approve.php Approval screen for uploaded CCR XML.
 *
...
<body class="body_top" >
<center>
<p><b><?php echo xlt('CCR Client Review');?></b></p>
</center>
                    ...
                    <tr>
                        <?php
                            $query_pd = sqlStatement("SELECT ad.id as adid, table_name, field_name, field_value FROM audit_master am JOIN audit_details ad ON ad.audit_master_id = am.id
								WHERE am.id = ? AND ad.table_name = 'client_data' ORDER BY ad.id", array($_REQUEST['amid']));
                            $i = 0;
                            while ($res_pd = sqlFetchArray($query_pd)) {
                                if ($res_pd['field_name'] != 'lname' && $res_pd['field_name'] != 'fname' && $res_pd['field_name'] != 'DOB') {
                                    $i++;
                                    $query_oldpd = sqlQuery("SELECT " . escape_sql_column_name($res_pd['field_name'], array("client_data")) . " AS val FROM client_data WHERE pid = ?", array($_REQUEST['pid']));
                                    if ($res_pd['field_name'] == 'sex') {
                                        echo "<td>" . ($client_data[$res_pd['field_name']] ? text($client_data[$res_pd['field_name']]): text($res_pd['field_name'])) . "</td>" .
        ...
        <tr>
            <td>
                <table border="0" width="95%" >
                    <tr>
                        <?php
                            $query_existing_medications = sqlStatement("SELECT * FROM prescriptions WHERE client_id = ?", array($_REQUEST['pid']));
                            $result = array();
        ...
        <tr>
            <td>
                <table border="0" width="95%" >
                    <tr>
                        <?php
                            $query_existing_immunizations = sqlStatement("SELECT * FROM immunizations WHERE client_id = ? AND added_erroneously = 0", array($_REQUEST['pid']));
                            $result = array();
                    ...
                    <tr>
                        <?php
                            $query_existing_lab_results = sqlStatement("SELECT * FROM procedure_order AS po LEFT JOIN procedure_order_code AS poc
								...
								LEFT JOIN procedure_result AS prs ON prs.procedure_report_id = pr.procedure_report_id WHERE client_id = ?", array($_REQUEST['pid']));
                            $result = array();
