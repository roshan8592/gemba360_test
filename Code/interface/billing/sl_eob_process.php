...
function era_callback(&$out)
{
    ...
    if (isset($_REQUEST['chk'.$chk_123])) {
        ...
        if ($pid && $encounter) {
            ...
            "FROM form_encounter AS e, client_data AS p WHERE " .
    ...
    // Simplify some claim attributes for cleaner code.
        ...
        if (empty($ferow['lname'])) {
              $client_name = $out['client_fname'] . ' ' . $out['client_lname'];
        } else {
            $client_name = $ferow['fname'] . ' ' . $ferow['lname'];
        }
        ...
        foreach ($out['svc'] as $svc) {
          ...
          // This key will then make its way into SQL-Ledger.
            ...
            if ($prev) {
                ...
                writeOldDetail($prev, $client_name, $invnumber, $service_date, $codekey, $bgcolor);
            ...
            // must be 25 characters or less in order to fit on client statements.
            foreach ($svc['adj'] as $adj) {
                $description = $adj['reason_code'] . ': ' .
                    BillingUtilities::claim_adjustment_reason_codes[$adj['reason_code']];
                if ($adj['group_code'] == 'PR' || !$primary) {
                    // Group code PR is Client Responsibility.  Enter these as zero
    ...
    // (if so, then insurance is not yet done with the claim).
        ...
        foreach ($codes as $code => $prev) {
          // writeOldDetail($prev, $arrow['name'], $invnumber, $service_date, $code, $bgcolor);
            writeOldDetail($prev, $client_name, $invnumber, $service_date, $code, $bgcolor);
            $got_response = false;
