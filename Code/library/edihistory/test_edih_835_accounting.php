...
function edih_835_accounting($segments, $delimiters)
{
     ...
     * $out['amount_client']    = trim($seg[5]); // pt responsibility, copay + deductible
     ...
     * // QC = Client
            ...
            $out['client_lname']     = trim($seg[3]);
            $out['client_fname']     = trim($seg[4]);
            $out['client_mname']     = trim($seg[5]);
            $out['client_member_id'] = trim($seg[9]);
        }
    ...
    foreach ($segments as $seg) {
        ...
        if (strncmp('CLP'.$de, $seg, 4) === 0) {
            ...
            //
            // This is the poorly-named "Client Account Number".  For 837p
            ...
            $out[$ck]['clp'][$i]['amount_client']  = (isset($sar[5]) && $sar[5]) ? trim($sar[5]) : ""; // pt responsibility, copay + deductible
        ...
        if (strncmp('CAS'.$de, $seg, 4) === 0) {
            ...
            if ($id == 'QC') {
                // QC Client
                $out[$ck]['clp'][$i]['client_lname'] = (isset($sar[3]) && $sar[3]) ? trim($sar[3]) : "";
                $out[$ck]['clp'][$i]['client_fname'] = (isset($sar[4]) && $sar[4]) ? trim($sar[4]) : "";
                $out[$ck]['clp'][$i]['client_mname'] = (isset($sar[5]) && $sar[5]) ? trim($sar[5]) : "";
                $out[$ck]['clp'][$i]['client_member_id'] = (isset($sar[9]) && $sar[9]) ? trim($sar[9]) : "";
            } elseif ($id == 'IL') {
                // IL = Insured or Subscriber
