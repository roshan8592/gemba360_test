...
$reviewauth = acl_check('clients', 'sign');
...
if ($_GET['set_pid'] && $form_review) {
  ...
  <script language='JavaScript'>
    parent.left_nav.setClient(<?php echo js_escape($result['fname'] . " " . $result['lname']) . "," . js_escape($pid) . "," . js_escape($result['pubpid']) . ",''," . js_escape(" " . xl('DOB') . ": " . oeFormatShortDate($result['DOB_YMD']) . " " . xl('Age') . ": " . getClientAge($result['DOB_YMD'])); ?>);
  </script>
    <?php
}

if (!$form_batch && !$pid && !$form_review) {
    die(xlt('There is no current client'));
}
...
<table width='100%' cellpadding='1' cellspacing='2'>
 ...
 <tr class='head'>
  <td colspan='2'><?php echo $form_batch ? xlt('Client') : xlt('Order'); ?></td>
