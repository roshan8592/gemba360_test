...
//During auditing if a new client demo is rejected will delete the client from DB

    public function delete_if_new_client($var)
    {
        ...
        if (UserService::valid($data_credentials)=='oemruser') {
            ...
            if ($rowfield['pid']>0) {
                $pid=$rowfield['pid'];
                $qry = "DELETE from  client_data WHERE pid=?";
                ...
                sqlStatement($qry, array($pid));
                $qry = "DELETE from client_access_offsite WHERE  pid=? ";
                sqlStatement($qry, array($pid));
                $qry = "DELETE from openemr_postcalendar_events WHERE  pc_pid=? ";// appointments approved, but client denied case.
                sqlStatement($qry, array($pid));
...
//Appointments and Demos are updated from the audit_details table to the actual transaction tables
    public function update_audited_data($var)
    {
          $data_credentials=$var[0];
                ...
                $validtables = array("client_data","employer_data","insurance_data","history_data","openemr_postcalendar_events","ar_session","documents_legal_master","documents_legal_detail","client_access_offsite");
        if (UserService::valid($data_credentials)) {
            $audit_master_id = $var['audit_master_id'];
            ...
            while ($row = sqlFetchArray($res)) {
                ...
                if ($cnt>0) {
                    while ($rowfield = sqlFetchArray($resfield)) {
                        ...
                        if ($table=='client_access_offsite') {
                            $newdata['client_access_offsite'][$rowfield['field_name']]=$rowfield['field_value'];
                            if ($rowfield['field_name'] == 'portal_pwd') {
                                $newdata['client_access_offsite']['pass_id']=$rowfield['id'];
                            }
                        }
                    }
                    ...
                    if ($table=='client_data') {
                        $pdrow = sqlQuery("SELECT id from client_data WHERE pid=?", array($pid));
                        if ($pdrow['id']) {
                            $newdata['client_data']['id'] = $pdrow['id'];
                            updateClientData($pid, $newdata['client_data']);
                        } else {
                            $prow = sqlQuery("SELECT IFNULL(MAX(pid)+1,1) AS pid FROM client_data");
                            $pid = $prow['pid'];
                            $newdata['client_data']['pubpid'] = $pid;
                        ...
                        sqlStatement("INSERT INTO ar_session ( " .
                         "payer_id, user_id, reference, check_date, pay_total, modified_time, payment_type, description, post_to_date, client_id, payment_method" .
                         ") VALUES ( " .
                        ...
                        if ($master_doc_id) {
                            ...
                            $mdoc_id = $newdata['documents_legal_detail']['dld_master_docid'];
                                ...
                                "dld_pid,dld_facility,dld_provider,dld_encounter,dld_master_docid,dld_signed,dld_signed_time,dld_filepath,dld_filename,dld_signing_person,dld_sign_level,dld_content,dld_file_for_pdf_generation,dld_denial_reason,dld_moved,dld_client_comments" .
                                ") VALUES ( " .
                                "'" . add_escape_custom($pid) . "', " .
                                ...
                                "'" . add_escape_custom($newdata['documents_legal_detail']['dld_client_comments']) . "')");
                    } elseif ($table=='client_access_offsite') {
                        $query = sqlStatement("SELECT * FROM client_access_offsite WHERE portal_username = '".$newdata['client_access_offsite']['portal_username']."'");
                        if (sqlNumRows($query) == 0) {
                            sqlStatement("INSERT INTO client_access_offsite ( " .
                              "pid,portal_username,portal_pwd,portal_pwd_status" .
                              ") VALUES ( " .
                              "'" . add_escape_custom($pid) . "', " .
                              "'" . add_escape_custom($newdata['client_access_offsite']['portal_username']) . "', " .
                              "'" . add_escape_custom($newdata['client_access_offsite']['portal_pwd']) . "', ".
                              "0)");
                            sqlQuery("UPDATE audit_details SET field_value = ? WHERE id = ?", array($this->generatePassword(),$newdata['client_access_offsite']['pass_id']));
                        }
                    }
                } else {
...
//according to the facility settings. audit_details is the child table of Audit_master
    ...
    public function insert_to_be_audit_data($var)
    {
        ...
        if (UserService::valid($data_credentials)) {
            ...
            if ((UserService::valid($data_credentials) == 'newclient' || UserService::valid($data_credentials) == 'newclienttoapprove') && $approval_status == 1) {
                $pid = 0;
            }
