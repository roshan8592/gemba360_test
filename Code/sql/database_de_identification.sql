...
#Filter the clients to include in the report, based on the drugs,immunizations, and diagnosis selected. 
call filter_pid();

#For each client, and table/column name to include in the report,select the data from the appropriate tables, and insert into the de_identified_data table.  Skip any tables/columns containing identifiers (names, telephone, etc). 
...
#temp_client_id : The list of clients to include in this report.
#temp_re_identification : Contains a re-identification code for each client.
#temp_client_record_id : A temporary table, contains the primary id of the record corresponding to a client.
#param_include_tables : Contains the tables/columns to include in this report.
#param_filter_pid : Contains the drugs/immunizations/diagnosis for filtering which clients to include 
...
DROP TABLE IF EXISTS temp_client_id_table;
create table temp_client_id_table (pid varchar(10));
...
DROP TABLE IF EXISTS temp_client_record_id;
create table temp_client_record_id(number int auto_increment, id int not null, key(number));
...
#The lexical_look_up_table is used to store the text of known client identifiers, such as client names (John Smith), telephone numbers (408-111-222), etc.  Later on, during the identification process, these text snippets will be removed from unstructured data, such as client notes. 
...
insert into transaction_metadata_de_identification (table_name,col_name,include_in_de_identification) select table_name, col_name, include_in_de_identification from metadata_de_identification where table_name = "client_data" || table_name = "history_data" || table_name = "lists" || table_name = "immunizations" || table_name = "prescriptions" || table_name = "transactions" || table_name = "insurance_data" || table_name = "billing" || table_name = "payments";
ELSE
SET noWords=LENGTH(include_tables) - LENGTH(REPLACE(include_tables, '#', '')) + 1;
SET include_tables = CONCAT(include_tables,'#');
insert into transaction_metadata_de_identification (table_name,col_name,include_in_de_identification) select table_name, col_name, include_in_de_identification from metadata_de_identification where table_name = "client_data";
WHILE( noWords ) do
...
#This creates a table (de_identified_data) containing all the client data to be included in the report.  Each table/column that is included in this report (such as history_data/tobacco) will have a corresponding column in the de_identified_data.  
...
BEGIN
#Retrieve a list of client ids that satisfy the selections picked in the de-identification Input screen.
#  The table param_filter_pid contains the parameters (start/end date, diagnosis, drugs, immunizations)
#for filter out which clients to select.  Store the selected client ids in the temp_client_id_table 
...
delete from temp_client_id_table;
insert into temp_client_id_table (pid) select pid from client_data;
#In parameter individual values are separated by '#'  
...
END IF;
DELETE FROM temp_client_id_table where pid NOT IN (SELECT pid FROM t1);
DELETE FROM t1;
...
SET subString = RTRIM(subString);
SET subString = SUBSTRING_INDEX(subString, '-', -1);
insert into t1 (pid) select client_id from prescriptions where drug = subString and start_date >= startDate and start_date <= endDate;
...
ELSE
insert into t1 (pid) select client_id from prescriptions where start_date >= startDate and start_date <= endDate;
...
SET subString = RTRIM(subString);
SET subString = SUBSTRING_INDEX(subString, '-', 1);
insert into t1 (pid) select client_id from immunizations where immunization_id = subString and administered_date >= startDate and administered_date <= endDate;
set noWords = noWords -1;
end while;
ELSE
insert into t1 (pid) select client_id from immunizations where administered_date >= startDate and administered_date <= endDate;
END IF;
DELETE FROM temp_client_id_table where pid NOT IN (SELECT pid FROM t1);
DELETE FROM t1;
...
begin
#In table de_identified_data, remove any empty columns (columns that contain an empty value, for every client). 
DECLARE done INT DEFAULT 0;
...
BEGIN
#Given some unstructured data (like client notes), replace any urls, dates, or names in the data with 'xxx'.  Then return the modified data. 
...
#The temp_client_id_table contains the list of clients to gather data for.
...
DECLARE client_id_cursor CURSOR FOR SELECT pid from temp_client_id_table;
...
FETCH client_id_cursor into clientId;

WHILE (done = 0) DO



#If the client id has no re-identification code associated with it, then create a new re-identification code using UUID() and assign it to the client id (store it in re_identification_code_data table)

IF(!( select count(*) from re_identification_code_data where pid = clientId)) THEN

insert into re_identification_code_data values (clientId, uuid());
...
TRUNCATE temp_client_record_id;

if (tableName = 'prescriptions' || tableName = 'immunizations') then

   SET @v = CONCAT("insert into temp_client_record_id (id) select id from ", tableName," where client_id = ", clientId);

   else SET @v = CONCAT("insert into temp_client_record_id (id) select id from ", tableName," where pid = ", clientId);
...
END IF;
  ...
  SELECT count(*) FROM temp_client_record_id into recordCount;
  ...
  select count(*) from de_identified_data where number = recordNumber and sub_number = recordCount AND re_identification_code = (select re_identification_code from re_identification_code_data where pid = clientId) into insertFlag;
  ...
  insert into de_identified_data (number,sub_number,re_identification_code) values (recordNumber,recordCount, (select re_identification_code from re_identification_code_data where pid = clientId));
  ...
  SELECT id FROM temp_client_record_id where number = recordCount into recordId;



#Case 4 :unstructured data(eg:client notes) perform lexical analysis - replace any identifying text (name, telephone, etc) with xxx
...
# Note that a single client can have multiple row entries in the de_identified_data.

# That is because a single client can have multiple entries for prescriptions, immunizations, etc.
...
update re_identified_data set pid = "client id" where number = 0;
...
select pid from re_identification_code_data where re_identification_code = reIdentificationCode INTO clientId;
update re_identified_data set pid = clientId where number = recordNumber;
...
FETCH metadata_cursor INTO colName, tableName;
   ...
   SET @v = CONCAT("update re_identified_data set ", colName, " = ( select ",colName," from ", tableName, "  where pid = ",clientId," ) where number = ",recordNumber );
       PREPARE stmt1 FROM @v;
       EXECUTE stmt1;
...
-- Table structure for table `de_identification_status`insert into re_identification_code_data values (clientId, uuid());
-- Table added to accomplish HIPAA De-identification
