<?php
/**
 * Remote access to a WordPress Client Portal.
 *
...
{
    $portal_url = $GLOBALS['gbl_portal_cms_address'] . "/wp-content/plugins/sunset-client-portal/webserve.php";
...
// Look up the OpenEMR client matching this request. More or less than 1 is an error.
function lookup_openemr_client($wp_login)
{
    if (empty($wp_login)) {
        die(xlt('The client was not logged in when submitting this form'));
    }

    $ptres = sqlStatement("SELECT pid FROM client_data WHERE cmsportal_login = ?", array($wp_login));
    if (sqlNumRows($ptres) < 1) {
        die(xlt('There is no client with portal login') . " '$wp_login'");
    }

    if (sqlNumRows($ptres) > 1) {
        die(xlt('There are multiple clients with portal login') . " '$wp_login'");
    }
