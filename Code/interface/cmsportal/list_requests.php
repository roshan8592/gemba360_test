...
// Get client name from OpenEMR, or empty if not there.
function clientNameFromLogin($login)
{
    $ptname = '';
    if ($login) {
        $tmp = sqlQuery(
            "SELECT fname, lname, mname, pid " .
            "FROM client_data WHERE cmsportal_login = ? ORDER BY id LIMIT 1",
            array($login)
        );
        if (!empty($tmp['pid'])) {
              $ptname = $tmp['lname'];
...
function openRequest(postid, type) {
 ...
 if (type.indexOf('Demographics') == 0) {
  document.location.href = 'client_select.php?postid=' + postid + '&csrf_token_form=<?php echo attr_url(CsrfUtils::collectCsrfToken()); ?>';
 } else
...
<table width='100%' cellpadding='1' cellspacing='2'>
 ...
 <tr class='head'>
  <th colspan='2'><?php echo xlt('Client'); ?></td>
...
while ($v1 || $v2) {
    ...
    if (!$v2 || $v1 && $v1[1]['datetime'] < $v2[1]['datetime']) {
        $postid = $v1[1]['postid'];
        $ptname = clientNameFromLogin($v1[1]['user']);
        ...
        $ptname = clientNameFromLogin($v2[1]['user']);
        ...
        echo "  <td>" . text($v2[1]['user'] == $v2[1]['fromuser'] ?
         xl('Message from client') : xl('Message to client')) . "</td>\n";
