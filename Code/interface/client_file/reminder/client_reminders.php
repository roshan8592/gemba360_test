...
<head>
    ...
    <title><?php echo xlt("Client Reminders"); ?></title>
...
if (!empty($client_id)) {
    //Only update one client
    $update_rem_log = update_reminders('', $client_id);
}

if ($mode == "simple") {
    // Collect the rules for the per client rules selection tab
...
<script language="javascript">
    ...
    function setclient(pid, lname, fname, dob) {
        ...
        f.client_id.value = pid;
    }

    // This invokes the find-client popup.
    function sel_client() {
        dlgopen('../../main/calendar/find_client_popup.php', '_blank', 500, 400);
    }
</script>
<?php
...
if ($mode == "simple") {
        $arrOeUiSettings = array(
            'heading_title' => xl('Client Reminders'),
            'include_client_name' => true,
            'expandable' => true,
            'expandable_files' => array('client_reminders_client_xpd'),//all file names need suffix _xpd
    ...
    $arrOeUiSettings = array(
            ...
            'expandable' => true,
            'expandable_files' => array('client_reminders_xpd'),//all file names need suffix _xpd
        ...
        <div class="row">
            <div class="col-sm-12">
                ...
                for ($i = 0; $i < count($sort); $i++) {
                    $sortlink[$i] = "<a class='arrowhead' href=\"client_reminders.php?client_id=" . attr_url($client_id) ."&mode=" . attr_url($mode) . "&sortby=" . attr_url($sort[$i]) . "&sortorder=asc\" onclick=\"top.restoreSession()\" title ='" . xla('Sort Up') . "'>" .
                ...
                for ($i = 0; $i < count($sort); $i++) {
                    if ($sortby == $sort[$i]) {
                        switch ($sortorder) {
                            case "asc":
                                $sortlink[$i] = "<a class='arrowhead' href=\"client_reminders.php?client_id=" . attr_url($client_id) . "&mode=" . attr_url($mode) . "&sortby=" . attr_url($sortby) . "&sortorder=desc\" onclick=\"top.restoreSession()\" title ='" . xla('Sort Up') . "'>" .
                                          "<i class='fa fa-sort-asc fa-lg' aria-hidden='true'></i></a>";
                                break;
                            case "desc":
                                $sortlink[$i] = "<a class='arrowhead' href=\"client_reminders.php?client_id=" . attr_url($client_id) . "&mode=" . attr_url($mode) . "&sortby=" . attr_url($sortby) . "&sortorder=asc\" onclick=\"top.restoreSession()\" title ='" . xla('Sort Down') . "'>" .
                                          "<i class='fa fa-sort-desc fa-lg' aria-hidden='true'></i></a>";
                                break;
                        } break;
                    }
                ...
                // This is for managing page numbering and display beneath the Client Reminders table.
                $listnumber = 25;
                $sqlBindArray = array();
                if (!empty($client_id)) {
                    $add_sql = "AND a.pid=? ";
                    array_push($sqlBindArray, $client_id);
                }

                $sql = "SELECT a.id, a.due_status, a.category, a.item, a.date_created, a.date_sent, b.fname, b.lname " .
                  "FROM `client_reminders` as a, `client_data` as b " .
                ...
                if ($prev >= 0) {
                    $prevlink = "<a href=\"client_reminders.php?client_id=" . attr_url($client_id) . "&mode=" . attr_url($mode) . "&sortby=" . attr_url($sortby) . "&sortorder=" . attr_url($sortorder) . "&begin=" . attr_url($prev) . "\" onclick=\"top.restoreSession()\"><<</a>";
                } else {
                    $prevlink = "<<";
                }

                if ($next < $total) {
                    $nextlink = "<a href=\"client_reminders.php?client_id=" . attr_url($client_id) . "&mode=" . attr_url($mode) . "&sortby=" . attr_url($sortby) . "&sortorder=" . attr_url($sortorder) . "&begin=" . attr_url($next) . "\" onclick=\"top.restoreSession()\">>></a>";
                } else {
                    $nextlink = ">>";
                }
                ?>

                <?php if ($mode == "simple") { // show the per client rule setting option ?>
                  <ul class="tabNav">
                          ...
                          <tr>
                            <td>
                              <div style='margin-left:15px'>
                                ...
                                <?php } else { ?>
                                <a href='client_reminders.php?client_id=<?php echo attr_url($client_id); ?>&mode=<?php echo attr_url($mode); ?>' class='css_button' onclick='top.restoreSession()'>
                ...
                <div id='report_results'>
                    <table>
                      <thead>
                        ...
                        <th><?php echo xlt('Client') . " " . $sortlink[1]; ?></th>
                ...
                foreach ($escapedsortby as $key => $columnName) {
                    $escapedsortby[$key] = escape_sql_column_name(trim($columnName), array('client_reminders','client_data'));
                }
                    ...
                    <div id='report_results'>
                      <table>
                        <tr>
                          ...
                          <th colspan="2"><?php echo xlt('Client Reminder'); ?></th>
                        </tr>
                        <tr>
                          <th><?php echo xlt('Client Setting'); ?></th>
                        ...
                        <?php foreach ($rules_default as $rule) { ?>
                          <tr>
                            ...
                            <td align="center">
                                <?php
                                $client_rule = collect_rule($rule['id'], $client_id);
                              // Set the client specific setting for gui
                                if (empty($client_rule)) {
                                    $select = "default";
                                } else {
                                    if ($client_rule['client_reminder_flag'] == "1") {
                                        $select = "on";
                                    } elseif ($client_rule['client_reminder_flag'] == "0") {
                                        $select = "off";
                                    } else { // $client_rule['client_reminder_flag'] == NULL
                                        $select = "default";
                                    }
                                } ?>
                              <select class="client_reminder" name="<?php echo attr($rule['id']); ?>">
                            ...
                            <td align="center" style="border-right:1px solid black;">
                                <?php
                                if ($rule['client_reminder_flag'] == "1") {
                                    echo xlt('On');
                                } else {
                                    echo xlt('Off');
                                }
...
$(document).ready(function(){
  ...
  $(".client_reminder").change(function() {
    top.restoreSession();
    $.post( "../../../library/ajax/rule_setting.php", {
      rule: this.name,
      type: 'client_reminder',
      setting: this.value,
      client_id: <?php echo js_escape($client_id); ?>,
 ...
 // Show a template popup of client reminders batch sending tool.
   ...
   $.post("../../../library/ajax/status_report.php",
     ...
     function(data){
       if (data == "PENDING") {
         ...
         top.restoreSession();
         link_report = "client_reminders.php?mode=admin&client_id=";
