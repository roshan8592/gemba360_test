...
<div id='docdiv' <?php echo $display_div; ?>>
    ...
    $query = "SELECT d.*,CONCAT(pd.fname,' ',pd.lname) AS pname,GROUP_CONCAT(n.note ORDER BY n.date DESC SEPARATOR '|') AS docNotes,
		...
		INNER JOIN client_data pd ON d.foreign_id = pd.pid
    ...
    <tr class='text bold'>
        ...
        <th align="left" width="20%"><?php echo xlt('Client'); ?></th>
    ...
    if (sqlNumRows($resultSet)) {
        while ($row = sqlFetchArray($resultSet)) {
            $url = $GLOBALS['webroot'] . "/controller.php?document&retrieve&client_id=" . attr_url($row["foreign_id"]) . "&document_id=" . attr_url($row["id"]) . '&as_file=false';
