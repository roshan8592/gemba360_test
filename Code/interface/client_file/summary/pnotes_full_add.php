...
<div>
    <div id="pnotes_title">
        <span class="title"><?php echo xlt('Client Message') . text($title_docname); ?></span>
    </div>
    <div>
        <?php if ($noteid) { ?>
            <!-- existing note -->
...
if ($noteid) {
    $body = $prow['body'];
    $body = preg_replace(array('/(\sto\s)-client-(\))/', '/(:\d{2}\s\()' . $client_id . '(\sto\s)/'), '${1}' . $clientname . '${2}', $body);
...
if ($_GET['set_pid']) {
    ...
    ?>
 parent.left_nav.setClient(<?php echo js_escape($ndata['fname']." ".$ndata['lname']) . "," . js_escape($client_id) . "," . js_escape($ndata['pubpid']) . ",window.name"; ?>);
    <?php
}
