 ...
 * Checkout Module.
 *
 * This module supports a popup window to handle client checkout
...
$client_id = empty($_GET['ptid']) ? $pid : 0 + $_GET['ptid'];

// Get the client's name and chart number.
$patdata = getClientData($client_id, 'fname,mname,lname,pubpid,street,city,state,postal_code');
...
// Generate a receipt from the last-billed invoice for this client,
// or for the encounter specified as a GET parameter.
//
function generate_receipt($client_id, $encounter = 0)
{
 //REMEMBER the entire receipt is generated here, have to echo DOC type etc and closing tags to create a valid webpsge
    ...
    $patdata = getClientData($client_id, 'fname,mname,lname,pubpid,street,city,state,postal_code,providerID');
    ...
    if ($encounter) {
        ...
        "WHERE pid = ? AND encounter = ?", array($client_id,$encounter));
    } else {
        $ferow = sqlQuery("SELECT id, date, encounter, provider_id FROM form_encounter " .
        "WHERE pid = ? " .
        "ORDER BY id DESC LIMIT 1", array($client_id));
    }
    if (empty($ferow)) {
        die(xlt("This client has no activity."));
    }
    $trans_id = $ferow['id'];
    ...
    "pid = ? AND encounter = ? LIMIT 1", array($client_id,$encounter));
    ...
    ?>
        ...
        function deleteme() {
         dlgopen('deleter.php?billing=' + <?php echo js_url($client_id.".".$encounter); ?> + '&csrf_token_form=' + <?php echo js_url(CsrfUtils::collectCsrfToken()); ?>, '_blank', 500, 450);
         return false;
        }
        ...
        <title><?php echo xlt('Client Checkout'); ?></title>
                    ...
                    <table class="table">
                        ...
                        <?php
                          $charges = 0.00;
                            ...
                            $inres = sqlStatement("SELECT s.sale_id, s.sale_date, s.fee, " .
                              ...
                              // "WHERE s.pid = '$client_id' AND s.encounter = '$encounter' AND s.fee != 0 " .
                              "WHERE s.pid = ? AND s.encounter = ? " .
                              "ORDER BY s.sale_id", array($client_id,$encounter));
                        while ($inrow = sqlFetchArray($inres)) {
                            ...
                            $inres = sqlStatement("SELECT * FROM billing WHERE " .
                              ...
                              "ORDER BY id", array($client_id,$encounter));
                        while ($inrow = sqlFetchArray($inres)) {
                            $charges += sprintf('%01.2f', $inrow['fee']);
                            receiptDetailLine(
                                $svcdate,
                            ...
                            $inres = sqlStatement("SELECT " .
                              ...
                              "a.adj_amount != 0 " .
                              "ORDER BY s.check_date, a.sequence_no", array($client_id,$encounter));
                        while ($inrow = sqlFetchArray($inres)) {
                ...
                <div class="form-group clearfix">
                    <div class="col-sm-12 text-center" id="hideonprint">
                        <div class="btn-group" role="group">
                            ...
                            <?php } ?>
                            <?php if ($details) { ?>
                                <button class="btn btn-default btn-hide" onclick="top.restoreSession(); window.location.href = 'pos_checkout.php?details=0&ptid=<?php echo attr_url($client_id); ?>&enc=<?php echo attr_url($encounter); ?>'"><?php echo xlt('Hide Details'); ?></button>
                            <?php } else { ?>
                                <button class="btn btn-default btn-show" onclick="top.restoreSession(); window.location.href = 'pos_checkout.php?details=1&ptid=<?php echo attr_url($client_id); ?>&enc=<?php echo attr_url($encounter); ?>'"><?php echo xlt('Show Details'); ?></button>
                            <?php } ?>
                        </div>
                    </div>
                </div>
...
?>
        ...
        if ($_POST['form_amount']) {
            ...
            if ($RowSearch = sqlFetchArray($ResultSearchNew)) {
              ...
              $session_id=sqlInsert(
                  ...
                  " VALUES ('0',?,?,now(),?,?,'','client','COPAY',?,?,'client_payment',now())",
    ...
    //
    if (!empty($_GET['enc'])) {
        generate_receipt($client_id, $_GET['enc']);
        exit();
    }

    // Get the unbilled billing table items for this client.
    ...
    $bres = sqlStatement($query, array($client_id));

    // Get the product sales for this client.
    ...
    $dres = sqlStatement($query, array($client_id));
    ...
    // List unbilled billing items (cpt, hcpcs, copays) for the client.
    // List unbilled product sales for the client.
...
<!DOCTYPE html>
    ...
    $arrOeUiSettings = array(
        'heading_title' => xl('Client Checkout'),
            ...
            <div class="row">
                <div class="col-sm-12">
                    <form action='pos_checkout.php' method='post'>
                        ...
                        <input name='form_pid' type='hidden' value='<?php echo attr($client_id) ?>'>
                        <fieldset>
                            ...
                            <div class= "table-responsive">
                                <table class = "table">
                                    ...
                                    // Process copays
                                    //
                                    $totalCopay = BillingUtilities::getClientCopay($client_id, $encounter);
                                    if ($totalCopay < 0) {
                                    ...
                                    if ($inv_encounter) {
                                        ...
                                        "pid = ? AND encounter = ? " .
                                        "ORDER BY id DESC LIMIT 1", array($client_id,$inv_encounter));
