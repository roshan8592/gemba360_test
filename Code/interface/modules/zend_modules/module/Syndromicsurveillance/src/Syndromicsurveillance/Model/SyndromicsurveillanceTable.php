...
class SyndromicsurveillanceTable extends AbstractTableGateway
{
    ...
    */
    ...
    /*
    * Fetch the list of clients having the reportable ICD9
    *
    ...
    *
    * @return   records         array       return the list of clients having the reportable ICD9 codes
    * @return   count           integer     return the count of clients having the reportable ICD9 codes
    */
    function fetch_result($fromDate, $toDate, $code_selected, $provider_selected, $start, $end, $get_count = null)
    {
        ...
        $query = "SELECT   c.code_text,l.pid AS clientid,p.language,l.diagnosis,CONCAT(p.fname, ' ', p.mname, ' ', p.lname) AS clientname,l.date AS issuedate, l.id AS issueid,l.title AS issuetitle 
			FROM
			  lists l, client_data p, codes c, form_encounter AS fe 
        ...
        $query .= " AND l.diagnosis LIKE 'ICD9:%' 
					...
					AND fe.pid = l.pid 
				UNION DISTINCT 
				SELECT c.code_text, b.pid AS clientid, p.language, b.code, CONCAT(p.fname, ' ', p.mname, ' ', p.lname) AS clientname, b.date AS issuedate,  b.id AS issueid, '' AS issuetitle 
				FROM
					billing b, client_data p, codes c, form_encounter fe 
				WHERE c.reportable = 1 
    ...
    */
    function generate_hl7($fromDate, $toDate, $code_selected, $provider_selected, $start, $end)
    {
        ...
        $query = "SELECT   c.code_text,l.pid AS clientid,p.language,l.diagnosis,
			...
			concat(p.fname, '^',p.mname,'^', p.lname) as clientname, l.id AS issueid,l.title AS issuetitle,fac.name,fac.facility_npi,p.race,p.ethnicity,p.postal_code,fe.encounter
			FROM
			  lists l, client_data p, codes c, form_encounter AS fe
        ...
        $query .= " AND l.diagnosis LIKE 'ICD9:%' 
				...
				AND fe.pid = l.pid 
			  UNION DISTINCT 
			  SELECT c.code_text, b.pid AS clientid, p.language, b.code, DATE_FORMAT(p.DOB,'%Y%m%d') as DOB, 
				  concat(p.street, '^',p.postal_code,'^', p.city, '^', p.state) as address, p.country_code, p.phone_home, p.phone_biz, p.status, 
				  p.sex, p.ethnoracial,p.county, c.code_text, c.code, c.code_type, DATE_FORMAT(fe.date,'%Y%m%d') as issuedate, concat(p.fname, '^',p.mname,'^', p.lname) as clientname,
        ...
        foreach ($result as $r) {
            ...
            $o_result       = $appTable->zQuery($o_query, array($r['encounter'],$r['clientid'],1));
            ...
            "1|" . // 1. Set id
            "|" . // 2. (B)Client id
            $r['clientid']."^^^^MR|". // 3. (R) Client indentifier list
            "|" . // 4. (B) Alternate PID
            "^^^^^^~^^^^^^S|" . // 5.R. Name
            ...
            //"|" . // 28.B Nationality
            //"|" . // 29. Client Death Date and Time
            //"|" . // 30. Client Death Indicator
