...
//
function frontPayment($client_id, $encounter, $method, $source, $amount1, $amount2, $timestamp, $auth = "")
{
    ...
    ") VALUES ( ?, ?, ?, ?, ?, ?, ?, ?)", array($client_id,$encounter,$timestamp,$auth,$method,$source,$amount1,$amount2));
    return $payid;
}
...
//Function inserts the distribution.Payment,Adjustment,Deductible,Takeback & Follow up reasons are inserted as seperate rows.
 ...
 //In the screen a drop down of Ins1,Ins2,Ins3,Pat are given.The posting can be done for any level.
    ...
    if (isset($_POST["Payment$CountRow"]) && $_POST["Payment$CountRow"]*1>0) {
        ...
        } elseif (trim(formData('type_name'))=='client') {
            $AccountCode="PP";
        }

        sqlBeginTrans();
        $sequence_no = sqlQuery("SELECT IFNULL(MAX(sequence_no),0) + 1 AS increment FROM ar_activity WHERE pid = ? AND encounter = ?", array(trim(formData('hidden_client_code')), trim(formData("HiddenEncounter$CountRow"))));
        sqlStatement("insert into ar_activity set "    .
        "pid = '"       . trim(formData('hidden_client_code')) .
    ...
    if ($Affected=='yes') {
        if (trim(formData('type_name'))!='client') {
            $ferow = sqlQuery("select last_level_closed from form_encounter  where
		pid ='".trim(formData('hidden_client_code'))."' and encounter='".trim(formData("HiddenEncounter$CountRow"))."'");
			...
			pid ='".trim(formData('hidden_client_code'))."' and encounter='".trim(formData("HiddenEncounter$CountRow"))."'");
                ...
                if ($new_payer_type <= 3 && !empty($ferow['last_level_closed']) || $new_payer_type == 0) {
                  ...
                  $new_payer_id = SLEOB::arGetPayerID(trim(formData('hidden_client_code')), $date_of_service, $new_payer_type);
                if ($new_payer_id>0) {
                        SLEOB::arSetupSecondary(trim(formData('hidden_client_code')), trim(formData("HiddenEncounter$CountRow")), 0);
                }
