...
class BillingExport
{
  ...
  //
    function addClaim($client_id, $encounter)
    {
        ...
        "WHERE p.pid = ? " .
        "LIMIT 1";
        $prow = sqlQuery($query, array($client_id, $client_id));
        ...
        $query = "SELECT e.date, e.facility, " .
          ...
          "LEFT OUTER JOIN forms ON forms.formdir = 'newclient' AND " .
          ...
          "LIMIT 1";
        $erow = sqlQuery($query, array($client_id, $client_id, $encounter));
        ...
        $ires = sqlStatement($query, array($erow['id'], $client_id));
