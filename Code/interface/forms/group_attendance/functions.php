...
require_once(dirname(__FILE__) . "/../../../library/client_tracker.inc.php");
...
function participant_insertions($form_id, $therapy_group, $group_encounter_data, $appt_data)
{
    ...
    foreach ($clientData as $pid => $client) {
        //Insert into therapy_groups_participants_attendance table
        insert_into_tgpa_table($form_id, $pid, $client);

        //Check if to create appt and encounter for each client (if has certain status and 'bottom' submit was pressed, not 'add_client' submit).
        $create_for_client = if_to_create_for_client($client['status']);
        if ($create_for_client) {
            //Create encounter for each client
            $encounter_id = insert_client_encounter($pid, $therapy_group, $group_encounter_data['date'], $client, $appt_data['pc_aid']);

            //Create appt for each client (if there is appointment connected to encounter)
            if (!empty($appt_data)) {
                $pc_eid = insert_client_appt($pid, $therapy_group, $appt_data['pc_aid'], $appt_data['pc_eventDate'], $appt_data['pc_startTime'], $client);
                manage_tracker_status($appt_data['pc_eventDate'], $appt_data['pc_startTime'], $pc_eid, $pid, $appt_data['pc_aid'], $client['status'], $appt_data['pc_room'], $encounter_id);
            }
        }
    }
}
...
{
    $sql_for_table_tgpa = "INSERT INTO therapy_groups_participant_attendance (form_id, pid, meeting_client_comment, meeting_client_status) " .
        "VALUES(?,?,?,?);";
    sqlStatement($sql_for_table_tgpa, array($form_id, $pid, $participantData['comment'], $participantData['status']));
}

/**
 * Creates an appointment for client from attendance form
 * @param $pid
 * @param $gid
 * @param $pc_aid
 * @param $pc_eventDate
 ...
 * @param $participantData
 */
function insert_client_appt($pid, $gid, $pc_aid, $pc_eventDate, $pc_startTime, $participantData)
{
...
/**
 * Creates an encounter for client from attendance form
 * @param $pid
 * @param $gid
 ...
 * @param $pc_aid
 */
function insert_client_encounter($pid, $gid, $group_encounter_date, $participantData, $pc_aid)
{
    ...
    if ($result_array) {
        ...
        addForm($enc_id, "New Client Encounter", $form_id, "newclient", $pid, $userauthorized, $group_encounter_date, '', '', null);
...
/**
 * Checks if to create appointment and encounter for client himself based on the status in the attendance form.
 ...
 * @return bool
 */
function if_to_create_for_client($status)
{
