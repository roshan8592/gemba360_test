...
if (isset($_POST["mode"])) {
    if ($_POST["mode"] == "ModifyPayments" || $_POST["mode"] == "FinishPayments") {
        ...
        if (trim($_POST['type_name'])=='insurance') {
            ...
            "', client_id = '"   . trim(formData('hidden_type_code')) ;
        }
            ...
            if ($payment_id*1>0) {//Distribution rows already in the database are displayed.
                ...
                if ($RowSearchSub = sqlFetchArray($ResultSearchSub)) {
                    do {
                        ...
                        $res = sqlStatement("SELECT fname,lname,mname FROM client_data where pid =?", [$PId]);
                ...
                <thead bgcolor="#DDDDDD" class="text">
                    ...
                    <td class="left top" ><?php echo xlt('Client Name'); ?></td>
                    ...
                    <td class="left top right" ><?php echo xlt('Follow Up Reason'); ?></td>
                            ...
                            while ($RowSearch = sqlFetchArray($ResultSearch)) {
                                ...
                                if ($Ins==0) {//Got just before Client
                                    $resMoneyGot = sqlStatement("SELECT sum(pay_amount) as MoneyGot FROM ar_activity where
                            ...
                            <tr bgcolor='<?php echo attr($bgcolor); ?>' class="text" id="trCharges<?php echo attr($CountIndex); ?>">
                                ...
                                <td align="left" class="<?php echo attr($StringClass); ?>"><input id="HiddenIns<?php echo attr($CountIndex); ?>" name="HiddenIns<?php echo attr($CountIndex); ?>" type="hidden" value="<?php echo attr($Ins); ?>"><?php echo generate_select_list("payment_ins$CountIndex", "payment_ins", "$Ins", "Insurance/Client", '', '', 'ActionOnInsPat("'.$CountIndex.'")'); ?></td>
...
<td align="center">&nbsp;</td>
            ...
            <div class="row">
                <input type="hidden" name="hidden_client_code" id="hidden_client_code" value="<?php echo attr($hidden_client_code);?>"/>
