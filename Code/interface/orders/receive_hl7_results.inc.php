...
//
function rhl7FlushMDM($client_id, $mdm_docname, $mdm_datetime, $mdm_text, $mdm_category_id, $provider)
{
    if ($client_id) {
        ...
        $d = new Document();
        $rc = $d->createDocument($client_id, $mdm_category_id, $filename, 'text/plain', $mdm_text);
        if (!$rc) {
            rhl7LogMsg(xl('Document created') . ": $filename", false);
            if ($provider) {
                $d->postClientNote($provider, $mdm_category_id, xl('Electronic document received'));
...
/**
 * Look for a client matching the given data.
 * Return values are:
 *  >0  Definite match, this is the pid.
 *   0  No client is close to a match.
 *  -1  It's not clear if there is a match.
 */
function match_client($ptarr)
{
    ...
    $client_id = 0;
    $res = sqlStatement(
        "SELECT pid FROM client_data WHERE " .
...
/**
 * Create a client using whatever client_data attributes are provided.
 */
function create_skeleton_client($client_data)
{
    $employer_data = array();
    $tmp = sqlQuery("SELECT MAX(pid)+1 AS pid FROM client_data");
    $ptid = empty($tmp['pid']) ? 1 : intval($tmp['pid']);
    if (!isset($client_data['pubpid'])) {
        $client_data['pubpid'] = $ptid;
    }

    updateClientData($ptid, $client_data, true);
 ...
 * @param  string  &$matchreq Array of shared client matching requests
 ...
 *                            value is an existing pid or 0 to specify creating a client
...
function receive_hl7_results(&$hl7, &$matchreq, $lab_id = 0, $direction = 'B', $dryrun = false, $matchresp = null)
{
  ...
  // Ensoftek: Different labs seem to send different EOLs. Edit HL7 input to a character we know.
    ...
    $in_report_status = '';
    $in_encounter = 0;
    $client_id = 0; // for results-only client matching logic
    $porow = false;
    $pcrow = false;
    $oprow = false;
  ...
  // We'll need the document category IDs for any embedded documents.
    ...
    foreach ($segs as $seg) {
        ...
        if ($a[0] == 'MSH') {
            ...
            $client_id = 0;
            // Client matching is needed for a results-only interface or MDM message type.
            if ('R' == $direction || 'MDM' == $msgtype) {
                ...
                $client_id = match_client($ptarr);
                if ($client_id == -1) {
                        // Result is indeterminate.
                        // Make a stringified form of $ptarr to use as a key.
                        $ptstring = serialize($ptarr);
                        // Check if the user has specified the client.
                    if (isset($matchresp[$ptstring])) {
                        // This will be an existing pid, or 0 to specify creating a client.
                        $client_id = intval($matchresp[$ptstring]);
                    } else {
                        if ($dryrun) {
                            ...
                            // Should not happen, but it would be bad to abort now.  Create the client.
                            $client_id = 0;
                            rhl7LogMsg(xl('Unexpected non-match, creating new client for segment') .
                              ' ' . $rhl7_segnum, false);
                        }
                    }
                }

                if ($client_id == 0 && !$dryrun) {
                          // We must create the client.
                          $client_id = create_skeleton_client($ptarr);
                }
            ...
            if ($direction == 'R') {
                ...
                // Need also provider_id (probably), client_id, date_ordered, lab_id.
                ...
                if (!$in_orderid) {
                    ...
                    $encrow = sqlQuery(
                        ...
                        array($client_id, $date_report, $date_report)
                    );
                    if (!empty($encrow)) {
                    ...
                    if (!$dryrun) {
                        ...
                        if ($encounter_id && $in_orderid) {
                                              addForm($encounter_id, "Procedure Order", $in_orderid, "procedure_order", $client_id);
                        }
                    }
                } // end no $porow
            } // end results-only
                ...
                if ($in_encounter) {
                    if ($direction != 'R' && $porow['encounter_id'] != $in_encounter) {
                        return rhl7LogMsg(xl('Encounter ID') .
                          ...
                          // might be done here to make sure the client seems to match.
                }
            ...
            if ($a[2] == 'ED') {
                ...
                // a normal client document in the assigned category for lab results.
                $tmp = explode($d2, $a[5]);
            ...
            // Ensoftek: Performing Organization Details. Goes into "Pending Review/Client Results--->Notes--->Facility" section.
 ...
 * $info['select'] = array of client matching responses where key is serialized client
 *   attributes and value is selected pid for this client, or 0 to create the client.
 * TO THE CALLER:
 * $info["$ppid/$filename"]['mssgs'] = array of messages from this function.
 * $info['match'] = array of client matching requests where key is serialized client
