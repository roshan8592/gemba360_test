...
class eRxXMLBuilder
{
    ...
    {
        $element = $this->getDocument()->createElement('ClientName');
        $element->appendChild($this->createElementTextDemographicsCheck('last', $this->trimData($this->stripSpecialCharacter($client['lname']), 35), xl('Client Last Name')));
        $element->appendChild($this->createElementTextDemographicsCheck('first', $this->trimData($this->stripSpecialCharacter($client['fname']), 35), xl('Client First Name')));
        $element->appendChild($this->createElementText('middle', $this->trimData($this->stripSpecialCharacter($client['mname']), 35)));
    ...
    {
        $client['street'] = $this->trimData($this->stripSpecialCharacter($client['street']), 35);
        $this->warningMessage($client['street'], xl('Client Street Address'));


        if (trim($client['country_code']) == '') {
            $eRxDefaultClientCountry = $this->getGlobals()->getDefaultClientCountry();

            if ($eRxDefaultClientCountry == '') {
                $this->demographicsCheck('', xl('Global Default Client Country'));
            } else {
                $client['country_code'] = $eRxDefaultClientCountry;
            }

            $this->demographicsCheck('', xl('Client Country'));
        }

        $element = $this->getDocument()->createElement('ClientAddress');
        $element->appendChild($this->createElementTextFieldEmpty('address1', $client['street'], xl('Client Street Address')));
        $element->appendChild($this->createElementTextDemographicsCheck('city', $client['city'], xl('Client City')));
        if ($client['state']) {
            $element->appendChild($this->createElementText('state', $client['state']));
        }

        if ($client['postal_code']) {
            $element->appendChild($this->createElementText('zip', $client['postal_code']));
        }

        $element->appendChild($this->createElementText('country', substr($client['country_code'], 0, 2)));

        return $element;
    }

    public function getClientContact($client)
    {
        $element = $this->getDocument()->createElement('ClientContact');
        if ($client['phone_home']) {
            $element->appendChild($this->createElementText('homeTelephone', preg_replace('/-/', '', $client['phone_home'])));
        }

        return $element;
    }

    public function getClientCharacteristics($client)
    {
        if (trim($client['date_of_birth']) == '' || $client['date_of_birth'] == '00000000') {
            $this->warningMessage('', xl('Client Date Of Birth'));
        }

        $this->warningMessage(trim($client['sex']), xl('Client Gender'));

        $element = $this->getDocument()->createElement('ClientCharacteristics');
        if ($client['date_of_birth'] && $client['date_of_birth'] != '00000000') {
            $element->appendChild($this->createElementText('dob', $client['date_of_birth']));
        }

        if ($client['sex']) {
            $element->appendChild($this->createElementText('gender', substr($client['sex'], 0, 1)));
        }

        return $element;
    }

    public function getClientFreeformHealthplans($clientId)
    {
        $healthplans = $this->getStore()
            ->getClientHealthplansByClientId($clientId);

        $elements = array();

        while ($healthplan = sqlFetchArray($healthplans)) {
            $element = $this->getDocument()->createElement('ClientFreeformHealthplans');
    ...
    public function getClientFreeformAllergy($clientId)
    {
        $allergyData = $this->getStore()
            ->getClientAllergiesByClientId($clientId);

        $elements = array();

        while ($allergy = sqlFetchArray($allergyData)) {
            $element = $this->getDocument()->createElement('ClientFreeformAllergy');
    ...
    {
        $clientData = $this->getStore()
            ->getClientByClientId($clientId);

        $element = $this->getDocument()->createElement('Client');
        $element->setAttribute('ID', $clientData['pid']);
        $element->appendChild($this->getClientName($clientData));
        $element->appendChild($this->getClientAddress($clientData));
        $element->appendChild($this->getClientContact($clientData));
        $element->appendChild($this->getClientCharacteristics($clientData));
        $this->appendChildren($element, $this->getClientFreeformHealthplans($clientId));
        $this->appendChildren($element, $this->getClientFreeformAllergy($clientId));
    ...
    public function getClientPrescriptions($prescriptionIds)
    {
        $elements = array();
    ...
    public function getClientMedication($clientId, $uploadActive, $count)
    {
        $medications = $this->getStore()
            ->selectMedicationsNotUploadedByClientId($clientId, $uploadActive, $count);
    ...
    public function getClientElements($clientId, $totalCount, $requestedPrescriptionIds)
    {
        $elements = array();
        ...
        if ($clientId) {
            ...
            $elements[] = $this->getClient($clientId);

            $selectPrescriptionIds = $this->getStore()
                ->selectPrescriptionIdsNotUploadedByClientId(
                    $clientId,
                    $uploadActive,
                    $totalCount
                );
            ...
            if (count($requestedPrescriptionIds) > 0) {
                $elements = array_merge($elements, $this->getClientPrescriptions($requestedPrescriptionIds));
            } elseif (count($prescriptionIds) > 0) {
                $elements = array_merge($elements, $this->getClientPrescriptions($prescriptionIds));
            } else {
                $this->getClientPrescriptions(array(0));
            }

            if ($selectPrescriptionIdsCount < $totalCount) {
                $elements = array_merge($elements, $this->getClientMedication($clientId, $uploadActive, $totalCount - $selectPrescriptionIdsCount));
            }
        }
