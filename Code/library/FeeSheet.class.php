...
class FeeSheet
{
    ...
    public $client_age = 0;                    // Age in years as of the visit date
    public $client_male = 0;                   // 1 if male
    public $client_pricelevel = '';            // From client_data.pricelevel
  ...
  // These variables are used to compute the initial consult service with highest CYP (IPPF).
    ...
    function __construct($pid = 0, $encounter = 0)
    {
        ...
        $clientrow = getClientData($this->pid, "DOB, sex, pricelevel");
        $this->client_age = $this->getAge($clientrow['DOB'], $this->visit_date);
        $this->client_male = strtoupper(substr($clientrow['sex'], 0, 1)) == 'M' ? 1 : 0;
        $this->client_pricelevel = $clientrow['pricelevel'];
    }
  ...
  // Gets the provider from the encounter, logged-in user or client demographics.
  ...
  //
    public function findProvider()
    {
        ...
        if (!$providerid) {
            $find_provider = sqlQuery("SELECT providerID FROM client_data " .
  ...
  //  pricelevel
    public function addServiceLineItem($args)
    {
        ...
        $pricelevel  = isset($args['pricelevel']) ? $args['pricelevel'] : $this->client_pricelevel;
        $del         = !empty($args['del']);
                ...
                if ($code_type == 'COPAY') {
                    ...
                    if (!$id) {
                        ...
                        $session_id = sqlInsert(
                            ...
                            "(payer_id, user_id, pay_total, payment_type, description, client_id, payment_method, " .
                            "adjustment_code, post_to_date) " .
                            "VALUES ('0',?,?,'client','COPAY',?,'','client_payment',now())",
  ...
  // Get price level from client demographics.
  //
    public function getPriceLevel()
    {
        return $this->client_pricelevel;
    }
  ...
  //
    ...
    {
        if (!empty($pricelevel)) {
            if ($this->client_pricelevel != $pricelevel) {
                $this->logFSMessage(xl('Price level changed'));
                sqlStatement(
                    "UPDATE client_data SET pricelevel = ? WHERE pid = ?",
                    array($pricelevel, $this->pid)
                );
                $this->client_pricelevel = $pricelevel;
            }
        }
    }
