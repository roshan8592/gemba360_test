...
if (!empty($_REQUEST['go'])) { ?>
    ...
    $arrOeUiSettings = array(
        ...
        'include_client_name' => false,// use only in appropriate pages
        'expandable' => false,
            ...
            <div class="col-sm-12">
                <fieldset>
                    <div class="col-sm-12 col-md-12 col-lg-12">
                        ...
                        switch ($task) {
                            case "add":
                                // Add a new message for a specific client; the message is documented in Client Notes.
                                // Add a new message; it's treated as a new note in Client Notes.
                                $note = $_POST['note'];
                                $noteid = $_POST['noteid'];
                                ...
                                foreach ($assigned_to_list as $assigned_to) {
                                    if ($noteid && $assigned_to != '-client-') {
                                        updatePnote($noteid, $note, $form_note_type, $assigned_to, $form_message_status, $datetime);
                                        $noteid = '';
                                    } else {
                                        if ($noteid && $assigned_to == '-client-') {
                                            // When $assigned_to == '-client-' we don't update the current note, but
                                            // instead create a new one with the current note's body prepended and
                                            // attributed to the client.  This seems to be all for the client portal.
                                            $row = getPnoteById($noteid);
                                            if (!$row) {
                                                die("getPnoteById() did not find id '" . text($noteid) . "'");
                                            }
                                            $pres = sqlQuery("SELECT lname, fname " .
                                                "FROM client_data WHERE pid = ?", array($reply_to[0]));
                                            $clientname = $pres['lname'] . ", " . $pres['fname'];
                                            $note .= "\n\n$clientname on " . $row['date'] . " wrote:\n\n";
                                            $note .= $row['body'];
                                        }
                                        // There's no note ID, and/or it's assigned to the client.
                                        // In these cases a new note is created.
                                        foreach ($reply_to as $client) {
                                            addPnote($client, $note, $userauthorized, '1', $form_note_type, $assigned_to, $datetime, $form_message_status);
                                        }
                                    }
                                }
                                break;
                            ...
                            case "save":
                                ...
                                if ($task == "save") {
                                    ...
                                    updatePnoteClient($noteid, $reply_to);
                                }
                                $task = "edit";
                                $note = $_POST['note'];
                            ...
                            case "edit":
                                ...
                                // Update the message if it already exists; it's appended to an existing note in Client Notes.
                        ...
                        if ($task == "addnew" or $task == "edit") {
                            ...
                            echo "<form name='form_client' id='new_note'
                            ...
                            if ($task == "addnew") {
                                ...
                                $onclick = "onclick=multi_sel_client()";
                            } elseif ($task == "edit") {
                                ...
                                <legend><?php echo text($message_legend); ?></legend>
                                        ...
                                        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-10 oe-custom-line col-lg-offset-1">
                                            <div class="row">
                                                <div class="col-xs-3 col-sm-3">
                                                    ...
                                                    // Added 6/2009 by BM to incorporate the client notes into the list_options listings.
                                                ...
                                                <div class="col-xs-4">
                                                    <label class="control-label" for="form_client">
                                                        <?php
                                                        if ($task != "addnew" && $result['pid'] != 0) { ?>
                                                            <a class="patLink"
                                                               onclick="goPid('<?php echo attr(addslashes($result['pid'])); ?>')"><?php echo xlt('Client'); ?>
                                                                :</a>
                                                            <?php
                                                        } else { ?>
                                                            <b class='<?php echo($task == "addnew" ? "required" : "") ?>'><?php echo xlt('Client'); ?>
                                                                :</b>
                                                            <?php
                                                        }
                                                        ?>
                                                    ...
                                                    if ($reply_to) {
                                                        $prow = sqlQuery("SELECT lname, fname,pid, pubpid, DOB  " .
                                                            "FROM client_data WHERE pid = ?", array($reply_to));
                                                        $clientname = $prow['lname'] . ", " . $prow['fname'];
                                                    }
                                                    if ($task == "addnew" || $result['pid']==0) {
                                                        $cursor = "oe-cursor-add";
                                                        $background = "oe-client-background";
                                                    } elseif ($task == "edit") {
                                                        $cursor = "oe-cursor-stop";
                                                        $background = '';
                                                    }
                                                    ?>
                                                    <input type='text'  id='form_client' name='form_client' class='form-control <?php echo $cursor . " " .$background;?>' onclick="multi_sel_client()" placeholder='<?php echo xla("Click to add client"); ?>' value='<?php echo attr($clientname); ?>' readonly/>
                                                    <input type='hidden' class="form-control" name='reply_to' id='reply_to' value='<?php echo attr($reply_to); ?>'/>
                                                </div>
                                                <div class="col-xs-2">
                                                    <?php
                                                    if ($task=="addnew" || $result['pid']==0) {
                                                        echo "<label class='control-label oe-empty-label' for='clear_clients'></label>";
                                                        echo '<button type="button" id="clear_clients"  class="btn btn-default btn-undo pull-left flip" value="' . xla('Clear') .'">' . xlt("Clear") . '</button>';
                                                    } ?>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        if ($noteid) {
                                            ...
                                            if (sqlNumRows($tmp)) {
                                                ...
                                                while ($gprow = sqlFetchArray($tmp)) {
                                                    ...
                                                    $str_dob = xl("DOB") . ":" . $prow['DOB'] . " " . xl("Age") . ":" . getClientAge($prow['DOB']);
                                    ...
                                    <div class="row">
                                        <div class='col-xs-12'>
                                            ...
                                            if ($noteid) {
                                                $body = preg_replace('/(:\d{2}\s\()' . $result['pid'] . '(\sto\s)/', '${1}' . $clientname . '${2}', $body);
                                                $body = preg_replace('/(\d{4}-\d{2}-\d{2} \d{2}:\d{2}\s\([^)(]+\s)(to)(\s[^)(]+\))/', '${1}' . xl('to') . '${3}', $body);
                                                $body =nl2br(text(oeFormatClientNote($body)));
                            ...
                            $sort = array("users.lname", "client_data.lname", "pnotes.title", "pnotes.date", "pnotes.message_status");
                            ...
                            while ($myrow = sqlFetchArray($result)) {
                                ...
                                $client = $myrow['pid'];
                                if ($client > 0) {
                                    $client = $myrow['client_data_lname'];
                                    if ($myrow['client_data_fname']) {
                                        $client .= ", " . $myrow['client_data_fname'];
                                    }
                                } else {
                                    $client = "* " . xl('Client must be set manually') . " *";
                                }
                                $count++;
                                echo "
                                    <tr id=\"row" . attr($count) . "\" style=\"background:white\" height=\"24\">
                                        ...
                                        <td style=\"border-bottom: 1px #000000 solid; border-right: 1px #000000 solid;\">
                                            ...
                                            text($client) . "</a></td><td width=5></td></tr>
                                            </table></td>
                                        <td style=\"border-bottom: 1px #000000 solid; border-right: 1px #000000 solid;\">
                                            <table cellspacing=0 cellpadding=0 width=100%><tr><td width=5></td><td class=\"text\">" .
                ...
                <fieldset>
                    ...
                    <div class="col-sm-4 col-md-4 col-lg-4">
                        ...
                        <form id="smsForm" class="input-group">
                            <input id="SMS_client" type="text" style="margin:0;max-width:100%;" class="form-control"
                                   placeholder="<?php echo xla("Client Name"); ?>" />
    ...
    <script language="javascript">
        ...
        $(function () {
            var f = $("#smsForm");
            $("#SMS_client").autocomplete({
                source: "save.php?go=sms_search",
                minLength: 2,
                select: function (event, ui) {
                    event.preventDefault();
                    $("#SMS_client").val(ui.item.label + ' ' + ui.item.mobile);
        ...
        $(function () {
            ...
            //clear inputs of clients
            $("#clear_clients").click(function(){
                $("#reply_to").val("");
                $("#form_client").val("");
            });
        });
        ...
        var PrintNote = function () {
            top.restoreSession();
            window.open('../../client_file/summary/pnotes_print.php?noteid=' + <?php echo js_url($noteid); ?>, '_blank', 'resizable=1,scrollbars=1,width=600,height=500');
        };
        ...
        function gotoReport(doc_id, pname, pid, pubpid, str_dob) {
            ...
            parent.left_nav.setClient(pname, pid, pubpid, '', str_dob);
            parent.left_nav.setClientEncounter(EncounterIdArray, EncounterDateArray, CalendarCategoryArray);
            <?php if ($GLOBALS['new_tabs_layout']) { ?>
            var docurl = '../controller.php?document&view' + "&client_id=" + encodeURIComponent(pid) + "&document_id=" + encodeURIComponent(doc_id) + "&";
            var paturl = 'client_file/summary/demographics.php?pid=' + encodeURIComponent(pid);
            parent.left_nav.loadFrame('dem1', 'pat', paturl);
            parent.left_nav.loadFrame('doc0', 'enc', docurl);
            top.activateTabByName('enc', true);
            <?php } else { ?>
            var docurl = '<?php  echo $GLOBALS['webroot'] . "/controller.php?document&view"; ?>' + "&client_id=" + encodeURIComponent(pid) + "&document_id=" + encodeURIComponent(doc_id) + "&";
            var paturl = '<?php  echo $GLOBALS['webroot'] . "/interface/client_file/summary/demographics.php?pid="; ?>' + encodeURIComponent(pid);
        ...
        // This is for callback by the find-client popup.
        function setclient(pid, lname, fname, dob) {
            var f = document.getElementById('new_note');
            f.form_client.value += lname + ', ' + fname + '; ';
            f.reply_to.value += pid + ';';
            <?php if ($noteid) { ?>
            //used when direct messaging service inserts a pnote with indeterminate client
            //to allow the user to assign the message to a client.
            top.restoreSession();
            $("#task").val("saveClient");
            $("#new_note").submit();
            <?php } ?>
        ...
        // This is for callback by the multi_clients_finder popup.
        function setMultiClients(clientsList) {
            var f = document.getElementById('new_note');
            f.form_client.value='';
            f.reply_to.value='';
            $.each(clientsList, function (key, client) {
                f.form_client.value += client.lname + ', ' + client.fname + '; ';
                f.reply_to.value += client.pid + ';';
            })
        ...
        // This invokes the find-client popup.
        function sel_client() {
            dlgopen('../../main/calendar/find_client_popup.php', '_blank', 625, 400);
        }

        function multi_sel_client() {
            $('#reply_to').trigger('click');
            var url = '../../main/finder/multi_clients_finder.php'
            // for edit selected list
            if ($('#reply_to').val() !== '') {
                url = url + '?clients=' + $('#reply_to').val() + '&csrf_token_form=<?php echo attr_url(CsrfUtils::collectCsrfToken()); ?>';
            }
        ...
        function SMS_direct() {
            ...
            if ((pid === '') || (m === '')) {
                ...
                alert('<?php echo xls("This client does not allow SMS messaging!"); ?>');
            } else {
                top.restoreSession();
