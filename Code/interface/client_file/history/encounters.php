...
function getDocListByEncID($encounter, $raw_encounter_date, $pid)
{
    ...
    if (!empty($documents) && count($documents) > 0) {
        foreach ($documents as $documentrow) {
            ...
            $docHref = $GLOBALS['webroot']."/controller.php?document&view&client_id=" . attr_url($pid) . "&doc_id=" . attr_url($documentrow['id']);
...
// This is called to generate a line of output for a client document.
//
...
function todocument(docid) {
  h = '<?php echo $GLOBALS['webroot'] ?>/controller.php?document&view&client_id=<?php echo attr_url($pid); ?>&doc_id=' + encodeURIComponent(docid);
  top.restoreSession();
  location.href = h;
}
...
<?php } else { ?>
    <?php if ($attendant_type == 'pid' && !$issue) { // only for client encounter and if listing for multiple issues?>
...
<?php if ($GLOBALS['enable_group_therapy'] && !$billing_view && $therapy_group == 0) { ?>
    <!-- Two new columns if therapy group is enable only in client  encounter - encounter type and group name (empty if isn't group type) -->
...
$drow = false;
if (!$billing_view) {
  // Query the documents for this client.  If this list is issue-specific
...
if ($attendant_type == 'pid') {
    $from = "FROM form_encounter AS fe " .
        ...
        "f.formdir = 'newclient' AND f.deleted = 0 ";
} else {
...
while ($result4 = sqlFetchArray($res4)) {
    ...
    if ($billing_view) {
        ...
        if ($attendant_type == 'pid' && !$issue) { // only for client encounter and if listing for multiple issues
        ...
        foreach ($encarr as $enc) {
            if ($enc['formdir'] == 'newclient' || $enc['formdir'] == 'newGroupEncounter') {
                continue;
            }
    ...
    if ($attendant_type == 'pid' && !$GLOBALS['ippf_specific']) {
        ...
        if ($auth_demo) {
            ...
            if ($responsible == 0) {
                $insured .= "<span class='text' style='color:red'>&nbsp;" . xlt('Client') .
                            "</span><br>\n";
            }
        } else {
