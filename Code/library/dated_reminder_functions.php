...
// --------- need to run client query seperately to allow for reminders not linked to a client
        $pRow = array();
        if ($drRow['pid'] > 0) {
            $pSQL = sqlStatement("SELECT pd.title ptitle, pd.fname pfname, pd.mname pmname, pd.lname plname FROM `client_data` pd WHERE pd.pid = ?", array($drRow['pid']));
            $pRow = sqlFetchArray($pSQL);
        }

// --------- fill the $reminders array
        $reminders[$i]['messageID'] = $drRow['dr_id'];
        $reminders[$i]['ClientID'] = $drRow['pid'];

// -------------------------------------  if there was a client linked, set the name, else set it to blank
        $reminders[$i]['ClientName'] = (empty($pRow) ? '' : $pRow['ptitle'] . ' ' . $pRow['pfname'] . ' ' . $pRow['pmname'] . ' ' . $pRow['plname']);
// -------------------------------------
...
// --------- check if reminder is  overdue
        ...
        $pdHTML .= '<p id="p_' . attr($r['messageID']) . '">
            ...
            <span title="' . ($r['ClientID'] > 0 ? xla('Click Client Name to Open Client File') : '') . '" class="' . attr($class) . '">' .
            $warning . '         
            <span onclick="goPid(' . attr(addslashes($r['ClientID'])) . ')" class="patLink" id="' . attr($r['ClientID']) . '">' .
            text($r['ClientName']) . '
            </span> ' .
...
// ---- returns string, blank if no current client
function getPatName($clientID)
{
    $clientID = intval($clientID);
    $pSQL = sqlStatement("SELECT pd.title ptitle, pd.fname pfname, pd.mname pmname, pd.lname plname FROM `client_data` pd WHERE pd.pid = ?", array($clientID));
    $pRow = sqlFetchArray($pSQL);
...
// --------- loop through the results
    for ($i = 0; $drRow = sqlFetchArray($drSQL); $i++) {
// --------- need to run client query seperately to allow for messages not linked to a client
        $pSQL = sqlStatement("SELECT pd.title ptitle, pd.fname pfname, pd.mname pmname, pd.lname plname FROM `client_data` pd WHERE pd.pid = ?", array($drRow['pid']));
        $pRow = sqlFetchArray($pSQL);
...
// -------------------------------------  if there was a client linked, set the name, else set it to blank
        $reminders[$i]['ClientName'] = (empty($pRow) ? 'N/A' : $pRow['ptitle'] . ' ' . $pRow['pfname'] . ' ' . $pRow['pmname'] . ' ' . $pRow['plname']);
// -------------------------------------
